...
import {
  ...
  normalizeWeekRect as normalizeRadioWeekRect,
  weekCellInRect as radioWeekCellInRect,
  ...
  normalizeWeekMergeSelection as normalizeRadioWeekMergeSelection,
  weekPlainClickPreservesWeekAreaSelection,
  deriveMergeEligibility as deriveRadioMergeEligibility,
  weekCellExportText,
...
type RadioWeekRectSelection = ExpertWeekRectSelection
type RadioMultiCellSelection = ExpertMultiCellSelection
type RadioWeekMergeSelectionNormalized = WeekMergeSelectionNormalized
 ...
 */
const RADIO_EXPERT_ROW_VIRTUALIZATION = true
...
export function RadioExpertGrid({
  ...
  const normalizeRowCacheRef = useRef(
    new Map<string, NormalizeRowCacheEntry<RadioExpertScheduleRow>>()
  )
  ...
  const rowMergeMapCacheRef = useRef(
    new Map<
      string,
      MapBuildCacheEntry<
        RadioExpertScheduleRow["mergedWeekSpans"],
        RadioRowMergeMap
      >
    >()
  )
  ...
  const pushRows = useCallback(
    (next: RadioExpertScheduleRow[]) => {
      const withDates = finalizeRowsPreservingIdentity(next, (r) => {
        ...
        const dates = deriveRadioExpertRowScheduleYmdFromRow(
          cleared,
          weekColumns,
          campaignStartDate,
          campaignEndDate,
  ...
  const rowReorderVirtual = useMemo(
    () =>
      RADIO_EXPERT_ROW_VIRTUALIZATION
        ? {
            rowCount: normalizedRows.length,
            estimateSizePx: RADIO_EXPERT_ROW_HEIGHT_PX,
  ...
  const ensureRowVisible = useCallback((rowIndex: number) => {
    if (!RADIO_EXPERT_ROW_VIRTUALIZATION) return
   ...
   * first obstruction (sibling span, populated week cell, or day-detailed week),
   * so resizing never absorbs or destroys values; the burst total is unchanged.
   */
            ...
            ? Math.min(
                ...
                Math.max(
                  ...
                  deltaDaysFromPx(prev.edgeYmd, dx, {
                    ...
                    dayKeysByWeekKey,
                    weekColumnWidths,
                    defaultWeekColWidthPx: RADIO_EXPERT_WEEK_COL_WIDTH_PX,
  ...
  const {
    ...
    getScrollElement: () => gridScrollRef.current,
    estimateSize: RADIO_EXPERT_ROW_HEIGHT_PX,
    overscan: RADIO_EXPERT_ROW_OVERSCAN,
  ...
  const virtualSpacerColSpan = useMemo(() => {
    ...
    return (
      1 + radioDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekCells
    )
  }, [
    weekColumns,
    expandedWeekKeys,
  ...
  const containerTotals = useMemo(() => {
    ...
    for (const row of normalizedRows) {
      const { net, fee } = expertRowCostSplit(row, weekKeys, feeradio)
      sumNet += net
      sumFee += fee
      for (const k of weekKeys) {
        ...
        <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2 border-b border-border/60 bg-muted/5 pb-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <Label htmlFor="radio-expert-row-count" className="sr-only">Add rows count</Label>
              <Input
                  ...
                  <thead ref={theadRef} className="[&_tr]:border-b-0">
                    <tr>
                      <ExpertGridRowReorderHeaderCell
                        ...
                        style={radioExpertHeaderCellBgStyle}
                      />
                    ...
                    {(() => {
                      ...
                      return (
                        <MemoExpertGridRow
                          ...
                          rowUiSig={rowUiSig}
                          render={() => {
                      const net = expertRowNetMedia(row, weekKeys, feeradio)
                      ...
                      return (
                        <tr
                          ...
                          data-radio-expert-row-index={rowIndex}
                          style={{
                            ...stripeStyle,
                            height: RADIO_EXPERT_ROW_HEIGHT_PX,
                            maxHeight: RADIO_EXPERT_ROW_HEIGHT_PX,
                          }}
                      ...
                      if (!RADIO_EXPERT_ROW_VIRTUALIZATION) {
