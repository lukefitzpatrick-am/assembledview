/**
 * F-28 Phase 2 — Programmatic Display Expert row virtualization smoke (Prompt A items 1,3,5,7,8).
 */
import assert from "node:assert/strict"
import test from "node:test"
import { format, startOfDay } from "date-fns"

import { mapProgDisplayExpertRowsToStandardLineItems } from "@/lib/mediaplan/expertChannelMappings"
import {
  reorderExpertRows,
  mergedSpanWidthPx,
...
const PROGDISPLAY_EXPERT_ROW_OVERSCAN = EXPERT_GRID_ROW_OVERSCAN_DEFAULT
...
function emptyRow(id: string): ProgDisplayExpertScheduleRow {
...
function buildRows(n: number): ProgDisplayExpertScheduleRow[] {
  return Array.from({ length: n }, (_, i) => emptyRow(`progdisplay-v-${i}`))
}

/** Mirrors ProgDisplayExpertGrid containerTotals over the FULL logical array. */
function computeProgDisplayWeeklyTotals(
...
test("3. DRAG REORDER across virtual boundary lands on correct logical index", () => {
  ...
  const dropIndex = virtualRowIndexFromOffsetY(
    280 * PROGDISPLAY_EXPERT_ROW_HEIGHT_PX + 1,
    PROGDISPLAY_EXPERT_ROW_HEIGHT_PX,
    300
  )
...
test("5. WEEK-COLUMN WIDTH respects resize overrides (sticky header/week resize path)", () => {
  ...
  assert.equal(PROGDISPLAY_EXPERT_ROW_HEIGHT_PX, 41)
})
...
test("7. WEEKLY TOTALS derived from full 300-row state", () => {
  ...
  const totals = computeProgDisplayWeeklyTotals(rows, 0)
  ...
  const after = computeProgDisplayWeeklyTotals(edited, 0)
...
test("8. PERF: mounted DOM window stays bounded while scrolling 300 rows", () => {
  ...
  const nearTop = expectedMountedRowRange(
    ...
    PROGDISPLAY_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    PROGDISPLAY_EXPERT_ROW_OVERSCAN
  )
  const mid = expectedMountedRowRange(
    150 * PROGDISPLAY_EXPERT_ROW_HEIGHT_PX,
    viewportHeight,
    PROGDISPLAY_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    PROGDISPLAY_EXPERT_ROW_OVERSCAN
  ...
  const nearBottom = expectedMountedRowRange(
    280 * PROGDISPLAY_EXPERT_ROW_HEIGHT_PX,
    viewportHeight,
    PROGDISPLAY_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    PROGDISPLAY_EXPERT_ROW_OVERSCAN
  ...
  const maxWindow =
    Math.ceil(viewportHeight / PROGDISPLAY_EXPERT_ROW_HEIGHT_PX) +
    2 * PROGDISPLAY_EXPERT_ROW_OVERSCAN +
    2
