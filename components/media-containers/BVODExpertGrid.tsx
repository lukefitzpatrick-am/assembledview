...
import {
  ...
  normalizeWeekRect as normalizeBvodWeekRect,
  weekCellInRect as bvodWeekCellInRect,
  ...
  normalizeWeekMergeSelection as normalizeBvodWeekMergeSelection,
  weekPlainClickPreservesWeekAreaSelection,
  deriveMergeEligibility as deriveBvodMergeEligibility,
  weekCellExportText,
...
type BvodWeekRectSelection = ExpertWeekRectSelection
type BvodMultiCellSelection = ExpertMultiCellSelection
type BvodWeekMergeSelectionNormalized = WeekMergeSelectionNormalized
 ...
 */
const BVOD_EXPERT_ROW_VIRTUALIZATION = true
const BVOD_EXPERT_ROW_HEIGHT_PX = OOH_EXPERT_ROW_HEIGHT_PX
const BVOD_EXPERT_ROW_OVERSCAN = OOH_EXPERT_ROW_OVERSCAN
...
export function BVODExpertGrid({
  ...
  const normalizeRowCacheRef = useRef(
    new Map<string, NormalizeRowCacheEntry<BvodExpertScheduleRow>>()
  )
  ...
  const rowMergeMapCacheRef = useRef(
    new Map<
      string,
      MapBuildCacheEntry<
        BvodExpertScheduleRow["mergedWeekSpans"],
        BvodRowMergeMap
      >
    >()
  )
  ...
  const pushRows = useCallback(
    (next: BvodExpertScheduleRow[]) => {
      const withDates = finalizeRowsPreservingIdentity(next, (r) => {
        ...
        const dates = deriveBvodExpertRowScheduleYmdFromRow(
          cleared,
          weekColumns,
          campaignStartDate,
          campaignEndDate,
  ...
  const rowReorderVirtual = useMemo(
    () =>
      BVOD_EXPERT_ROW_VIRTUALIZATION
        ? {
            rowCount: normalizedRows.length,
            estimateSizePx: BVOD_EXPERT_ROW_HEIGHT_PX,
  ...
  const ensureRowVisible = useCallback((rowIndex: number) => {
    if (!BVOD_EXPERT_ROW_VIRTUALIZATION) return
  ...
  const widthForWeekKey = useCallback(
    (k: string): number => {
      ...
      return weekColumnWidths[k] ?? BVOD_EXPERT_WEEK_COL_WIDTH_PX
    },
            ...
            ? Math.min(
                ...
                Math.max(
                  ...
                  deltaDaysFromPx(prev.edgeYmd, dx, {
                    ...
                    dayKeysByWeekKey,
                    weekColumnWidths,
                    defaultWeekColWidthPx: BVOD_EXPERT_WEEK_COL_WIDTH_PX,
  ...
  const {
    ...
    getScrollElement: () => gridScrollRef.current,
    estimateSize: BVOD_EXPERT_ROW_HEIGHT_PX,
    overscan: BVOD_EXPERT_ROW_OVERSCAN,
  ...
  const virtualSpacerColSpan = useMemo(() => {
    ...
    return (
      1 + bvodDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekCells
    )
  }, [
    weekColumns,
    expandedWeekKeys,
  ...
  const containerTotals = useMemo(() => {
    ...
    for (const row of normalizedRows) {
      const { net, fee } = expertRowCostSplit(row, weekKeys, feebvod)
      sumNet += net
      sumFee += fee
      for (const k of weekKeys) {
  ...
  }, [dayKeysByWeekKey, feebvod, normalizedRows, weekKeys])
                  ...
                  <thead ref={theadRef} className="[&_tr]:border-b-0">
                    <tr>
                      <ExpertGridRowReorderHeaderCell
                        ...
                        style={bvodExpertHeaderCellBgStyle}
                      />
                    ...
                    {(() => {
                      ...
                      return (
                        <MemoExpertGridRow
                          ...
                          rowUiSig={rowUiSig}
                          render={() => {
                      const net = expertRowNetMedia(row, weekKeys, feebvod)
                      ...
                      return (
                        <tr
                          ...
                          data-bvod-expert-row-index={rowIndex}
                          style={{
                            ...stripeStyle,
                            height: BVOD_EXPERT_ROW_HEIGHT_PX,
                            maxHeight: BVOD_EXPERT_ROW_HEIGHT_PX,
                          }}
                          ...
                          {(() => {
                            ...
                            for (let wi = 0; wi < weekColumns.length; wi += 1) {
                              ...
                              const tdClassName = cn(
                                ...
                                !isMergedAnchorCell &&
                                  mergeReadyOnCell &&
                                  "bvod-expert-week-cell--merge-ready z-[6] bg-pacing-behind-bg ring-2 ring-inset ring-pacing-behind shadow-e0",
                      ...
                      if (!BVOD_EXPERT_ROW_VIRTUALIZATION) {
