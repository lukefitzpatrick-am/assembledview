...
- **MBA billing override row precedence (MB-20):** `pendingBillingOverrideRows` (Applied, unsaved) > `billingOverrideRowsForPanels` (saved table) > computed auto. Pending is derived only via `layerDraftMonthsOntoOverrideRows` (`buildPendingBillingOverrideRows`). Do not invent a second draft→rows path. Apply is state-only (MB-23); Cancel / X / Escape discard pending; Done keeps it. Campaign save is the only commit point (MB-22 REPLACE-SET).
