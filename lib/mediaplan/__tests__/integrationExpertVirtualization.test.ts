/**
 * F-28 Phase 2 — Integration Expert row virtualization smoke (Prompt A items 1,3,5,7,8).
 */
import assert from "node:assert/strict"
import test from "node:test"
import { format, startOfDay } from "date-fns"

import { mapIntegrationExpertRowsToStandardLineItems } from "@/lib/mediaplan/expertChannelMappings"
import {
  reorderExpertRows,
  mergedSpanWidthPx,
...
const INTEGRATION_EXPERT_ROW_OVERSCAN = EXPERT_GRID_ROW_OVERSCAN_DEFAULT
...
function emptyRow(id: string): IntegrationExpertScheduleRow {
...
function buildRows(n: number): IntegrationExpertScheduleRow[] {
  return Array.from({ length: n }, (_, i) => emptyRow(`integration-v-${i}`))
}

/** Mirrors IntegrationExpertGrid containerTotals over the FULL logical array. */
function computeIntegrationWeeklyTotals(
...
test("3. DRAG REORDER across virtual boundary lands on correct logical index", () => {
  ...
  const dropIndex = virtualRowIndexFromOffsetY(
    280 * INTEGRATION_EXPERT_ROW_HEIGHT_PX + 1,
    INTEGRATION_EXPERT_ROW_HEIGHT_PX,
    300
  )
...
test("5. WEEK-COLUMN WIDTH respects resize overrides (sticky header/week resize path)", () => {
  ...
  assert.equal(INTEGRATION_EXPERT_ROW_HEIGHT_PX, 41)
})
...
test("7. WEEKLY TOTALS derived from full 300-row state", () => {
  ...
  const totals = computeIntegrationWeeklyTotals(rows, 0)
  ...
  const after = computeIntegrationWeeklyTotals(edited, 0)
...
test("8. PERF: mounted DOM window stays bounded while scrolling 300 rows", () => {
  ...
  const nearTop = expectedMountedRowRange(
    ...
    INTEGRATION_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    INTEGRATION_EXPERT_ROW_OVERSCAN
  )
  const mid = expectedMountedRowRange(
    150 * INTEGRATION_EXPERT_ROW_HEIGHT_PX,
    viewportHeight,
    INTEGRATION_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    INTEGRATION_EXPERT_ROW_OVERSCAN
  ...
  const nearBottom = expectedMountedRowRange(
    280 * INTEGRATION_EXPERT_ROW_HEIGHT_PX,
    viewportHeight,
    INTEGRATION_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    INTEGRATION_EXPERT_ROW_OVERSCAN
  ...
  const maxWindow =
    Math.ceil(viewportHeight / INTEGRATION_EXPERT_ROW_HEIGHT_PX) +
    2 * INTEGRATION_EXPERT_ROW_OVERSCAN +
    2
