...
function line(bursts: any[]): StandardInfluencersFormLineItem {
  return {
    platform: "P", objective: "", campaign: "", bidStrategy: "", buyType: "bonus", targetingAttribute: "", creativeTargeting: "", creative: "", buyingDemo: "", market: "",
...
test("sub-week interior burst imports as day-detail and round-trips", () => {
  ...
  const rows = mapStandardInfluencersLineItemsToExpertRows(std, cols, CS, CE)
  const wk0 = cols[0]!
  assert.equal(weekHasDailyValues(rows[0]!.dailyValues ?? {}, weekDayKeys(wk0, CS, CE)), true)
  const back = mapInfluencersExpertRowsToStandardLineItems(rows, cols, CS, CE, { feePctInfluencers: 0 })
