...
| `campaign_reads` | 0 (0079 not applied) | Dashboard six-beat campaign read. One row per generation. `status` draft\|published. Publish unpublishes earlier rows for the same mba+version. Edits update a row in place and revert published → draft. RLS on. No `ava_readonly` grant |
