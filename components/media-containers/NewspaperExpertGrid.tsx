...
import {
  ...
  WEEK_COL_WIDTH_PX as NEWSPAPER_EXPERT_WEEK_COL_WIDTH_PX,
  WEEK_SCROLLER_EDGE as NEWSPAPER_EXPERT_WEEK_SCROLLER_EDGE,
  WEEK_CELL_VISUAL_CLASSES as NEWSPAPER_WEEK_CELL_VISUAL_CLASSES,
  ...
  normalizeWeekRect as normalizeNewspaperWeekRect,
  weekCellInRect as newspaperWeekCellInRect,
  ...
  normalizeWeekMergeSelection as normalizeNewspaperWeekMergeSelection,
  weekPlainClickPreservesWeekAreaSelection,
  deriveMergeEligibility as deriveNewspaperMergeEligibility,
  weekCellExportText,
...
type NewspaperWeekRectSelection = ExpertWeekRectSelection
type NewspaperMultiCellSelection = ExpertMultiCellSelection
type NewspaperWeekMergeSelectionNormalized = WeekMergeSelectionNormalized
 ...
 */
const NEWSPAPER_EXPERT_ROW_VIRTUALIZATION = true
const NEWSPAPER_EXPERT_ROW_HEIGHT_PX = OOH_EXPERT_ROW_HEIGHT_PX
const NEWSPAPER_EXPERT_ROW_OVERSCAN = OOH_EXPERT_ROW_OVERSCAN
...
export function NewspaperExpertGrid({
  ...
  const normalizeRowCacheRef = useRef(
    new Map<string, NormalizeRowCacheEntry<NewspaperExpertScheduleRow>>()
  )
  ...
  const rowMergeMapCacheRef = useRef(
    new Map<
      string,
      MapBuildCacheEntry<
        NewspaperExpertScheduleRow["mergedWeekSpans"],
        NewspaperRowMergeMap
      >
    >()
  )
  ...
  const pushRows = useCallback(
    (next: NewspaperExpertScheduleRow[]) => {
      const withDates = finalizeRowsPreservingIdentity(next, (r) => {
        ...
        const dates = deriveNewspaperExpertRowScheduleYmdFromRow(
          cleared,
          weekColumns,
          campaignStartDate,
          campaignEndDate,
  ...
  const rowReorderVirtual = useMemo(
    () =>
      NEWSPAPER_EXPERT_ROW_VIRTUALIZATION
        ? {
            rowCount: normalizedRows.length,
            estimateSizePx: NEWSPAPER_EXPERT_ROW_HEIGHT_PX,
  ...
  const ensureRowVisible = useCallback((rowIndex: number) => {
    if (!NEWSPAPER_EXPERT_ROW_VIRTUALIZATION) return
  ...
  const widthForWeekKey = useCallback(
    (k: string): number => {
      ...
      return weekColumnWidths[k] ?? NEWSPAPER_EXPERT_WEEK_COL_WIDTH_PX
    },
            ...
            ? Math.min(
                ...
                Math.max(
                  ...
                  deltaDaysFromPx(prev.edgeYmd, dx, {
                    ...
                    dayKeysByWeekKey,
                    weekColumnWidths,
                    defaultWeekColWidthPx: NEWSPAPER_EXPERT_WEEK_COL_WIDTH_PX,
  ...
  const {
    ...
    getScrollElement: () => gridScrollRef.current,
    estimateSize: NEWSPAPER_EXPERT_ROW_HEIGHT_PX,
    overscan: NEWSPAPER_EXPERT_ROW_OVERSCAN,
  ...
  const virtualSpacerColSpan = useMemo(() => {
    ...
    return (
      1 + newspaperDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekCells
    )
  }, [
    weekColumns,
    expandedWeekKeys,
  ...
  const containerTotals = useMemo(() => {
    ...
    for (const row of normalizedRows) {
      const { net, fee } = expertRowCostSplit(row, weekKeys, feenewspapers)
      sumNet += net
      sumFee += fee
      for (const k of weekKeys) {
  ...
  }, [dayKeysByWeekKey, feenewspapers, normalizedRows, weekKeys])
        ...
        <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2 border-b border-border/60 bg-muted/5 pb-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <Label htmlFor="newspaper-expert-row-count" className="sr-only">Add rows count</Label>
              <Input
                        ...
                        return (
                          ...
                          >
                            ...
                            <ExpertGridWeekResizeHandle
                              weekKey={col.weekKey}
                              currentWidth={weekColumnWidths[col.weekKey] ?? NEWSPAPER_EXPERT_WEEK_COL_WIDTH_PX}
                    ...
                    {(() => {
                      ...
                      return (
                        <MemoExpertGridRow
                          ...
                          rowUiSig={rowUiSig}
                          render={() => {
                      const net = expertRowNetMedia(row, weekKeys, feenewspapers)
                      ...
                      return (
                        <tr
                          ...
                          style={{
                            ...stripeStyle,
                            height: NEWSPAPER_EXPERT_ROW_HEIGHT_PX,
                            maxHeight: NEWSPAPER_EXPERT_ROW_HEIGHT_PX,
                          }}
                          ...
                          {(() => {
                            ...
                            for (let wi = 0; wi < weekColumns.length; wi += 1) {
                              ...
                              const tdClassName = cn(
                                ...
                                !isMergedAnchorCell &&
                                  mergeReadyOnCell &&
                                  "newspaper-expert-week-cell--merge-ready z-[6] bg-pacing-behind-bg ring-2 ring-inset ring-pacing-behind shadow-e0",
                      ...
                      if (!NEWSPAPER_EXPERT_ROW_VIRTUALIZATION) {
