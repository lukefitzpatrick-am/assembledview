...
const form = useForm<MagazinesFormValues>({
  ...
  useStableHydration(
    ...
    (items) => {
      ...
      form.reset({
        magazineslineItems: stampBurstReactKeys(transformedLineItems),
  ...
  const handleValueChange = useCallback((lineItemIndex: number, burstIndex: number, budgetIncludesFeesOverride?: boolean) => {
    ...
    const buyTypeRaw = form.getValues(`magazineslineItems.${lineItemIndex}.buyType`);
    ...
    const bt = coerceBuyTypeWithDevWarn(String(buyTypeRaw || ""), "MagazinesContainer.handleValueChange");
