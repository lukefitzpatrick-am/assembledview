...
import {
  ...
  WEEK_COL_WIDTH_PX as PROGAUDIO_EXPERT_WEEK_COL_WIDTH_PX,
  WEEK_SCROLLER_EDGE as PROGAUDIO_EXPERT_WEEK_SCROLLER_EDGE,
  WEEK_CELL_VISUAL_CLASSES as PROGAUDIO_WEEK_CELL_VISUAL_CLASSES,
  ...
  normalizeWeekRect as normalizeProgAudioWeekRect,
  weekCellInRect as progAudioWeekCellInRect,
  ...
  normalizeWeekMergeSelection as normalizeProgAudioWeekMergeSelection,
  weekPlainClickPreservesWeekAreaSelection,
  deriveMergeEligibility as deriveProgAudioMergeEligibility,
  weekCellExportText,
...
type ProgAudioWeekRectSelection = ExpertWeekRectSelection
type ProgAudioMultiCellSelection = ExpertMultiCellSelection
type ProgAudioWeekMergeSelectionNormalized = WeekMergeSelectionNormalized
 ...
 */
const PROGAUDIO_EXPERT_ROW_VIRTUALIZATION = true
const PROGAUDIO_EXPERT_ROW_HEIGHT_PX = OOH_EXPERT_ROW_HEIGHT_PX
const PROGAUDIO_EXPERT_ROW_OVERSCAN = OOH_EXPERT_ROW_OVERSCAN
...
export function ProgAudioExpertGrid({
  ...
  const normalizeRowCacheRef = useRef(
    new Map<string, NormalizeRowCacheEntry<ProgAudioExpertScheduleRow>>()
  )
  ...
  const rowMergeMapCacheRef = useRef(
    new Map<
      string,
      MapBuildCacheEntry<
        ProgAudioExpertScheduleRow["mergedWeekSpans"],
        ProgAudioRowMergeMap
      >
    >()
  )
  ...
  const rowReorderVirtual = useMemo(
    () =>
      PROGAUDIO_EXPERT_ROW_VIRTUALIZATION
        ? {
            rowCount: normalizedRows.length,
            estimateSizePx: PROGAUDIO_EXPERT_ROW_HEIGHT_PX,
  ...
  const ensureRowVisible = useCallback((rowIndex: number) => {
    if (!PROGAUDIO_EXPERT_ROW_VIRTUALIZATION) return
  ...
  const widthForWeekKey = useCallback(
    (k: string): number => {
      ...
      return weekColumnWidths[k] ?? PROGAUDIO_EXPERT_WEEK_COL_WIDTH_PX
    },
            ...
            ? Math.min(
                ...
                Math.max(
                  ...
                  deltaDaysFromPx(prev.edgeYmd, dx, {
                    ...
                    dayKeysByWeekKey,
                    weekColumnWidths,
                    defaultWeekColWidthPx: PROGAUDIO_EXPERT_WEEK_COL_WIDTH_PX,
  ...
  const {
    ...
    getScrollElement: () => gridScrollRef.current,
    estimateSize: PROGAUDIO_EXPERT_ROW_HEIGHT_PX,
    overscan: PROGAUDIO_EXPERT_ROW_OVERSCAN,
  ...
  const virtualSpacerColSpan = useMemo(() => {
    ...
    return (
      1 + progAudioDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekCells
    )
  }, [
    weekColumns,
    expandedWeekKeys,
  ...
  const containerTotals = useMemo(() => {
    ...
    for (const row of normalizedRows) {
      const { net, fee } = expertRowCostSplit(row, weekKeys, feeprogaudio)
      sumNet += net
      sumFee += fee
      for (const k of weekKeys) {
  ...
  }, [dayKeysByWeekKey, feeprogaudio, normalizedRows, weekKeys])
        ...
        <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2 border-b border-border/60 bg-muted/5 pb-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <Label htmlFor="progaudio-expert-row-count" className="sr-only">Add rows count</Label>
              <Input
                  ...
                  <thead ref={theadRef} className="[&_tr]:border-b-0">
                    <tr>
                      <ExpertGridRowReorderHeaderCell
                        ...
                        style={progAudioExpertHeaderCellBgStyle}
                      />
                        ...
                        return (
                          ...
                          >
                            ...
                            <ExpertGridWeekResizeHandle
                              weekKey={col.weekKey}
                              currentWidth={weekColumnWidths[col.weekKey] ?? PROGAUDIO_EXPERT_WEEK_COL_WIDTH_PX}
                    ...
                    {(() => {
                      ...
                      return (
                        <MemoExpertGridRow
                          ...
                          rowUiSig={rowUiSig}
                          render={() => {
                      const net = expertRowNetMedia(row, weekKeys, feeprogaudio)
                      ...
                      return (
                        <tr
                          ...
                          data-progaudio-expert-row-index={rowIndex}
                          style={{
                            ...stripeStyle,
                            height: PROGAUDIO_EXPERT_ROW_HEIGHT_PX,
                            maxHeight: PROGAUDIO_EXPERT_ROW_HEIGHT_PX,
                          }}
                          ...
                          {(() => {
                            ...
                            for (let wi = 0; wi < weekColumns.length; wi += 1) {
                              ...
                              const tdClassName = cn(
                                ...
                                !isMergedAnchorCell &&
                                  mergeReadyOnCell &&
                                  "progaudio-expert-week-cell--merge-ready z-[6] bg-pacing-behind-bg ring-2 ring-inset ring-pacing-behind shadow-e0",
                      ...
                      if (!PROGAUDIO_EXPERT_ROW_VIRTUALIZATION) {
