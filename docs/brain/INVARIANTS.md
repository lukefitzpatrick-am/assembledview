...
- `DATA_BACKEND` defaults to `xano`. Per-domain `DATA_BACKEND_<DOMAIN>` overrides when set. `shadow` serves Xano and must not alter user-visible payloads; `postgres` serves Supabase for wired domains only (reference → publishers/clients → …). New domains opt in one PR at a time.
