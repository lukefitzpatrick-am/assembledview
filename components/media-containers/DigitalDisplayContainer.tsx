...
} from "@/components/media-containers/DigitalDisplayExpertGrid"
...
export default function DigiDisplayContainer({
  ...
  const [expertDigiDisplayRows, setExpertDigiDisplayRows] = useState<
    DigitalDisplayExpertScheduleRow[]
  >([])
  const [digiDisplayExpertModalOpen, setDigiDisplayExpertModalOpen] =
    useState(false)
  ...
  if (currentLineItemIndexForNewSite !== null) {
    form.setValue(`digidisplaylineItems.${currentLineItemIndexForNewSite}.publisher`, createdSite.platform, { shouldDirty: true })
    form.setValue(`digidisplaylineItems.${currentLineItemIndexForNewSite}.platform`, createdSite.platform, { shouldDirty: true })
    form.setValue(`digidisplaylineItems.${currentLineItemIndexForNewSite}.site`, createdSite.site, { shouldDirty: true })
  }
  ...
  const handleExpertDigiDisplayRowsChange = useCallback(
    (next: DigitalDisplayExpertScheduleRow[]) => {
      setExpertDigiDisplayRows(next)
    },
    []
  )
  ...
  const openDigiDisplayExpertModal = useCallback(() => {
    ...
    const weekKeys = digiDisplayExpertWeekColumns.map((c) => c.weekKey)
    const rows: DigitalDisplayExpertScheduleRow[] =
      mapped.length > 0
        ? mapped
        : [
            createEmptyDigitalDisplayExpertRow(
    ...
    digiDisplayExpertRowsBaselineRef.current =
      serializeDigiDisplayExpertRowsBaseline(rows)
    setExpertDigiDisplayRows(rows)
  ...
  const handleDigiDisplayExpertApply = useCallback(() => {
    const prevLineItems = form.getValues("digidisplaylineItems") || []
    const standard = mapDigitalDisplayExpertRowsToStandardLineItems(
      expertDigiDisplayRows,
      digiDisplayExpertWeekColumns,
      campaignStartDate,
      campaignEndDate,
      {
        feePctDigiDisplay: feedigidisplay,
...
}, [watchedLineItems, feedigidisplay, mbaNumber, form, onLineItemsChange]);
          ...
          <CardHeader className="pb-3">
            <div className="flex flex-col gap-3">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div
                  role="group"
                  aria-label="Digital Display entry mode"
                                        ...
                                        <div className="flex-1 flex items-center space-x-1">
                                          <FormControl>
                                            <Combobox
                                              ...
                                              options={filteredDigiDisplaySites.map((digidisplaySite) => ({
                                                value: digidisplaySite.site || `site-${digidisplaySite.id}`,
                                                label: digidisplaySite.site || "(Unnamed site)",
                                              }))}
                                            />
                                          </FormControl>
                                            <Button
...
<FormItem>
  ...
  <FormControl>
                                    ...
                                    {(buyType === "cpc" || buyType === "cpv") && (
                                      <FormField
                                        control={form.control}
                                        name={`digidisplaylineItems.${lineItemIndex}.bursts.${burstIndex}.adServingRatePct`}
                                        render={({ field }) => (
                                          <FormItem>
                                    ...
                                    {buyType === "fixed_cost" && (
                                      <FormField
                                        control={form.control}
                                        name={`digidisplaylineItems.${lineItemIndex}.bursts.${burstIndex}.adServingImpressions`}
                                        render={({ field }) => (
                                          <FormItem>
      ...
      <Dialog
        open={digiDisplayExpertModalOpen}
        onOpenChange={handleDigiDisplayExpertModalOpenChange}
      >
        <DialogContent className="max-w-[95vw] w-[95vw] max-h-[95vh] h-[95vh] flex flex-col p-4 gap-0 overflow-hidden">
          ...
          <DialogFooter className="flex-shrink-0 border-t pt-3 mt-2">
            <div className="mr-auto flex flex-col gap-1.5">
              <ExpertIncompleteRowsSummary rows={expertDigiDisplayRows} />
              {expertApplyPendingPageSave ? (
            ...
            <Button type="button" onClick={handleDigiDisplayExpertApply}>
      ...
      <Dialog
        open={digiDisplayExpertExitConfirmOpen}
        onOpenChange={(open) => {
          if (!open) dismissDigiDisplayExpertExitConfirm()
        }}
      >
        <DialogContent
          ...
          onClick={(e) => {
            if (
              (e.target as HTMLElement).closest(
                "[data-digitaldisplay-expert-exit-yes]"
              )
            ) {
              return
            }
            dismissDigiDisplayExpertExitConfirm()
          }}
        >
          <DialogHeader>
            <DialogTitle>Leave Digital Display Expert Mode?</DialogTitle>
            <DialogDescription>
              You have unsaved changes in Expert Mode. Apply saves them to the
              Digital Display section; leaving now discards those edits.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
              type="button"
              variant="outline"
              onClick={dismissDigiDisplayExpertExitConfirm}
            >
              No, keep editing
            </Button>
            <Button
              ...
              variant="destructive"
              data-digitaldisplay-expert-exit-yes
              onClick={confirmDigiDisplayExpertExitWithoutSaving}
            >
