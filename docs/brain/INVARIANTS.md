...
- On publish, `media_plan_versions.approved_slice` is frozen once (`{ totalCents, lines[] }`); never mutate after write. Missing slice at MBA document render is derived from billing `schedule_months` at read time (`deriveApprovedSliceFromScheduleRows`); the derived object is never written back. Widened C1 gate (`SAVE_GATE_FULL_SCOPE=off|log|enforce`, default **off**) compares billing `schedule_months` full scope to `approved_slice.totalCents` ± 1¢. Fee-component `billing_overrides` attached at first publish flow into slice `feeCents` via override-aware financials. `mba_fee_snapshots.fees` is fee-rate JSON, not override dollars.
...
- MBA / media-plan doc APIs render from persisted `schedule_months` + `approved_slice` + fee snapshot (`{mba_number, version_number}`); when `approved_slice` was never frozen, `buildMbaFromPersisted` derives it from billing `schedule_months` at read time and does **not** persist it. Require admin; 422 unless campaign_status is past Draft. Footer `v{n} · {hash8}` from `snapshot_checksum` (sha256; written on publish). `/api/mba/generate` returns `X-Slice-Source: persisted|derived` for support (not shown in the PDF).
