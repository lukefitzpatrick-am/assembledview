...
export interface GroupedLineItemForKPI {
  ...
  oohType: string
  market: string
  spend: number       // sum of grossMedia across all bursts
  deliverables: number // sum of deliverables across all bursts
}
...
export function groupLineItemsForKPI(items: LineItem[]): GroupedLineItemForKPI[] {
  ...
  for (const item of items) {
    // Resolve stable lineItemId — containers use either field name
    const id: string =
    ...
    const spend =
      parseFloat(String((item as any).grossMedia ?? (item as any).totalMedia ?? '0').replace(/[^0-9.-]/g, '')) || 0
    const deliverables =
      parseFloat(String((item as any).deliverables ?? (item as any).calculatedValue ?? '0').replace(/[^0-9.-]/g, '')) || 0
    ...
    if (map.has(id)) {
      ...
      map.set(id, {
        ...
        targeting:   String((item as any).targeting   ?? (item as any).creativeTargeting ?? (item as any).creative_targeting ?? ''),
