...
- Fee snapshot on publish is write-once when `FEE_SNAPSHOT_WRITE_ONCE=on` (default **off** until Luke confirms audit §5.3); overwrite only via `POST /api/admin/fee-snapshots/resnapshot` (reason required, audited to `finance_edits`).
