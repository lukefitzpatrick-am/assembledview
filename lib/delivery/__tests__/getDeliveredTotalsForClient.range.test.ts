/**
 * getDeliveredTotalsForClient — no range must match the unbounded loader call.
 */
...
const campaign = { mbaNumber: "foo001", mediaTypes: ["Search"] }

test(
  "getDeliveredTotalsForClient with no range equals the unbounded call",
  { skip },
  async () => {
    const { getDeliveredTotalsForClient } = await import("../getDeliveredTotalsForClient.js")

    loadDeliverySnapshot.mock.resetCalls()
    const unbounded = await getDeliveredTotalsForClient([campaign])
    ...
    loadDeliverySnapshot.mock.resetCalls()
    const explicitEmpty = await getDeliveredTotalsForClient([campaign], {
      startDate: null,
      endDate: null,
    })
...
test(
  "getDeliveredTotalsForClient passes DATE_DAY from/to when a range is set",
  { skip },
  async () => {
    const { getDeliveredTotalsForClient } = await import("../getDeliveredTotalsForClient.js")
    loadDeliverySnapshot.mock.resetCalls()
    await getDeliveredTotalsForClient([campaign], {
