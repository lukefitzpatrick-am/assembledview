...
- **Prompt 2 IN PROGRESS:** `db/schema/*.ts` authored from the two SQL files (45 tables); `db/index.ts`, `drizzle.config.ts`, npm `db:*` scripts, `lineItemAttrs` zod + golden test green; `drizzle-kit generate` is empty against the local baseline in `db/drizzle/`. **Blocked on live-DB empty-diff gate** until `DATABASE_URL` + `DIRECT_URL` are in `.env.local` (pooler 6543 + direct 5432). Do not run `db:migrate` against Supabase — schema already applied via SQL Editor.
