/**
 * F-28 Phase 2 — Digital Display Expert row virtualization smoke (Prompt A items 1,3,5,7,8).
 */
import assert from "node:assert/strict"
import test from "node:test"
import { format, startOfDay } from "date-fns"

import { mapDigitalDisplayExpertRowsToStandardLineItems } from "@/lib/mediaplan/expertChannelMappings"
import {
  reorderExpertRows,
  mergedSpanWidthPx,
...
const DIGITALDISPLAY_EXPERT_ROW_OVERSCAN = EXPERT_GRID_ROW_OVERSCAN_DEFAULT
...
function emptyRow(id: string): DigitalDisplayExpertScheduleRow {
...
function buildRows(n: number): DigitalDisplayExpertScheduleRow[] {
  return Array.from({ length: n }, (_, i) => emptyRow(`digitaldisplay-v-${i}`))
}

/** Mirrors DigitalDisplayExpertGrid containerTotals over the FULL logical array. */
function computeDigitalDisplayWeeklyTotals(
...
test("3. DRAG REORDER across virtual boundary lands on correct logical index", () => {
  ...
  const dropIndex = virtualRowIndexFromOffsetY(
    280 * DIGITALDISPLAY_EXPERT_ROW_HEIGHT_PX + 1,
    DIGITALDISPLAY_EXPERT_ROW_HEIGHT_PX,
    300
  )
...
test("5. WEEK-COLUMN WIDTH respects resize overrides (sticky header/week resize path)", () => {
  ...
  assert.equal(DIGITALDISPLAY_EXPERT_ROW_HEIGHT_PX, 41)
})
...
test("7. WEEKLY TOTALS derived from full 300-row state", () => {
  ...
  const totals = computeDigitalDisplayWeeklyTotals(rows, 0)
  ...
  const after = computeDigitalDisplayWeeklyTotals(edited, 0)
...
test("8. PERF: mounted DOM window stays bounded while scrolling 300 rows", () => {
  ...
  const nearTop = expectedMountedRowRange(
    ...
    DIGITALDISPLAY_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    DIGITALDISPLAY_EXPERT_ROW_OVERSCAN
  )
  const mid = expectedMountedRowRange(
    150 * DIGITALDISPLAY_EXPERT_ROW_HEIGHT_PX,
    viewportHeight,
    DIGITALDISPLAY_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    DIGITALDISPLAY_EXPERT_ROW_OVERSCAN
  ...
  const nearBottom = expectedMountedRowRange(
    280 * DIGITALDISPLAY_EXPERT_ROW_HEIGHT_PX,
    viewportHeight,
    DIGITALDISPLAY_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    DIGITALDISPLAY_EXPERT_ROW_OVERSCAN
  ...
  const maxWindow =
    Math.ceil(viewportHeight / DIGITALDISPLAY_EXPERT_ROW_HEIGHT_PX) +
    2 * DIGITALDISPLAY_EXPERT_ROW_OVERSCAN +
    2
