/**
 * F-28 Phase 2 — Production Expert row virtualization smoke (Prompt A items 1,3,5,7,8).
 */
import assert from "node:assert/strict"
import test from "node:test"
import { format, startOfDay } from "date-fns"

import { mapProductionExpertRowsToStandardLineItems } from "@/lib/mediaplan/expertChannelMappings"
import {
  reorderExpertRows,
  mergedSpanWidthPx,
...
import { expertRowGrossCost } from "@/lib/mediaplan/expertRowCost"
import type {
  ExpertWeeklyValues,
...
const PRODUCTION_EXPERT_ROW_OVERSCAN = EXPERT_GRID_ROW_OVERSCAN_DEFAULT
...
function emptyRow(id: string): ProductionExpertScheduleRow {
...
function buildRows(n: number): ProductionExpertScheduleRow[] {
  return Array.from({ length: n }, (_, i) => emptyRow(`production-v-${i}`))
}

/** Mirrors ProductionExpertGrid containerTotals over the FULL logical array. */
function computeProductionWeeklyTotals(
  rows: readonly ProductionExpertScheduleRow[]
) {
  let sumGross = 0
  let sumQty = 0
...
test("1. APPLY CORRECTNESS: edit on off-window row survives Apply (full state, not DOM)", () => {
  ...
  const patched = updateRowAtIndex(rows, offWindowIndex, {
    description: "OFF-WINDOW-DESCRIPTION-MARK",
  ...
  assert.equal(standard[offWindowIndex]!.description, "OFF-WINDOW-DESCRIPTION-MARK")
  assert.equal(standard[offWindowIndex]!.publisher, "OFF-WINDOW-PUBLISHER")
  assert.equal(standard[0]!.description, "")
  assert.notEqual(standard[0]!.description, "OFF-WINDOW-DESCRIPTION-MARK")
})
...
test("3. DRAG REORDER across virtual boundary lands on correct logical index", () => {
  ...
  const dropIndex = virtualRowIndexFromOffsetY(
    280 * PRODUCTION_EXPERT_ROW_HEIGHT_PX + 1,
    PRODUCTION_EXPERT_ROW_HEIGHT_PX,
    300
  )
  ...
  assert.ok(next)
  assert.equal(next![280]!.description, "P2")
  assert.equal(next![2]!.description, "P3")
  ...
  assert.equal(standard[280]!.description, "P2")
...
test("5. WEEK-COLUMN WIDTH respects resize overrides (sticky header/week resize path)", () => {
  ...
  assert.equal(PRODUCTION_EXPERT_ROW_HEIGHT_PX, 41)
})
...
test("7. WEEKLY TOTALS derived from full 300-row state", () => {
  ...
  const totals = computeProductionWeeklyTotals(rows)
  ...
  const after = computeProductionWeeklyTotals(edited)
...
test("8. PERF: mounted DOM window stays bounded while scrolling 300 rows", () => {
  ...
  const nearTop = expectedMountedRowRange(
    ...
    PRODUCTION_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    PRODUCTION_EXPERT_ROW_OVERSCAN
  )
  const mid = expectedMountedRowRange(
    150 * PRODUCTION_EXPERT_ROW_HEIGHT_PX,
    viewportHeight,
    PRODUCTION_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    PRODUCTION_EXPERT_ROW_OVERSCAN
  ...
  const nearBottom = expectedMountedRowRange(
    280 * PRODUCTION_EXPERT_ROW_HEIGHT_PX,
    viewportHeight,
    PRODUCTION_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    PRODUCTION_EXPERT_ROW_OVERSCAN
  ...
  const maxWindow =
    Math.ceil(viewportHeight / PRODUCTION_EXPERT_ROW_HEIGHT_PX) +
    2 * PRODUCTION_EXPERT_ROW_OVERSCAN +
    2
