...
import {
  ...
  normalizeWeekRect as normalizeMagazinesWeekRect,
  weekCellInRect as magazinesWeekCellInRect,
...
type MagazinesWeekRectSelection = ExpertWeekRectSelection
type MagazinesMultiCellSelection = ExpertMultiCellSelection
type MagazinesWeekMergeSelectionNormalized = WeekMergeSelectionNormalized
 ...
 */
const MAGAZINES_EXPERT_ROW_VIRTUALIZATION = true
const MAGAZINES_EXPERT_ROW_HEIGHT_PX = OOH_EXPERT_ROW_HEIGHT_PX
const MAGAZINES_EXPERT_ROW_OVERSCAN = OOH_EXPERT_ROW_OVERSCAN
...
export function MagazinesExpertGrid({
  ...
  const normalizeRowCacheRef = useRef(
    new Map<string, NormalizeRowCacheEntry<MagazinesExpertScheduleRow>>()
  )
  ...
  const rowMergeMapCacheRef = useRef(
    new Map<
      string,
      MapBuildCacheEntry<
        MagazinesExpertScheduleRow["mergedWeekSpans"],
        MagazinesRowMergeMap
      >
    >()
  )
  ...
  const pushRows = useCallback(
    (next: MagazinesExpertScheduleRow[]) => {
      const withDates = finalizeRowsPreservingIdentity(next, (r) => {
        ...
        const dates = deriveMagazineExpertRowScheduleYmdFromRow(
          cleared,
          weekColumns,
          campaignStartDate,
          campaignEndDate,
  ...
  const rowReorderVirtual = useMemo(
    () =>
      MAGAZINES_EXPERT_ROW_VIRTUALIZATION
        ? {
            rowCount: normalizedRows.length,
            estimateSizePx: MAGAZINES_EXPERT_ROW_HEIGHT_PX,
  ...
  const ensureRowVisible = useCallback((rowIndex: number) => {
    if (!MAGAZINES_EXPERT_ROW_VIRTUALIZATION) return
            ...
            ? Math.min(
                ...
                Math.max(
                  ...
                  deltaDaysFromPx(prev.edgeYmd, dx, {
                    ...
                    dayKeysByWeekKey,
                    weekColumnWidths,
                    defaultWeekColWidthPx: MAGAZINES_EXPERT_WEEK_COL_WIDTH_PX,
  ...
  const {
    ...
    getScrollElement: () => gridScrollRef.current,
    estimateSize: MAGAZINES_EXPERT_ROW_HEIGHT_PX,
    overscan: MAGAZINES_EXPERT_ROW_OVERSCAN,
  ...
  const virtualSpacerColSpan = useMemo(() => {
    ...
    return (
      1 + magazinesDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekCells
    )
  }, [
    weekColumns,
    expandedWeekKeys,
  ...
  const containerTotals = useMemo(() => {
    ...
    for (const row of normalizedRows) {
      const { net, fee } = expertRowCostSplit(row, weekKeys, feemagazines)
      sumNet += net
      sumFee += fee
      for (const k of weekKeys) {
        ...
        <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2 border-b border-border/60 bg-muted/5 pb-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <Label htmlFor="magazines-expert-row-count" className="sr-only">Add rows count</Label>
              <Input
                  ...
                  <thead ref={theadRef} className="[&_tr]:border-b-0">
                    <tr>
                      <ExpertGridRowReorderHeaderCell
                        ...
                        style={magazinesExpertHeaderCellBgStyle}
                      />
                    ...
                    {(() => {
                      ...
                      return (
                        <tr
                          ...
                          data-magazines-expert-row-index={rowIndex}
                          style={{
                            ...stripeStyle,
                            height: MAGAZINES_EXPERT_ROW_HEIGHT_PX,
                            maxHeight: MAGAZINES_EXPERT_ROW_HEIGHT_PX,
                          }}
                          ...
                          {(() => {
                            ...
                            for (let wi = 0; wi < weekColumns.length; wi += 1) {
                              ...
                              const tdClassName = cn(
                                ...
                                !isMergedAnchorCell &&
                                  mergeReadyOnCell &&
                                  "magazines-expert-week-cell--merge-ready z-[6] bg-pacing-behind-bg ring-2 ring-inset ring-pacing-behind shadow-e0",
                      ...
                      if (!MAGAZINES_EXPERT_ROW_VIRTUALIZATION) {
