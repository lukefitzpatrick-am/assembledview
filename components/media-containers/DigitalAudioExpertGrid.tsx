...
const DIGIAUDIO_EXPERT_WEEK_COL_WIDTH_PX = 112

/** Week column width on `<th>` and week `<td>`; inputs use `w-full` inside the cell. */
const digiAudioExpertWeekColLayoutStyle = {
  width: DIGIAUDIO_EXPERT_WEEK_COL_WIDTH_PX,
  minWidth: DIGIAUDIO_EXPERT_WEEK_COL_WIDTH_PX,
  maxWidth: DIGIAUDIO_EXPERT_WEEK_COL_WIDTH_PX,
  boxSizing: "border-box" as const,
}

const MEDIA_ACCENT_HEX = getMediaTypeThemeHex("digiaudio")

const digiAudioExpertHeaderCellBgStyle = {
  backgroundColor: rgbaFromHex(MEDIA_ACCENT_HEX, 0.08),
}

const digiAudioExpertTotalsRowBgStyle = {
  backgroundColor: rgbaFromHex(MEDIA_ACCENT_HEX, 0.06),
}

/** Right edge of sticky block before the horizontally scrolling week area. */
const DIGIAUDIO_EXPERT_WEEK_SCROLLER_EDGE =
...
): DigitalAudioExpertMergedWeekSpan | null {
...
type DigitalAudioMultiCellSelection = {
  startRow: number
  endRow: number
  startCol: number
  endCol: number
...
type DigitalAudioCopiedCells = {
  data: (string | number)[][]
  sourceRows: number
  sourceCols: number
  selection: DigitalAudioMultiCellSelection
}

function weeklyCellDisplayValue(
  cellValue: number | string | undefined,
  mergedSpan: DigitalAudioExpertMergedWeekSpan | null
): string {
  if (mergedSpan) {
...
const DIGIAUDIO_WEEK_CELL_VISUAL_CLASSES = {
...
function selectionOverlapsMergedSpan(
  rowMergeMap: DigitalAudioRowMergeMap | undefined,
...
type DigitalAudioWeekRectSelection = {
  rowStart: number
  rowEnd: number
  weekKeyStart: string
  weekKeyEnd: string
...
function normalizeDigitalAudioWeekRect(
  row0: number,
  wk0: string,
  row1: number,
  wk1: string,
  weekKeys: readonly string[]
): DigitalAudioWeekRectSelection {
...
function digiAudioWeekCellInRect(
  rowIndex: number,
  weekKey: string,
  rect: DigitalAudioWeekRectSelection | null,
  weekKeys: readonly string[]
): boolean {
...
function rectToMultiCellSelection(
  rect: DigitalAudioWeekRectSelection | null,
  weekKeys: readonly string[]
): DigitalAudioMultiCellSelection | null {
  if (!rect) return null
  const i0 = weekKeys.indexOf(rect.weekKeyStart)
  const i1 = weekKeys.indexOf(rect.weekKeyEnd)
...
function coerceWeeklySelectionBounds(
  ...
  weekRectSelection: DigitalAudioWeekRectSelection | null,
  ...
  if (
    weekRectSelection &&
    digiAudioWeekCellInRect(anchorRow, anchorWeekKey, weekRectSelection, weekKeys)
  ) {
...
function applyWeeklyPasteMatrixToSelection(args: {
  ...
  weekKeys: readonly string[]
  rowCount: number
  nextRows: DigitalAudioExpertScheduleRow[]
}): {
  applied: number
  errorReasons: string[]
  layout: WeeklyPasteLayoutMode
...
function oohMergeKeysFromRect(
  rect: DigitalAudioWeekRectSelection,
  weekKeys: readonly string[]
): string[] | null {
...
type DigitalAudioWeekMergeSelectionNormalized = {
  rowIndex: number
 ...
 */
function normalizeDigitalAudioWeekMergeSelection(
  rect: DigitalAudioWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): DigitalAudioWeekMergeSelectionNormalized | null {
...
function weekPlainClickPreservesWeekAreaSelection(
  ...
  const mergeNorm = normalizeDigitalAudioWeekMergeSelection(rect, multi, weekKeys)
  if (
    mergeNorm &&
  ...
  if (rect && digiAudioWeekCellInRect(rowIndex, weekKey, rect, weekKeys)) {
    const isSingleCell =
...
function deriveDigitalAudioMergeEligibility(
  rect: DigitalAudioWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): DigitalAudioWeekMergeSelectionNormalized | null {
  return normalizeDigitalAudioWeekMergeSelection(rect, multi, weekKeys)
}
...
function mergedWeekSpansAfterCutRect(
  spans: DigitalAudioExpertMergedWeekSpan[] | undefined,
  wi0: number,
  wi1: number,
  weekKeys: readonly string[]
): DigitalAudioExpertMergedWeekSpan[] {
...
type WeeklyExportSelection =
  | { kind: "rect"; rect: DigitalAudioWeekRectSelection }
...
function resolveWeeklyExportSelection(
  ...
  weekKeys: readonly string[],
  rows: readonly DigitalAudioExpertScheduleRow[] | null
...
function applyWeeklyCutToRows(
  ...
  rows: readonly DigitalAudioExpertScheduleRow[],
  weekKeys: readonly string[]
): DigitalAudioExpertScheduleRow[] | null {
  switch (selection.kind) {
    case "rect": {
      const rect = selection.rect
      const wi0 = Math.min(
      ...
      if (wi0 < 0 || wi1 < 0) return null
      const next: DigitalAudioExpertScheduleRow[] = rows.map((r) => ({
        ...r,
    ...
    case "mergeContiguous": {
      ...
      const next: DigitalAudioExpertScheduleRow[] = rows.map((row) => ({
        ...row,
...
/** Match labels/values on {@link DigitalAudioContainer} buy-type combobox. */
const DIGIAUDIO_BUY_TYPE_OPTIONS: ComboboxOption[] = [
...
function normalizeDigitalAudioBuyTypePaste(raw: string): string {
  const v = raw.trim()
  if (!v) return ""
  const byValue = DIGIAUDIO_BUY_TYPE_OPTIONS.find(
    (o) => o.value.toLowerCase() === v.toLowerCase()
  )
  if (byValue) return byValue.value
  const byLabel = DIGIAUDIO_BUY_TYPE_OPTIONS.find(
...
function normalizeDigitalAudioPublisherPaste(raw: string, publisherNames: string[]): string {
  const v = raw.trim()
  if (!v) return ""
...
function normalizeDigitalAudioSitePaste(raw: string, siteNames: string[]): string {
  const v = raw.trim()
  if (!v) return ""
...
export interface DigitalAudioExpertGridProps {
  ...
  /** Digital Audio sites (platform + site label) for site combobox + fuzzy matching */
  digiAudioSites?: {
...
type DigitalAudioRowMergeSpanMeta = Readonly<{
  id: string
  startWeekKey: string
  endWeekKey: string
  totalQty: number
...
type DigitalAudioRowMergeMap = Readonly<{
  anchorByWeekKey: Readonly<Record<string, string>>
  interiorByWeekKey: Readonly<Record<string, string>>
  spanById: Readonly<Record<string, DigitalAudioExpertMergedWeekSpan>>
  spanMetaByAnchorWeekKey: Readonly<Record<string, DigitalAudioRowMergeSpanMeta>>
}>
...
export function DigitalAudioExpertGrid({
  ...
  const [focusedCell, setFocusedCell] = useState<DigitalAudioExpertFocusedCell | null>(
    null
  )
  const focusedCellRef = useRef<DigitalAudioExpertFocusedCell | null>(null)
  ...
  const siteComboboxOptionsForRow = useCallback(
    (row: DigitalAudioExpertScheduleRow): ComboboxOption[] => {
      const key = normalizeDigitalAudioKey(row.publisher)
      const src = !key
        ? digiAudioSites
        : digiAudioSites.filter((s) => normalizeDigitalAudioKey(s.platform) === key)
      const names = [
        ...new Set(
          src
            .map((s) => String(s.site ?? "").trim())
  ...
  const [weekRectSelection, setWeekRectSelection] =
    useState<DigitalAudioWeekRectSelection | null>(null)
  // Multi-cell selection state for copy/paste operations.
  const [multiCellSelection, setMultiCellSelection] =
    useState<DigitalAudioMultiCellSelection | null>(null)
  const [isSelecting, setIsSelecting] = useState(false)
  const [copiedCells, setCopiedCells] = useState<DigitalAudioCopiedCells | null>(null)
  ...
  const lastDragRectDuringGestureRef = useRef<DigitalAudioWeekRectSelection | null>(null)
  /**
   ...
   */
  const postDragWeekClickRectRef = useRef<DigitalAudioWeekRectSelection | null>(null)
  const weekRectSelectionRef = useRef<DigitalAudioWeekRectSelection | null>(null)
  ...
  const rowMergeMaps = useMemo<readonly DigitalAudioRowMergeMap[]>(() => {
    const maps = normalizedRows.map((row) => {
      const anchorByWeekKey: Record<string, string> = {}
      const interiorByWeekKey: Record<string, string> = {}
      const spanById: Record<string, DigitalAudioExpertMergedWeekSpan> = {}
      const spanMetaByAnchorWeekKey: Record<string, DigitalAudioRowMergeSpanMeta> = {}
      ...
      for (const span of row.mergedWeekSpans ?? []) {
        if (spanById[span.id]) {
          if (DEBUG_DIGIAUDIO_MERGE) {
            console.debug("[Digital Audio merge] occupancy duplicate span id ignored", {
              rowId: row.id,
        ...
        if (keys.some((k) => occupiedWeekKeys.has(k))) {
          if (DEBUG_DIGIAUDIO_MERGE) {
            console.debug("[Digital Audio merge] occupancy overlap ignored", {
              rowId: row.id,
              rowIndex: normalizedRows.indexOf(row),
              spanId: span.id,
              keys,
  ...
  useEffect(() => {
    if (!DEBUG_DIGIAUDIO_MERGE) return
    ...
    console.debug("[Digital Audio merge] pending selection updated", pendingMergeSelection)
  ...
  const updateRow = useCallback(
    (rowIndex: number, patch: Partial<DigitalAudioExpertScheduleRow>) => {
  ...
  const tryFuzzyMatch = useCallback(
    (rowIndex: number, field: FuzzyMatchField, value: string) => {
      ...
      if (corrected) {
        updateRow(rowIndex, { [field]: corrected } as Partial<DigitalAudioExpertScheduleRow>)
        return
      }
      ...
      if (fuzzyMatchAutoApplyRef.current) {
        updateRow(rowIndex, { [field]: match.matched } as Partial<DigitalAudioExpertScheduleRow>)
      } else {
  ...
  const navColCount =
    digiAudioDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekKeys.length
  const firstWeekNavColIndex = digiAudioDescriptorKeys.length + WEEK_GRID_COL_OFFSET
  const unitRateNavColIndex = digiAudioDescriptorKeys.indexOf("unitRate")
  ...
  const unmergeWeekSpan = useCallback(
    (rowIndex: number, spanId: string) => {
      ...
      if (DEBUG_DIGIAUDIO_MERGE) {
        console.debug("[Digital Audio merge] unmerge applied", { rowIndex, spanId })
      }
    },
  ...
  const duplicateRow = useCallback(
    (rowIndex: number) => {
      ...
      const id =
        typeof crypto !== "undefined" && crypto.randomUUID
          ? crypto.randomUUID()
          : `digiaudio-expert-${Date.now()}-${rowIndex}`
  ...
  const clearWeekSelectionWhereMergedAnchorInvolved = useCallback(
    (rowIndex: number, weekKey: string) => {
      setWeekRectSelection((prev) =>
        prev && digiAudioWeekCellInRect(rowIndex, weekKey, prev, weekKeys)
          ? null
          : prev
      )
  ...
  const rangeWeekMultiSelect = useCallback(
    (rowIndex: number, anchorKey: string, endKey: string) => {
      ...
      setWeekRectSelection(
        normalizeDigitalAudioWeekRect(rowIndex, anchorKey, rowIndex, endKey, weekKeys)
      )
      setWeekStripSelection(null)
    },
    [weekKeys]
  ...
  const lockPendingMergeSelectionFromCurrentSelection = useCallback(() => {
    const n = deriveDigitalAudioMergeEligibility(
  ...
  const derivedMergeTarget = useMemo(() => {
    const n = deriveDigitalAudioMergeEligibility(weekRectSelection, weekMultiSelect, weekKeys)
  ...
  useEffect(() => {
    const endDrag = () => {
      ...
      if (DEBUG_DIGIAUDIO_MERGE) {
        console.debug("[Digital Audio merge] drag end", {
          lastRect: r,
  ...
  const handleMergeSelectedWeeks = useCallback(() => {
    ...
    if (selectionOverlapsMergedSpan(rowMergeMaps[rowIndex], sorted, weekKeys)) {
      if (DEBUG_DIGIAUDIO_MERGE) {
        console.debug("[Digital Audio merge] validation overlap failure", {
          rowIndex,
    ...
    const newId =
      typeof crypto !== "undefined" && crypto.randomUUID
        ? crypto.randomUUID()
        : `merge-${Date.now()}`
    const newSpan: DigitalAudioExpertMergedWeekSpan = {
      id: newId,
    ...
    if (DEBUG_DIGIAUDIO_MERGE) {
      console.debug("[Digital Audio merge] merge applied", {
        rowIndex,
        spanId: newSpan.id,
  ...
  useEffect(() => {
    ...
    const cellId = expertGridCellId(
      ...
      digiAudioDescriptorKeys.length + WEEK_GRID_COL_OFFSET + lastWi
    )
    document
      .getElementById(cellId)
      ?.scrollIntoView({ behavior: "smooth", block: "nearest", inline: "nearest" })
  }, [domGridId, mergeTarget, mergeWeeksReady, digiAudioDescriptorKeys.length, weekKeys])

  useEffect(() => {
    if (!DEBUG_DIGIAUDIO_MERGE) return
    console.debug("[Digital Audio merge] merge eligibility derived", {
      mergeTarget,
      derivedMergeTarget,
      mergeWeeksReady,
      weekRectSelection,
  ...
  const pasteMatrixIntoGrid = useCallback(
    (matrix: string[][]) => {
      ...
      if (fcIsWeek) {
        ...
        const nextRows: DigitalAudioExpertScheduleRow[] = normalizedRows.map((r) => ({
          ...r,
      ...
      for (let dr = 0; dr < working.length; dr++) {
        ...
        for (let dc = 0; dc < pasteRow.length; dc++) {
          ...
          const field = target.field as keyof DigitalAudioExpertScheduleRow
          ...
          if (field === "unitRate") {
            ...
            nextRows[targetRow] = {
              ...cur,
              [field]: truthy,
            } as DigitalAudioExpertScheduleRow
            applied += 1
          } else if (field === "buyType") {
            nextRows[targetRow] = {
              ...cur,
              buyType: normalizeDigitalAudioBuyTypePaste(raw),
            }
            applied += 1
          } else if (field === "publisher") {
            const pub = normalizeDigitalAudioPublisherPaste(raw, publisherNames)
            nextRows[targetRow] = {
              ...cur,
              publisher: pub,
            ...
            nextRows[targetRow] = {
              ...cur,
              site: normalizeDigitalAudioSitePaste(raw, siteNames),
            }
            applied += 1
          } else {
            const v = raw.trim()
            nextRows[targetRow] = { ...cur, [field]: v } as DigitalAudioExpertScheduleRow
            applied += 1
          }
        }
      }
                    ...
                    <tr>
                        ...
                        <th
                          ...
                          className={stickyThCorner(
                            i === descriptorHeadLabels.length - 1
                              ? DIGIAUDIO_EXPERT_WEEK_SCROLLER_EDGE
                              : undefined
                          )}
                          style={{
                      ...
                      {weekColumns.map((col) => (
                        <th
                          ...
                          style={{
                            ...digiAudioExpertWeekColLayoutStyle,
                            ...digiAudioExpertHeaderCellBgStyle,
                          }}
                          title={col.labelFull}
                        >
                    ...
                    {normalizedRows.map((row, rowIndex) => {
                      ...
                      const stripeStyle =
                        rowIndex % 2 === 0
                          ...
                          : undefined
                      const grossCol = digiAudioDescriptorKeys.length
                      const actionsCol = digiAudioDescriptorKeys.length + 1
                        ...
                        >
                          ...
                          >
                            <Input
                              ...
                              onFocus={() =>
                                handleCellFocus(rowIndex, "targetingAttribute")
                              }
                              onKeyDown={(e) =>
                          ...
                          {(() => {
                            ...
                            for (let wi = 0; wi < weekColumns.length; wi += 1) {
                              ...
                              const tdClassName = cn(
                                ...
                                !isMergedAnchorCell &&
                                  mergeReadyOnCell &&
                                  "digiaudio-expert-week-cell--merge-ready z-[6] bg-amber-500/24 ring-2 ring-inset ring-amber-600/75 shadow-sm dark:bg-amber-500/20 dark:ring-amber-400/70",
                              ...
                              const mergedAnchorWrapperClassName = cn(
                                ...
                                isMergedAnchorCell &&
                                  DIGIAUDIO_WEEK_CELL_VISUAL_CLASSES.mergedSurface,
                              ...
                              const inputClassName = cn(
                                ...
                                isMergedAnchorCell
                                  ? DIGIAUDIO_WEEK_CELL_VISUAL_CLASSES.mergedInput
                              ...
                              renderedWeekCells.push(
                                <td
                                  ...
                                  style={
                                    isMergedAnchorCell
                                      ? {
                                          width: DIGIAUDIO_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                          minWidth:
                                            DIGIAUDIO_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                          maxWidth:
                                            DIGIAUDIO_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                  ...
                                  onMouseEnter={(e) => {
                                    ...
                                    const next = normalizeDigitalAudioWeekRect(
                                      drag.rowIndex,
                                      drag.weekKey,
                                      rowIndex,
                                      col.weekKey,
                                    ...
                                    setWeekStripSelection(null)
                                    if (DEBUG_DIGIAUDIO_MERGE) {
                                      console.debug("[Digital Audio merge] drag update", {
                                        rect: next,
                                      })
                                    }
                                  }}
                                ...
                                >
                                  ...
                                  >
                                    <Input
                                      ...
                                      setIsSelecting(true)
                                      if (DEBUG_DIGIAUDIO_MERGE) {
                                        console.debug("[Digital Audio merge] drag start", {
                                          rowIndex,
                                          weekKey: col.weekKey,
                                        })
                                      }
                                      ...
                                      if (shift && lastWeekAnchorRef.current) {
                                        ...
                                        if (
                                          ...
                                          const rect = normalizeDigitalAudioWeekRect(
                                            lastWeekAnchorRef.current.rowIndex,
                                            lastWeekAnchorRef.current.weekKey,
                                            rowIndex,
                                            col.weekKey,
                    ...
                    >
                      <td
                        ...
                        style={{
                          ...
                          ...digiAudioExpertTotalsRowBgStyle,
                        }}
                        colSpan={digiAudioDescriptorKeys.length}
                      >
                      ...
                      <td
                        className={cn(
                          stickyTd(digiAudioDescriptorKeys.length),
                          "h-8 px-1 text-xs tabular-nums"
                        )}
                        style={{
                          ...stickyStyleBody(digiAudioDescriptorKeys.length),
                          ...digiAudioExpertTotalsRowBgStyle,
                        }}
                      >
                      ...
                      <td
                        className={cn(
                          stickyTd(digiAudioDescriptorKeys.length + 1),
                          "h-8"
                        )}
                        style={{
                          ...stickyStyleBody(digiAudioDescriptorKeys.length + 1),
                          ...digiAudioExpertTotalsRowBgStyle,
                        }}
                      />
                      <td
                        className={cn(
                          stickyTd(digiAudioDescriptorKeys.length + 2),
                          "h-8 px-1 text-xs tabular-nums text-muted-foreground",
                          DIGIAUDIO_EXPERT_WEEK_SCROLLER_EDGE
                        )}
                        style={{
                          ...stickyStyleBody(digiAudioDescriptorKeys.length + 2),
                          ...digiAudioExpertTotalsRowBgStyle,
                        }}
                      >
