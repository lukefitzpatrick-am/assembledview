/**
 * F-28 Phase 2 — Newspaper Expert row virtualization smoke (Prompt A items 1,3,5,7,8).
 */
import assert from "node:assert/strict"
import test from "node:test"
import { format, startOfDay } from "date-fns"

import { mapNewspaperExpertRowsToStandardLineItems } from "@/lib/mediaplan/expertChannelMappings"
import {
  reorderExpertRows,
  mergedSpanWidthPx,
...
const NEWSPAPER_EXPERT_ROW_OVERSCAN = EXPERT_GRID_ROW_OVERSCAN_DEFAULT
...
function emptyRow(id: string): NewspaperExpertScheduleRow {
...
function buildRows(n: number): NewspaperExpertScheduleRow[] {
  return Array.from({ length: n }, (_, i) => emptyRow(`newspaper-v-${i}`))
}

/** Mirrors NewspaperExpertGrid containerTotals over the FULL logical array. */
function computeNewspaperWeeklyTotals(
...
test("1. APPLY CORRECTNESS: edit on off-window row survives Apply (full state, not DOM)", () => {
  ...
  const patched = updateRowAtIndex(rows, offWindowIndex, {
    publisher: "OFF-WINDOW-PUBLISHER-MARK",
  ...
  assert.equal(standard[offWindowIndex]!.publisher, "OFF-WINDOW-PUBLISHER-MARK")
  assert.equal(standard[offWindowIndex]!.network, "OFF-WINDOW-NETWORK")
  assert.equal(standard[0]!.publisher, "")
  assert.notEqual(standard[0]!.publisher, "OFF-WINDOW-PUBLISHER-MARK")
})
...
test("3. DRAG REORDER across virtual boundary lands on correct logical index", () => {
  ...
  const dropIndex = virtualRowIndexFromOffsetY(
    280 * NEWSPAPER_EXPERT_ROW_HEIGHT_PX + 1,
    NEWSPAPER_EXPERT_ROW_HEIGHT_PX,
    300
  )
  ...
  assert.ok(next)
  assert.equal(next![280]!.publisher, "P2")
  assert.equal(next![2]!.publisher, "P3")
  ...
  assert.equal(standard[280]!.publisher, "P2")
...
test("5. WEEK-COLUMN WIDTH respects resize overrides (sticky header/week resize path)", () => {
  ...
  assert.equal(NEWSPAPER_EXPERT_ROW_HEIGHT_PX, 41)
})
...
test("7. WEEKLY TOTALS derived from full 300-row state", () => {
  ...
  const totals = computeNewspaperWeeklyTotals(rows, 0)
  ...
  const after = computeNewspaperWeeklyTotals(edited, 0)
...
test("8. PERF: mounted DOM window stays bounded while scrolling 300 rows", () => {
  ...
  const nearTop = expectedMountedRowRange(
    ...
    NEWSPAPER_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    NEWSPAPER_EXPERT_ROW_OVERSCAN
  )
  const mid = expectedMountedRowRange(
    150 * NEWSPAPER_EXPERT_ROW_HEIGHT_PX,
    viewportHeight,
    NEWSPAPER_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    NEWSPAPER_EXPERT_ROW_OVERSCAN
  ...
  const nearBottom = expectedMountedRowRange(
    280 * NEWSPAPER_EXPERT_ROW_HEIGHT_PX,
    viewportHeight,
    NEWSPAPER_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    NEWSPAPER_EXPERT_ROW_OVERSCAN
  ...
  const maxWindow =
    Math.ceil(viewportHeight / NEWSPAPER_EXPERT_ROW_HEIGHT_PX) +
    2 * NEWSPAPER_EXPERT_ROW_OVERSCAN +
    2
