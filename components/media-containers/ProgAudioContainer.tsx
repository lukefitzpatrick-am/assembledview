...
export default function ProgAudioContainer({
  ...
  const form = useForm<ProgAudioFormValues>({
    resolver: zodResolver(progAudioFormSchema),
    defaultValues: {
      lineItems: [
        {
          platform: "",
    ...
    const lineItemTotals = watchedLineItems.map((lineItem, index) => {
      ...
      lineItem.bursts.forEach((burst) => {
        ...
        if (lineItem.budgetIncludesFees) {
          lineFee += (budget / 100) * (feeprogaudio || 0);
          lineMedia += (budget / 100) * (100 - (feeprogaudio || 0));
        } else {
          lineMedia += budget;
...
useEffect(() => {
  ...
  }, [watchedLineItems, feeprogaudio, onInvestmentChange, onBurstsChange, onTotalMediaChange, form]);
