...
export function ExpertGrid({
  ...
  const bidStrategyOptions = useMemo(
    () =>
      config.descriptorCore.find((c) => c.key === "bidStrategy")?.options ?? [],
    [config]
  )
  ...
  const pushRows = useCallback(
    (next: SearchExpertScheduleRow[]) => {
      const withDates = finalizeRowsPreservingIdentity(next, (r) => {
        ...
        const dates = config.deriveScheduleYmdFromRow(
          cleared,
          weekColumns,
          campaignStartDate,
          campaignEndDate,
  ...
  const descriptorHeadLabels = useMemo(
    () => expertGridDescriptorHeadLabels(config, showBillingCols),
    [config, showBillingCols]
  )
                    ...
                    {(() => {
                      ...
                      return (
                        <MemoExpertGridRow
                          ...
                          rowUiSig={rowUiSig}
                          render={() => {
                      const net = expertRowNetMedia(row, weekKeys, feePercent)
