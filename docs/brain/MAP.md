...
- `lib/ava/skills/registry.ts` — skill guidance loaded on demand. `assembled-campaign-read` writes the dashboard six-beat read via `POST /api/campaign-reads/generate` (202 + `after()` job + campaign-scoped tools).
