...
| C-22 | **MBA GET still reads Xano unconditionally** (`/api/mediaplans/mba/[mba_number]`) — duplicate-read-path inheritance; `nextVersionNumber` from Xano master watermark can lag Postgres tip until T6. O4.6 makes publish safe by resolving `version_number` server-side in `savePlanVersion` (Postgres tip+1) and bumping the Xano watermark on publish mirror; GET/Xano path itself dies at T6 | Open — mitigated (O4.6); dies at T6 |
