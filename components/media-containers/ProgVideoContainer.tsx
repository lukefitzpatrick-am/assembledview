...
export default function ProgVideoContainer({
  ...
  const form = useForm<ProgVideoFormValues>({
    resolver: zodResolver(progVideoFormSchema),
    defaultValues: {
      lineItems: [
        {
          platform: "",
    ...
    const lineItemTotals = watchedLineItems.map((lineItem, index) => {
      ...
      lineItem.bursts.forEach((burst) => {
        ...
        if (lineItem.budgetIncludesFees) {
          lineFee += (budget / 100) * (feeprogvideo || 0);
          lineMedia += (budget / 100) * (100 - (feeprogvideo || 0));
        } else {
          lineMedia += budget;
...
useEffect(() => {
  ...
  }, [watchedLineItems, feeprogvideo, onInvestmentChange, onBurstsChange, onTotalMediaChange, form]);
