...
const PROGBVOD_EXPERT_WEEK_COL_WIDTH_PX = 112

/** Week column width on `<th>` and week `<td>`; inputs use `w-full` inside the cell. */
const progBvodExpertWeekColLayoutStyle = {
  width: PROGBVOD_EXPERT_WEEK_COL_WIDTH_PX,
  minWidth: PROGBVOD_EXPERT_WEEK_COL_WIDTH_PX,
  maxWidth: PROGBVOD_EXPERT_WEEK_COL_WIDTH_PX,
  boxSizing: "border-box" as const,
}

const MEDIA_ACCENT_HEX = getMediaTypeThemeHex("progbvod")

const progBvodExpertHeaderCellBgStyle = {
  backgroundColor: rgbaFromHex(MEDIA_ACCENT_HEX, 0.08),
}

const progBvodExpertTotalsRowBgStyle = {
  backgroundColor: rgbaFromHex(MEDIA_ACCENT_HEX, 0.06),
}

/** Right edge of sticky block before the horizontally scrolling week area. */
const PROGBVOD_EXPERT_WEEK_SCROLLER_EDGE =
  "border-r-2 border-border/80 shadow-[2px_0_8px_-4px_rgba(0,0,0,0.08)] dark:shadow-[2px_0_8px_-4px_rgba(0,0,0,0.22)]"

const DEBUG_PROGBVOD_MERGE = false

function normalizeProgBvodKey(input: unknown): string {
  return String(input ?? "")
    .trim()
    .toLowerCase()
    .replace(/\s+/g, " ")
...
type ProgBvodMultiCellSelection = {
  startRow: number
  endRow: number
  startCol: number
  endCol: number
...
type ProgBvodCopiedCells = {
  data: (string | number)[][]
  sourceRows: number
  sourceCols: number
  selection: ProgBvodMultiCellSelection
}
...
const PROGBVOD_WEEK_CELL_VISUAL_CLASSES = {
...
function selectionOverlapsMergedSpan(
  rowMergeMap: ProgBvodRowMergeMap | undefined,
...
type ProgBvodWeekRectSelection = {
  rowStart: number
  rowEnd: number
  weekKeyStart: string
  weekKeyEnd: string
...
function normalizeProgBvodWeekRect(
  row0: number,
  wk0: string,
  row1: number,
  wk1: string,
  weekKeys: readonly string[]
): ProgBvodWeekRectSelection {
...
function progBvodWeekCellInRect(
  rowIndex: number,
  weekKey: string,
  rect: ProgBvodWeekRectSelection | null,
  weekKeys: readonly string[]
): boolean {
...
function rectToMultiCellSelection(
  rect: ProgBvodWeekRectSelection | null,
  weekKeys: readonly string[]
): ProgBvodMultiCellSelection | null {
  if (!rect) return null
  const i0 = weekKeys.indexOf(rect.weekKeyStart)
  const i1 = weekKeys.indexOf(rect.weekKeyEnd)
...
function coerceWeeklySelectionBounds(
  ...
  weekRectSelection: ProgBvodWeekRectSelection | null,
  ...
  if (
    weekRectSelection &&
    progBvodWeekCellInRect(anchorRow, anchorWeekKey, weekRectSelection, weekKeys)
  ) {
...
function applyWeeklyPasteMatrixToSelection(args: {
  ...
  weekKeys: readonly string[]
  rowCount: number
  nextRows: ProgBvodExpertScheduleRow[]
}): {
  applied: number
  errorReasons: string[]
  layout: WeeklyPasteLayoutMode
...
function oohMergeKeysFromRect(
  rect: ProgBvodWeekRectSelection,
  weekKeys: readonly string[]
): string[] | null {
...
type ProgBvodWeekMergeSelectionNormalized = {
  rowIndex: number
 ...
 */
function normalizeProgBvodWeekMergeSelection(
  rect: ProgBvodWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): ProgBvodWeekMergeSelectionNormalized | null {
...
function weekPlainClickPreservesWeekAreaSelection(
  ...
  const mergeNorm = normalizeProgBvodWeekMergeSelection(rect, multi, weekKeys)
  if (
    mergeNorm &&
  ...
  if (rect && progBvodWeekCellInRect(rowIndex, weekKey, rect, weekKeys)) {
    const isSingleCell =
...
function deriveProgBvodMergeEligibility(
  rect: ProgBvodWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): ProgBvodWeekMergeSelectionNormalized | null {
  return normalizeProgBvodWeekMergeSelection(rect, multi, weekKeys)
}
...
type WeeklyExportSelection =
  | { kind: "rect"; rect: ProgBvodWeekRectSelection }
...
function resolveWeeklyExportSelection(
  ...
  weekKeys: readonly string[],
  rows: readonly ProgBvodExpertScheduleRow[] | null
...
function buildWeeklyExportTsv(
  selection: WeeklyExportSelection,
  rows: readonly ProgBvodExpertScheduleRow[],
  weekKeys: readonly string[]
): string | null {
  switch (selection.kind) {
    case "rect": {
...
function applyWeeklyCutToRows(
  ...
  rows: readonly ProgBvodExpertScheduleRow[],
  weekKeys: readonly string[]
): ProgBvodExpertScheduleRow[] | null {
  switch (selection.kind) {
    case "rect": {
      const rect = selection.rect
      const wi0 = Math.min(
      ...
      if (wi0 < 0 || wi1 < 0) return null
      const next: ProgBvodExpertScheduleRow[] = rows.map((r) => ({
        ...r,
    ...
    case "mergeContiguous": {
      ...
      const next: ProgBvodExpertScheduleRow[] = rows.map((row) => ({
        ...row,
...
const PROGBVOD_BID_STRATEGY_OPTIONS: ComboboxOption[] = [
...
function normalizeProgBvodBidStrategyPaste(raw: string): string {
  const v = raw.trim()
  if (!v) return ""
  const byValue = PROGBVOD_BID_STRATEGY_OPTIONS.find((o) => o.value.toLowerCase() === v.toLowerCase())
  if (byValue) return byValue.value
  const byLabel = PROGBVOD_BID_STRATEGY_OPTIONS.find((o) => o.label.toLowerCase() === v.toLowerCase())
  if (byLabel) return byLabel.value
  return v
}

function normalizeProgBvodPlatformPaste(raw: string, platformNames: string[]): string {
  const v = raw.trim()
  if (!v) return ""
...
const PROGBVOD_DESCRIPTOR_CORE: readonly (keyof ProgBvodExpertScheduleRow)[] = [
  "startDate",
  "endDate",
  "platform",
  "bidStrategy",
...
const PROGBVOD_BILLING_FLAG_KEYS: readonly (keyof ProgBvodExpertScheduleRow)[] = [
  "fixedCostMedia",
  "clientPaysForMedia",
  "budgetIncludesFees",
  "noadserving",
...
const PROGBVOD_DESCRIPTOR_TAIL: readonly (keyof ProgBvodExpertScheduleRow)[] = ["unitRate"]
...
type ProgBvodRowMergeSpanMeta = Readonly<{
  id: string
  startWeekKey: string
  endWeekKey: string
  totalQty: number
...
type ProgBvodRowMergeMap = Readonly<{
  ...
  spanMetaByAnchorWeekKey: Readonly<Record<string, ProgBvodRowMergeSpanMeta>>
}>
...
export function ProgBvodExpertGrid({
  ...
  const [focusedCell, setFocusedCell] = useState<ProgBvodExpertFocusedCell | null>(
    null
  )
  const focusedCellRef = useRef<ProgBvodExpertFocusedCell | null>(null)
  ...
  const [weekRectSelection, setWeekRectSelection] =
    useState<ProgBvodWeekRectSelection | null>(null)
  // Multi-cell selection state for copy/paste operations.
  const [multiCellSelection, setMultiCellSelection] =
    useState<ProgBvodMultiCellSelection | null>(null)
  const [isSelecting, setIsSelecting] = useState(false)
  const [copiedCells, setCopiedCells] = useState<ProgBvodCopiedCells | null>(null)
  ...
  const lastDragRectDuringGestureRef = useRef<ProgBvodWeekRectSelection | null>(null)
  /**
   ...
   */
  const postDragWeekClickRectRef = useRef<ProgBvodWeekRectSelection | null>(null)
  const weekRectSelectionRef = useRef<ProgBvodWeekRectSelection | null>(null)
  ...
  const progBvodDescriptorKeys = useMemo(
    () =>
      [
        ...PROGBVOD_DESCRIPTOR_CORE,
        ...PROGBVOD_BILLING_FLAG_KEYS,
        ...PROGBVOD_DESCRIPTOR_TAIL,
      ] as (keyof ProgBvodExpertScheduleRow)[],
    []
  )
  ...
  useEffect(() => {
    if (!DEBUG_PROGBVOD_MERGE) return
    ...
    console.debug("[Programmatic BVOD merge] pending selection updated", pendingMergeSelection)
  ...
  const tryFuzzyMatch = useCallback(
    (rowIndex: number, field: FuzzyMatchField, value: string) => {
      ...
      if (corrected) {
        updateRow(rowIndex, { [field]: corrected } as Partial<ProgBvodExpertScheduleRow>)
        return
      }
      ...
      if (fuzzyMatchAutoApplyRef.current) {
        updateRow(rowIndex, { [field]: match.matched } as Partial<ProgBvodExpertScheduleRow>)
      } else {
  ...
  const navColCount =
    progBvodDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekKeys.length
  ...
  const duplicateRow = useCallback(
    (rowIndex: number) => {
      ...
      const id =
        typeof crypto !== "undefined" && crypto.randomUUID
          ? crypto.randomUUID()
          : `progbvod-expert-${Date.now()}-${rowIndex}`
  ...
  const clearWeekSelectionWhereMergedAnchorInvolved = useCallback(
    (rowIndex: number, weekKey: string) => {
      setWeekRectSelection((prev) =>
        prev && progBvodWeekCellInRect(rowIndex, weekKey, prev, weekKeys)
          ? null
          : prev
      )
  ...
  const rangeWeekMultiSelect = useCallback(
    (rowIndex: number, anchorKey: string, endKey: string) => {
      ...
      setWeekRectSelection(
        normalizeProgBvodWeekRect(rowIndex, anchorKey, rowIndex, endKey, weekKeys)
      )
      setWeekStripSelection(null)
    },
    [weekKeys]
  ...
  const lockPendingMergeSelectionFromCurrentSelection = useCallback(() => {
    const n = deriveProgBvodMergeEligibility(
  ...
  const derivedMergeTarget = useMemo(() => {
    const n = deriveProgBvodMergeEligibility(weekRectSelection, weekMultiSelect, weekKeys)
  ...
  useEffect(() => {
    const endDrag = () => {
      ...
      if (DEBUG_PROGBVOD_MERGE) {
        console.debug("[Programmatic BVOD merge] drag end", {
          lastRect: r,
  ...
  const handleMergeSelectedWeeks = useCallback(() => {
    ...
    if (selectionOverlapsMergedSpan(rowMergeMaps[rowIndex], sorted, weekKeys)) {
      if (DEBUG_PROGBVOD_MERGE) {
        console.debug("[Programmatic BVOD merge] validation overlap failure", {
          rowIndex,
    ...
    if (DEBUG_PROGBVOD_MERGE) {
      console.debug("[Programmatic BVOD merge] merge applied", {
        rowIndex,
        spanId: newSpan.id,
  ...
  useEffect(() => {
    ...
    const cellId = expertGridCellId(
      ...
      progBvodDescriptorKeys.length + WEEK_GRID_COL_OFFSET + lastWi
    )
    document
      .getElementById(cellId)
      ?.scrollIntoView({ behavior: "smooth", block: "nearest", inline: "nearest" })
  }, [domGridId, mergeTarget, mergeWeeksReady, progBvodDescriptorKeys.length, weekKeys])

  useEffect(() => {
    if (!DEBUG_PROGBVOD_MERGE) return
    console.debug("[Programmatic BVOD merge] merge eligibility derived", {
      mergeTarget,
      derivedMergeTarget,
      mergeWeeksReady,
      weekRectSelection,
  ...
  const pasteMatrixIntoGrid = useCallback(
    (matrix: string[][]) => {
      ...
      if (fcIsWeek) {
        ...
        const nextRows: ProgBvodExpertScheduleRow[] = normalizedRows.map((r) => ({
          ...r,
      ...
      for (let dr = 0; dr < working.length; dr++) {
        ...
        for (let dc = 0; dc < pasteRow.length; dc++) {
          ...
          const field = target.field as keyof ProgBvodExpertScheduleRow
          ...
          if (field === "unitRate") {
            ...
            nextRows[targetRow] = {
              ...cur,
              [field]: truthy,
            } as ProgBvodExpertScheduleRow
            applied += 1
          } else if (field === "buyType") {
            nextRows[targetRow] = {
              ...cur,
              buyType: normalizeProgBvodBuyTypePaste(raw),
            }
            applied += 1
          } else if (field === "bidStrategy") {
            nextRows[targetRow] = {
              ...cur,
              bidStrategy: normalizeProgBvodBidStrategyPaste(raw),
            }
            applied += 1
          } else if (field === "platform") {
            const plt = normalizeProgBvodPlatformPaste(raw, platformNames)
            nextRows[targetRow] = {
              ...cur,
              platform: plt,
            }
            ...
            const v = raw.trim()
            nextRows[targetRow] = { ...cur, [field]: v } as ProgBvodExpertScheduleRow
            applied += 1
          }
        }
      }
  ...
  const containerTotals = useMemo(() => {
    ...
    const fee =
      feeprogbvod > 0 && feeprogbvod < 100 ? (sumGross * feeprogbvod) / (100 - feeprogbvod) : 0
    const totalWithFee = sumGross + fee

    return { sumGross, sumQty, perWeek, fee, totalWithFee }
  }, [feeprogbvod, normalizedRows, weekKeys])
  ...
  const colIndexOf = useCallback(
    (key: keyof ProgBvodExpertScheduleRow) => progBvodDescriptorKeys.indexOf(key),
    [progBvodDescriptorKeys]
  )
      ...
      <Card className="relative overflow-hidden border-0 shadow-md">
        ...
        <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2 border-b border-border/60 bg-muted/5 pb-3">
          <CardTitle className="text-base font-semibold tracking-tight">
            Programmatic BVOD — Expert Schedule
          </CardTitle>
                ...
                <table className="w-max min-w-full border-collapse text-sm">
                    ...
                    <tr>
                      ...
                      {weekColumns.map((col) => (
                        <th
                          ...
                          style={{
                            ...progBvodExpertWeekColLayoutStyle,
                            ...progBvodExpertHeaderCellBgStyle,
                          }}
                          title={col.labelFull}
                        >
                  ...
                  <tbody>
                    {normalizedRows.map((row, rowIndex) => {
                      ...
                      const stripeStyle =
                        rowIndex % 2 === 0
                          ...
                          : undefined
                      const grossCol = progBvodDescriptorKeys.length
                      const actionsCol = progBvodDescriptorKeys.length + 1
                      const sigmaCol = progBvodDescriptorKeys.length + 2
                            ...
                            for (let wi = 0; wi < weekColumns.length; wi += 1) {
                              ...
                              const tdClassName = cn(
                                ...
                                isPopulatedNonMergedCell &&
                                  PROGBVOD_WEEK_CELL_VISUAL_CLASSES.populatedSingleTd,
                                ...
                                !isMergedAnchorCell &&
                                  mergeReadyOnCell &&
                                  "progbvod-expert-week-cell--merge-ready z-[6] bg-amber-500/24 ring-2 ring-inset ring-amber-600/75 shadow-sm dark:bg-amber-500/20 dark:ring-amber-400/70",
                              ...
                              const mergedAnchorWrapperClassName = cn(
                                ...
                                isMergedAnchorCell &&
                                  PROGBVOD_WEEK_CELL_VISUAL_CLASSES.mergedSurface,
                              ...
                              const inputClassName = cn(
                                ...
                                isMergedAnchorCell
                                  ? PROGBVOD_WEEK_CELL_VISUAL_CLASSES.mergedInput
                              ...
                              renderedWeekCells.push(
                                <td
                                  ...
                                  style={
                                    isMergedAnchorCell
                                      ? {
                                          width: PROGBVOD_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                          minWidth:
                                            PROGBVOD_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                          maxWidth:
                                            PROGBVOD_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                  ...
                                  onMouseEnter={(e) => {
                                    ...
                                    const next = normalizeProgBvodWeekRect(
                                      drag.rowIndex,
                                      drag.weekKey,
                                      rowIndex,
                                      col.weekKey,
                                    ...
                                    setWeekStripSelection(null)
                                    if (DEBUG_PROGBVOD_MERGE) {
                                      console.debug("[Programmatic BVOD merge] drag update", {
                                        rect: next,
                                      })
                                    }
                                  }}
                                ...
                                >
                                  ...
                                  >
                                    <Input
                                      ...
                                      setIsSelecting(true)
                                      if (DEBUG_PROGBVOD_MERGE) {
                                        console.debug("[Programmatic BVOD merge] drag start", {
                                          rowIndex,
                                          weekKey: col.weekKey,
                                        })
                                      }
                                      ...
                                      if (shift && lastWeekAnchorRef.current) {
                                        ...
                                        if (
                                          ...
                                          const rect = normalizeProgBvodWeekRect(
                                            lastWeekAnchorRef.current.rowIndex,
                                            lastWeekAnchorRef.current.weekKey,
                                            rowIndex,
                                            col.weekKey,
                    ...
                    >
                      ...
                      <td
                        className={cn(
                          stickyTd(progBvodDescriptorKeys.length + 2),
