const fs = require("fs");
const path =
  "c:/Projects/avmediaplan/.worktrees/refactor-expertgrid-consolidation/components/media-containers/ProgDisplayContainer.tsx";
...
console.log("ProgDisplay ExpertCard replace ok", {
  removedChars: end + endMarker.length - start,
  newLen: next.length,
  hasMapClose: next.includes("});"),
  afterSnippet: JSON.stringify(next.slice(start + replacement.length, start + replacement.length + 40)),
});
