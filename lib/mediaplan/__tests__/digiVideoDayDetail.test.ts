...
function line(bursts: any[]): StandardDigiVideoFormLineItem {
  return {
    platform: "P", publisher: "Pub", site: "S", bidStrategy: "", buyType: "bonus",
    placement: "", size: "", targetingAttribute: "", creativeTargeting: "", creative: "",
...
test("sub-week interior burst imports as day-detail and round-trips", () => {
  ...
  const rows = mapStandardDigiVideoLineItemsToExpertRows(std, cols, CS, CE)
  const wk0 = cols[0]!
  assert.equal(weekHasDailyValues(rows[0]!.dailyValues ?? {}, weekDayKeys(wk0, CS, CE)), true)
  const back = mapDigiVideoExpertRowsToStandardLineItems(rows, cols, CS, CE, { feePctDigiVideo: 0 })
