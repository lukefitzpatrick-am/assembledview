...
export default function ProgDisplayContainer({
  ...
  const form = useForm<ProgDisplayFormValues>({
    resolver: zodResolver(progDisplayFormSchema),
    defaultValues: {
      lineItems: [
        {
          platform: "",
    ...
    const lineItemTotals = watchedLineItems.map((lineItem, index) => {
      ...
      lineItem.bursts.forEach((burst) => {
        ...
        if (lineItem.budgetIncludesFees) {
          lineFee += (budget / 100) * (feeprogdisplay || 0);
          lineMedia += (budget / 100) * (100 - (feeprogdisplay || 0));
        } else {
          lineMedia += budget;
...
useEffect(() => {
  ...
  }, [watchedLineItems, feeprogdisplay, onInvestmentChange, onBurstsChange, onTotalMediaChange, form]);
