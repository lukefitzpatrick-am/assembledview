...
import { lineCardFromSocial } from "@/lib/pacing/channel/lineCardModel";
...
export function SocialCampaignsClient({ isAdmin: _isAdmin }: SocialCampaignsClientProps) {
  ...
  const { layout } = useChannelLayout("social");
  ...
  const boardItems = useMemo(
    () => displayed.map((row) => ({ model: lineCardFromSocial(row, asOf), row })),
    [displayed, asOf],
  );
    ...
    <div className="space-y-4 p-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <ChannelLayoutToggle channel="social" />
        </div>
      </div>
      {total === 0 ? (
        <EmptyState
