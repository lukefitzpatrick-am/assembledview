...
The last three are mirrored but their callers use raw `sql`. Sync is four crons from 00:15 UTC (invoices, import, contacts, pdfs), 15 minutes apart. Each writes a `running` `xero_sync_log` row first and updates it. `stage` and `duration_ms` are migration `0087` (apply before deploy; no backfill). Resume uses the newest success or incomplete row for that stage, skipping `running` and `failed`. Ops-health "Xero sync freshness" is red when any stage's newest success is missing or older than 36 hours. AR `mba_number` is filled from the Xero Reference by `matchMba.ts` (MBA token, then `scope_of_work.scope_id`); a scope hit does not write `mba_number`. `pdf_file` is Blob-backed `{url, pathname, filename}` on success; ETL left a non-null Xano stub with no `url` key. `sync_pdfs` pending = `IS NULL OR NOT (pdf_file ? 'url')`, FY26+, batch 10 (`XERO_PDF_BATCH_SIZE`), 40s budget.
