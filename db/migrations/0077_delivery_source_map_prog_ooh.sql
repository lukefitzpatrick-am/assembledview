-- Migration 0077: seed Vistar and Broadsign onto delivery_source_map
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM public.migration_markers WHERE key = '0077_delivery_source_map_prog_ooh') THEN
    RAISE NOTICE '0077 already applied — no-op';
    RETURN;
  END IF;

  IF to_regclass('public.delivery_source_map') IS NULL THEN
    RAISE EXCEPTION '0077 requires delivery_source_map (apply 0063 first)';
  END IF;
  ...
  INSERT INTO public.delivery_source_map (
    ...
    (
      ...
      false,
      true,
      'Vistar exchange CSV via partner-ingest. PACING_FACT Programmatic - OOH.'
    ),
    (
      'broadsign',
      'partner_file',
      false,
      true,
      'Broadsign buys on the Vistar exchange file. PACING_FACT Programmatic - OOH.'
    )
  ON CONFLICT (publisher_key) DO NOTHING;

  INSERT INTO public.migration_markers (key, note)
  VALUES (
    '0077_delivery_source_map_prog_ooh',
    'delivery_source_map: Vistar + Broadsign partner_file seed. App reads TS seed, not this table (C-76).'
  )
