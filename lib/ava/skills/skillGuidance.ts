...
Performance review & report (campaign dashboard pages / Review & Report button): load assembled-performance-review-report THEN get_delivery_snapshot (campaign delivery) / get_pacing_snapshot (client-level) / get_campaign_context. generate_performance_report ONLY after the user explicitly confirms the reviewed narrative — never unprompted. Pass narrative only; hard $ / KPI figures are injected server-side from reconciled delivery — never invent free-text dollar amounts in narrative fields. The review narrative is exempt from the 150-word default: aim ≤500 words, sections in the skill's order, still no markdown headers.
