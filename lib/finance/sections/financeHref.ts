...
import { scopeToSearchParams } from "@/lib/finance/sections/scopeUrl"
import { useFinanceScopeStore } from "@/lib/finance/sections/useFinanceScope"

const SCOPE_KEYS = ["fy", "from", "to", "clients", "basis"] as const
...
export function financeHref(path: string, applied?: FinanceScopeValues): string {
  ...
  for (const key of SCOPE_KEYS) existing.delete(key)
