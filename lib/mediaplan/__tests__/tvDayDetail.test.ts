...
function line(bursts: any[]): StandardTelevisionFormLineItem {
  return {
    market: "M", network: "N", station: "S", daypart: "", placement: "", buyType: "tarps", buyingDemo: "",
    bidStrategy: "", creativeTargeting: "", creative: "",
...
test("sub-week interior burst imports as day-detail and round-trips", () => {
  const std = [line([{ budget: "600", buyAmount: "100", startDate: new Date(2026,0,7), endDate: new Date(2026,0,9), calculatedValue: 6, tarps: "6", size: "30s" }])]
  const rows = mapStandardTvLineItemsToExpertRows(std, cols, CS, CE)
  const wk0 = cols[0]!
  assert.equal(weekHasDailyValues(rows[0]!.dailyValues ?? {}, weekDayKeys(wk0, CS, CE)), true)
  const back = mapTvExpertRowsToStandardLineItems(rows, cols, CS, CE, { feePctTelevision: 0 })
...
test("full-week burst stays weekly (no dailyValues)", () => {
  const std = [line([{ budget: "1000", buyAmount: "100", startDate: new Date(2026,0,11), endDate: new Date(2026,0,17), calculatedValue: 10, tarps: "10", size: "30s" }])]
  const rows = mapStandardTvLineItemsToExpertRows(std, cols, CS, CE)
