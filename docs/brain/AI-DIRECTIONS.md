...
| Task shape | Start at |
|---|---|
| Add or change a media channel | `db/schema/lineChannelValues.ts` (the value tuple; `enums.ts` builds `lineChannelEnum` from it) → `lineItemAttrs.ts` → `lib/api/media-containers.ts` → `lib/data/planShapes.ts` → the ~12-map list in `BLAST-RADIUS.md`. **All of them, or the channel half-works** |
...
| Client import of a server-only sink | Split a `*.server.ts` sibling (`import "server-only"`, static `@/db`). `await import("@/db")` still bundles. Gate: `npm run check:client-server-only` — sinks are `server-only`, `@/db`, `db/schema` (drizzle table builders; only `lineChannelValues.ts` is client-safe) and the `snowflake-sdk` / `drizzle-orm` / `postgres` / `pg` packages. For a plain value list the UI needs, move the values to their own drizzle-free module rather than allowlisting. Do not add `webpackIgnore` except the isomorphic `lib/api.ts` allowlist |
...
| Partner file ingest | `lib/partner-ingest/` + `GET /api/cron/partner-ingest`. Raw lines first. Range replace via `withSnowflakeSession` only. No UPDATE on RAW. Two-factor sender match. |
