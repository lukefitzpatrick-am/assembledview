...
const DIGIVIDEO_EXPERT_WEEK_COL_WIDTH_PX = 112

/** Week column width on `<th>` and week `<td>`; inputs use `w-full` inside the cell. */
const digiVideoExpertWeekColLayoutStyle = {
  width: DIGIVIDEO_EXPERT_WEEK_COL_WIDTH_PX,
  minWidth: DIGIVIDEO_EXPERT_WEEK_COL_WIDTH_PX,
  maxWidth: DIGIVIDEO_EXPERT_WEEK_COL_WIDTH_PX,
  boxSizing: "border-box" as const,
}

const MEDIA_ACCENT_HEX = getMediaTypeThemeHex("digivideo")

const digiVideoExpertHeaderCellBgStyle = {
  backgroundColor: rgbaFromHex(MEDIA_ACCENT_HEX, 0.08),
}

const digiVideoExpertTotalsRowBgStyle = {
  backgroundColor: rgbaFromHex(MEDIA_ACCENT_HEX, 0.06),
}

/** Right edge of sticky block before the horizontally scrolling week area. */
const DIGIVIDEO_EXPERT_WEEK_SCROLLER_EDGE =
  "border-r-2 border-border/80 shadow-[2px_0_8px_-4px_rgba(0,0,0,0.08)] dark:shadow-[2px_0_8px_-4px_rgba(0,0,0,0.22)]"

const DEBUG_DIGIVIDEO_MERGE = false

function normalizeDigiVideoKey(input: unknown): string {
  return String(input ?? "")
    .trim()
    .toLowerCase()
    .replace(/\s+/g, " ")
...
function sumMergedQuantities(row: DigiVideoExpertScheduleRow): number {
...
): DigiVideoExpertMergedWeekSpan | null {
...
type DigiVideoMultiCellSelection = {
  startRow: number
  endRow: number
  startCol: number
  endCol: number
...
type DigiVideoCopiedCells = {
  data: (string | number)[][]
  sourceRows: number
  sourceCols: number
  selection: DigiVideoMultiCellSelection
}

function weeklyCellDisplayValue(
  cellValue: number | string | undefined,
  mergedSpan: DigiVideoExpertMergedWeekSpan | null
): string {
  if (mergedSpan) {
...
const DIGIVIDEO_WEEK_CELL_VISUAL_CLASSES = {
...
function selectionOverlapsMergedSpan(
  rowMergeMap: DigiVideoRowMergeMap | undefined,
...
type DigiVideoWeekRectSelection = {
  rowStart: number
  rowEnd: number
  weekKeyStart: string
  weekKeyEnd: string
...
function normalizeDigiVideoWeekRect(
  row0: number,
  wk0: string,
  row1: number,
  wk1: string,
  weekKeys: readonly string[]
): DigiVideoWeekRectSelection {
...
function digiVideoWeekCellInRect(
  rowIndex: number,
  weekKey: string,
  rect: DigiVideoWeekRectSelection | null,
  weekKeys: readonly string[]
): boolean {
...
function rectToMultiCellSelection(
  rect: DigiVideoWeekRectSelection | null,
  weekKeys: readonly string[]
): DigiVideoMultiCellSelection | null {
  if (!rect) return null
  const i0 = weekKeys.indexOf(rect.weekKeyStart)
  const i1 = weekKeys.indexOf(rect.weekKeyEnd)
...
function coerceWeeklySelectionBounds(
  ...
  weekRectSelection: DigiVideoWeekRectSelection | null,
  ...
  if (
    weekRectSelection &&
    digiVideoWeekCellInRect(anchorRow, anchorWeekKey, weekRectSelection, weekKeys)
  ) {
...
function applyWeeklyPasteMatrixToSelection(args: {
  ...
  weekKeys: readonly string[]
  rowCount: number
  nextRows: DigiVideoExpertScheduleRow[]
}): {
  applied: number
  errorReasons: string[]
  layout: WeeklyPasteLayoutMode
...
function oohMergeKeysFromRect(
  rect: DigiVideoWeekRectSelection,
  weekKeys: readonly string[]
): string[] | null {
...
type DigiVideoWeekMergeSelectionNormalized = {
  rowIndex: number
 ...
 */
function normalizeDigiVideoWeekMergeSelection(
  rect: DigiVideoWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): DigiVideoWeekMergeSelectionNormalized | null {
...
function weekPlainClickPreservesWeekAreaSelection(
  ...
  const mergeNorm = normalizeDigiVideoWeekMergeSelection(rect, multi, weekKeys)
  if (
    mergeNorm &&
  ...
  if (rect && digiVideoWeekCellInRect(rowIndex, weekKey, rect, weekKeys)) {
    const isSingleCell =
...
function deriveDigiVideoMergeEligibility(
  rect: DigiVideoWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): DigiVideoWeekMergeSelectionNormalized | null {
  return normalizeDigiVideoWeekMergeSelection(rect, multi, weekKeys)
}
...
function mergedWeekSpansAfterCutRect(
  spans: DigiVideoExpertMergedWeekSpan[] | undefined,
  wi0: number,
  wi1: number,
  weekKeys: readonly string[]
): DigiVideoExpertMergedWeekSpan[] {
...
type WeeklyExportSelection =
  | { kind: "rect"; rect: DigiVideoWeekRectSelection }
...
function resolveWeeklyExportSelection(
  ...
  weekKeys: readonly string[],
  rows: readonly DigiVideoExpertScheduleRow[] | null
...
function applyWeeklyCutToRows(
  ...
  rows: readonly DigiVideoExpertScheduleRow[],
  weekKeys: readonly string[]
): DigiVideoExpertScheduleRow[] | null {
  switch (selection.kind) {
    case "rect": {
      const rect = selection.rect
      const wi0 = Math.min(
      ...
      if (wi0 < 0 || wi1 < 0) return null
      const next: DigiVideoExpertScheduleRow[] = rows.map((r) => ({
        ...r,
    ...
    case "mergeContiguous": {
      ...
      const next: DigiVideoExpertScheduleRow[] = rows.map((row) => ({
        ...row,
...
/** Match labels/values on {@link DigitalVideoContainer} buy-type combobox. */
const DIGIVIDEO_BUY_TYPE_OPTIONS: ComboboxOption[] = [
...
function normalizeDigiVideoBuyTypePaste(raw: string): string {
  const v = raw.trim()
  if (!v) return ""
  const byValue = DIGIVIDEO_BUY_TYPE_OPTIONS.find(
    (o) => o.value.toLowerCase() === v.toLowerCase()
  )
  if (byValue) return byValue.value
  const byLabel = DIGIVIDEO_BUY_TYPE_OPTIONS.find(
...
function normalizeDigiVideoPublisherPaste(raw: string, publisherNames: string[]): string {
  const v = raw.trim()
  if (!v) return ""
...
function normalizeDigiVideoSitePaste(raw: string, siteNames: string[]): string {
  const v = raw.trim()
  if (!v) return ""
...
export interface DigitalVideoExpertGridProps {
  ...
  /** Digi Video sites (platform + site label) for site combobox + fuzzy matching */
  digiVideoSites?: {
...
type DigiVideoRowMergeSpanMeta = Readonly<{
  id: string
  startWeekKey: string
  endWeekKey: string
  totalQty: number
...
type DigiVideoRowMergeMap = Readonly<{
  anchorByWeekKey: Readonly<Record<string, string>>
  interiorByWeekKey: Readonly<Record<string, string>>
  spanById: Readonly<Record<string, DigiVideoExpertMergedWeekSpan>>
  spanMetaByAnchorWeekKey: Readonly<Record<string, DigiVideoRowMergeSpanMeta>>
}>
...
export function DigitalVideoExpertGrid({
  ...
  const [focusedCell, setFocusedCell] = useState<DigiVideoExpertFocusedCell | null>(
    null
  )
  const focusedCellRef = useRef<DigiVideoExpertFocusedCell | null>(null)
  ...
  const siteComboboxOptionsForRow = useCallback(
    (row: DigiVideoExpertScheduleRow): ComboboxOption[] => {
      const key = normalizeDigiVideoKey(row.publisher)
      const src = !key
        ? digiVideoSites
        : digiVideoSites.filter((s) => normalizeDigiVideoKey(s.platform) === key)
      const names = [
        ...new Set(
          src
            .map((s) => String(s.site ?? "").trim())
  ...
  const [weekRectSelection, setWeekRectSelection] =
    useState<DigiVideoWeekRectSelection | null>(null)
  ...
  const lastDragRectDuringGestureRef = useRef<DigiVideoWeekRectSelection | null>(null)
  /**
   ...
   */
  const postDragWeekClickRectRef = useRef<DigiVideoWeekRectSelection | null>(null)
  const weekRectSelectionRef = useRef<DigiVideoWeekRectSelection | null>(null)
  ...
  const rowMergeMaps = useMemo<readonly DigiVideoRowMergeMap[]>(() => {
    const maps = normalizedRows.map((row) => {
      const anchorByWeekKey: Record<string, string> = {}
      const interiorByWeekKey: Record<string, string> = {}
      const spanById: Record<string, DigiVideoExpertMergedWeekSpan> = {}
      const spanMetaByAnchorWeekKey: Record<string, DigiVideoRowMergeSpanMeta> = {}
      ...
      for (const span of row.mergedWeekSpans ?? []) {
        if (spanById[span.id]) {
          if (DEBUG_DIGIVIDEO_MERGE) {
            console.debug("[Digi Video merge] occupancy duplicate span id ignored", {
              rowId: row.id,
        ...
        if (keys.some((k) => occupiedWeekKeys.has(k))) {
          if (DEBUG_DIGIVIDEO_MERGE) {
            console.debug("[Digi Video merge] occupancy overlap ignored", {
              rowId: row.id,
              rowIndex: normalizedRows.indexOf(row),
              spanId: span.id,
              keys,
  ...
  useEffect(() => {
    if (!DEBUG_DIGIVIDEO_MERGE) return
    ...
    console.debug("[Digi Video merge] pending selection updated", pendingMergeSelection)
  ...
  const updateRow = useCallback(
    (rowIndex: number, patch: Partial<DigiVideoExpertScheduleRow>) => {
  ...
  const tryFuzzyMatch = useCallback(
    (rowIndex: number, field: FuzzyMatchField, value: string) => {
      ...
      if (corrected) {
        updateRow(rowIndex, { [field]: corrected } as Partial<DigiVideoExpertScheduleRow>)
        return
      }
      ...
      if (fuzzyMatchAutoApplyRef.current) {
        updateRow(rowIndex, { [field]: match.matched } as Partial<DigiVideoExpertScheduleRow>)
      } else {
  ...
  const navColCount =
    digiVideoDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekKeys.length
  ...
  const unmergeWeekSpan = useCallback(
    (rowIndex: number, spanId: string) => {
      ...
      if (DEBUG_DIGIVIDEO_MERGE) {
        console.debug("[Digi Video merge] unmerge applied", { rowIndex, spanId })
      }
    },
  ...
  const duplicateRow = useCallback(
    (rowIndex: number) => {
      ...
      const id =
        typeof crypto !== "undefined" && crypto.randomUUID
          ? crypto.randomUUID()
          : `digivideo-expert-${Date.now()}-${rowIndex}`
  ...
  const clearWeekSelectionWhereMergedAnchorInvolved = useCallback(
    (rowIndex: number, weekKey: string) => {
      setWeekRectSelection((prev) =>
        prev && digiVideoWeekCellInRect(rowIndex, weekKey, prev, weekKeys)
          ? null
          : prev
      )
  ...
  const rangeWeekMultiSelect = useCallback(
    (rowIndex: number, anchorKey: string, endKey: string) => {
      ...
      setWeekRectSelection(
        normalizeDigiVideoWeekRect(rowIndex, anchorKey, rowIndex, endKey, weekKeys)
      )
      setWeekStripSelection(null)
    },
    [weekKeys]
  ...
  const lockPendingMergeSelectionFromCurrentSelection = useCallback(() => {
    const n = deriveDigiVideoMergeEligibility(
  ...
  const derivedMergeTarget = useMemo(() => {
    const n = deriveDigiVideoMergeEligibility(weekRectSelection, weekMultiSelect, weekKeys)
  ...
  useEffect(() => {
    const endDrag = () => {
      ...
      if (DEBUG_DIGIVIDEO_MERGE) {
        console.debug("[Digi Video merge] drag end", {
          lastRect: r,
  ...
  const handleMergeSelectedWeeks = useCallback(() => {
    ...
    if (selectionOverlapsMergedSpan(rowMergeMaps[rowIndex], sorted, weekKeys)) {
      if (DEBUG_DIGIVIDEO_MERGE) {
        console.debug("[Digi Video merge] validation overlap failure", {
          rowIndex,
    ...
    const newId =
      typeof crypto !== "undefined" && crypto.randomUUID
        ? crypto.randomUUID()
        : `merge-${Date.now()}`
    const newSpan: DigiVideoExpertMergedWeekSpan = {
      id: newId,
    ...
    if (DEBUG_DIGIVIDEO_MERGE) {
      console.debug("[Digi Video merge] merge applied", {
        rowIndex,
        spanId: newSpan.id,
  ...
  useEffect(() => {
    ...
    const cellId = expertGridCellId(
      ...
      digiVideoDescriptorKeys.length + WEEK_GRID_COL_OFFSET + lastWi
    )
    document
      .getElementById(cellId)
      ?.scrollIntoView({ behavior: "smooth", block: "nearest", inline: "nearest" })
  }, [domGridId, mergeTarget, mergeWeeksReady, digiVideoDescriptorKeys.length, weekKeys])

  useEffect(() => {
    if (!DEBUG_DIGIVIDEO_MERGE) return
    console.debug("[Digi Video merge] merge eligibility derived", {
      mergeTarget,
      derivedMergeTarget,
      mergeWeeksReady,
      weekRectSelection,
  ...
  const pasteMatrixIntoGrid = useCallback(
    (matrix: string[][]) => {
      ...
      if (fcIsWeek) {
        ...
        const nextRows: DigiVideoExpertScheduleRow[] = normalizedRows.map((r) => ({
          ...r,
      ...
      for (let dr = 0; dr < working.length; dr++) {
        ...
        for (let dc = 0; dc < pasteRow.length; dc++) {
          ...
          const field = target.field as keyof DigiVideoExpertScheduleRow
          ...
          if (field === "unitRate") {
            ...
            nextRows[targetRow] = {
              ...cur,
              [field]: truthy,
            } as DigiVideoExpertScheduleRow
            applied += 1
          } else if (field === "buyType") {
            nextRows[targetRow] = {
              ...cur,
              buyType: normalizeDigiVideoBuyTypePaste(raw),
            }
            applied += 1
          } else if (field === "publisher") {
            const pub = normalizeDigiVideoPublisherPaste(raw, publisherNames)
            nextRows[targetRow] = {
              ...cur,
              publisher: pub,
            ...
            nextRows[targetRow] = {
              ...cur,
              site: normalizeDigiVideoSitePaste(raw, siteNames),
            }
            applied += 1
          } else {
            const v = raw.trim()
            nextRows[targetRow] = { ...cur, [field]: v } as DigiVideoExpertScheduleRow
            applied += 1
          }
        }
      }
                ...
                <table className="w-max min-w-full border-collapse text-sm">
                    ...
                    <tr>
                      ...
                      {weekColumns.map((col) => (
                        <th
                          ...
                          style={{
                            ...digiVideoExpertWeekColLayoutStyle,
                            ...digiVideoExpertHeaderCellBgStyle,
                          }}
                          title={col.labelFull}
                        >
                  ...
                  <tbody>
                    {normalizedRows.map((row, rowIndex) => {
                      ...
                      const stripeStyle =
                        rowIndex % 2 === 0
                          ...
                          : undefined
                      const grossCol = digiVideoDescriptorKeys.length
                      const actionsCol = digiVideoDescriptorKeys.length + 1
                          ...
                          {(() => {
                            ...
                            for (let wi = 0; wi < weekColumns.length; wi += 1) {
                              ...
                              const tdClassName = cn(
                                ...
                                !isMergedAnchorCell &&
                                  mergeReadyOnCell &&
                                  "digivideo-expert-week-cell--merge-ready z-[6] bg-amber-500/24 ring-2 ring-inset ring-amber-600/75 shadow-sm dark:bg-amber-500/20 dark:ring-amber-400/70",
                              ...
                              const mergedAnchorWrapperClassName = cn(
                                ...
                                isMergedAnchorCell &&
                                  DIGIVIDEO_WEEK_CELL_VISUAL_CLASSES.mergedSurface,
                              ...
                              const inputClassName = cn(
                                ...
                                isMergedAnchorCell
                                  ? DIGIVIDEO_WEEK_CELL_VISUAL_CLASSES.mergedInput
                              ...
                              renderedWeekCells.push(
                                <td
                                  ...
                                  style={
                                    isMergedAnchorCell
                                      ? {
                                          width: DIGIVIDEO_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                          minWidth:
                                            DIGIVIDEO_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                          maxWidth:
                                            DIGIVIDEO_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                  ...
                                  onMouseEnter={(e) => {
                                    ...
                                    const next = normalizeDigiVideoWeekRect(
                                      drag.rowIndex,
                                      drag.weekKey,
                                      rowIndex,
                                      col.weekKey,
                                    ...
                                    setWeekStripSelection(null)
                                    if (DEBUG_DIGIVIDEO_MERGE) {
                                      console.debug("[Digi Video merge] drag update", {
                                        rect: next,
                                      })
                                    }
                                  }}
                                ...
                                >
                                  ...
                                  >
                                    <Input
                                      ...
                                      setIsSelecting(true)
                                      if (DEBUG_DIGIVIDEO_MERGE) {
                                        console.debug("[Digi Video merge] drag start", {
                                          rowIndex,
                                          weekKey: col.weekKey,
                                        })
                                      }
                                      ...
                                      if (shift && lastWeekAnchorRef.current) {
                                        ...
                                        if (
                                          ...
                                          const rect = normalizeDigiVideoWeekRect(
                                            lastWeekAnchorRef.current.rowIndex,
                                            lastWeekAnchorRef.current.weekKey,
                                            rowIndex,
                                            col.weekKey,
                    ...
                    >
                      ...
                      <td
                        className={cn(
                          stickyTd(digiVideoDescriptorKeys.length + 2),
