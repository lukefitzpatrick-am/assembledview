...
test("sub-week interior burst imports as day-detail and round-trips", () => {
  ...
  const rows = mapStandardDigiAudioLineItemsToExpertRows(std, cols, CS, CE)
  const wk0 = cols[0]!
  assert.equal(weekHasDailyValues(rows[0]!.dailyValues ?? {}, weekDayKeys(wk0, CS, CE)), true)
  const back = mapDigitalAudioExpertRowsToStandardLineItems(rows, cols, CS, CE, { feePctDigiAudio: 0 })
