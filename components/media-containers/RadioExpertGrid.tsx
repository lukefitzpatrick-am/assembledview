...
function sumMergedQuantities(row: RadioExpertScheduleRow): number {
...
): RadioExpertMergedWeekSpan | null {
...
type RadioCopiedCells = {
  data: (string | number)[][]
  sourceRows: number
  sourceCols: number
  selection: RadioMultiCellSelection
}

function weeklyCellDisplayValue(
  cellValue: number | string | undefined,
  mergedSpan: RadioExpertMergedWeekSpan | null
): string {
  if (mergedSpan) {
...
const RADIO_WEEK_CELL_VISUAL_CLASSES = {
...
function selectionOverlapsMergedSpan(
  rowMergeMap: RadioRowMergeMap | undefined,
...
function normalizeRadioWeekRect(
  row0: number,
  wk0: string,
  row1: number,
  wk1: string,
  weekKeys: readonly string[]
): RadioWeekRectSelection {
...
function radioWeekCellInRect(
  rowIndex: number,
  weekKey: string,
  rect: RadioWeekRectSelection | null,
  weekKeys: readonly string[]
): boolean {
...
function coerceWeeklySelectionBounds(
  ...
  weekRectSelection: RadioWeekRectSelection | null,
  ...
  if (
    weekRectSelection &&
    radioWeekCellInRect(anchorRow, anchorWeekKey, weekRectSelection, weekKeys)
  ) {
...
function applyWeeklyPasteMatrixToSelection(args: {
  ...
  weekKeys: readonly string[]
  rowCount: number
  nextRows: RadioExpertScheduleRow[]
}): {
  applied: number
  errorReasons: string[]
  layout: WeeklyPasteLayoutMode
...
type RadioWeekMergeSelectionNormalized = {
  rowIndex: number
 ...
 */
function normalizeRadioWeekMergeSelection(
  rect: RadioWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): RadioWeekMergeSelectionNormalized | null {
...
function weekPlainClickPreservesWeekAreaSelection(
  ...
  const mergeNorm = normalizeRadioWeekMergeSelection(rect, multi, weekKeys)
  if (
    mergeNorm &&
  ...
  if (rect && radioWeekCellInRect(rowIndex, weekKey, rect, weekKeys)) {
    const isSingleCell =
...
function deriveRadioMergeEligibility(
  rect: RadioWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): RadioWeekMergeSelectionNormalized | null {
  return normalizeRadioWeekMergeSelection(rect, multi, weekKeys)
}
...
function mergedWeekSpansAfterCutRect(
  spans: RadioExpertMergedWeekSpan[] | undefined,
  wi0: number,
  wi1: number,
  weekKeys: readonly string[]
): RadioExpertMergedWeekSpan[] {
...
type WeeklyExportSelection =
  | { kind: "rect"; rect: RadioWeekRectSelection }
...
function resolveWeeklyExportSelection(
  ...
  weekKeys: readonly string[],
  rows: readonly RadioExpertScheduleRow[] | null
...
function applyWeeklyCutToRows(
  ...
  switch (selection.kind) {
    case "rect": {
      ...
      if (wi0 < 0 || wi1 < 0) return null
      const next: RadioExpertScheduleRow[] = rows.map((r) => ({
        ...r,
    ...
    case "mergeContiguous": {
      ...
      const next: RadioExpertScheduleRow[] = rows.map((row) => ({
        ...row,
...
/** Match labels/values on {@link RadioContainer} buy-type combobox. */
const RADIO_BUY_TYPE_OPTIONS: ComboboxOption[] = [
...
function normalizeRadioBuyTypePaste(raw: string): string {
  const v = raw.trim()
  if (!v) return ""
  const byValue = RADIO_BUY_TYPE_OPTIONS.find(
    (o) => o.value.toLowerCase() === v.toLowerCase()
  )
  if (byValue) return byValue.value
  const byLabel = RADIO_BUY_TYPE_OPTIONS.find(
...
function normalizeRadioFormatPaste(raw: string): string {
  return raw.trim()
}

function normalizeRadioNetworkPaste(raw: string, networkNames: string[]): string {
  const v = raw.trim()
  if (!v) return ""
...
function normalizeRadioStationPaste(raw: string, stationNames: string[]): string {
  const v = raw.trim()
  if (!v) return ""
...
function rowGrossCost(row: RadioExpertScheduleRow, weekKeys: string[]): number {
...
const RADIO_DESCRIPTOR_CORE: readonly (keyof RadioExpertScheduleRow)[] = [
  "startDate",
  "endDate",
  "network",
  "station",
...
const RADIO_BILLING_FLAG_KEYS: readonly (keyof RadioExpertScheduleRow)[] = [
  "fixedCostMedia",
  "clientPaysForMedia",
  "budgetIncludesFees",
]

const RADIO_DESCRIPTOR_TAIL: readonly (keyof RadioExpertScheduleRow)[] = ["unitRate"]
...
type RadioRowMergeSpanMeta = Readonly<{
  id: string
  startWeekKey: string
  endWeekKey: string
  totalQty: number
...
type RadioRowMergeMap = Readonly<{
  anchorByWeekKey: Readonly<Record<string, string>>
  interiorByWeekKey: Readonly<Record<string, string>>
  spanById: Readonly<Record<string, RadioExpertMergedWeekSpan>>
  spanMetaByAnchorWeekKey: Readonly<Record<string, RadioRowMergeSpanMeta>>
}>
...
export function RadioExpertGrid({
  ...
  const [focusedCell, setFocusedCell] = useState<RadioExpertFocusedCell | null>(
    null
  )
  const focusedCellRef = useRef<RadioExpertFocusedCell | null>(null)
  ...
  const [weekRectSelection, setWeekRectSelection] =
    useState<RadioWeekRectSelection | null>(null)
  // Multi-cell selection state for copy/paste operations.
  const [multiCellSelection, setMultiCellSelection] =
    useState<RadioMultiCellSelection | null>(null)
  const [isSelecting, setIsSelecting] = useState(false)
  const [copiedCells, setCopiedCells] = useState<RadioCopiedCells | null>(null)
  ...
  const lastDragRectDuringGestureRef = useRef<RadioWeekRectSelection | null>(null)
  /**
   ...
   */
  const postDragWeekClickRectRef = useRef<RadioWeekRectSelection | null>(null)
  const weekRectSelectionRef = useRef<RadioWeekRectSelection | null>(null)
  ...
  const radioDescriptorKeys = useMemo(
    () =>
      [
        ...RADIO_DESCRIPTOR_CORE,
        ...RADIO_BILLING_FLAG_KEYS,
        ...RADIO_DESCRIPTOR_TAIL,
      ] as (keyof RadioExpertScheduleRow)[],
    []
  )

  const descriptorColWidths = useMemo(
    () => [
      48, 48, 120, 120, 96, 110, 80, 96, 110, 96, 40, 40, 40, 88,
    ],
    []
  )
  ...
  const rowMergeMaps = useMemo<readonly RadioRowMergeMap[]>(() => {
    const maps = normalizedRows.map((row) => {
      const anchorByWeekKey: Record<string, string> = {}
      const interiorByWeekKey: Record<string, string> = {}
      const spanById: Record<string, RadioExpertMergedWeekSpan> = {}
      const spanMetaByAnchorWeekKey: Record<string, RadioRowMergeSpanMeta> = {}
      ...
      for (const span of row.mergedWeekSpans ?? []) {
        if (spanById[span.id]) {
          if (DEBUG_RADIO_MERGE) {
            console.debug("[Radio merge] occupancy duplicate span id ignored", {
              rowId: row.id,
        ...
        if (keys.some((k) => occupiedWeekKeys.has(k))) {
          if (DEBUG_RADIO_MERGE) {
            console.debug("[Radio merge] occupancy overlap ignored", {
              rowId: row.id,
              rowIndex: normalizedRows.indexOf(row),
              spanId: span.id,
              keys,
  ...
  useEffect(() => {
    ...
    console.debug("[Radio merge] pending selection updated", pendingMergeSelection)
  ...
  const updateRow = useCallback(
    (rowIndex: number, patch: Partial<RadioExpertScheduleRow>) => {
  ...
  const tryFuzzyMatch = useCallback(
    (rowIndex: number, field: FuzzyMatchField, value: string) => {
      ...
      if (corrected) {
        updateRow(rowIndex, { [field]: corrected } as Partial<RadioExpertScheduleRow>)
        return
      }
      ...
      if (fuzzyMatchAutoApplyRef.current) {
        updateRow(rowIndex, { [field]: match.matched } as Partial<RadioExpertScheduleRow>)
      } else {
  ...
  const navColCount =
    radioDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekKeys.length
  const firstWeekNavColIndex = radioDescriptorKeys.length + WEEK_GRID_COL_OFFSET
  const unitRateNavColIndex = radioDescriptorKeys.indexOf("unitRate")
  ...
  const unmergeWeekSpan = useCallback(
    (rowIndex: number, spanId: string) => {
      ...
      if (DEBUG_RADIO_MERGE) {
        console.debug("[Radio merge] unmerge applied", { rowIndex, spanId })
      }
    },
  ...
  const clearWeekSelectionWhereMergedAnchorInvolved = useCallback(
    (rowIndex: number, weekKey: string) => {
      setWeekRectSelection((prev) =>
        prev && radioWeekCellInRect(rowIndex, weekKey, prev, weekKeys)
          ? null
          : prev
      )
  ...
  const rangeWeekMultiSelect = useCallback(
    (rowIndex: number, anchorKey: string, endKey: string) => {
      ...
      setWeekRectSelection(
        normalizeRadioWeekRect(rowIndex, anchorKey, rowIndex, endKey, weekKeys)
      )
      setWeekStripSelection(null)
    },
    [weekKeys]
  ...
  const lockPendingMergeSelectionFromCurrentSelection = useCallback(() => {
    const n = deriveRadioMergeEligibility(
  ...
  const derivedMergeTarget = useMemo(() => {
    const n = deriveRadioMergeEligibility(weekRectSelection, weekMultiSelect, weekKeys)
  ...
  useEffect(() => {
    const endDrag = () => {
      ...
      if (DEBUG_RADIO_MERGE) {
        console.debug("[Radio merge] drag end", {
          lastRect: r,
  ...
  const handleMergeSelectedWeeks = useCallback(() => {
    ...
    if (selectionOverlapsMergedSpan(rowMergeMaps[rowIndex], sorted, weekKeys)) {
      if (DEBUG_RADIO_MERGE) {
        console.debug("[Radio merge] validation overlap failure", {
          rowIndex,
    ...
    const newId =
      typeof crypto !== "undefined" && crypto.randomUUID
        ? crypto.randomUUID()
        : `merge-${Date.now()}`
    const newSpan: RadioExpertMergedWeekSpan = {
      id: newId,
    ...
    if (DEBUG_RADIO_MERGE) {
      console.debug("[Radio merge] merge applied", {
        rowIndex,
        spanId: newSpan.id,
  ...
  useEffect(() => {
    ...
    const cellId = expertGridCellId(
      ...
      radioDescriptorKeys.length + WEEK_GRID_COL_OFFSET + lastWi
    )
    document
      .getElementById(cellId)
      ?.scrollIntoView({ behavior: "smooth", block: "nearest", inline: "nearest" })
  }, [domGridId, mergeTarget, mergeWeeksReady, radioDescriptorKeys.length, weekKeys])

  useEffect(() => {
    if (!DEBUG_RADIO_MERGE) return
    console.debug("[Radio merge] merge eligibility derived", {
      mergeTarget,
      derivedMergeTarget,
      mergeWeeksReady,
      weekRectSelection,
  ...
  const pasteMatrixIntoGrid = useCallback(
    (matrix: string[][]) => {
      ...
      if (fcIsWeek) {
        ...
        const nextRows: RadioExpertScheduleRow[] = normalizedRows.map((r) => ({
          ...r,
      ...
      for (let dr = 0; dr < working.length; dr++) {
        ...
        for (let dc = 0; dc < pasteRow.length; dc++) {
          ...
          const field = target.field as keyof RadioExpertScheduleRow
          ...
          if (field === "unitRate") {
            ...
            nextRows[targetRow] = {
              ...cur,
              [field]: truthy,
            } as RadioExpertScheduleRow
            applied += 1
          } else if (field === "buyType") {
            nextRows[targetRow] = {
              ...cur,
              buyType: normalizeRadioBuyTypePaste(raw),
            }
            applied += 1
          } else if (field === "format") {
            nextRows[targetRow] = {
              ...cur,
              format: normalizeRadioFormatPaste(raw),
            }
            applied += 1
          } else if (field === "network") {
            nextRows[targetRow] = {
              ...cur,
              network: normalizeRadioNetworkPaste(raw, networkNames),
            }
            applied += 1
          } else if (field === "station") {
            nextRows[targetRow] = {
              ...cur,
              station: normalizeRadioStationPaste(raw, stationNames),
            }
            applied += 1
          } else {
            const v = raw.trim()
            nextRows[targetRow] = { ...cur, [field]: v } as RadioExpertScheduleRow
            applied += 1
          }
        }
      }
  ...
  const containerTotals = useMemo(() => {
    ...
    const fee =
      feeradio > 0 && feeradio < 100 ? (sumGross * feeradio) / (100 - feeradio) : 0
  ...
  const colIndexOf = useCallback(
    (key: keyof RadioExpertScheduleRow) => radioDescriptorKeys.indexOf(key),
    [radioDescriptorKeys]
  )
      ...
      <Card className="relative overflow-hidden border-0 shadow-md">
        ...
        <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2 border-b border-border/60 bg-muted/5 pb-3">
          <CardTitle className="text-base font-semibold tracking-tight">
            Radio — Expert Schedule
          </CardTitle>
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2">
              <Label htmlFor="radio-expert-row-count" className="text-sm whitespace-nowrap">
                Rows:
              </Label>
              <Input
                id="radio-expert-row-count"
                type="number"
                min={1}
                max={500}
                        ...
                        <th
                          ...
                          className={stickyThCorner(
                            i === descriptorHeadLabels.length - 1
                              ? RADIO_EXPERT_WEEK_SCROLLER_EDGE
                              : undefined
                          )}
                          style={{
                  ...
                  <tbody>
                    {normalizedRows.map((row, rowIndex) => {
                      ...
                      const stripeStyle =
                        rowIndex % 2 === 0
                          ...
                          : undefined
                      const grossCol = radioDescriptorKeys.length
                      const actionsCol = radioDescriptorKeys.length + 1
                      const sigmaCol = radioDescriptorKeys.length + 2
                      ...
                      const cDur = colIndexOf("duration")
                      ...
                      return (
                        ...
                        >
                          ...
                          <td
                            className={stickyTd(cDur)}
                            style={stickyStyleBody(cDur)}
                          >
                            <Input
                              id={expertGridCellId(
                                domGridId,
                              ...
                              value={row.duration}
                              onFocus={() =>
                                handleCellFocus(rowIndex, "duration")
                              }
                              onKeyDown={(e) =>
                                handleGridInputKeyDown(rowIndex, cDur, e)
                              }
                              onChange={(e) =>
                                updateRow(rowIndex, {
                            ...
                            for (let wi = 0; wi < weekColumns.length; wi += 1) {
                              ...
                              const tdClassName = cn(
                                ...
                                isPopulatedNonMergedCell &&
                                  RADIO_WEEK_CELL_VISUAL_CLASSES.populatedSingleTd,
                              ...
                              const mergedAnchorWrapperClassName = cn(
                                ...
                                isMergedAnchorCell &&
                                  RADIO_WEEK_CELL_VISUAL_CLASSES.mergedSurface,
                              ...
                              const inputClassName = cn(
                                ...
                                isMergedAnchorCell
                                  ? RADIO_WEEK_CELL_VISUAL_CLASSES.mergedInput
                              ...
                              renderedWeekCells.push(
                                <td
                                  ...
                                  onMouseEnter={(e) => {
                                    ...
                                    const next = normalizeRadioWeekRect(
                                      drag.rowIndex,
                                      drag.weekKey,
                                      rowIndex,
                                      col.weekKey,
                                    ...
                                    setWeekStripSelection(null)
                                    if (DEBUG_RADIO_MERGE) {
                                      console.debug("[Radio merge] drag update", {
                                        rect: next,
                                      })
                                    }
                                  }}
                                ...
                                >
                                  ...
                                  >
                                    <Input
                                      ...
                                      setIsSelecting(true)
                                      if (DEBUG_RADIO_MERGE) {
                                        console.debug("[Radio merge] drag start", {
                                          rowIndex,
                                          weekKey: col.weekKey,
                                        })
                                      }
                                      ...
                                      if (shift && lastWeekAnchorRef.current) {
                                        ...
                                        if (
                                          ...
                                          const rect = normalizeRadioWeekRect(
                                            lastWeekAnchorRef.current.rowIndex,
                                            lastWeekAnchorRef.current.weekKey,
                                            rowIndex,
                                            col.weekKey,
                    ...
                    >
                      <td
                        ...
                        colSpan={radioDescriptorKeys.length}
                      >
                      ...
                      <td
                        className={cn(
                          stickyTd(radioDescriptorKeys.length),
                          "h-8 px-1 text-xs tabular-nums"
                        )}
                        style={{
                          ...stickyStyleBody(radioDescriptorKeys.length),
                          ...radioExpertTotalsRowBgStyle,
                        }}
                      >
                      ...
                      <td
                        className={cn(
                          stickyTd(radioDescriptorKeys.length + 1),
                          "h-8"
                        )}
                        style={{
                          ...stickyStyleBody(radioDescriptorKeys.length + 1),
                          ...radioExpertTotalsRowBgStyle,
                        }}
                      />
                      <td
                        className={cn(
                          stickyTd(radioDescriptorKeys.length + 2),
                          "h-8 px-1 text-xs tabular-nums text-muted-foreground",
                          RADIO_EXPERT_WEEK_SCROLLER_EDGE
                        )}
                        style={{
                          ...stickyStyleBody(radioDescriptorKeys.length + 2),
                          ...radioExpertTotalsRowBgStyle,
                        }}
                      >
        ...
        {DEBUG_RADIO_MERGE ? (
          <div className="pointer-events-none absolute bottom-2 right-2 z-50 max-w-[360px] rounded-md border bg-background/95 p-2 text-[11px] leading-snug shadow-lg">
            <div className="mb-1 font-medium text-foreground">Radio Merge Debug</div>
