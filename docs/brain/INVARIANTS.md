...
- `DATA_BACKEND` defaults to `xano`. `shadow` serves Xano and must not alter user-visible payloads; `postgres` serves Supabase for wired domains only (reference tables first). New domains opt in one PR at a time.
