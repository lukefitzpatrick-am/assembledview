...
/** Net media when budget is gross incl. fee — must match `getInfluencersBursts` / burst row readouts (linear split). */
...
export default function InfluencersContainer({
  ...
  const handleInfluencersExpertApply = useCallback(() => {
    ...
    form.setValue("lineItems", merged as InfluencersFormValues["lineItems"], {
      shouldDirty: true,
  ...
  const handleBuyTypeChange = useCallback(
   ...
   const handleValueChange = useCallback((lineItemIndex: number, burstIndex: number, budgetIncludesFeesOverride?: boolean) => {
    ...
    const budget = netMediaFeeMarkup(rawBudget, budgetIncludesFees, feeinfluencers || 0);
              ...
              <div className="flex items-center gap-6 text-sm font-semibold tabular-nums">
                ...
                <div className="text-right">
                  ...
                  <span style={mediaTypeAccentTextStyle(MEDIA_ACCENT_HEX)}>
                    {formatMoney(overallTotals.overallCost, { locale: "en-AU", currency: "AUD" })}
                  </span>
                </div>
              </div>
            </div>
                            ...
                            <Button
                              ...
                              aria-label={
                                collapsedLineItems.has(lineItemIndex)
                                  ? `Expand details for influencers line item ${lineItemIndex + 1}`
                                  : `Collapse details for influencers line item ${lineItemIndex + 1}`
                              }
