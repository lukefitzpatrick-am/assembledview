...
| `app/api/mediaplans/[id]/download/route.ts` | A | Stored file download — unpublished 422 `NOT_APPROVED`; missing jsonb url/`path` → 404 `NOT_SAVED`; kinds include `aa_media_plan`; tenant `checkClientMbaAccess` | `published_at` + stored jsonb | Never generate; never `requireRole(admin)` alone |
