...
- Publisher forecast `*_comms` are unconditional whole-percent (`rate / 100`) in `applyForecastCommissionRate`. Values in (0, 1] are a data error — log `[forecast-commission-scale]` once (never block / never reinterpret as decimal fraction).
