/**
 * F-28 Phase 2 — Television Expert row virtualization smoke (Prompt A items 1,3,5,7,8).
 */
import assert from "node:assert/strict"
import test from "node:test"
import { format, startOfDay } from "date-fns"

import { mapTvExpertRowsToStandardLineItems } from "@/lib/mediaplan/expertChannelMappings"
import {
  reorderExpertRows,
  mergedSpanWidthPx,
...
const TV_EXPERT_ROW_OVERSCAN = EXPERT_GRID_ROW_OVERSCAN_DEFAULT
...
function emptyRow(id: string): TelevisionExpertScheduleRow {
...
function buildRows(n: number): TelevisionExpertScheduleRow[] {
  return Array.from({ length: n }, (_, i) => emptyRow(`tv-v-${i}`))
}

/** Mirrors TelevisionExpertGrid containerTotals over the FULL logical array. */
function computeTvWeeklyTotals(
...
test("3. DRAG REORDER across virtual boundary lands on correct logical index", () => {
  ...
  const dropIndex = virtualRowIndexFromOffsetY(
    280 * TV_EXPERT_ROW_HEIGHT_PX + 1,
    TV_EXPERT_ROW_HEIGHT_PX,
    300
  )
...
test("5. WEEK-COLUMN WIDTH respects resize overrides (sticky header/week resize path)", () => {
  ...
  assert.equal(TV_EXPERT_ROW_HEIGHT_PX, 41)
})
...
test("7. WEEKLY TOTALS derived from full 300-row state", () => {
  ...
  const totals = computeTvWeeklyTotals(rows, 0)
  ...
  const after = computeTvWeeklyTotals(edited, 0)
...
test("8. PERF: mounted DOM window stays bounded while scrolling 300 rows", () => {
  ...
  const nearTop = expectedMountedRowRange(
    ...
    TV_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    TV_EXPERT_ROW_OVERSCAN
  )
  const mid = expectedMountedRowRange(
    150 * TV_EXPERT_ROW_HEIGHT_PX,
    viewportHeight,
    TV_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    TV_EXPERT_ROW_OVERSCAN
  ...
  const maxWindow =
    Math.ceil(viewportHeight / TV_EXPERT_ROW_HEIGHT_PX) +
    2 * TV_EXPERT_ROW_OVERSCAN +
    2
