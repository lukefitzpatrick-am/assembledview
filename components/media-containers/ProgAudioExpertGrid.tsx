...
const PROGAUDIO_EXPERT_WEEK_COL_WIDTH_PX = 112

/** Week column width on `<th>` and week `<td>`; inputs use `w-full` inside the cell. */
const progAudioExpertWeekColLayoutStyle = {
  width: PROGAUDIO_EXPERT_WEEK_COL_WIDTH_PX,
  minWidth: PROGAUDIO_EXPERT_WEEK_COL_WIDTH_PX,
  maxWidth: PROGAUDIO_EXPERT_WEEK_COL_WIDTH_PX,
  boxSizing: "border-box" as const,
}

const MEDIA_ACCENT_HEX = getMediaTypeThemeHex("progaudio")

const progAudioExpertHeaderCellBgStyle = {
  backgroundColor: rgbaFromHex(MEDIA_ACCENT_HEX, 0.08),
}

const progAudioExpertTotalsRowBgStyle = {
  backgroundColor: rgbaFromHex(MEDIA_ACCENT_HEX, 0.06),
}

/** Right edge of sticky block before the horizontally scrolling week area. */
const PROGAUDIO_EXPERT_WEEK_SCROLLER_EDGE =
  "border-r-2 border-border/80 shadow-[2px_0_8px_-4px_rgba(0,0,0,0.08)] dark:shadow-[2px_0_8px_-4px_rgba(0,0,0,0.22)]"

const DEBUG_PROGAUDIO_MERGE = false

function normalizeProgAudioKey(input: unknown): string {
  return String(input ?? "")
    .trim()
    .toLowerCase()
    .replace(/\s+/g, " ")
...
function sumMergedQuantities(row: ProgAudioExpertScheduleRow): number {
...
type ProgAudioMultiCellSelection = {
  startRow: number
  endRow: number
  startCol: number
  endCol: number
...
type ProgAudioCopiedCells = {
  data: (string | number)[][]
  sourceRows: number
  sourceCols: number
  selection: ProgAudioMultiCellSelection
}

function weeklyCellDisplayValue(
  cellValue: number | string | undefined,
  mergedSpan: ProgExpertMergedWeekSpan | null
): string {
  if (mergedSpan) {
...
const PROGAUDIO_WEEK_CELL_VISUAL_CLASSES = {
...
function selectionOverlapsMergedSpan(
  rowMergeMap: ProgAudioRowMergeMap | undefined,
...
type ProgAudioWeekRectSelection = {
  rowStart: number
  rowEnd: number
  weekKeyStart: string
  weekKeyEnd: string
...
function normalizeProgAudioWeekRect(
  row0: number,
  wk0: string,
  row1: number,
  wk1: string,
  weekKeys: readonly string[]
): ProgAudioWeekRectSelection {
...
function progAudioWeekCellInRect(
  rowIndex: number,
  weekKey: string,
  rect: ProgAudioWeekRectSelection | null,
  weekKeys: readonly string[]
): boolean {
...
function rectToMultiCellSelection(
  rect: ProgAudioWeekRectSelection | null,
  weekKeys: readonly string[]
): ProgAudioMultiCellSelection | null {
  if (!rect) return null
  const i0 = weekKeys.indexOf(rect.weekKeyStart)
  const i1 = weekKeys.indexOf(rect.weekKeyEnd)
...
function coerceWeeklySelectionBounds(
  ...
  weekRectSelection: ProgAudioWeekRectSelection | null,
  ...
  if (
    weekRectSelection &&
    progAudioWeekCellInRect(anchorRow, anchorWeekKey, weekRectSelection, weekKeys)
  ) {
...
function applyWeeklyPasteMatrixToSelection(args: {
  ...
  weekKeys: readonly string[]
  rowCount: number
  nextRows: ProgAudioExpertScheduleRow[]
}): {
  applied: number
  errorReasons: string[]
  layout: WeeklyPasteLayoutMode
...
function oohMergeKeysFromRect(
  rect: ProgAudioWeekRectSelection,
  weekKeys: readonly string[]
): string[] | null {
...
type ProgAudioWeekMergeSelectionNormalized = {
  rowIndex: number
 ...
 */
function normalizeProgAudioWeekMergeSelection(
  rect: ProgAudioWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): ProgAudioWeekMergeSelectionNormalized | null {
...
function weekPlainClickPreservesWeekAreaSelection(
  ...
  const mergeNorm = normalizeProgAudioWeekMergeSelection(rect, multi, weekKeys)
  if (
    mergeNorm &&
  ...
  if (rect && progAudioWeekCellInRect(rowIndex, weekKey, rect, weekKeys)) {
    const isSingleCell =
...
function deriveProgAudioMergeEligibility(
  rect: ProgAudioWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): ProgAudioWeekMergeSelectionNormalized | null {
  return normalizeProgAudioWeekMergeSelection(rect, multi, weekKeys)
}
...
function mergedWeekSpansAfterCutRect(
  spans: ProgExpertMergedWeekSpan[] | undefined,
  wi0: number,
  wi1: number,
  weekKeys: readonly string[]
): ProgExpertMergedWeekSpan[] {
...
type WeeklyExportSelection =
  | { kind: "rect"; rect: ProgAudioWeekRectSelection }
...
function resolveWeeklyExportSelection(
  ...
  weekKeys: readonly string[],
  rows: readonly ProgAudioExpertScheduleRow[] | null
...
function buildWeeklyExportTsv(
  selection: WeeklyExportSelection,
  rows: readonly ProgAudioExpertScheduleRow[],
  weekKeys: readonly string[]
): string | null {
  switch (selection.kind) {
    case "rect": {
...
function applyWeeklyCutToRows(
  ...
  rows: readonly ProgAudioExpertScheduleRow[],
  weekKeys: readonly string[]
): ProgAudioExpertScheduleRow[] | null {
  switch (selection.kind) {
    case "rect": {
      const rect = selection.rect
      const wi0 = Math.min(
      ...
      if (wi0 < 0 || wi1 < 0) return null
      const next: ProgAudioExpertScheduleRow[] = rows.map((r) => ({
        ...r,
    ...
    case "mergeContiguous": {
      ...
      const next: ProgAudioExpertScheduleRow[] = rows.map((row) => ({
        ...row,
...
const PROGAUDIO_BID_STRATEGY_OPTIONS: ComboboxOption[] = [
...
function normalizeProgAudioBidStrategyPaste(raw: string): string {
  const v = raw.trim()
  if (!v) return ""
  const byValue = PROGAUDIO_BID_STRATEGY_OPTIONS.find((o) => o.value.toLowerCase() === v.toLowerCase())
  if (byValue) return byValue.value
  const byLabel = PROGAUDIO_BID_STRATEGY_OPTIONS.find((o) => o.label.toLowerCase() === v.toLowerCase())
  if (byLabel) return byLabel.value
  return v
}

function normalizeProgAudioPlatformPaste(raw: string, platformNames: string[]): string {
  const v = raw.trim()
  if (!v) return ""
...
function rowGrossCost(row: ProgAudioExpertScheduleRow, weekKeys: string[]): number {
...
const PROGAUDIO_DESCRIPTOR_CORE: readonly (keyof ProgAudioExpertScheduleRow)[] = [
  "startDate",
  "endDate",
  "platform",
  "bidStrategy",
...
const PROGAUDIO_BILLING_FLAG_KEYS: readonly (keyof ProgAudioExpertScheduleRow)[] = [
  "fixedCostMedia",
  "clientPaysForMedia",
  "budgetIncludesFees",
  "noadserving",
...
const PROGAUDIO_DESCRIPTOR_TAIL: readonly (keyof ProgAudioExpertScheduleRow)[] = ["unitRate"]
...
type ProgAudioExpertFocusedCell = { rowIndex: number; columnKey: string }
type WeekDragSource =
  | {
      type: "single"
      rowIndex: number
...
type ProgAudioRowMergeSpanMeta = Readonly<{
  id: string
  startWeekKey: string
  endWeekKey: string
  totalQty: number
...
type ProgAudioRowMergeMap = Readonly<{
  ...
  spanMetaByAnchorWeekKey: Readonly<Record<string, ProgAudioRowMergeSpanMeta>>
}>
...
export function ProgAudioExpertGrid({
  ...
  const [focusedCell, setFocusedCell] = useState<ProgAudioExpertFocusedCell | null>(
    null
  )
  const focusedCellRef = useRef<ProgAudioExpertFocusedCell | null>(null)
  ...
  const [weekRectSelection, setWeekRectSelection] =
    useState<ProgAudioWeekRectSelection | null>(null)
  // Multi-cell selection state for copy/paste operations.
  const [multiCellSelection, setMultiCellSelection] =
    useState<ProgAudioMultiCellSelection | null>(null)
  const [isSelecting, setIsSelecting] = useState(false)
  const [copiedCells, setCopiedCells] = useState<ProgAudioCopiedCells | null>(null)
  ...
  const lastDragRectDuringGestureRef = useRef<ProgAudioWeekRectSelection | null>(null)
  /**
   ...
   */
  const postDragWeekClickRectRef = useRef<ProgAudioWeekRectSelection | null>(null)
  const weekRectSelectionRef = useRef<ProgAudioWeekRectSelection | null>(null)
  ...
  const progAudioDescriptorKeys = useMemo(
    () =>
      [
        ...PROGAUDIO_DESCRIPTOR_CORE,
        ...PROGAUDIO_BILLING_FLAG_KEYS,
        ...PROGAUDIO_DESCRIPTOR_TAIL,
      ] as (keyof ProgAudioExpertScheduleRow)[],
    []
  )
  ...
  const rowMergeMaps = useMemo<readonly ProgAudioRowMergeMap[]>(() => {
    const maps = normalizedRows.map((row) => {
      ...
      const spanMetaByAnchorWeekKey: Record<string, ProgAudioRowMergeSpanMeta> = {}
      ...
      for (const span of row.mergedWeekSpans ?? []) {
        if (spanById[span.id]) {
          if (DEBUG_PROGAUDIO_MERGE) {
            console.debug("[Programmatic Audio merge] occupancy duplicate span id ignored", {
              rowId: row.id,
        ...
        if (keys.some((k) => occupiedWeekKeys.has(k))) {
          if (DEBUG_PROGAUDIO_MERGE) {
            console.debug("[Programmatic Audio merge] occupancy overlap ignored", {
              rowId: row.id,
              rowIndex: normalizedRows.indexOf(row),
              spanId: span.id,
              keys,
  ...
  useEffect(() => {
    if (!DEBUG_PROGAUDIO_MERGE) return
    ...
    console.debug("[Programmatic Audio merge] pending selection updated", pendingMergeSelection)
  ...
  const updateRow = useCallback(
    (rowIndex: number, patch: Partial<ProgAudioExpertScheduleRow>) => {
  ...
  const tryFuzzyMatch = useCallback(
    (rowIndex: number, field: FuzzyMatchField, value: string) => {
      ...
      if (corrected) {
        updateRow(rowIndex, { [field]: corrected } as Partial<ProgAudioExpertScheduleRow>)
        return
      }
      ...
      if (fuzzyMatchAutoApplyRef.current) {
        updateRow(rowIndex, { [field]: match.matched } as Partial<ProgAudioExpertScheduleRow>)
      } else {
  ...
  const navColCount =
    progAudioDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekKeys.length
  const firstWeekNavColIndex = progAudioDescriptorKeys.length + WEEK_GRID_COL_OFFSET
  const unitRateNavColIndex = progAudioDescriptorKeys.indexOf("unitRate")
  ...
  const unmergeWeekSpan = useCallback(
    (rowIndex: number, spanId: string) => {
      ...
      if (DEBUG_PROGAUDIO_MERGE) {
        console.debug("[Programmatic Audio merge] unmerge applied", { rowIndex, spanId })
      }
    },
  ...
  const duplicateRow = useCallback(
    (rowIndex: number) => {
      ...
      const id =
        typeof crypto !== "undefined" && crypto.randomUUID
          ? crypto.randomUUID()
          : `progaudio-expert-${Date.now()}-${rowIndex}`
  ...
  const clearWeekSelectionWhereMergedAnchorInvolved = useCallback(
    (rowIndex: number, weekKey: string) => {
      setWeekRectSelection((prev) =>
        prev && progAudioWeekCellInRect(rowIndex, weekKey, prev, weekKeys)
          ? null
          : prev
      )
  ...
  const rangeWeekMultiSelect = useCallback(
    (rowIndex: number, anchorKey: string, endKey: string) => {
      ...
      setWeekRectSelection(
        normalizeProgAudioWeekRect(rowIndex, anchorKey, rowIndex, endKey, weekKeys)
      )
      setWeekStripSelection(null)
    },
    [weekKeys]
  ...
  const lockPendingMergeSelectionFromCurrentSelection = useCallback(() => {
    const n = deriveProgAudioMergeEligibility(
  ...
  const derivedMergeTarget = useMemo(() => {
    const n = deriveProgAudioMergeEligibility(weekRectSelection, weekMultiSelect, weekKeys)
  ...
  useEffect(() => {
    const endDrag = () => {
      ...
      if (DEBUG_PROGAUDIO_MERGE) {
        console.debug("[Programmatic Audio merge] drag end", {
          lastRect: r,
  ...
  const handleMergeSelectedWeeks = useCallback(() => {
    ...
    if (selectionOverlapsMergedSpan(rowMergeMaps[rowIndex], sorted, weekKeys)) {
      if (DEBUG_PROGAUDIO_MERGE) {
        console.debug("[Programmatic Audio merge] validation overlap failure", {
          rowIndex,
    ...
    if (DEBUG_PROGAUDIO_MERGE) {
      console.debug("[Programmatic Audio merge] merge applied", {
        rowIndex,
        spanId: newSpan.id,
  ...
  useEffect(() => {
    ...
    const cellId = expertGridCellId(
      ...
      progAudioDescriptorKeys.length + WEEK_GRID_COL_OFFSET + lastWi
    )
    document
      .getElementById(cellId)
      ?.scrollIntoView({ behavior: "smooth", block: "nearest", inline: "nearest" })
  }, [domGridId, mergeTarget, mergeWeeksReady, progAudioDescriptorKeys.length, weekKeys])

  useEffect(() => {
    if (!DEBUG_PROGAUDIO_MERGE) return
    console.debug("[Programmatic Audio merge] merge eligibility derived", {
      mergeTarget,
      derivedMergeTarget,
      mergeWeeksReady,
      weekRectSelection,
  ...
  const pasteMatrixIntoGrid = useCallback(
    (matrix: string[][]) => {
      ...
      if (fcIsWeek) {
        ...
        const nextRows: ProgAudioExpertScheduleRow[] = normalizedRows.map((r) => ({
          ...r,
      ...
      for (let dr = 0; dr < working.length; dr++) {
        ...
        for (let dc = 0; dc < pasteRow.length; dc++) {
          ...
          const field = target.field as keyof ProgAudioExpertScheduleRow
          ...
          if (field === "unitRate") {
            ...
            nextRows[targetRow] = {
              ...cur,
              [field]: truthy,
            } as ProgAudioExpertScheduleRow
            applied += 1
          } else if (field === "buyType") {
            nextRows[targetRow] = {
              ...cur,
              buyType: normalizeProgAudioBuyTypePaste(raw),
            }
            applied += 1
          } else if (field === "bidStrategy") {
            nextRows[targetRow] = {
              ...cur,
              bidStrategy: normalizeProgAudioBidStrategyPaste(raw),
            }
            applied += 1
          } else if (field === "platform") {
            const plt = normalizeProgAudioPlatformPaste(raw, platformNames)
            nextRows[targetRow] = {
              ...cur,
              platform: plt,
            }
            ...
            const v = raw.trim()
            nextRows[targetRow] = { ...cur, [field]: v } as ProgAudioExpertScheduleRow
            applied += 1
          }
        }
      }
  ...
  const containerTotals = useMemo(() => {
    ...
    const fee =
      feeprogaudio > 0 && feeprogaudio < 100 ? (sumGross * feeprogaudio) / (100 - feeprogaudio) : 0
    const totalWithFee = sumGross + fee

    return { sumGross, sumQty, perWeek, fee, totalWithFee }
  }, [feeprogaudio, normalizedRows, weekKeys])
  ...
  const colIndexOf = useCallback(
    (key: keyof ProgAudioExpertScheduleRow) => progAudioDescriptorKeys.indexOf(key),
    [progAudioDescriptorKeys]
  )
      ...
      <Card className="relative overflow-hidden border-0 shadow-md">
        ...
        <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2 border-b border-border/60 bg-muted/5 pb-3">
          <CardTitle className="text-base font-semibold tracking-tight">
            Programmatic Audio — Expert Schedule
          </CardTitle>
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2">
              <Label htmlFor="progaudio-expert-row-count" className="text-sm whitespace-nowrap">
                Rows:
              </Label>
              <Input
                id="progaudio-expert-row-count"
                type="number"
                min={1}
                max={500}
          ...
          <div className="overflow-hidden rounded-lg border border-border/80 bg-card/30 shadow-sm">
            {normalizedRows.length === 0 ? (
              ...
              <div
                ...
                data-progaudio-expert-grid-scroll=""
              >
                    ...
                    <tr>
                        ...
                        <th
                          ...
                          className={stickyThCorner(
                            i === descriptorHeadLabels.length - 1
                              ? PROGAUDIO_EXPERT_WEEK_SCROLLER_EDGE
                              : undefined
                          )}
                          style={{
                            ...stickyStyleHeaderCorner(i),
                            ...progAudioExpertHeaderCellBgStyle,
                          }}
                        >
                          {label === "Unit Rate" ? (
                            <Tooltip>
                      ...
                      {weekColumns.map((col) => (
                        <th
                          ...
                          style={{
                            ...progAudioExpertWeekColLayoutStyle,
                            ...progAudioExpertHeaderCellBgStyle,
                          }}
                          title={col.labelFull}
                        >
                    ...
                    {normalizedRows.map((row, rowIndex) => {
                      ...
                      const stripeStyle =
                        rowIndex % 2 === 0
                          ...
                          : undefined
                      const grossCol = progAudioDescriptorKeys.length
                      const actionsCol = progAudioDescriptorKeys.length + 1
                      const sigmaCol = progAudioDescriptorKeys.length + 2
                        ...
                        >
                          ...
                          {(() => {
                            ...
                            for (let wi = 0; wi < weekColumns.length; wi += 1) {
                              ...
                              const colIndex =
                                progAudioDescriptorKeys.length +
                              ...
                              const tdClassName = cn(
                                ...
                                isPopulatedNonMergedCell &&
                                  PROGAUDIO_WEEK_CELL_VISUAL_CLASSES.populatedSingleTd,
                                ...
                                !isMergedAnchorCell &&
                                  mergeReadyOnCell &&
                                  "progaudio-expert-week-cell--merge-ready z-[6] bg-amber-500/24 ring-2 ring-inset ring-amber-600/75 shadow-sm dark:bg-amber-500/20 dark:ring-amber-400/70",
                              ...
                              const mergedAnchorWrapperClassName = cn(
                                ...
                                isMergedAnchorCell &&
                                  PROGAUDIO_WEEK_CELL_VISUAL_CLASSES.mergedSurface,
                              ...
                              const inputClassName = cn(
                                ...
                                isMergedAnchorCell
                                  ? PROGAUDIO_WEEK_CELL_VISUAL_CLASSES.mergedInput
                              ...
                              renderedWeekCells.push(
                                <td
                                  ...
                                  style={
                                    isMergedAnchorCell
                                      ? {
                                          width: PROGAUDIO_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                          minWidth:
                                            PROGAUDIO_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                          maxWidth:
                                            PROGAUDIO_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                  ...
                                  onMouseEnter={(e) => {
                                    ...
                                    const next = normalizeProgAudioWeekRect(
                                      drag.rowIndex,
                                      drag.weekKey,
                                      rowIndex,
                                      col.weekKey,
                                    ...
                                    setWeekStripSelection(null)
                                    if (DEBUG_PROGAUDIO_MERGE) {
                                      console.debug("[Programmatic Audio merge] drag update", {
                                        rect: next,
                                      })
                                    }
                                  }}
                                ...
                                >
                                  ...
                                  >
                                    <Input
                                      ...
                                      setIsSelecting(true)
                                      if (DEBUG_PROGAUDIO_MERGE) {
                                        console.debug("[Programmatic Audio merge] drag start", {
                                          rowIndex,
                                          weekKey: col.weekKey,
                                        })
                                      }
                                      ...
                                      if (shift && lastWeekAnchorRef.current) {
                                        ...
                                        if (
                                          ...
                                          const rect = normalizeProgAudioWeekRect(
                                            lastWeekAnchorRef.current.rowIndex,
                                            lastWeekAnchorRef.current.weekKey,
                                            rowIndex,
                                            col.weekKey,
                    ...
                    >
                      <td
                        ...
                        style={{
                          ...
                          ...progAudioExpertTotalsRowBgStyle,
                        }}
                        colSpan={progAudioDescriptorKeys.length}
                      >
                      ...
                      <td
                        className={cn(
                          stickyTd(progAudioDescriptorKeys.length),
                          "h-8 px-1 text-xs tabular-nums"
                        )}
                        style={{
                          ...stickyStyleBody(progAudioDescriptorKeys.length),
                          ...progAudioExpertTotalsRowBgStyle,
                        }}
                      >
                      ...
                      <td
                        className={cn(
                          stickyTd(progAudioDescriptorKeys.length + 1),
                          "h-8"
                        )}
                        style={{
                          ...stickyStyleBody(progAudioDescriptorKeys.length + 1),
                          ...progAudioExpertTotalsRowBgStyle,
                        }}
                      />
                      <td
                        className={cn(
                          stickyTd(progAudioDescriptorKeys.length + 2),
                          "h-8 px-1 text-xs tabular-nums text-muted-foreground",
                          PROGAUDIO_EXPERT_WEEK_SCROLLER_EDGE
                        )}
                        style={{
                          ...stickyStyleBody(progAudioDescriptorKeys.length + 2),
                          ...progAudioExpertTotalsRowBgStyle,
                        }}
                      >
