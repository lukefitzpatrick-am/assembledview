...
- `0079_campaign_reads.sql` — `campaign_reads` (serial PK, mba+version, draft|published, beats jsonb, body_markdown, sources, generate/edit/publish stamps). Indexes `(mba_number, version_number, status)` and `(mba_number, generated_at desc)`. One row per generation; publish unpublishes earlier rows for the same mba+version. RLS on; no `ava_readonly` grant. Marker `0079_campaign_reads`. AUTHOR ONLY. Do not apply until Luke runs it. Do not drizzle-kit. Mirror: `db/schema/campaignReads.ts`. Do not SELECT this table against live Postgres before applying (C-76).
