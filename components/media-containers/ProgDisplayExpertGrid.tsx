 ...
 */
const PROGDISPLAY_EXPERT_ROW_VIRTUALIZATION = true
const PROGDISPLAY_EXPERT_ROW_HEIGHT_PX = OOH_EXPERT_ROW_HEIGHT_PX
const PROGDISPLAY_EXPERT_ROW_OVERSCAN = OOH_EXPERT_ROW_OVERSCAN
...
export function ProgDisplayExpertGrid({
  ...
  const normalizeRowCacheRef = useRef(
    new Map<string, NormalizeRowCacheEntry<ProgDisplayExpertScheduleRow>>()
  )
  ...
  const rowMergeMapCacheRef = useRef(
    new Map<
      string,
      MapBuildCacheEntry<
        ProgDisplayExpertScheduleRow["mergedWeekSpans"],
        ProgDisplayRowMergeMap
      >
    >()
  )
  ...
  const rowReorderVirtual = useMemo(
    () =>
      PROGDISPLAY_EXPERT_ROW_VIRTUALIZATION
        ? {
            rowCount: normalizedRows.length,
            estimateSizePx: PROGDISPLAY_EXPERT_ROW_HEIGHT_PX,
  ...
  const ensureRowVisible = useCallback((rowIndex: number) => {
    if (!PROGDISPLAY_EXPERT_ROW_VIRTUALIZATION) return
  ...
  const widthForWeekKey = useCallback(
    (k: string): number => {
      ...
      return weekColumnWidths[k] ?? PROGDISPLAY_EXPERT_WEEK_COL_WIDTH_PX
    },
            ...
            ? Math.min(
                ...
                Math.max(
                  ...
                  deltaDaysFromPx(prev.edgeYmd, dx, {
                    ...
                    dayKeysByWeekKey,
                    weekColumnWidths,
                    defaultWeekColWidthPx: PROGDISPLAY_EXPERT_WEEK_COL_WIDTH_PX,
  ...
  const {
    ...
    getScrollElement: () => gridScrollRef.current,
    estimateSize: PROGDISPLAY_EXPERT_ROW_HEIGHT_PX,
    overscan: PROGDISPLAY_EXPERT_ROW_OVERSCAN,
  ...
  const virtualSpacerColSpan = useMemo(() => {
    ...
    return (
      1 + progDisplayDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekCells
    )
  }, [
    weekColumns,
    expandedWeekKeys,
  ...
  const containerTotals = useMemo(() => {
    ...
    for (const row of normalizedRows) {
      const { net, fee } = expertRowCostSplit(row, weekKeys, feeprogdisplay)
      sumNet += net
      sumFee += fee
      for (const k of weekKeys) {
  ...
  }, [dayKeysByWeekKey, feeprogdisplay, normalizedRows, weekKeys])
        ...
        <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2 border-b border-border/60 bg-muted/5 pb-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <Label htmlFor="progdisplay-expert-row-count" className="sr-only">Add rows count</Label>
              <Input
                        ...
                        return (
                          ...
                          >
                            ...
                            <ExpertGridWeekResizeHandle
                              weekKey={col.weekKey}
                              currentWidth={weekColumnWidths[col.weekKey] ?? PROGDISPLAY_EXPERT_WEEK_COL_WIDTH_PX}
                  ...
                  <tbody>
                    {(() => {
                      ...
                      return (
                        <MemoExpertGridRow
                          ...
                          rowUiSig={rowUiSig}
                          render={() => {
                      const net = expertRowNetMedia(row, weekKeys, feeprogdisplay)
                      ...
                      return (
                        <tr
                          ...
                          data-progdisplay-expert-row-index={rowIndex}
                          style={{
                            ...stripeStyle,
                            height: PROGDISPLAY_EXPERT_ROW_HEIGHT_PX,
                            maxHeight: PROGDISPLAY_EXPERT_ROW_HEIGHT_PX,
                          }}
                          ...
                          {(() => {
                            ...
                            for (let wi = 0; wi < weekColumns.length; wi += 1) {
                              ...
                              const tdClassName = cn(
                                ...
                                !isMergedAnchorCell &&
                                  mergeReadyOnCell &&
                                  "progdisplay-expert-week-cell--merge-ready z-[6] bg-pacing-behind-bg ring-2 ring-inset ring-pacing-behind shadow-e0",
                      ...
                      if (!PROGDISPLAY_EXPERT_ROW_VIRTUALIZATION) {
                    ...
                    >
                      ...
                      <td
                        ...
                        style={stickyStyleBody(progDisplayDescriptorKeys.length)}
                      >
                      ...
                      <td
                        ...
                        style={stickyStyleBody(progDisplayDescriptorKeys.length + 1)}
                      />
                      <td
                        className={cn(
                        ...
                        style={stickyStyleBody(progDisplayDescriptorKeys.length + 2)}
                      >
