...
| `app/api/cron/xero-sync` | Writes `xero_*` + `finance_billing_records` (`xero:` keys) + Blob PDFs; Xano task still active until T6 — dual writers; parity via `db:xero-parity` before any `db:etl` |
