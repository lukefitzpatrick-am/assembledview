...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.error("[api/pacing/alert-subscriptions GET]", e)
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const payload = await xanoPacingPost(PACING_ALERT_SUBS_PATH, body)
    return pacingJsonOk(payload, { status: 201 })
  } catch (e) {
    console.error("[api/pacing/alert-subscriptions POST]", e)
    return pacingJsonError(e instanceof Error ? e.message : "xano_error", 500)
  }
}
