...
- **MBA billing override row precedence (MB-20 / MB-24):** `pendingBillingOverrideRows` (Applied, unsaved) > `savedBillingOverrideRows` (fetch-only table) > computed auto. `savedBillingOverrideRows` is written only by successful fetch and plan-change reset — never optimistic. Pending is derived only via `layerDraftMonthsOntoOverrideRows` (`buildPendingBillingOverrideRows`). Do not invent a second draft→rows path. Apply is state-only (MB-23); Cancel / X / Escape discard pending (saved cache untouched); Done keeps pending. Campaign save merges pending over saved only — un-Applied draft never reaches the payload (MB-22 REPLACE-SET).
