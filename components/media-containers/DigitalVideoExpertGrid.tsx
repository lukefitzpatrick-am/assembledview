...
import { deriveDigiVideoExpertRowScheduleYmdFromRow } from "@/lib/mediaplan/expertChannelMappings"
import {
  expertRowCostSplit,
  expertRowNetMedia,
...
import {
  ...
  WEEK_COL_WIDTH_PX as DIGIVIDEO_EXPERT_WEEK_COL_WIDTH_PX,
  WEEK_SCROLLER_EDGE as DIGIVIDEO_EXPERT_WEEK_SCROLLER_EDGE,
  WEEK_CELL_VISUAL_CLASSES as DIGIVIDEO_WEEK_CELL_VISUAL_CLASSES,
  ...
  normalizeWeekRect as normalizeDigiVideoWeekRect,
  weekCellInRect as digiVideoWeekCellInRect,
  ...
  normalizeWeekMergeSelection as normalizeDigiVideoWeekMergeSelection,
  weekPlainClickPreservesWeekAreaSelection,
  deriveMergeEligibility as deriveDigiVideoMergeEligibility,
  weekCellExportText,
...
type DigiVideoWeekRectSelection = ExpertWeekRectSelection
type DigiVideoMultiCellSelection = ExpertMultiCellSelection
type DigiVideoWeekMergeSelectionNormalized = WeekMergeSelectionNormalized
...
export function DigitalVideoExpertGrid({
  ...
  const normalizeRowCacheRef = useRef(
    new Map<string, NormalizeRowCacheEntry<DigiVideoExpertScheduleRow>>()
  )
  ...
  const rowMergeMapCacheRef = useRef(
    new Map<
      string,
      MapBuildCacheEntry<
        DigiVideoExpertScheduleRow["mergedWeekSpans"],
        DigiVideoRowMergeMap
      >
    >()
  )
  ...
  const pushRows = useCallback(
    (next: DigiVideoExpertScheduleRow[]) => {
      const withDates = finalizeRowsPreservingIdentity(next, (r) => {
        ...
        const dates = deriveDigiVideoExpertRowScheduleYmdFromRow(
          cleared,
          weekColumns,
          campaignStartDate,
          campaignEndDate,
  ...
  const widthForWeekKey = useCallback(
    (k: string): number => {
      ...
      return weekColumnWidths[k] ?? DIGIVIDEO_EXPERT_WEEK_COL_WIDTH_PX
    },
            ...
            ? Math.min(
                ...
                Math.max(
                  ...
                  deltaDaysFromPx(prev.edgeYmd, dx, {
                    ...
                    dayKeysByWeekKey,
                    weekColumnWidths,
                    defaultWeekColWidthPx: DIGIVIDEO_EXPERT_WEEK_COL_WIDTH_PX,
  ...
  const containerTotals = useMemo(() => {
    ...
    for (const row of normalizedRows) {
      const { net, fee } = expertRowCostSplit(row, weekKeys, feedigivideo)
      sumNet += net
      sumFee += fee
      for (const k of weekKeys) {
  ...
  }, [dayKeysByWeekKey, feedigivideo, normalizedRows, weekKeys])
                        ...
                        return (
                          ...
                          >
                            ...
                            <ExpertGridWeekResizeHandle
                              weekKey={col.weekKey}
                              currentWidth={weekColumnWidths[col.weekKey] ?? DIGIVIDEO_EXPERT_WEEK_COL_WIDTH_PX}
                    ...
                    {(() => {
                      ...
                      return (
                        <MemoExpertGridRow
                          ...
                          rowUiSig={rowUiSig}
                          render={() => {
                      const net = expertRowNetMedia(row, weekKeys, feedigivideo)
                          ...
                          {(() => {
                            ...
                            for (let wi = 0; wi < weekColumns.length; wi += 1) {
                              ...
                              const tdClassName = cn(
                                ...
                                !isMergedAnchorCell &&
                                  mergeReadyOnCell &&
                                  "digivideo-expert-week-cell--merge-ready z-[6] bg-pacing-behind-bg ring-2 ring-inset ring-pacing-behind shadow-e0",
