...
- Finance-hub schedule-spend treemaps (`GET /api/dashboard/global-monthly-{publisher,client}-spend` → `getGlobalMonthlyPublisherSpend` / `getGlobalMonthlyClientSpend`): list rows are `{ month, publisher|client, amount }` (month ISO `YYYY-MM-DD` string, identifiers strings, amount AUD dollars number). When `getDataBackendFor("plans") === "postgres"`, rows come from `lib/data/dashboardMonthlySpend.ts` (delivery-basis `schedule_months` on `masters.published_version_id`, publisher from `line_items`, client from `mp_client_name`; FY filter stays app-side). xano/shadow still call Xano `dashboard_monthly_*`. Soft-fail → empty months; UI honest fallback to FY client billing stays.
