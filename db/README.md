...
- `0079_campaign_reads.sql` — `campaign_reads` (six-beat dashboard campaign read; one row per generation; `status` draft\|published; indexes `(mba_number, version_number, status)` and `(mba_number, generated_at desc)`). RLS on; no `ava_readonly` grant. Marker `0079_campaign_reads`. AUTHOR ONLY. Do not apply until Luke runs it. Do not drizzle-kit. Mirror: `db/schema/campaignReads.ts`. Do not SELECT this table against live Postgres before applying (C-76).
