...
const SEARCH_EXPERT_WEEK_COL_WIDTH_PX = 112

/** Week column width on `<th>` and week `<td>`; inputs use `w-full` inside the cell. */
const searchExpertWeekColLayoutStyle = {
  width: SEARCH_EXPERT_WEEK_COL_WIDTH_PX,
  minWidth: SEARCH_EXPERT_WEEK_COL_WIDTH_PX,
  maxWidth: SEARCH_EXPERT_WEEK_COL_WIDTH_PX,
  boxSizing: "border-box" as const,
}

const MEDIA_ACCENT_HEX = getMediaTypeThemeHex("search")

const searchExpertHeaderCellBgStyle = {
  backgroundColor: rgbaFromHex(MEDIA_ACCENT_HEX, 0.08),
}

const searchExpertTotalsRowBgStyle = {
  backgroundColor: rgbaFromHex(MEDIA_ACCENT_HEX, 0.06),
}

/** Right edge of sticky block before the horizontally scrolling week area. */
const SEARCH_EXPERT_WEEK_SCROLLER_EDGE =
  "border-r-2 border-border/80 shadow-[2px_0_8px_-4px_rgba(0,0,0,0.08)] dark:shadow-[2px_0_8px_-4px_rgba(0,0,0,0.22)]"

const DEBUG_SEARCH_MERGE = false

function normalizeSearchKey(input: unknown): string {
  return String(input ?? "")
    .trim()
    .toLowerCase()
    .replace(/\s+/g, " ")
...
function sumMergedQuantities(row: SearchExpertScheduleRow): number {
...
): SearchExpertMergedWeekSpan | null {
...
function weeklyCellDisplayValue(
  ...
  mergedSpan: SearchExpertMergedWeekSpan | null
): string {
  if (mergedSpan) {
...
const SEARCH_WEEK_CELL_VISUAL_CLASSES = {
...
function selectionOverlapsMergedSpan(
  rowMergeMap: SearchRowMergeMap | undefined,
...
function normalizeSearchWeekRect(
  row0: number,
  wk0: string,
  row1: number,
  wk1: string,
  weekKeys: readonly string[]
): SearchWeekRectSelection {
...
function searchWeekCellInRect(
  rowIndex: number,
  weekKey: string,
  rect: SearchWeekRectSelection | null,
  weekKeys: readonly string[]
): boolean {
...
function coerceWeeklySelectionBounds(
  ...
  if (
    weekRectSelection &&
    searchWeekCellInRect(anchorRow, anchorWeekKey, weekRectSelection, weekKeys)
  ) {
...
function applyWeeklyPasteMatrixToSelection(args: {
  ...
  weekKeys: readonly string[]
  rowCount: number
  nextRows: SearchExpertScheduleRow[]
}): {
  applied: number
  errorReasons: string[]
  layout: WeeklyPasteLayoutMode
...
type SearchWeekMergeSelectionNormalized = {
  rowIndex: number
 ...
 */
function normalizeSearchWeekMergeSelection(
  rect: SearchWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): SearchWeekMergeSelectionNormalized | null {
...
function weekPlainClickPreservesWeekAreaSelection(
  ...
  const mergeNorm = normalizeSearchWeekMergeSelection(rect, multi, weekKeys)
  if (
    mergeNorm &&
  ...
  if (rect && searchWeekCellInRect(rowIndex, weekKey, rect, weekKeys)) {
    const isSingleCell =
...
function deriveSearchMergeEligibility(
  rect: SearchWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): SearchWeekMergeSelectionNormalized | null {
  return normalizeSearchWeekMergeSelection(rect, multi, weekKeys)
}
...
function mergedWeekSpansAfterCutRect(
  spans: SearchExpertMergedWeekSpan[] | undefined,
  wi0: number,
  wi1: number,
  weekKeys: readonly string[]
): SearchExpertMergedWeekSpan[] {
...
type WeeklyExportSelection =
  | { kind: "rect"; rect: SearchWeekRectSelection }
...
function resolveWeeklyExportSelection(
  ...
  weekKeys: readonly string[],
  rows: readonly SearchExpertScheduleRow[] | null
...
function buildWeeklyExportTsv(
  selection: WeeklyExportSelection,
  rows: readonly SearchExpertScheduleRow[],
  weekKeys: readonly string[]
): string | null {
  switch (selection.kind) {
    case "rect": {
...
function applyWeeklyCutToRows(
  ...
  switch (selection.kind) {
    case "rect": {
      ...
      if (wi0 < 0 || wi1 < 0) return null
      const next: SearchExpertScheduleRow[] = rows.map((r) => ({
        ...r,
    ...
    case "mergeContiguous": {
      ...
      const next: SearchExpertScheduleRow[] = rows.map((row) => ({
        ...row,
...
/** Match labels/values on {@link SearchContainer} buy-type combobox. */
const SEARCH_BUY_TYPE_OPTIONS: ComboboxOption[] = [
...
function normalizeSearchBuyTypePaste(raw: string): string {
  const v = raw.trim()
  if (!v) return ""
  const byValue = SEARCH_BUY_TYPE_OPTIONS.find(
...
function normalizeSearchPlatformPaste(raw: string, platformNames: string[]): string {
  const v = raw.trim()
  if (!v) return ""
...
function rowGrossCost(row: SearchExpertScheduleRow, weekKeys: string[]): number {
...
const SEARCH_DESCRIPTOR_CORE: readonly (keyof SearchExpertScheduleRow)[] = [
  "startDate",
  "endDate",
  "platform",
  "bidStrategy",
...
const SEARCH_BILLING_FLAG_KEYS: readonly (keyof SearchExpertScheduleRow)[] = [
  "fixedCostMedia",
  "clientPaysForMedia",
  "budgetIncludesFees",
]

const SEARCH_DESCRIPTOR_TAIL: readonly (keyof SearchExpertScheduleRow)[] = ["unitRate"]
...
type SearchRowMergeSpanMeta = Readonly<{
  id: string
  startWeekKey: string
  endWeekKey: string
  totalQty: number
...
export function SearchExpertGrid({
  campaignStartDate,
  campaignEndDate,
  feesearch,
  rows,
  onRowsChange,
  publishers = [],
}: SearchExpertGridProps) {
  const { toast } = useToast()
  const domGridId = useId().replace(/:/g, "")
  const [focusedCell, setFocusedCell] = useState<SearchExpertFocusedCell | null>(
    null
  )
  const focusedCellRef = useRef<SearchExpertFocusedCell | null>(null)
  ...
  const [weekRectSelection, setWeekRectSelection] =
    useState<SearchWeekRectSelection | null>(null)
  ...
  const lastDragRectDuringGestureRef = useRef<SearchWeekRectSelection | null>(null)
  /**
   ...
   */
  const postDragWeekClickRectRef = useRef<SearchWeekRectSelection | null>(null)
  const weekRectSelectionRef = useRef<SearchWeekRectSelection | null>(null)
  ...
  const searchDescriptorKeys = useMemo(
    () =>
      [
        ...SEARCH_DESCRIPTOR_CORE,
        ...SEARCH_BILLING_FLAG_KEYS,
        ...SEARCH_DESCRIPTOR_TAIL,
      ] as (keyof SearchExpertScheduleRow)[],
    []
  )
  ...
  const rowMergeMaps = useMemo<readonly SearchRowMergeMap[]>(() => {
  ...
  const navColCount =
    searchDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekKeys.length
  const firstWeekNavColIndex = searchDescriptorKeys.length + WEEK_GRID_COL_OFFSET
  const unitRateNavColIndex = searchDescriptorKeys.indexOf("unitRate")
  ...
  const duplicateRow = useCallback(
    (rowIndex: number) => {
      ...
      const id =
        typeof crypto !== "undefined" && crypto.randomUUID
          ? crypto.randomUUID()
          : `search-expert-${Date.now()}-${rowIndex}`
  ...
  const clearWeekSelectionWhereMergedAnchorInvolved = useCallback(
    (rowIndex: number, weekKey: string) => {
      setWeekRectSelection((prev) =>
        prev && searchWeekCellInRect(rowIndex, weekKey, prev, weekKeys)
          ? null
          : prev
      )
  ...
  const rangeWeekMultiSelect = useCallback(
    (rowIndex: number, anchorKey: string, endKey: string) => {
      ...
      setWeekRectSelection(
        normalizeSearchWeekRect(rowIndex, anchorKey, rowIndex, endKey, weekKeys)
      )
      setWeekStripSelection(null)
    },
    [weekKeys]
  ...
  const lockPendingMergeSelectionFromCurrentSelection = useCallback(() => {
    const n = deriveSearchMergeEligibility(
  ...
  useEffect(() => {
    ...
    const cellId = expertGridCellId(
      ...
      searchDescriptorKeys.length + WEEK_GRID_COL_OFFSET + lastWi
    )
    document
      .getElementById(cellId)
      ?.scrollIntoView({ behavior: "smooth", block: "nearest", inline: "nearest" })
  }, [domGridId, mergeTarget, mergeWeeksReady, searchDescriptorKeys.length, weekKeys])
  ...
  const containerTotals = useMemo(() => {
    ...
    const fee =
      feesearch > 0 && feesearch < 100 ? (sumGross * feesearch) / (100 - feesearch) : 0
    const totalWithFee = sumGross + fee

    return { sumGross, sumQty, perWeek, fee, totalWithFee }
  }, [feesearch, normalizedRows, weekKeys])
      ...
      <Card className="relative overflow-hidden border-0 shadow-md">
        ...
        <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2 border-b border-border/60 bg-muted/5 pb-3">
          <CardTitle className="text-base font-semibold tracking-tight">
            Search Media — Expert Schedule
          </CardTitle>
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2">
              <Label htmlFor="search-expert-row-count" className="text-sm whitespace-nowrap">
                Rows:
              </Label>
              <Input
                id="search-expert-row-count"
                type="number"
                min={1}
                max={500}
                    ...
                    <tr>
                      ...
                      {weekColumns.map((col) => (
                        <th
                          ...
                          style={{
                            ...searchExpertWeekColLayoutStyle,
                            ...searchExpertHeaderCellBgStyle,
                          }}
                          title={col.labelFull}
                        >
                  ...
                  <tbody>
                            ...
                            for (let wi = 0; wi < weekColumns.length; wi += 1) {
                              ...
                              const tdClassName = cn(
                                ...
                                isPopulatedNonMergedCell &&
                                  SEARCH_WEEK_CELL_VISUAL_CLASSES.populatedSingleTd,
                                ...
                                !isMergedAnchorCell &&
                                  mergeReadyOnCell &&
                                  "search-expert-week-cell--merge-ready z-[6] bg-amber-500/24 ring-2 ring-inset ring-amber-600/75 shadow-sm dark:bg-amber-500/20 dark:ring-amber-400/70",
                              ...
                              const mergedAnchorWrapperClassName = cn(
                                ...
                                isMergedAnchorCell &&
                                  SEARCH_WEEK_CELL_VISUAL_CLASSES.mergedSurface,
                              ...
                              const inputClassName = cn(
                                ...
                                isMergedAnchorCell
                                  ? SEARCH_WEEK_CELL_VISUAL_CLASSES.mergedInput
                              ...
                              renderedWeekCells.push(
                                <td
                                  ...
                                  style={
                                    isMergedAnchorCell
                                      ? {
                                          width: SEARCH_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                          minWidth:
                                            SEARCH_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                          maxWidth:
                                            SEARCH_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                  ...
                                  onMouseEnter={(e) => {
                                    ...
                                    const next = normalizeSearchWeekRect(
                                      drag.rowIndex,
                                      drag.weekKey,
                                      rowIndex,
                                      col.weekKey,
                    ...
                    >
                      ...
                      <td
                        ...
                        style={{
                          ...stickyStyleBody(searchDescriptorKeys.length),
                          ...searchExpertTotalsRowBgStyle,
                        }}
                      >
                      ...
                      <td
                        ...
                        style={{
                          ...stickyStyleBody(searchDescriptorKeys.length + 1),
                          ...searchExpertTotalsRowBgStyle,
                        }}
                      />
                      <td
                        ...
                        style={{
                          ...stickyStyleBody(searchDescriptorKeys.length + 2),
                          ...searchExpertTotalsRowBgStyle,
                        }}
                      >
