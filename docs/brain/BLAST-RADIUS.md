...
| `lib/mediaplan/drafts/*` / `0012_plan_working_drafts.sql` / `POST /api/plans/drafts` / create+edit draft chrome / `hooks/usePlanDraftSession.ts` | Stage 2b working-draft persistence + offer always on (GET/PUT/DELETE ungated); `NEXT_PUBLIC_PLAN_DRAFTS` gates autosave chrome only (default off; local may be on — not Vercel from Cursor). **Flag off retains rows** — offer + save-on-published stay reachable. Pill always from `describePlanSavePill`; edit header trail from `describeVersionHeaderTrail(planDraft.modeResolved)` (same resolver — unpublished tip never shows "Next: vN"); stale-base publish 409 when `baseVersionId` sent; Discard deletes the row; docs/pacing keep serving published tip |
...
| `lib/fireflies/{sync,runSync,timeEntryDrafts}.ts` / `lib/myhours/proposalRepo.ts` / `ava_time_entry_proposals` | A successful client-attributed Fireflies note fans out to roster-attendee time drafts. Re-sync of an existing eligible `client_notes` row repeats this upsert so note-insert/draft-upsert partial failures recover. Eligibility is non-internal + client id + attendee email intersecting active `team_members`; meeting date is Sydney civil date. Upsert refreshes proposed/blocked rows only — confirmed/skipped decisions are terminal. |
| `lib/mediaplans/ingest/*` / `app/admin/schedule-ingest` / `POST /api/admin/ingest/{review,remap,accept,ava-mapping}` / `0023_line_item_panels` / `0027_line_item_panel_flights` / `0024_publisher_profiles` / `0033_publisher_profiles_sen` / `0034_publisher_profiles_money_map` / `0035_publisher_profiles_reference_ignore` | Human gate for publisher schedules. Accept → `savePlanVersion` then panel+flight insert in one txn (never bypass). Remap mutates `publisher_profiles.column_map`. Proposal never mints `line_item_id`. Money: typed `column_map` targets (`media_rate:*`, `media_amount:stated`, `charge:*`); stated line total wins when present (SCA Client Total / JCD MEDIA VALUE); derived rate×period is authoritative only when no stated column (QMS weekly); derived-vs-stated >0.5% → warning only; file-total Δ>0.5% blocks Accept (409). `reference:ignore` is acknowledged (not unmapped / not AVA / not imported). Unmapped samples are deduped by header across line-item sheets before the AVA gate. Mapping rows key by sheet+header+index. Panels collapsed by default in review UI (flight summary per panel). AVA mapping proposals only when publisher confidence < 90% + unmapped columns — client POSTs samples to `/api/admin/ingest/ava-mapping` (`avaColumnMapping.server.ts`); `avaColumnMapping.ts` is client-safe — never auto-accept; never parses values/bursts/grid_semantics. Seeds: QMS, SCA, JCDecaux, SEN (JSON + migration must agree). **OOH panel-line count > `OOH_PANEL_LINE_EXPERT_THRESHOLD` (30)** → Accept sets `preferOohExpertView`; `OOHContainer` suppresses card list (summary + open expert). Flights: no money; dark letters = no row. |
