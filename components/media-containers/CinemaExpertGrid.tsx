...
import {
  ...
  WEEK_COL_WIDTH_PX as CINEMA_EXPERT_WEEK_COL_WIDTH_PX,
  WEEK_SCROLLER_EDGE as CINEMA_EXPERT_WEEK_SCROLLER_EDGE,
  WEEK_CELL_VISUAL_CLASSES as CINEMA_WEEK_CELL_VISUAL_CLASSES,
  ...
  normalizeWeekRect as normalizeCinemaWeekRect,
  weekCellInRect as cinemaWeekCellInRect,
  ...
  weekRangeOutlineFlags as cinemaWeekRangeOutlineFlags,
  weekOutlineEdgeClasses as cinemaWeekOutlineEdgeClasses,
  mergeReadyOutlineFlags as oohMergeReadyOutlineFlags,
  mergeReadyOutlineEdgeClasses as oohMergeReadyOutlineEdgeClasses,
  weekCellInMergePulseHighlight as cinemaWeekCellInMergePulseHighlight,
  mergeKeysFromRect as oohMergeKeysFromRect,
  normalizeWeekMergeSelection as normalizeCinemaWeekMergeSelection,
  weekPlainClickPreservesWeekAreaSelection,
  deriveMergeEligibility as deriveCinemaMergeEligibility,
  weekCellExportText,
...
type CinemaWeekRectSelection = ExpertWeekRectSelection
type CinemaMultiCellSelection = ExpertMultiCellSelection
type CinemaWeekMergeSelectionNormalized = WeekMergeSelectionNormalized

const MEDIA_ACCENT_HEX = getMediaTypeThemeHex("cinema")

const cinemaExpertHeaderCellBgStyle = {
  backgroundColor: rgbaFromHex(MEDIA_ACCENT_HEX, 0.08),
}

const cinemaExpertTotalsRowBgStyle = {
  backgroundColor: rgbaFromHex(MEDIA_ACCENT_HEX, 0.06),
}

const DEBUG_CINEMA_MERGE = false
 ...
 */
const CINEMA_EXPERT_ROW_VIRTUALIZATION = true
...
type CinemaCopiedCells = {
  data: (string | number)[][]
  sourceRows: number
  sourceCols: number
  selection: CinemaMultiCellSelection
}

/** Match labels/values on {@link CinemaContainer} buy-type combobox. */
const CINEMA_BUY_TYPE_OPTIONS: ComboboxOption[] = [
...
function normalizeCinemaBuyTypePaste(raw: string): string {
  const v = raw.trim()
  if (!v) return ""
  const byValue = CINEMA_BUY_TYPE_OPTIONS.find(
    (o) => o.value.toLowerCase() === v.toLowerCase()
  )
  if (byValue) return byValue.value
  const byLabel = CINEMA_BUY_TYPE_OPTIONS.find(
...
function normalizeCinemaFormatPaste(raw: string): string {
  return raw.trim()
}

function normalizeCinemaNetworkPaste(raw: string, networkNames: string[]): string {
  const v = raw.trim()
  if (!v) return ""
...
function normalizeCinemaStationPaste(raw: string, stationNames: string[]): string {
  const v = raw.trim()
  if (!v) return ""
...
const CINEMA_DESCRIPTOR_CORE: readonly (keyof CinemaExpertScheduleRow)[] = [
  "startDate",
  "endDate",
  "network",
  "station",
...
const CINEMA_BILLING_FLAG_KEYS: readonly (keyof CinemaExpertScheduleRow)[] = [
  "fixedCostMedia",
  "clientPaysForMedia",
  "budgetIncludesFees",
]

const CINEMA_DESCRIPTOR_TAIL: readonly (keyof CinemaExpertScheduleRow)[] = [
  "market",
  "buyingDemo",
  "unitRate",
]
...
export interface CinemaExpertGridProps {
  ...
  cinemaStations?: { station?: string | null; id?: number | string | null }[]
  onReorder?: () => void
}


type CinemaExpertFocusedCell = { rowIndex: number; columnKey: string }
type WeekDragSource =
  | {
      type: "single"
      rowIndex: number
...
type CinemaRowMergeSpanMeta = Readonly<{
  id: string
  startWeekKey: string
  endWeekKey: string
  totalQty: number
...
type CinemaRowMergeMap = Readonly<{
  anchorByWeekKey: Readonly<Record<string, string>>
  interiorByWeekKey: Readonly<Record<string, string>>
  spanById: Readonly<Record<string, CinemaExpertMergedWeekSpan>>
  spanMetaByAnchorWeekKey: Readonly<Record<string, CinemaRowMergeSpanMeta>>
}>
...
export function CinemaExpertGrid({
  ...
  const [focusedCell, setFocusedCell] = useState<CinemaExpertFocusedCell | null>(
    null
  )
  const focusedCellRef = useRef<CinemaExpertFocusedCell | null>(null)
  ...
  const [weekRectSelection, setWeekRectSelection] =
    useState<CinemaWeekRectSelection | null>(null)
  // Multi-cell selection state for copy/paste operations.
  const [multiCellSelection, setMultiCellSelection] =
    useState<CinemaMultiCellSelection | null>(null)
  const [isSelecting, setIsSelecting] = useState(false)
  const [copiedCells, setCopiedCells] = useState<CinemaCopiedCells | null>(null)
  ...
  const lastDragRectDuringGestureRef = useRef<CinemaWeekRectSelection | null>(null)
  /**
   ...
   */
  const postDragWeekClickRectRef = useRef<CinemaWeekRectSelection | null>(null)
  const weekRectSelectionRef = useRef<CinemaWeekRectSelection | null>(null)
  ...
  const cinemaDescriptorKeys = useMemo(
    () =>
      [
        ...(showBillingCols ? CINEMA_BILLING_FLAG_KEYS : []),
        ...CINEMA_DESCRIPTOR_CORE,
        ...CINEMA_DESCRIPTOR_TAIL,
      ] as (keyof CinemaExpertScheduleRow)[],
    [showBillingCols]
  )
  ...
  const normalizeRowCacheRef = useRef(
    new Map<string, NormalizeRowCacheEntry<CinemaExpertScheduleRow>>()
  )
  ...
  const rowMergeMapCacheRef = useRef(
    new Map<
      string,
      MapBuildCacheEntry<
        CinemaExpertScheduleRow["mergedWeekSpans"],
        CinemaRowMergeMap
      >
    >()
  )
  const rowMergeMaps = useMemo<readonly CinemaRowMergeMap[]>(() => {
    const { maps, cache } = buildMapsPreservingIdentity(
      ...
      (row) => {
        ...
        const spanById: Record<string, CinemaExpertMergedWeekSpan> = {}
        const spanMetaByAnchorWeekKey: Record<string, CinemaRowMergeSpanMeta> = {}
        ...
        for (const span of row.mergedWeekSpans ?? []) {
          if (spanById[span.id]) {
            if (DEBUG_CINEMA_MERGE) {
              console.debug("[Cinema merge] occupancy duplicate span id ignored", {
                rowId: row.id,
                spanId: span.id,
              })
            }
          ...
          if (keys.some((k) => occupiedWeekKeys.has(k))) {
            if (DEBUG_CINEMA_MERGE) {
              console.debug("[Cinema merge] occupancy overlap ignored", {
                rowId: row.id,
                spanId: span.id,
                keys,
              })
  ...
  useEffect(() => {
    ...
    console.debug("[Cinema merge] pending selection updated", pendingMergeSelection)
  ...
  const pushRows = useCallback(
    (next: CinemaExpertScheduleRow[]) => {
      const withDates = finalizeRowsPreservingIdentity(next, (r) => {
        ...
        const dates = deriveCinemaExpertRowScheduleYmdFromRow(
          cleared,
          weekColumns,
          campaignStartDate,
          campaignEndDate,
  ...
  const rowReorderVirtual = useMemo(
    () =>
      CINEMA_EXPERT_ROW_VIRTUALIZATION
        ? {
            rowCount: normalizedRows.length,
            estimateSizePx: CINEMA_EXPERT_ROW_HEIGHT_PX,
  ...
  const updateRow = useCallback(
    (rowIndex: number, patch: Partial<CinemaExpertScheduleRow>) => {
  ...
  const tryFuzzyMatch = useCallback(
    (rowIndex: number, field: FuzzyMatchField, value: string) => {
      ...
      if (corrected) {
        updateRow(rowIndex, { [field]: corrected } as Partial<CinemaExpertScheduleRow>)
        return
      }
      ...
      if (fuzzyMatchAutoApplyRef.current) {
        updateRow(rowIndex, { [field]: match.matched } as Partial<CinemaExpertScheduleRow>)
      } else {
  ...
  const navColCount =
    cinemaDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekKeys.length
  const firstWeekNavColIndex = cinemaDescriptorKeys.length + WEEK_GRID_COL_OFFSET
  const unitRateNavColIndex = cinemaDescriptorKeys.indexOf("unitRate")
  ...
  const ensureRowVisible = useCallback((rowIndex: number) => {
    if (!CINEMA_EXPERT_ROW_VIRTUALIZATION) return
            ...
            ? Math.min(
                ...
                Math.max(
                  ...
                  deltaDaysFromPx(prev.edgeYmd, dx, {
                    ...
                    dayKeysByWeekKey,
                    weekColumnWidths,
                    defaultWeekColWidthPx: CINEMA_EXPERT_WEEK_COL_WIDTH_PX,
  ...
  const unmergeWeekSpan = useCallback(
    (rowIndex: number, spanId: string) => {
      ...
      if (DEBUG_CINEMA_MERGE) {
        console.debug("[Cinema merge] unmerge applied", { rowIndex, spanId })
      }
    },
    ...
    (rowIndex: number) => {
      const next = duplicateExpertRow(
        ...
        () =>
          typeof crypto !== "undefined" && crypto.randomUUID
            ? crypto.randomUUID()
            : `cinema-expert-${Date.now()}-${rowIndex}`,
        (i) =>
          typeof crypto !== "undefined" && crypto.randomUUID
            ? crypto.randomUUID()
            : `sp-${Date.now()}-${i}`
  ...
  const clearWeekSelectionWhereMergedAnchorInvolved = useCallback(
    (rowIndex: number, weekKey: string) => {
      setWeekRectSelection((prev) =>
        prev && cinemaWeekCellInRect(rowIndex, weekKey, prev, weekKeys)
          ? null
          : prev
      )
  ...
  const rangeWeekMultiSelect = useCallback(
    (rowIndex: number, anchorKey: string, endKey: string) => {
      ...
      setWeekRectSelection(
        normalizeCinemaWeekRect(rowIndex, anchorKey, rowIndex, endKey, weekKeys)
      )
      setWeekStripSelection(null)
    },
    [weekKeys]
  ...
  const lockPendingMergeSelectionFromCurrentSelection = useCallback(() => {
    const n = deriveCinemaMergeEligibility(
  ...
  const derivedMergeTarget = useMemo(() => {
    const n = deriveCinemaMergeEligibility(weekRectSelection, weekMultiSelect, weekKeys)
  ...
  useEffect(() => {
    const endDrag = () => {
      ...
      if (DEBUG_CINEMA_MERGE) {
        console.debug("[Cinema merge] drag end", {
          lastRect: r,
  ...
  const handleMergeSelectedWeeks = useCallback(() => {
    ...
    if (selectionOverlapsMergedSpan(rowMergeMaps[rowIndex], sorted, weekKeys)) {
      if (DEBUG_CINEMA_MERGE) {
        console.debug("[Cinema merge] validation overlap failure", {
          rowIndex,
    ...
    const newId =
      typeof crypto !== "undefined" && crypto.randomUUID
        ? crypto.randomUUID()
        : `merge-${Date.now()}`
    const newSpan: CinemaExpertMergedWeekSpan = {
      id: newId,
    ...
    if (DEBUG_CINEMA_MERGE) {
      console.debug("[Cinema merge] merge applied", {
        rowIndex,
        spanId: newSpan.id,
  ...
  useEffect(() => {
    ...
    const cellId = expertGridCellId(
      ...
      cinemaDescriptorKeys.length + WEEK_GRID_COL_OFFSET + lastWi
    )
    document
      .getElementById(cellId)
      ?.scrollIntoView({ behavior: "smooth", block: "nearest", inline: "nearest" })
  }, [domGridId, mergeTarget, mergeWeeksReady, cinemaDescriptorKeys.length, weekKeys])

  useEffect(() => {
    if (!DEBUG_CINEMA_MERGE) return
    console.debug("[Cinema merge] merge eligibility derived", {
      mergeTarget,
      derivedMergeTarget,
      mergeWeeksReady,
      weekRectSelection,
  ...
  const pasteMatrixIntoGrid = useCallback(
    (matrix: string[][]) => {
      ...
      if (fcIsWeek) {
        ...
        const nextRows: CinemaExpertScheduleRow[] = normalizedRows.map((r) => ({
          ...r,
      ...
      for (let dr = 0; dr < working.length; dr++) {
        ...
        for (let dc = 0; dc < pasteRow.length; dc++) {
          ...
          const field = target.field as keyof CinemaExpertScheduleRow
          ...
          if (field === "unitRate") {
            ...
            nextRows[targetRow] = {
              ...cur,
              [field]: truthy,
            } as CinemaExpertScheduleRow
            applied += 1
          } else if (field === "buyType") {
            nextRows[targetRow] = {
              ...cur,
              buyType: normalizeCinemaBuyTypePaste(raw),
            }
            applied += 1
          } else if (field === "format") {
            nextRows[targetRow] = {
              ...cur,
              format: normalizeCinemaFormatPaste(raw),
            }
            applied += 1
          } else if (field === "network") {
            nextRows[targetRow] = {
              ...cur,
              network: normalizeCinemaNetworkPaste(raw, networkNames),
            }
            applied += 1
          } else if (field === "station") {
            nextRows[targetRow] = {
              ...cur,
              station: normalizeCinemaStationPaste(raw, stationNames),
            }
            applied += 1
          } else {
            const v = raw.trim()
            nextRows[targetRow] = { ...cur, [field]: v } as CinemaExpertScheduleRow
            applied += 1
          }
        }
      }
  ...
  const containerTotals = useMemo(() => {
    ...
    for (const row of normalizedRows) {
      const { net, fee } = expertRowCostSplit(row, weekKeys, feecinema)
      sumNet += net
      sumFee += fee
      for (const k of weekKeys) {
  ...
  }, [dayKeysByWeekKey, feecinema, normalizedRows, weekKeys])
  ...
  const colIndexOf = useCallback(
    (key: keyof CinemaExpertScheduleRow) => cinemaDescriptorKeys.indexOf(key),
    [cinemaDescriptorKeys]
  )
  ...
  const {
    ...
    getScrollElement: () => gridScrollRef.current,
    estimateSize: CINEMA_EXPERT_ROW_HEIGHT_PX,
    overscan: CINEMA_EXPERT_ROW_OVERSCAN,
  ...
  const virtualSpacerColSpan = useMemo(() => {
    ...
    return (
      1 + cinemaDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekCells
    )
  }, [
    weekColumns,
    expandedWeekKeys,
      ...
      <Card className="relative overflow-hidden border-0 shadow-md">
        ...
        <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2 border-b border-border/60 bg-muted/5 pb-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <Label htmlFor="cinema-expert-row-count" className="sr-only">Add rows count</Label>
              <Input
                id="cinema-expert-row-count"
                type="number"
                min={1}
                max={500}
          ...
          <div className="overflow-hidden rounded-lg border border-border/80 bg-card/30 shadow-sm">
            ...
            {normalizedRows.length === 0 ? (
              ...
              <div
                ...
                data-cinema-expert-grid-scroll=""
              >
                  ...
                  <thead ref={theadRef} className="[&_tr]:border-b-0">
                    <tr>
                      <ExpertGridRowReorderHeaderCell
                        ...
                        style={cinemaExpertHeaderCellBgStyle}
                      />
                        ...
                        <th
                          ...
                          className={stickyThCorner(
                            i === descriptorHeadLabels.length - 1
                              ? CINEMA_EXPERT_WEEK_SCROLLER_EDGE
                              : undefined
                          )}
                          style={{
                  ...
                  <tbody>
                    {(() => {
                      ...
                      const stripeStyle =
                        rowIndex % 2 === 0
                          ...
                          : undefined
                      const grossCol = cinemaDescriptorKeys.length
                      const actionsCol = cinemaDescriptorKeys.length + 1
                      const sigmaCol = cinemaDescriptorKeys.length + 2
                      ...
                      return (
                        <tr
                          ...
                          data-cinema-expert-row-index={rowIndex}
                          style={{
                            ...stripeStyle,
                            height: CINEMA_EXPERT_ROW_HEIGHT_PX,
                            maxHeight: CINEMA_EXPERT_ROW_HEIGHT_PX,
                          }}
                            ...
                            for (let wi = 0; wi < weekColumns.length; wi += 1) {
                              ...
                              const stripOutlineRow =
                                ...
                                weekStripSelection.rowIndex === rowIndex
                                  ...
                                  : null
                              const rangeOutline = cinemaWeekRangeOutlineFlags(
                                rowIndex,
                                wi,
                                weekRectSelection,
                                stripOutlineRow,
                              ...
                              const inMergePulseHighlight = spanKeys.some(
                                (key) =>
                                  cinemaWeekCellInMergePulseHighlight(
                                    rowIndex,
                                    key,
                              ...
                              const tdClassName = cn(
                                ...
                                isPopulatedNonMergedCell &&
                                  CINEMA_WEEK_CELL_VISUAL_CLASSES.populatedSingleTd,
                                ...
                                !isMergedAnchorCell &&
                                  mergeReadyOnCell &&
                                  "cinema-expert-week-cell--merge-ready z-[6] bg-pacing-behind-bg ring-2 ring-inset ring-pacing-behind shadow-e0",
                                ...
                                isDragDropTargetCell &&
                                  !isDragDropTargetValid &&
                                  "cursor-not-allowed",
                                cinemaWeekOutlineEdgeClasses(rangeOutline),
                                !isMergedAnchorCell &&
                              ...
                              const mergedAnchorWrapperClassName = cn(
                                ...
                                isMergedAnchorCell &&
                                  CINEMA_WEEK_CELL_VISUAL_CLASSES.mergedSurface,
                              ...
                              const inputClassName = cn(
                                ...
                                isMergedAnchorCell
                                  ? CINEMA_WEEK_CELL_VISUAL_CLASSES.mergedInput
                              ...
                              renderedWeekCells.push(
                                <td
                                  ...
                                  onMouseEnter={(e) => {
                                    ...
                                    const next = normalizeCinemaWeekRect(
                                      drag.rowIndex,
                                      drag.weekKey,
                                      rowIndex,
                                      col.weekKey,
                                    ...
                                    setWeekStripSelection(null)
                                    if (DEBUG_CINEMA_MERGE) {
                                      console.debug("[Cinema merge] drag update", {
                                        rect: next,
                                      })
                                    }
                                  }}
                                ...
                                >
                                  ...
                                  >
                                    <Input
                                      ...
                                      setIsSelecting(true)
                                      if (DEBUG_CINEMA_MERGE) {
                                        console.debug("[Cinema merge] drag start", {
                                          rowIndex,
                                          weekKey: col.weekKey,
                                        })
                                      }
                                      ...
                                      if (shift && lastWeekAnchorRef.current) {
                                        ...
                                        if (
                                          ...
                                          const rect = normalizeCinemaWeekRect(
                                            lastWeekAnchorRef.current.rowIndex,
                                            lastWeekAnchorRef.current.weekKey,
                                            rowIndex,
                                            col.weekKey,
                      ...
                      if (!CINEMA_EXPERT_ROW_VIRTUALIZATION) {
                    ...
                    >
                      ...
                      <td
                        ...
                        colSpan={cinemaDescriptorKeys.length}
                      >
                      ...
                      <td
                        className={cn(
                          stickyTd(cinemaDescriptorKeys.length),
                          "h-8 px-1 text-xs tabular-nums"
                        )}
                        style={{
                          ...stickyStyleBody(cinemaDescriptorKeys.length),
                          ...cinemaExpertTotalsRowBgStyle,
                        }}
                      >
                      ...
                      <td
                        className={cn(
                          stickyTd(cinemaDescriptorKeys.length + 1),
                          "h-8"
                        )}
                        style={{
                          ...stickyStyleBody(cinemaDescriptorKeys.length + 1),
                          ...cinemaExpertTotalsRowBgStyle,
                        }}
                      />
                      <td
                        className={cn(
                          stickyTd(cinemaDescriptorKeys.length + 2),
                          "h-8 px-1 text-xs tabular-nums text-muted-foreground",
                          CINEMA_EXPERT_WEEK_SCROLLER_EDGE
                        )}
                        style={{
                          ...stickyStyleBody(cinemaDescriptorKeys.length + 2),
                          ...cinemaExpertTotalsRowBgStyle,
                        }}
                      >
        ...
        {DEBUG_CINEMA_MERGE ? (
          <div className="pointer-events-none absolute bottom-2 right-2 z-50 max-w-[360px] rounded-md border bg-background/95 p-2 text-[11px] leading-snug shadow-lg">
            <div className="mb-1 font-medium text-foreground">Cinema Merge Debug</div>
