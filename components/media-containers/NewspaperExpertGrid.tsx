...
function sumMergedQuantities(row: NewspaperExpertScheduleRow): number {
...
): NewspaperExpertMergedWeekSpan | null {
...
type NewspaperMultiCellSelection = {
  startRow: number
  endRow: number
  startCol: number
  endCol: number
...
type NewspaperCopiedCells = {
  data: (string | number)[][]
  sourceRows: number
  sourceCols: number
  selection: NewspaperMultiCellSelection
}

function weeklyCellDisplayValue(
  cellValue: number | string | undefined,
  mergedSpan: NewspaperExpertMergedWeekSpan | null
): string {
  if (mergedSpan) {
...
const NEWSPAPER_WEEK_CELL_VISUAL_CLASSES = {
...
function selectionOverlapsMergedSpan(
  rowMergeMap: NewspaperRowMergeMap | undefined,
...
type NewspaperWeekRectSelection = {
  rowStart: number
  rowEnd: number
  weekKeyStart: string
  weekKeyEnd: string
...
function normalizeNewspaperWeekRect(
  row0: number,
  wk0: string,
  row1: number,
  wk1: string,
  weekKeys: readonly string[]
): NewspaperWeekRectSelection {
...
function newspaperWeekCellInRect(
  rowIndex: number,
  weekKey: string,
  rect: NewspaperWeekRectSelection | null,
  weekKeys: readonly string[]
): boolean {
...
function rectToMultiCellSelection(
  rect: NewspaperWeekRectSelection | null,
  weekKeys: readonly string[]
): NewspaperMultiCellSelection | null {
  if (!rect) return null
  const i0 = weekKeys.indexOf(rect.weekKeyStart)
  const i1 = weekKeys.indexOf(rect.weekKeyEnd)
...
function coerceWeeklySelectionBounds(
  ...
  weekRectSelection: NewspaperWeekRectSelection | null,
  ...
  if (
    weekRectSelection &&
    newspaperWeekCellInRect(anchorRow, anchorWeekKey, weekRectSelection, weekKeys)
  ) {
...
function applyWeeklyPasteMatrixToSelection(args: {
  ...
  weekKeys: readonly string[]
  rowCount: number
  nextRows: NewspaperExpertScheduleRow[]
}): {
  applied: number
  errorReasons: string[]
  layout: WeeklyPasteLayoutMode
...
function oohMergeKeysFromRect(
  rect: NewspaperWeekRectSelection,
  weekKeys: readonly string[]
): string[] | null {
...
type NewspaperWeekMergeSelectionNormalized = {
  rowIndex: number
 ...
 */
function normalizeNewspaperWeekMergeSelection(
  rect: NewspaperWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): NewspaperWeekMergeSelectionNormalized | null {
...
function weekPlainClickPreservesWeekAreaSelection(
  ...
  const mergeNorm = normalizeNewspaperWeekMergeSelection(rect, multi, weekKeys)
  if (
    mergeNorm &&
  ...
  if (rect && newspaperWeekCellInRect(rowIndex, weekKey, rect, weekKeys)) {
    const isSingleCell =
...
function deriveNewspaperMergeEligibility(
  rect: NewspaperWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): NewspaperWeekMergeSelectionNormalized | null {
  return normalizeNewspaperWeekMergeSelection(rect, multi, weekKeys)
}
...
function mergedWeekSpansAfterCutRect(
  spans: NewspaperExpertMergedWeekSpan[] | undefined,
  wi0: number,
  wi1: number,
  weekKeys: readonly string[]
): NewspaperExpertMergedWeekSpan[] {
...
type WeeklyExportSelection =
  | { kind: "rect"; rect: NewspaperWeekRectSelection }
...
function resolveWeeklyExportSelection(
  ...
  weekKeys: readonly string[],
  rows: readonly NewspaperExpertScheduleRow[] | null
...
function buildWeeklyExportTsv(
  selection: WeeklyExportSelection,
  rows: readonly NewspaperExpertScheduleRow[],
  weekKeys: readonly string[]
): string | null {
  switch (selection.kind) {
    case "rect": {
...
function applyWeeklyCutToRows(
  ...
  rows: readonly NewspaperExpertScheduleRow[],
  weekKeys: readonly string[]
): NewspaperExpertScheduleRow[] | null {
  switch (selection.kind) {
    case "rect": {
      const rect = selection.rect
      const wi0 = Math.min(
      ...
      if (wi0 < 0 || wi1 < 0) return null
      const next: NewspaperExpertScheduleRow[] = rows.map((r) => ({
        ...r,
    ...
    case "mergeContiguous": {
      ...
      const next: NewspaperExpertScheduleRow[] = rows.map((row) => ({
        ...row,
...
/** Match labels/values on {@link NewspaperContainer} buy-type combobox. */
const NEWSPAPER_BUY_TYPE_OPTIONS: ComboboxOption[] = [
...
function normalizeNewspaperBuyTypePaste(raw: string): string {
  const v = raw.trim()
  if (!v) return ""
  const byValue = NEWSPAPER_BUY_TYPE_OPTIONS.find(
...
function normalizeNewspaperNetworkPaste(raw: string, networkNames: string[]): string {
  const v = raw.trim()
  if (!v) return ""
...
const NEWSPAPER_DESCRIPTOR_CORE: readonly (keyof NewspaperExpertScheduleRow)[] = [
  "startDate",
  "endDate",
  "network",
  "publisher",
...
type NewspaperRowMergeSpanMeta = Readonly<{
  id: string
  startWeekKey: string
  endWeekKey: string
  totalQty: number
...
type NewspaperRowMergeMap = Readonly<{
  anchorByWeekKey: Readonly<Record<string, string>>
  interiorByWeekKey: Readonly<Record<string, string>>
  spanById: Readonly<Record<string, NewspaperExpertMergedWeekSpan>>
  spanMetaByAnchorWeekKey: Readonly<Record<string, NewspaperRowMergeSpanMeta>>
}>
...
export function NewspaperExpertGrid({
  ...
  const [focusedCell, setFocusedCell] = useState<NewspaperExpertFocusedCell | null>(
    null
  )
  const focusedCellRef = useRef<NewspaperExpertFocusedCell | null>(null)
  ...
  const [weekRectSelection, setWeekRectSelection] =
    useState<NewspaperWeekRectSelection | null>(null)
  // Multi-cell selection state for copy/paste operations.
  const [multiCellSelection, setMultiCellSelection] =
    useState<NewspaperMultiCellSelection | null>(null)
  const [isSelecting, setIsSelecting] = useState(false)
  const [copiedCells, setCopiedCells] = useState<NewspaperCopiedCells | null>(null)
  ...
  const lastDragRectDuringGestureRef = useRef<NewspaperWeekRectSelection | null>(null)
  /**
   ...
   */
  const postDragWeekClickRectRef = useRef<NewspaperWeekRectSelection | null>(null)
  const weekRectSelectionRef = useRef<NewspaperWeekRectSelection | null>(null)
  ...
  const rowMergeMaps = useMemo<readonly NewspaperRowMergeMap[]>(() => {
    const maps = normalizedRows.map((row) => {
      const anchorByWeekKey: Record<string, string> = {}
      const interiorByWeekKey: Record<string, string> = {}
      const spanById: Record<string, NewspaperExpertMergedWeekSpan> = {}
      const spanMetaByAnchorWeekKey: Record<string, NewspaperRowMergeSpanMeta> = {}
      ...
      for (const span of row.mergedWeekSpans ?? []) {
        if (spanById[span.id]) {
          if (DEBUG_NEWSPAPER_MERGE) {
            console.debug("[Newspaper merge] occupancy duplicate span id ignored", {
              rowId: row.id,
        ...
        if (keys.some((k) => occupiedWeekKeys.has(k))) {
          if (DEBUG_NEWSPAPER_MERGE) {
            console.debug("[Newspaper merge] occupancy overlap ignored", {
              rowId: row.id,
              rowIndex: normalizedRows.indexOf(row),
              spanId: span.id,
              keys,
  ...
  useEffect(() => {
    if (!DEBUG_NEWSPAPER_MERGE) return
    ...
    console.debug("[Newspaper merge] pending selection updated", pendingMergeSelection)
  ...
  const updateRow = useCallback(
    (rowIndex: number, patch: Partial<NewspaperExpertScheduleRow>) => {
  ...
  const tryFuzzyMatch = useCallback(
    (rowIndex: number, field: FuzzyMatchField, value: string) => {
      ...
      if (corrected) {
        updateRow(rowIndex, { [field]: corrected } as Partial<NewspaperExpertScheduleRow>)
        return
      }
      ...
      if (fuzzyMatchAutoApplyRef.current) {
        updateRow(rowIndex, { [field]: match.matched } as Partial<NewspaperExpertScheduleRow>)
      } else {
  ...
  const navColCount =
    newspaperDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekKeys.length
  ...
  const unmergeWeekSpan = useCallback(
    (rowIndex: number, spanId: string) => {
      ...
      if (DEBUG_NEWSPAPER_MERGE) {
        console.debug("[Newspaper merge] unmerge applied", { rowIndex, spanId })
      }
    },
  ...
  const duplicateRow = useCallback(
    (rowIndex: number) => {
      ...
      const id =
        typeof crypto !== "undefined" && crypto.randomUUID
          ? crypto.randomUUID()
          : `newspaper-expert-${Date.now()}-${rowIndex}`
  ...
  const clearWeekSelectionWhereMergedAnchorInvolved = useCallback(
    (rowIndex: number, weekKey: string) => {
      setWeekRectSelection((prev) =>
        prev && newspaperWeekCellInRect(rowIndex, weekKey, prev, weekKeys)
          ? null
          : prev
      )
  ...
  const rangeWeekMultiSelect = useCallback(
    (rowIndex: number, anchorKey: string, endKey: string) => {
      ...
      setWeekRectSelection(
        normalizeNewspaperWeekRect(rowIndex, anchorKey, rowIndex, endKey, weekKeys)
      )
      setWeekStripSelection(null)
    },
    [weekKeys]
  ...
  const lockPendingMergeSelectionFromCurrentSelection = useCallback(() => {
    const n = deriveNewspaperMergeEligibility(
  ...
  const derivedMergeTarget = useMemo(() => {
    const n = deriveNewspaperMergeEligibility(weekRectSelection, weekMultiSelect, weekKeys)
  ...
  useEffect(() => {
    const endDrag = () => {
      ...
      if (DEBUG_NEWSPAPER_MERGE) {
        console.debug("[Newspaper merge] drag end", {
          lastRect: r,
  ...
  const handleMergeSelectedWeeks = useCallback(() => {
    ...
    if (selectionOverlapsMergedSpan(rowMergeMaps[rowIndex], sorted, weekKeys)) {
      if (DEBUG_NEWSPAPER_MERGE) {
        console.debug("[Newspaper merge] validation overlap failure", {
          rowIndex,
    ...
    const newId =
      typeof crypto !== "undefined" && crypto.randomUUID
        ? crypto.randomUUID()
        : `merge-${Date.now()}`
    const newSpan: NewspaperExpertMergedWeekSpan = {
      id: newId,
    ...
    if (DEBUG_NEWSPAPER_MERGE) {
      console.debug("[Newspaper merge] merge applied", {
        rowIndex,
        spanId: newSpan.id,
  ...
  useEffect(() => {
    ...
    const cellId = expertGridCellId(
      ...
      newspaperDescriptorKeys.length + WEEK_GRID_COL_OFFSET + lastWi
    )
    document
      .getElementById(cellId)
      ?.scrollIntoView({ behavior: "smooth", block: "nearest", inline: "nearest" })
  }, [domGridId, mergeTarget, mergeWeeksReady, newspaperDescriptorKeys.length, weekKeys])

  useEffect(() => {
    if (!DEBUG_NEWSPAPER_MERGE) return
    console.debug("[Newspaper merge] merge eligibility derived", {
      mergeTarget,
      derivedMergeTarget,
      mergeWeeksReady,
      weekRectSelection,
  ...
  const pasteMatrixIntoGrid = useCallback(
    (matrix: string[][]) => {
      ...
      if (fcIsWeek) {
        ...
        const nextRows: NewspaperExpertScheduleRow[] = normalizedRows.map((r) => ({
          ...r,
      ...
      for (let dr = 0; dr < working.length; dr++) {
        ...
        for (let dc = 0; dc < pasteRow.length; dc++) {
          ...
          const field = target.field as keyof NewspaperExpertScheduleRow
          ...
          if (field === "unitRate") {
            ...
            nextRows[targetRow] = {
              ...cur,
              network: normalizeNewspaperNetworkPaste(raw, networkNames),
            }
            applied += 1
          } else {
            const v = raw.trim()
            nextRows[targetRow] = { ...cur, [field]: v } as NewspaperExpertScheduleRow
            applied += 1
          }
        }
      }
      ...
      <Card className="relative overflow-hidden border-0 shadow-md">
        ...
        <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2 border-b border-border/60 bg-muted/5 pb-3">
          <CardTitle className="text-base font-semibold tracking-tight">
            Newspaper Media — Expert Schedule
          </CardTitle>
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2">
              <Label htmlFor="newspaper-expert-row-count" className="text-sm whitespace-nowrap">
                Rows:
              </Label>
              <Input
                id="newspaper-expert-row-count"
                type="number"
                min={1}
                max={500}
          ...
          <div className="overflow-hidden rounded-lg border border-border/80 bg-card/30 shadow-sm">
            {normalizedRows.length === 0 ? (
              ...
              <div
                ...
                data-newspaper-expert-grid-scroll=""
              >
                <table className="w-max min-w-full border-collapse text-sm">
                    ...
                    <tr>
                      ...
                      {weekColumns.map((col) => (
                        <th
                          ...
                          style={{
                            ...newspaperExpertWeekColLayoutStyle,
                            ...newspaperExpertHeaderCellBgStyle,
                          }}
                          title={col.labelFull}
                        >
                  ...
                  <tbody>
                    {normalizedRows.map((row, rowIndex) => {
                      ...
                      const stripeStyle =
                        rowIndex % 2 === 0
                          ...
                          : undefined
                      const grossCol = newspaperDescriptorKeys.length
                      const actionsCol = newspaperDescriptorKeys.length + 1
                      const sigmaCol = newspaperDescriptorKeys.length + 2
                        ...
                        >
                          ...
                          {(() => {
                            ...
                            for (let wi = 0; wi < weekColumns.length; wi += 1) {
                              ...
                              const colIndex =
                                newspaperDescriptorKeys.length +
                              ...
                              const tdClassName = cn(
                                ...
                                !isMergedAnchorCell &&
                                  mergeReadyOnCell &&
                                  "newspaper-expert-week-cell--merge-ready z-[6] bg-amber-500/24 ring-2 ring-inset ring-amber-600/75 shadow-sm dark:bg-amber-500/20 dark:ring-amber-400/70",
                              ...
                              const mergedAnchorWrapperClassName = cn(
                                ...
                                isMergedAnchorCell &&
                                  NEWSPAPER_WEEK_CELL_VISUAL_CLASSES.mergedSurface,
                              ...
                              const inputClassName = cn(
                                ...
                                isMergedAnchorCell
                                  ? NEWSPAPER_WEEK_CELL_VISUAL_CLASSES.mergedInput
                              ...
                              renderedWeekCells.push(
                                <td
                                  ...
                                  style={
                                    isMergedAnchorCell
                                      ? {
                                          width: NEWSPAPER_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                          minWidth:
                                            NEWSPAPER_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                          maxWidth:
                                            NEWSPAPER_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                  ...
                                  onMouseEnter={(e) => {
                                    ...
                                    const next = normalizeNewspaperWeekRect(
                                      drag.rowIndex,
                                      drag.weekKey,
                                      rowIndex,
                                      col.weekKey,
                                    ...
                                    setWeekStripSelection(null)
                                    if (DEBUG_NEWSPAPER_MERGE) {
                                      console.debug("[Newspaper merge] drag update", {
                                        rect: next,
                                      })
                                    }
                                  }}
                                ...
                                >
                                  ...
                                  >
                                    <Input
                                      ...
                                      setIsSelecting(true)
                                      if (DEBUG_NEWSPAPER_MERGE) {
                                        console.debug("[Newspaper merge] drag start", {
                                          rowIndex,
                                          weekKey: col.weekKey,
                                        })
                                      }
                                      ...
                                      if (shift && lastWeekAnchorRef.current) {
                                        ...
                                        if (
                                          ...
                                          const rect = normalizeNewspaperWeekRect(
                                            lastWeekAnchorRef.current.rowIndex,
                                            lastWeekAnchorRef.current.weekKey,
                                            rowIndex,
                                            col.weekKey,
                    ...
                    >
                      ...
                      <td
                        className={cn(
                          stickyTd(newspaperDescriptorKeys.length + 2),
        ...
        {DEBUG_NEWSPAPER_MERGE ? (
          <div className="pointer-events-none absolute bottom-2 right-2 z-50 max-w-[360px] rounded-md border bg-background/95 p-2 text-[11px] leading-snug shadow-lg">
            <div className="mb-1 font-medium text-foreground">Newspaper merge debug</div>
