/**
 * F-28 Phase 2 — Search Expert row virtualization smoke (Prompt A items 1,3,5,7,8).
 */
import assert from "node:assert/strict"
import test from "node:test"
import { format, startOfDay } from "date-fns"

import { mapSearchExpertRowsToStandardLineItems } from "@/lib/mediaplan/expertChannelMappings"
import {
  reorderExpertRows,
  mergedSpanWidthPx,
...
function emptyRow(id: string): SearchExpertScheduleRow {
...
function buildRows(n: number): SearchExpertScheduleRow[] {
  return Array.from({ length: n }, (_, i) => emptyRow(`search-v-${i}`))
}

/** Mirrors SearchExpertGrid containerTotals over the FULL logical array. */
function computeSearchWeeklyTotals(
...
test("1. APPLY CORRECTNESS: edit on off-window row survives Apply (full state, not DOM)", () => {
  ...
  const patched = updateRowAtIndex(rows, offWindowIndex, {
    ...
    bidStrategy: "OFF-WINDOW-BID-STRATEGY",
    buyType: "cpc",
    unitRate: 100,
    weeklyValues: {
  ...
  assert.equal(standard[offWindowIndex]!.bidStrategy, "OFF-WINDOW-BID-STRATEGY")
...
test("3. DRAG REORDER across virtual boundary lands on correct logical index", () => {
  ...
  const dropIndex = virtualRowIndexFromOffsetY(
    280 * SEARCH_EXPERT_ROW_HEIGHT_PX + 1,
    SEARCH_EXPERT_ROW_HEIGHT_PX,
    300
  )
...
test("5. WEEK-COLUMN WIDTH respects resize overrides (sticky header/week resize path)", () => {
  ...
  assert.equal(SEARCH_EXPERT_ROW_HEIGHT_PX, 41)
})
...
test("7. WEEKLY TOTALS derived from full 300-row state", () => {
  ...
  const totals = computeSearchWeeklyTotals(rows, 0)
  ...
  const after = computeSearchWeeklyTotals(edited, 0)
...
test("8. PERF: mounted DOM window stays bounded while scrolling 300 rows", () => {
  ...
  const nearTop = expectedMountedRowRange(
    ...
    SEARCH_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    SEARCH_EXPERT_ROW_OVERSCAN
  )
  const mid = expectedMountedRowRange(
    150 * SEARCH_EXPERT_ROW_HEIGHT_PX,
    viewportHeight,
    SEARCH_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    SEARCH_EXPERT_ROW_OVERSCAN
  ...
  const nearBottom = expectedMountedRowRange(
    280 * SEARCH_EXPERT_ROW_HEIGHT_PX,
    viewportHeight,
    SEARCH_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    SEARCH_EXPERT_ROW_OVERSCAN
  ...
  const maxWindow =
    Math.ceil(viewportHeight / SEARCH_EXPERT_ROW_HEIGHT_PX) +
    2 * SEARCH_EXPERT_ROW_OVERSCAN +
    2
