...
function line(bursts: any[]): StandardProgAudioFormLineItem {
  return {
    platform: "P", bidStrategy: "", buyType: "bonus", site: "", placement: "", targetingAttribute: "", creativeTargeting: "", creative: "", buyingDemo: "", market: "",
...
test("sub-week interior burst imports as day-detail and round-trips", () => {
  ...
  const rows = mapStandardProgAudioLineItemsToExpertRows(std, cols, CS, CE)
  const wk0 = cols[0]!
  assert.equal(weekHasDailyValues(rows[0]!.dailyValues ?? {}, weekDayKeys(wk0, CS, CE)), true)
  const back = mapProgAudioExpertRowsToStandardLineItems(rows, cols, CS, CE, { feePctProgAudio: 0 })
