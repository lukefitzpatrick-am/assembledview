...
| Client import of `@/db` or `server-only` | Split a `*.server.ts` sibling (`import "server-only"`, static `@/db`). `await import("@/db")` still bundles. Gate: `npm run check:client-server-only`. Do not add `webpackIgnore` except the isomorphic `lib/api.ts` allowlist |
