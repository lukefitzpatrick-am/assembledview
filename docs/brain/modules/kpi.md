...
- `lib/kpi/kpiReview.ts` — campaign MBA per-channel KPI review (`buildKpiReview({ groups, lineItemTargets })`). Groups are the same channel-platform buckets `channelCoverage` produces (Social · Meta, Prog Video · Channel Factory, BVOD). Targets stay at line grain — shared value, else planned-spend weighted; never averaged across channels. A set metric carries `targetSource` `target` (saved `campaign_kpi`) or `benchmark` (industry backfill, `benchmark_ref` on hover). 0/null is "No target set" at the row (`targetSource` null) — it does not inherit `publisher_kpi`. When every row on a card is unset, the card is `noTargets` and the UI replaces the rows: admin “No KPI targets saved for {channel}. Set targets on the media plan →” (`/mediaplans/mba/{mba}/edit#builder-section-kpis`); client “KPI targets pending.” Mixed cards still grey omitted metrics for admins and hide them from clients. `shouldShowKpiReview` hides the section from clients unless a card has a saved or benchmark target; admins always see it when groups exist. Delivered: CTR = clicks/impressions, conversion_rate = results/clicks, CPV = card spend / views (VIDEO_3S_VIEWS social, views Channel Factory, completes BVOD; modelled tag when the card is modelled). VTR = completes/impressions only where COMPLETED_VIEWS exists (Channel Factory partner file — PACING_FACT dual-writes it into VIDEO_3S_VIEWS; CM360 and Meta → "Not tracked for this source"). Frequency → "Not tracked yet" (C-134). Status via `deliveryStatusFromPct` (higher-is-better for CTR/CVR/VTR; lower-is-better for CPV). Mount: `components/dashboard/campaign/KpiReview.tsx`. Tests: `npm run test:kpi-review`.
...
- `lib/kpi/kpiWorkbook.ts` — client-side standalone Campaign KPIs workbook (`buildKpiWorkbookBlob` + `toKpiSheetRows`). `KPISection` Download KPIs and both create/edit zip-all paths use it; media-plan xlsx append uses the same row mapping. No plan save, no KPI write, no network. Tests: `npm run test:kpi-writes`.
- `components/kpis/{KPIEditModal,KPISection,kpiHost}` — shared modal + the `KpiHost` contract with two persistence semantics: the media-plan host persists `campaign_kpi` immediately when the plan has an identity (create defers until the first save in session); the pacing host syncs immediately. Plan save still runs its own sync for the version it just wrote (including VP-1 increments). `KPISection` Download KPIs is outline, disabled when `kpiRows` is empty or loading, and writes `KPIs_<campaign name>.xlsx` via `buildKpiWorkbookBlob`.
