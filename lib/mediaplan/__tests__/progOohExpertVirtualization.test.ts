/**
 * F-28 Phase 2 — Programmatic OOH Expert row virtualization smoke (Prompt A items 1,3,5,7,8).
 */
import assert from "node:assert/strict"
import test from "node:test"
import { format, startOfDay } from "date-fns"

import { mapProgOohExpertRowsToStandardLineItems } from "@/lib/mediaplan/expertChannelMappings"
import {
  reorderExpertRows,
  mergedSpanWidthPx,
...
function emptyRow(id: string): ProgOohExpertScheduleRow {
...
function buildRows(n: number): ProgOohExpertScheduleRow[] {
  return Array.from({ length: n }, (_, i) => emptyRow(`progooh-v-${i}`))
}

/** Mirrors ProgOOHExpertGrid containerTotals over the FULL logical array. */
function computeProgOohWeeklyTotals(
...
test("5. WEEK-COLUMN WIDTH respects resize overrides (sticky header/week resize path)", () => {
  ...
  assert.equal(PROGOOH_EXPERT_ROW_HEIGHT_PX, 41)
})
...
test("7. WEEKLY TOTALS derived from full 300-row state", () => {
  ...
  const totals = computeProgOohWeeklyTotals(rows, 0)
  ...
  const after = computeProgOohWeeklyTotals(edited, 0)
