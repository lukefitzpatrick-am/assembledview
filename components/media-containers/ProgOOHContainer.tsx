...
export default function ProgOOHContainer({
  ...
  const form = useForm<ProgOOHFormValues>({
    resolver: zodResolver(progOOHFormSchema),
    defaultValues: {
      lineItems: [
        {
          platform: "",
    ...
    const lineItemTotals = watchedLineItems.map((lineItem, index) => {
      ...
      lineItem.bursts.forEach((burst) => {
        ...
        if (lineItem.budgetIncludesFees) {
          lineFee += (budget / 100) * (feeprogooh || 0);
          lineMedia += (budget / 100) * (100 - (feeprogooh || 0));
        } else {
          lineMedia += budget;
...
useEffect(() => {
  ...
  }, [watchedLineItems, feeprogooh, onInvestmentChange, onBurstsChange, onTotalMediaChange, form]);
