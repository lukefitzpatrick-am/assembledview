...
**FN7 status (2026-08-01):** Classic hub cut over to Finance sections IA. Register items **Load-gate (F5.3 / UX-1)**, **$0/empty landing (UX-2)**, **dead hub Overview treemaps (F10 / UX-3)**, **CF1** are **fixed pending live verification** — not FIXED until live smoke passes (see `docs/brain/KNOWN-ISSUES.md` UX-* rows). This file remains historical discovery; product surface is `components/finance/sections/*`.
...
- **F5.3 (~85%)** Load required before any data appears. Load button is far from the primary action. **fixed pending live verification (FN7)** — classic hub deleted; sections Apply → auto-load (UX-1).
...
- **F10.1 (~80%)** Treemaps show relative size, not absolute amounts or trends. Wrong visualisation for the question. **fixed pending live verification (FN7)** for hub surface — Overview treemaps removed with hub (UX-3); C-26 residual on dashboard path only.
- **F10.2 (~70%)** No drilldown from the treemap to contributing line items. **fixed pending live verification (FN7)** — hub surface gone.
- **F10.3 (~60%)** These charts live on Overview, which the finance team opens least often. Data is in the wrong place for the users who'd benefit from it. **fixed pending live verification (FN7)** — hub Overview gone.
...
- **CF1 (~90%)** KPI hero is fixed on all tabs. Useful on Overview, noise on the four working tabs. ~150px of fixed vertical real estate paid for on every load. **fixed pending live verification (FN7)** — hub hero deleted; sections landing has no tab-strip KPI hero (UX-2).
