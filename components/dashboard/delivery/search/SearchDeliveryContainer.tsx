...
import { clampProgress, SmallProgressCard } from "@/components/dashboard/delivery/SmallProgressCard"
...
type SearchDeliveryContainerProps = {
  clientSlug: string
  mbaNumber: string
  lineItemIds: string[]
  searchLineItems?: any[]
...
export default function SearchDeliveryContainer({
  clientSlug,
  mbaNumber,
  lineItemIds,
  searchLineItems,
  ...
  kpiTargets,
}: SearchDeliveryContainerProps): React.ReactElement | null {
  ...
  const [clicksChartMode, setClicksChartMode] = useState<"cumulative" | "daily">(
  ...
  useEffect(() => {
    // Guard: Skip auto-fetch if initialSearchData is provided (from DeliveryDataProviderWrapper)
  ...
  // --- Stage 3c-i: cumulative target curve for clicks ---

  /**
   * Derive the TargetCurveLineItem shape from the already-computed
   * scheduleByLineItemId map (which stores normalised bursts) plus the raw
   * searchLineItems prop (for publisher/bidStrategy/buyType).
   */
  const targetCurveLineItems = useMemo<TargetCurveLineItem[]>(() => {
    const items = Array.isArray(searchLineItems) ? searchLineItems : []
    return items
      .map((item): TargetCurveLineItem | null => {
        ...
        for (const b of bursts) {
          if (!earliestStartISO || b.startISO < earliestStartISO) {
            earliestStartISO = b.startISO
          }
          if (!latestEndISO || b.endISO > latestEndISO) {
            latestEndISO = b.endISO
          }
        }
        ...
        const publisher = String(
          item?.platform ?? item?.site ?? item?.publisher ?? "",
        )
          .trim()
          .toLowerCase()
        const bidStrategy = String(
          item?.bid_strategy ?? item?.bidStrategy ?? buyType ?? "",
        )
          .trim()
          .toLowerCase()
  ...
  }, [searchLineItems, scheduleByLineItemId])
  ...
  const dailyClicksByDate = useMemo(() => {
    const m = new Map<string, number>()
  ...
  }, [targetCurve, dailyClicksByDate])

  const cumulativeActualByDate = useMemo(() => {
    const m = new Map<string, number>()
    for (const row of cumulativeActual) m.set(row.date, row.actual)
    return m
  }, [cumulativeActual])

  const onTrackStatus: OnTrackStatus = useMemo(
    () => evaluateOnTrack(targetCurve, cumulativeActualByDate, refLineISO),
    [targetCurve, cumulativeActualByDate, refLineISO],
  )

  /**
   * Merged chart data for the cumulative view. Rows are joined by date, so
   * `actual` and `target`/`targetLow`/`targetHigh` align on the x-axis.
   */
  const chartClicksCumulative = useMemo(() => {
    if (targetCurve.length === 0) return []
    const actualMap = cumulativeActualByDate
    return targetCurve.map((p) => ({
      date: p.date,
      target: p.target,
      targetLow: p.targetLow,
      targetHigh: p.targetHigh,
      actual: actualMap.get(p.date) ?? 0,
    }))
  }, [targetCurve, cumulativeActualByDate])
  ...
  return (
    <DeliveryCard
      icon={Search}
      title={c("Search delivery", "Search Pacing")}
      ...
      <div className="space-y-6">
        ...
        {isLoading ? (
          ...
          <div className="flex min-h-[200px] items-center justify-center text-sm text-muted-foreground">
            {c("No Search delivery data available", "No Search pacing data available")}
          </div>
        ) : (
          <>
            ...
            <DeliverySubCard
              icon={LineChartIcon}
              title={
                clicksChartMode === "cumulative" && clientFacingLabels && targetCurve.length > 0
                  ? "Cumulative Clicks vs Target"
                  : "Daily Clicks + Spend"
              }
              subtitle={daily.length ? `${daily.length} days` : "—"}
            >
              {clientFacingLabels && targetCurve.length > 0 ? (
                  ...
                  <div className="inline-flex rounded-full border border-border/60 bg-muted/40 p-0.5 text-[11px]">
                    <button
                      ...
                      className={cn(
                        ...
                        clicksChartMode === "cumulative"
                          ? "bg-background text-foreground shadow-sm"
                          : "text-muted-foreground hover:text-foreground",
                      )}
                      onClick={() => setClicksChartMode("cumulative")}
                    >
                    ...
                    <button
                      ...
                      className={cn(
                        "rounded-full px-3 py-1 transition",
                        clicksChartMode === "daily"
                          ? "bg-background text-foreground shadow-sm"
                          : "text-muted-foreground hover:text-foreground",
                      )}
                      onClick={() => setClicksChartMode("daily")}
                    >
                      Daily
                    </button>
                  </div>
                  ...
                  >
                    <span
                      className={cn(
                        ...
                        onTrackStatus === "on-track" && "bg-emerald-500",
                        onTrackStatus === "ahead" && "bg-blue-500",
                        onTrackStatus === "behind" && "bg-amber-500",
                        onTrackStatus === "no-data" && "bg-muted-foreground",
                      )}
                      aria-hidden
                    />
                    {onTrackStatus === "on-track" && "On track"}
                    {onTrackStatus === "ahead" && "Ahead of pace"}
                    {onTrackStatus === "behind" && "Behind pace"}
                    {onTrackStatus === "no-data" && "Awaiting data"}
                  </span>
                </div>
              ) : null}
              ...
              {clicksChartMode === "cumulative" && clientFacingLabels && targetCurve.length > 0 ? (
                <ChartContainer
                  config={{
                    actual: { label: "Cumulative clicks", color: searchSeriesPalette.clicks },
                    target: { label: "Target", color: searchCostAccent },
                  }}
                  className="h-[320px] w-full"
                  ref={clicksSpendChartRef}
                >
                  <LineChart data={chartClicksCumulative} height={320} margin={{ left: 12, right: 12, top: 4, bottom: 0 }}>
                    <defs>
                      <linearGradient id="search-target-band" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor={searchCostAccent} stopOpacity={0.22} />
                        <stop offset="95%" stopColor={searchCostAccent} stopOpacity={0.04} />
                      </linearGradient>
                    </defs>
                    ...
                    <XAxis
                      ...
                      axisLine={false}
                      minTickGap={16}
                      angle={chartClicksCumulative.length > 10 ? -45 : 0}
                      textAnchor={chartClicksCumulative.length > 10 ? "end" : "middle"}
                      height={chartClicksCumulative.length > 10 ? 56 : 30}
                    ...
                    {/*
                      The shaded target band is built by rendering two Area
                      series: first a FULL area from 0 to targetHigh with the
                      gradient fill, then a SOLID-coloured area from 0 to
                      targetLow that matches the chart background — the net
                      visual is a band between targetLow and targetHigh.
                    */}
                    <Area
                      type="monotone"
                      dataKey="targetHigh"
                      stroke="none"
                      fill="url(#search-target-band)"
                      name="Target range"
                        ...
                        <AccordionTrigger className="px-4 py-3 text-left text-sm font-semibold transition-all hover:no-underline">
                          <div className="grid w-full grid-cols-[1fr_auto] items-center gap-2">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <StatusBadge pacingPct={typeof budgetPacingPct === "number" ? budgetPacingPct : 0} size="sm" />
                            </div>
                          </div>
                        </AccordionTrigger>
