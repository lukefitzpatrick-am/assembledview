...
- `lib/billing/` — `computeSchedule`, `buildBillingSchedule` (drops $0 lines — the reason `injectMissingFeeLinesFromMedia` exists for client-pays fee lines), `parsePersistedBillingScheduleToMonths` (legacy-shape inference), `seedLineFees`, `applyBillingLineMode` (billingMode auto/manual — see INVARIANTS), `exportBillingScheduleExcel`, **PC4** `balancer.ts` / `collisionWorksheet.ts` / `rebalanceLineOnSchedule.ts` (flag `NEXT_PUBLIC_BILLING_BALANCER`, default **off**).
