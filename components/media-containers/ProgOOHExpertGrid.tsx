...
import {
  ...
  normalizeWeekRect as normalizeProgOohWeekRect,
  weekCellInRect as progOohWeekCellInRect,
  ...
  normalizeWeekMergeSelection as normalizeProgOohWeekMergeSelection,
  weekPlainClickPreservesWeekAreaSelection,
  deriveMergeEligibility as deriveProgOohMergeEligibility,
  weekCellExportText,
...
type ProgOohWeekRectSelection = ExpertWeekRectSelection
type ProgOohMultiCellSelection = ExpertMultiCellSelection
type ProgOohWeekMergeSelectionNormalized = WeekMergeSelectionNormalized
 ...
 */
const PROGOOH_EXPERT_ROW_VIRTUALIZATION = true
...
export function ProgOohExpertGrid({
  ...
  const normalizeRowCacheRef = useRef(
    new Map<string, NormalizeRowCacheEntry<ProgOohExpertScheduleRow>>()
  )
  ...
  const rowMergeMapCacheRef = useRef(
    new Map<
      string,
      MapBuildCacheEntry<
        ProgOohExpertScheduleRow["mergedWeekSpans"],
        ProgOohRowMergeMap
      >
    >()
  )
  ...
  const rowReorderVirtual = useMemo(
    () =>
      PROGOOH_EXPERT_ROW_VIRTUALIZATION
        ? {
            rowCount: normalizedRows.length,
            estimateSizePx: PROGOOH_EXPERT_ROW_HEIGHT_PX,
  ...
  const ensureRowVisible = useCallback((rowIndex: number) => {
    if (!PROGOOH_EXPERT_ROW_VIRTUALIZATION) return
  ...
  const widthForWeekKey = useCallback(
    (k: string): number => {
      ...
      return weekColumnWidths[k] ?? PROGOOH_EXPERT_WEEK_COL_WIDTH_PX
    },
            ...
            ? Math.min(
                ...
                Math.max(
                  ...
                  deltaDaysFromPx(prev.edgeYmd, dx, {
                    ...
                    dayKeysByWeekKey,
                    weekColumnWidths,
                    defaultWeekColWidthPx: PROGOOH_EXPERT_WEEK_COL_WIDTH_PX,
  ...
  const {
    ...
    getScrollElement: () => gridScrollRef.current,
    estimateSize: PROGOOH_EXPERT_ROW_HEIGHT_PX,
    overscan: PROGOOH_EXPERT_ROW_OVERSCAN,
  ...
  const virtualSpacerColSpan = useMemo(() => {
    ...
    return (
      1 + progOohDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekCells
    )
  }, [
    weekColumns,
    expandedWeekKeys,
  ...
  const containerTotals = useMemo(() => {
    ...
    for (const row of normalizedRows) {
      const { net, fee } = expertRowCostSplit(row, weekKeys, feeprogooh)
      sumNet += net
      sumFee += fee
      for (const k of weekKeys) {
  ...
  }, [dayKeysByWeekKey, feeprogooh, normalizedRows, weekKeys])
        ...
        <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2 border-b border-border/60 bg-muted/5 pb-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <Label htmlFor="progooh-expert-row-count" className="sr-only">Add rows count</Label>
              <Input
                  ...
                  <tbody>
                    {(() => {
                      ...
                      return (
                        <MemoExpertGridRow
                          ...
                          rowUiSig={rowUiSig}
                          render={() => {
                      const net = expertRowNetMedia(row, weekKeys, feeprogooh)
                      ...
                      return (
                        <tr
                          ...
                          data-progooh-expert-row-index={rowIndex}
                          style={{
                            ...stripeStyle,
                            height: PROGOOH_EXPERT_ROW_HEIGHT_PX,
                            maxHeight: PROGOOH_EXPERT_ROW_HEIGHT_PX,
                          }}
                          ...
                          {(() => {
                            ...
                            for (let wi = 0; wi < weekColumns.length; wi += 1) {
                              ...
                              const tdClassName = cn(
                                ...
                                !isMergedAnchorCell &&
                                  mergeReadyOnCell &&
                                  "progooh-expert-week-cell--merge-ready z-[6] bg-pacing-behind-bg ring-2 ring-inset ring-pacing-behind shadow-e0",
                      ...
                      if (!PROGOOH_EXPERT_ROW_VIRTUALIZATION) {
                    ...
                    >
                      ...
                      <td
                        ...
                        style={stickyStyleBody(progOohDescriptorKeys.length)}
                      >
                      ...
                      <td
                        ...
                        style={stickyStyleBody(progOohDescriptorKeys.length + 1)}
                      />
                      <td
                        className={cn(
                        ...
                        style={stickyStyleBody(progOohDescriptorKeys.length + 2)}
                      >
