...
- `app/admin/fireflies-unattributed` — Fireflies notes with no MBA/domain match; Assign writes `client_domains` (MR-5 learn) via `GET|POST /api/admin/fireflies-unattributed`. Cron `GET/POST /api/cron/fireflies-sync` (`FIREFLIES_API_KEY` + `CRON_SECRET`) pulls transcripts into `client_notes`, creates `ava_task_proposals` from action items (never tasks), and advances `fireflies_sync_state` cursor. Human accept lives on `/tasks` Inbox. Publisher Hub Meetings (`PublisherMeetingsSection`) is read-only `client_notes` for this `publisher_id` + `attributed_type='publisher'` via `GET /api/publishers/[publisherId]/meetings` (`requireAdmin`); empty state points here.
