...
test("sub-week interior burst imports as day-detail and round-trips", () => {
  ...
  const rows = mapStandardProgVideoLineItemsToExpertRows(std, cols, CS, CE)
  const wk0 = cols[0]!
  assert.equal(weekHasDailyValues(rows[0]!.dailyValues ?? {}, weekDayKeys(wk0, CS, CE)), true)
  const back = mapProgVideoExpertRowsToStandardLineItems(rows, cols, CS, CE, { feePctProgVideo: 0 })
