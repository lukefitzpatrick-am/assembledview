...
export default function ProgBVODContainer({
  ...
  // Form initialization
  const form = useForm<ProgBvodFormValues>({
    resolver: zodResolver(progBvodFormSchema),
    defaultValues: {
      lineItems: [
        {
          platform: "",
    ...
    const lineItemTotals = watchedLineItems.map((lineItem, index) => {
      ...
      lineItem.bursts.forEach((burst) => {
        ...
        if (lineItem.budgetIncludesFees) {
          lineFee += (budget / 100) * (feeprogbvod || 0);
          lineMedia += (budget / 100) * (100 - (feeprogbvod || 0));
        } else {
          lineMedia += budget;
...
useEffect(() => {
  ...
  }, [watchedLineItems, feeprogbvod, onInvestmentChange, onBurstsChange, onTotalMediaChange, form]);
                                  ...
                                  <div className="grid grid-cols-5 gap-4 items-center flex-grow">
                                    ...
                                    <div className="flex flex-col space-y-2">
                                      <div className="grid grid-cols-2 gap-2">
                                        ...
                                        <div className="flex flex-col">
                                          <FormLabel className="text-xs">Fee ({feeprogbvod}%)</FormLabel>
                                          <Input
                                            type="text"
                                            className="w-full"
                                            value={new Intl.NumberFormat("en-US", {
