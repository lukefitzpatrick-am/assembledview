...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (originalCount > 500) {
      console.warn("[api/pacing/programmatic/display][" + requestId + "] ids truncated", {
        mbaNumber,
        originalCount,
        truncatedTo: 500,
      })
    ...
    console.log("[api/pacing/programmatic/display][" + requestId + "] start", {
      mbaNumber,
      startDate: start,
      endDate: end,
    ...
    const cacheKey = buildPacingCacheKey({
      scope: "pacing:programmatic-display",
      mba: mbaNumber,
      startDate: start,
      endDate: end,
      lineItemIds,
    ...
    const cacheResult = await getPacingCache(cacheKey, async () => {
      ...
      if (hitRowLimit) {
        console.warn("[api/pacing/programmatic/display] potential truncation: hit row limit", {
          mbaNumber,
          startDate: start,
          endDate: end,
      ...
      if (DEBUG) {
        console.info("[api/pacing/programmatic/display] rows", {
          mbaNumber,
          startDate: start,
          endDate: end,
          lineItemIds,
    ...
    if (DEBUG && cacheResult.state === "STALE" && cacheResult.staleError) {
      console.warn("[api/pacing/programmatic/display] served stale cache", {
        mbaNumber,
        startDate: start,
        endDate: end,
    ...
    return NextResponse.json(
      { error: "Failed to fetch DV360 Display pacing rows", message },
      { status: 500 }
    )
  }
}
