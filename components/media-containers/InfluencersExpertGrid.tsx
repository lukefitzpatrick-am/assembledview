...
import type {
  ExpertWeeklyValues,
  InfluencersExpertMergedWeekSpan,
...
const INFLUENCERS_EXPERT_WEEK_COL_WIDTH_PX = 112

/** Week column width on `<th>` and week `<td>`; inputs use `w-full` inside the cell. */
const influencersExpertWeekColLayoutStyle = {
  width: INFLUENCERS_EXPERT_WEEK_COL_WIDTH_PX,
  minWidth: INFLUENCERS_EXPERT_WEEK_COL_WIDTH_PX,
  maxWidth: INFLUENCERS_EXPERT_WEEK_COL_WIDTH_PX,
  boxSizing: "border-box" as const,
}

const MEDIA_ACCENT_HEX = getMediaTypeThemeHex("influencers")

const influencersExpertHeaderCellBgStyle = {
  backgroundColor: rgbaFromHex(MEDIA_ACCENT_HEX, 0.08),
}

const influencersExpertTotalsRowBgStyle = {
  backgroundColor: rgbaFromHex(MEDIA_ACCENT_HEX, 0.06),
}

/** Right edge of sticky block before the horizontally scrolling week area. */
const INFLUENCERS_EXPERT_WEEK_SCROLLER_EDGE =
  "border-r-2 border-border/80 shadow-[2px_0_8px_-4px_rgba(0,0,0,0.08)] dark:shadow-[2px_0_8px_-4px_rgba(0,0,0,0.22)]"

const DEBUG_INFLUENCERS_MERGE = false

function normalizeInfluencersKey(input: unknown): string {
  return String(input ?? "")
    .trim()
    .toLowerCase()
    .replace(/\s+/g, " ")
...
function sumMergedQuantities(row: InfluencersExpertScheduleRow): number {
...
): InfluencersExpertMergedWeekSpan | null {
...
type InfluencersMultiCellSelection = {
  startRow: number
  endRow: number
  startCol: number
  endCol: number
...
type InfluencersCopiedCells = {
  data: (string | number)[][]
  sourceRows: number
  sourceCols: number
  selection: InfluencersMultiCellSelection
}

function weeklyCellDisplayValue(
  cellValue: number | string | undefined,
  mergedSpan: InfluencersExpertMergedWeekSpan | null
): string {
  if (mergedSpan) {
...
const INFLUENCERS_WEEK_CELL_VISUAL_CLASSES = {
...
function selectionOverlapsMergedSpan(
  rowMergeMap: InfluencersRowMergeMap | undefined,
...
type InfluencersWeekRectSelection = {
  rowStart: number
  rowEnd: number
  weekKeyStart: string
  weekKeyEnd: string
...
function normalizeInfluencersWeekRect(
  row0: number,
  wk0: string,
  row1: number,
  wk1: string,
  weekKeys: readonly string[]
): InfluencersWeekRectSelection {
...
function influencersWeekCellInRect(
  rowIndex: number,
  weekKey: string,
  rect: InfluencersWeekRectSelection | null,
  weekKeys: readonly string[]
): boolean {
...
function rectToMultiCellSelection(
  rect: InfluencersWeekRectSelection | null,
  weekKeys: readonly string[]
): InfluencersMultiCellSelection | null {
  if (!rect) return null
  const i0 = weekKeys.indexOf(rect.weekKeyStart)
  const i1 = weekKeys.indexOf(rect.weekKeyEnd)
...
function coerceWeeklySelectionBounds(
  ...
  weekRectSelection: InfluencersWeekRectSelection | null,
  ...
  if (
    weekRectSelection &&
    influencersWeekCellInRect(anchorRow, anchorWeekKey, weekRectSelection, weekKeys)
  ) {
...
function applyWeeklyPasteMatrixToSelection(args: {
  ...
  weekKeys: readonly string[]
  rowCount: number
  nextRows: InfluencersExpertScheduleRow[]
}): {
  applied: number
  errorReasons: string[]
  layout: WeeklyPasteLayoutMode
...
function oohMergeKeysFromRect(
  rect: InfluencersWeekRectSelection,
  weekKeys: readonly string[]
): string[] | null {
...
type InfluencersWeekMergeSelectionNormalized = {
  rowIndex: number
 ...
 */
function normalizeInfluencersWeekMergeSelection(
  rect: InfluencersWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): InfluencersWeekMergeSelectionNormalized | null {
...
function weekPlainClickPreservesWeekAreaSelection(
  ...
  const mergeNorm = normalizeInfluencersWeekMergeSelection(rect, multi, weekKeys)
  if (
    mergeNorm &&
  ...
  if (rect && influencersWeekCellInRect(rowIndex, weekKey, rect, weekKeys)) {
    const isSingleCell =
...
function deriveInfluencersMergeEligibility(
  rect: InfluencersWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): InfluencersWeekMergeSelectionNormalized | null {
  return normalizeInfluencersWeekMergeSelection(rect, multi, weekKeys)
}
...
function mergedWeekSpansAfterCutRect(
  spans: InfluencersExpertMergedWeekSpan[] | undefined,
  wi0: number,
  wi1: number,
  weekKeys: readonly string[]
): InfluencersExpertMergedWeekSpan[] {
...
type WeeklyExportSelection =
  | { kind: "rect"; rect: InfluencersWeekRectSelection }
...
function resolveWeeklyExportSelection(
  ...
  weekKeys: readonly string[],
  rows: readonly InfluencersExpertScheduleRow[] | null
...
function applyWeeklyCutToRows(
  ...
  rows: readonly InfluencersExpertScheduleRow[],
  weekKeys: readonly string[]
): InfluencersExpertScheduleRow[] | null {
  switch (selection.kind) {
    case "rect": {
      const rect = selection.rect
      const wi0 = Math.min(
      ...
      if (wi0 < 0 || wi1 < 0) return null
      const next: InfluencersExpertScheduleRow[] = rows.map((r) => ({
        ...r,
    ...
    case "mergeContiguous": {
      ...
      const next: InfluencersExpertScheduleRow[] = rows.map((row) => ({
        ...row,
...
/** Match labels/values on {@link InfluencersContainer} buy-type combobox. */
const INFLUENCERS_BUY_TYPE_OPTIONS: ComboboxOption[] = [
...
const INFLUENCERS_BID_STRATEGY_OPTIONS: ComboboxOption[] = [
...
function normalizeInfluencersBuyTypePaste(raw: string): string {
  const v = raw.trim()
  if (!v) return ""
  const byValue = INFLUENCERS_BUY_TYPE_OPTIONS.find(
    (o) => o.value.toLowerCase() === v.toLowerCase()
  )
  if (byValue) return byValue.value
  const byLabel = INFLUENCERS_BUY_TYPE_OPTIONS.find(
...
function normalizeInfluencersBidStrategyPaste(raw: string): string {
  const v = raw.trim()
  if (!v) return ""
  const byValue = INFLUENCERS_BID_STRATEGY_OPTIONS.find((o) => o.value.toLowerCase() === v.toLowerCase())
  if (byValue) return byValue.value
  const byLabel = INFLUENCERS_BID_STRATEGY_OPTIONS.find((o) => o.label.toLowerCase() === v.toLowerCase())
  if (byLabel) return byLabel.value
  return v
}

function normalizeInfluencersPlatformPaste(raw: string, platformNames: string[]): string {
  const v = raw.trim()
  if (!v) return ""
...
function rowGrossCost(row: InfluencersExpertScheduleRow, weekKeys: string[]): number {
...
const INFLUENCERS_DESCRIPTOR_CORE: readonly (keyof InfluencersExpertScheduleRow)[] = [
  "startDate",
  "endDate",
  "platform",
  "objective",
...
const INFLUENCERS_BILLING_FLAG_KEYS: readonly (keyof InfluencersExpertScheduleRow)[] = [
  "fixedCostMedia",
  "clientPaysForMedia",
  "budgetIncludesFees",
]

const INFLUENCERS_DESCRIPTOR_TAIL: readonly (keyof InfluencersExpertScheduleRow)[] = [
  "unitRate",
]
...
export interface InfluencersExpertGridProps {
  ...
  /** Platform names (influencers publishers API) for platform combobox + fuzzy matching */
  publishers?: { publisher_name: string }[]
}

const moneyOpts = { locale: "en-US" as const, currency: "USD" as const }

type InfluencersExpertFocusedCell = { rowIndex: number; columnKey: string }
type WeekDragSource =
  | {
      type: "single"
      rowIndex: number
...
type InfluencersRowMergeSpanMeta = Readonly<{
  id: string
  startWeekKey: string
  endWeekKey: string
  totalQty: number
...
type InfluencersRowMergeMap = Readonly<{
  anchorByWeekKey: Readonly<Record<string, string>>
  interiorByWeekKey: Readonly<Record<string, string>>
  spanById: Readonly<Record<string, InfluencersExpertMergedWeekSpan>>
  spanMetaByAnchorWeekKey: Readonly<Record<string, InfluencersRowMergeSpanMeta>>
}>
...
export function InfluencersExpertGrid({
  ...
  const [focusedCell, setFocusedCell] = useState<InfluencersExpertFocusedCell | null>(
    null
  )
  const focusedCellRef = useRef<InfluencersExpertFocusedCell | null>(null)
  ...
  const [weekRectSelection, setWeekRectSelection] =
    useState<InfluencersWeekRectSelection | null>(null)
  // Multi-cell selection state for copy/paste operations.
  const [multiCellSelection, setMultiCellSelection] =
    useState<InfluencersMultiCellSelection | null>(null)
  const [isSelecting, setIsSelecting] = useState(false)
  const [copiedCells, setCopiedCells] = useState<InfluencersCopiedCells | null>(null)
  ...
  const lastDragRectDuringGestureRef = useRef<InfluencersWeekRectSelection | null>(null)
  /**
   ...
   */
  const postDragWeekClickRectRef = useRef<InfluencersWeekRectSelection | null>(null)
  const weekRectSelectionRef = useRef<InfluencersWeekRectSelection | null>(null)
  ...
  const influencersDescriptorKeys = useMemo(
    () =>
      [
        ...INFLUENCERS_DESCRIPTOR_CORE,
        ...INFLUENCERS_BILLING_FLAG_KEYS,
        ...INFLUENCERS_DESCRIPTOR_TAIL,
      ] as (keyof InfluencersExpertScheduleRow)[],
    []
  )

  const descriptorColWidths = useMemo(
    () => [
      48, 48, 120, 110, 110, 110, 96, 120, 120, 110, 96, 40, 40, 40, 88,
    ],
    []
  )
  ...
  const rowMergeMaps = useMemo<readonly InfluencersRowMergeMap[]>(() => {
    const maps = normalizedRows.map((row) => {
      const anchorByWeekKey: Record<string, string> = {}
      const interiorByWeekKey: Record<string, string> = {}
      const spanById: Record<string, InfluencersExpertMergedWeekSpan> = {}
      const spanMetaByAnchorWeekKey: Record<string, InfluencersRowMergeSpanMeta> = {}
      ...
      for (const span of row.mergedWeekSpans ?? []) {
        if (spanById[span.id]) {
          if (DEBUG_INFLUENCERS_MERGE) {
            console.debug("[Influencers merge] occupancy duplicate span id ignored", {
              rowId: row.id,
        ...
        if (keys.some((k) => occupiedWeekKeys.has(k))) {
          if (DEBUG_INFLUENCERS_MERGE) {
            console.debug("[Influencers merge] occupancy overlap ignored", {
              rowId: row.id,
              rowIndex: normalizedRows.indexOf(row),
              spanId: span.id,
              keys,
  ...
  useEffect(() => {
    if (!DEBUG_INFLUENCERS_MERGE) return
    ...
    console.debug("[Influencers merge] pending selection updated", pendingMergeSelection)
  ...
  const updateRow = useCallback(
    (rowIndex: number, patch: Partial<InfluencersExpertScheduleRow>) => {
  ...
  const tryFuzzyMatch = useCallback(
    (rowIndex: number, field: FuzzyMatchField, value: string) => {
      ...
      if (corrected) {
        updateRow(rowIndex, { [field]: corrected } as Partial<InfluencersExpertScheduleRow>)
        return
      }
      ...
      if (fuzzyMatchAutoApplyRef.current) {
        updateRow(rowIndex, { [field]: match.matched } as Partial<InfluencersExpertScheduleRow>)
      } else {
  ...
  const navColCount =
    influencersDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekKeys.length
  const firstWeekNavColIndex = influencersDescriptorKeys.length + WEEK_GRID_COL_OFFSET
  const unitRateNavColIndex = influencersDescriptorKeys.indexOf("unitRate")
  ...
  const unmergeWeekSpan = useCallback(
    (rowIndex: number, spanId: string) => {
      ...
      if (DEBUG_INFLUENCERS_MERGE) {
        console.debug("[Influencers merge] unmerge applied", { rowIndex, spanId })
      }
    },
  ...
  const duplicateRow = useCallback(
    (rowIndex: number) => {
      ...
      const id =
        typeof crypto !== "undefined" && crypto.randomUUID
          ? crypto.randomUUID()
          : `influencers-expert-${Date.now()}-${rowIndex}`
  ...
  const clearWeekSelectionWhereMergedAnchorInvolved = useCallback(
    (rowIndex: number, weekKey: string) => {
      setWeekRectSelection((prev) =>
        prev && influencersWeekCellInRect(rowIndex, weekKey, prev, weekKeys)
          ? null
          : prev
      )
  ...
  const rangeWeekMultiSelect = useCallback(
    (rowIndex: number, anchorKey: string, endKey: string) => {
      ...
      setWeekRectSelection(
        normalizeInfluencersWeekRect(rowIndex, anchorKey, rowIndex, endKey, weekKeys)
      )
      setWeekStripSelection(null)
    },
    [weekKeys]
  ...
  const lockPendingMergeSelectionFromCurrentSelection = useCallback(() => {
    const n = deriveInfluencersMergeEligibility(
  ...
  const derivedMergeTarget = useMemo(() => {
    const n = deriveInfluencersMergeEligibility(weekRectSelection, weekMultiSelect, weekKeys)
  ...
  useEffect(() => {
    const endDrag = () => {
      ...
      if (DEBUG_INFLUENCERS_MERGE) {
        console.debug("[Influencers merge] drag end", {
          lastRect: r,
  ...
  const handleMergeSelectedWeeks = useCallback(() => {
    ...
    if (selectionOverlapsMergedSpan(rowMergeMaps[rowIndex], sorted, weekKeys)) {
      if (DEBUG_INFLUENCERS_MERGE) {
        console.debug("[Influencers merge] validation overlap failure", {
          rowIndex,
    ...
    const newId =
      typeof crypto !== "undefined" && crypto.randomUUID
        ? crypto.randomUUID()
        : `merge-${Date.now()}`
    const newSpan: InfluencersExpertMergedWeekSpan = {
      id: newId,
    ...
    if (DEBUG_INFLUENCERS_MERGE) {
      console.debug("[Influencers merge] merge applied", {
        rowIndex,
        spanId: newSpan.id,
  ...
  useEffect(() => {
    ...
    const cellId = expertGridCellId(
      ...
      influencersDescriptorKeys.length + WEEK_GRID_COL_OFFSET + lastWi
    )
    document
      .getElementById(cellId)
      ?.scrollIntoView({ behavior: "smooth", block: "nearest", inline: "nearest" })
  }, [domGridId, mergeTarget, mergeWeeksReady, influencersDescriptorKeys.length, weekKeys])

  useEffect(() => {
    if (!DEBUG_INFLUENCERS_MERGE) return
    console.debug("[Influencers merge] merge eligibility derived", {
      mergeTarget,
      derivedMergeTarget,
      mergeWeeksReady,
      weekRectSelection,
  ...
  const pasteMatrixIntoGrid = useCallback(
    (matrix: string[][]) => {
      ...
      if (fcIsWeek) {
        ...
        const nextRows: InfluencersExpertScheduleRow[] = normalizedRows.map((r) => ({
          ...r,
      ...
      for (let dr = 0; dr < working.length; dr++) {
        ...
        for (let dc = 0; dc < pasteRow.length; dc++) {
          ...
          const field = target.field as keyof InfluencersExpertScheduleRow
          ...
          if (field === "unitRate") {
            ...
            nextRows[targetRow] = {
              ...cur,
              [field]: truthy,
            } as InfluencersExpertScheduleRow
            applied += 1
          } else if (field === "buyType") {
            nextRows[targetRow] = {
              ...cur,
              buyType: normalizeInfluencersBuyTypePaste(raw),
            }
            applied += 1
          } else if (field === "bidStrategy") {
            nextRows[targetRow] = {
              ...cur,
              bidStrategy: normalizeInfluencersBidStrategyPaste(raw),
            }
            applied += 1
          } else if (field === "platform") {
            const plt = normalizeInfluencersPlatformPaste(raw, platformNames)
            nextRows[targetRow] = {
              ...cur,
              platform: plt,
            }
            ...
            const v = raw.trim()
            nextRows[targetRow] = { ...cur, [field]: v } as InfluencersExpertScheduleRow
            applied += 1
          }
        }
      }
  ...
  const containerTotals = useMemo(() => {
    ...
    const fee =
      feeinfluencers > 0 && feeinfluencers < 100 ? (sumGross * feeinfluencers) / (100 - feeinfluencers) : 0
    const totalWithFee = sumGross + fee

    return { sumGross, sumQty, perWeek, fee, totalWithFee }
  }, [feeinfluencers, normalizedRows, weekKeys])
  ...
  const colIndexOf = useCallback(
    (key: keyof InfluencersExpertScheduleRow) => influencersDescriptorKeys.indexOf(key),
    [influencersDescriptorKeys]
  )
      ...
      <Card className="relative overflow-hidden border-0 shadow-md">
        ...
        <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2 border-b border-border/60 bg-muted/5 pb-3">
          <CardTitle className="text-base font-semibold tracking-tight">
            Influencers Media — Expert Schedule
          </CardTitle>
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2">
              <Label htmlFor="influencers-expert-row-count" className="text-sm whitespace-nowrap">
                Rows:
              </Label>
              <Input
                id="influencers-expert-row-count"
                type="number"
                min={1}
                max={500}
          ...
          <div className="overflow-hidden rounded-lg border border-border/80 bg-card/30 shadow-sm">
            {normalizedRows.length === 0 ? (
              ...
              <div
                ...
                data-influencers-expert-grid-scroll=""
              >
                    ...
                    <tr>
                        ...
                        <th
                          ...
                          className={stickyThCorner(
                            i === descriptorHeadLabels.length - 1
                              ? INFLUENCERS_EXPERT_WEEK_SCROLLER_EDGE
                              : undefined
                          )}
                          style={{
                            ...stickyStyleHeaderCorner(i),
                            ...influencersExpertHeaderCellBgStyle,
                          }}
                        >
                          {label === "Unit Rate" ? (
                            <Tooltip>
                      ...
                      {weekColumns.map((col) => (
                        <th
                          ...
                          style={{
                            ...influencersExpertWeekColLayoutStyle,
                            ...influencersExpertHeaderCellBgStyle,
                          }}
                          title={col.labelFull}
                        >
                    ...
                    {normalizedRows.map((row, rowIndex) => {
                      ...
                      const stripeStyle =
                        rowIndex % 2 === 0
                          ...
                          : undefined
                      const grossCol = influencersDescriptorKeys.length
                      const actionsCol = influencersDescriptorKeys.length + 1
                      const sigmaCol = influencersDescriptorKeys.length + 2
                      const cStart = colIndexOf("startDate")
                      const cEnd = colIndexOf("endDate")
                      const cPlt = colIndexOf("platform")
                      const cObj = colIndexOf("objective")
                      const cCamp = colIndexOf("campaign")
                      ...
                      return (
                        ...
                        >
                          ...
                          <td
                            className={stickyTd(cObj)}
                            style={stickyStyleBody(cObj)}
                          >
                            <Input
                              id={expertGridCellId(
                                domGridId,
                              ...
                              value={row.objective}
                              onFocus={() =>
                                handleCellFocus(rowIndex, "objective")
                              }
                              onKeyDown={(e) =>
                                handleGridInputKeyDown(rowIndex, cObj, e)
                              }
                              onChange={(e) =>
                                updateRow(rowIndex, {
                          ...
                          <td
                            className={stickyTd(cCamp)}
                            style={stickyStyleBody(cCamp)}
                          >
                            <Input
                              id={expertGridCellId(
                                domGridId,
                              ...
                              value={row.campaign}
                              onFocus={() =>
                                handleCellFocus(rowIndex, "campaign")
                              }
                              onKeyDown={(e) =>
                                handleGridInputKeyDown(rowIndex, cCamp, e)
                              }
                              onChange={(e) =>
                                updateRow(rowIndex, {
                          ...
                          <td
                            className={stickyTd(cTgtAttr)}
                            style={stickyStyleBody(cTgtAttr)}
                          >
                            <Input
                              id={expertGridCellId(
                                domGridId,
                              ...
                              onKeyDown={(e) =>
                                handleGridInputKeyDown(rowIndex, cTgtAttr, e)
                              }
                              onChange={(e) =>
                                updateRow(rowIndex, {
                                  targetingAttribute: e.target.value,
                                })
                              }
                            />
                          </td>
                          ...
                          {(() => {
                            ...
                            for (let wi = 0; wi < weekColumns.length; wi += 1) {
                              ...
                              const colIndex =
                                influencersDescriptorKeys.length +
                              ...
                              const tdClassName = cn(
                                ...
                                isPopulatedNonMergedCell &&
                                  INFLUENCERS_WEEK_CELL_VISUAL_CLASSES.populatedSingleTd,
                                ...
                                !isMergedAnchorCell &&
                                  mergeReadyOnCell &&
                                  "influencers-expert-week-cell--merge-ready z-[6] bg-amber-500/24 ring-2 ring-inset ring-amber-600/75 shadow-sm dark:bg-amber-500/20 dark:ring-amber-400/70",
                              ...
                              const mergedAnchorWrapperClassName = cn(
                                ...
                                isMergedAnchorCell &&
                                  INFLUENCERS_WEEK_CELL_VISUAL_CLASSES.mergedSurface,
                              ...
                              const inputClassName = cn(
                                ...
                                isMergedAnchorCell
                                  ? INFLUENCERS_WEEK_CELL_VISUAL_CLASSES.mergedInput
                              ...
                              renderedWeekCells.push(
                                <td
                                  ...
                                  style={
                                    isMergedAnchorCell
                                      ? {
                                          width: INFLUENCERS_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                          minWidth:
                                            INFLUENCERS_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                          maxWidth:
                                            INFLUENCERS_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                  ...
                                  onMouseEnter={(e) => {
                                    ...
                                    const next = normalizeInfluencersWeekRect(
                                      drag.rowIndex,
                                      drag.weekKey,
                                      rowIndex,
                                      col.weekKey,
                                    ...
                                    setWeekStripSelection(null)
                                    if (DEBUG_INFLUENCERS_MERGE) {
                                      console.debug("[Influencers merge] drag update", {
                                        rect: next,
                                      })
                                    }
                                  }}
                                ...
                                >
                                  ...
                                  >
                                    <Input
                                      ...
                                      setIsSelecting(true)
                                      if (DEBUG_INFLUENCERS_MERGE) {
                                        console.debug("[Influencers merge] drag start", {
                                          rowIndex,
                                          weekKey: col.weekKey,
                                        })
                                      }
                                      ...
                                      if (shift && lastWeekAnchorRef.current) {
                                        ...
                                        if (
                                          ...
                                          const rect = normalizeInfluencersWeekRect(
                                            lastWeekAnchorRef.current.rowIndex,
                                            lastWeekAnchorRef.current.weekKey,
                                            rowIndex,
                                            col.weekKey,
                    ...
                    >
                      <td
                        ...
                        style={{
                          ...
                          ...influencersExpertTotalsRowBgStyle,
                        }}
                        colSpan={influencersDescriptorKeys.length}
                      >
                      ...
                      <td
                        className={cn(
                          stickyTd(influencersDescriptorKeys.length),
                          "h-8 px-1 text-xs tabular-nums"
                        )}
                        style={{
                          ...stickyStyleBody(influencersDescriptorKeys.length),
                          ...influencersExpertTotalsRowBgStyle,
                        }}
                      >
                      ...
                      <td
                        className={cn(
                          stickyTd(influencersDescriptorKeys.length + 1),
                          "h-8"
                        )}
                        style={{
                          ...stickyStyleBody(influencersDescriptorKeys.length + 1),
                          ...influencersExpertTotalsRowBgStyle,
                        }}
                      />
                      <td
                        className={cn(
                          stickyTd(influencersDescriptorKeys.length + 2),
                          "h-8 px-1 text-xs tabular-nums text-muted-foreground",
                          INFLUENCERS_EXPERT_WEEK_SCROLLER_EDGE
                        )}
                        style={{
                          ...stickyStyleBody(influencersDescriptorKeys.length + 2),
                          ...influencersExpertTotalsRowBgStyle,
                        }}
                      >
        ...
        {DEBUG_INFLUENCERS_MERGE ? (
          <div className="pointer-events-none absolute bottom-2 right-2 z-50 max-w-[360px] rounded-md border bg-background/95 p-2 text-[11px] leading-snug shadow-lg">
            <div className="mb-1 font-medium text-foreground">Influencers merge debug</div>
