...
const PROGVIDEO_EXPERT_WEEK_COL_WIDTH_PX = 112

/** Week column width on `<th>` and week `<td>`; inputs use `w-full` inside the cell. */
const progVideoExpertWeekColLayoutStyle = {
  width: PROGVIDEO_EXPERT_WEEK_COL_WIDTH_PX,
  minWidth: PROGVIDEO_EXPERT_WEEK_COL_WIDTH_PX,
  maxWidth: PROGVIDEO_EXPERT_WEEK_COL_WIDTH_PX,
  boxSizing: "border-box" as const,
}

const MEDIA_ACCENT_HEX = getMediaTypeThemeHex("progvideo")

const progVideoExpertHeaderCellBgStyle = {
  backgroundColor: rgbaFromHex(MEDIA_ACCENT_HEX, 0.08),
}

const progVideoExpertTotalsRowBgStyle = {
  backgroundColor: rgbaFromHex(MEDIA_ACCENT_HEX, 0.06),
}

/** Right edge of sticky block before the horizontally scrolling week area. */
const PROGVIDEO_EXPERT_WEEK_SCROLLER_EDGE =
  "border-r-2 border-border/80 shadow-[2px_0_8px_-4px_rgba(0,0,0,0.08)] dark:shadow-[2px_0_8px_-4px_rgba(0,0,0,0.22)]"

const DEBUG_PROGVIDEO_MERGE = false

function normalizeProgVideoKey(input: unknown): string {
  return String(input ?? "")
    .trim()
    .toLowerCase()
    .replace(/\s+/g, " ")
...
function sumMergedQuantities(row: ProgVideoExpertScheduleRow): number {
...
type ProgVideoMultiCellSelection = {
  startRow: number
  endRow: number
  startCol: number
  endCol: number
...
type ProgVideoCopiedCells = {
  data: (string | number)[][]
  sourceRows: number
  sourceCols: number
  selection: ProgVideoMultiCellSelection
}
...
const PROGVIDEO_WEEK_CELL_VISUAL_CLASSES = {
...
function selectionOverlapsMergedSpan(
  rowMergeMap: ProgVideoRowMergeMap | undefined,
...
type ProgVideoWeekRectSelection = {
  rowStart: number
  rowEnd: number
  weekKeyStart: string
  weekKeyEnd: string
...
function normalizeProgVideoWeekRect(
  row0: number,
  wk0: string,
  row1: number,
  wk1: string,
  weekKeys: readonly string[]
): ProgVideoWeekRectSelection {
...
function progVideoWeekCellInRect(
  rowIndex: number,
  weekKey: string,
  rect: ProgVideoWeekRectSelection | null,
  weekKeys: readonly string[]
): boolean {
...
function rectToMultiCellSelection(
  rect: ProgVideoWeekRectSelection | null,
  weekKeys: readonly string[]
): ProgVideoMultiCellSelection | null {
  if (!rect) return null
  const i0 = weekKeys.indexOf(rect.weekKeyStart)
  const i1 = weekKeys.indexOf(rect.weekKeyEnd)
...
function coerceWeeklySelectionBounds(
  ...
  weekRectSelection: ProgVideoWeekRectSelection | null,
  ...
  if (
    weekRectSelection &&
    progVideoWeekCellInRect(anchorRow, anchorWeekKey, weekRectSelection, weekKeys)
  ) {
...
function applyWeeklyPasteMatrixToSelection(args: {
  ...
  weekKeys: readonly string[]
  rowCount: number
  nextRows: ProgVideoExpertScheduleRow[]
}): {
  applied: number
  errorReasons: string[]
  layout: WeeklyPasteLayoutMode
...
function oohMergeKeysFromRect(
  rect: ProgVideoWeekRectSelection,
  weekKeys: readonly string[]
): string[] | null {
...
type ProgVideoWeekMergeSelectionNormalized = {
  rowIndex: number
 ...
 */
function normalizeProgVideoWeekMergeSelection(
  rect: ProgVideoWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): ProgVideoWeekMergeSelectionNormalized | null {
...
function weekPlainClickPreservesWeekAreaSelection(
  ...
  const mergeNorm = normalizeProgVideoWeekMergeSelection(rect, multi, weekKeys)
  if (
    mergeNorm &&
  ...
  if (rect && progVideoWeekCellInRect(rowIndex, weekKey, rect, weekKeys)) {
    const isSingleCell =
...
function deriveProgVideoMergeEligibility(
  rect: ProgVideoWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): ProgVideoWeekMergeSelectionNormalized | null {
  return normalizeProgVideoWeekMergeSelection(rect, multi, weekKeys)
}
...
type WeeklyExportSelection =
  | { kind: "rect"; rect: ProgVideoWeekRectSelection }
...
function resolveWeeklyExportSelection(
  ...
  weekKeys: readonly string[],
  rows: readonly ProgVideoExpertScheduleRow[] | null
...
function buildWeeklyExportTsv(
  selection: WeeklyExportSelection,
  rows: readonly ProgVideoExpertScheduleRow[],
  weekKeys: readonly string[]
): string | null {
  switch (selection.kind) {
    case "rect": {
...
function applyWeeklyCutToRows(
  ...
  rows: readonly ProgVideoExpertScheduleRow[],
  weekKeys: readonly string[]
): ProgVideoExpertScheduleRow[] | null {
  switch (selection.kind) {
    case "rect": {
      const rect = selection.rect
      const wi0 = Math.min(
      ...
      if (wi0 < 0 || wi1 < 0) return null
      const next: ProgVideoExpertScheduleRow[] = rows.map((r) => ({
        ...r,
    ...
    case "mergeContiguous": {
      ...
      const next: ProgVideoExpertScheduleRow[] = rows.map((row) => ({
        ...row,
...
function normalizeProgVideoPlatformPaste(raw: string, platformNames: string[]): string {
  const v = raw.trim()
  if (!v) return ""
...
function rowGrossCost(row: ProgVideoExpertScheduleRow, weekKeys: string[]): number {
...
const PROGVIDEO_DESCRIPTOR_CORE: readonly (keyof ProgVideoExpertScheduleRow)[] = [
  "startDate",
  "endDate",
  "platform",
  "bidStrategy",
...
const PROGVIDEO_BILLING_FLAG_KEYS: readonly (keyof ProgVideoExpertScheduleRow)[] = [
  "fixedCostMedia",
  "clientPaysForMedia",
  "budgetIncludesFees",
  "noadserving",
...
const PROGVIDEO_DESCRIPTOR_TAIL: readonly (keyof ProgVideoExpertScheduleRow)[] = ["unitRate"]
...
type ProgVideoRowMergeSpanMeta = Readonly<{
  id: string
  startWeekKey: string
  endWeekKey: string
  totalQty: number
...
type ProgVideoRowMergeMap = Readonly<{
  ...
  spanMetaByAnchorWeekKey: Readonly<Record<string, ProgVideoRowMergeSpanMeta>>
}>
...
export function ProgVideoExpertGrid({
  ...
  const [focusedCell, setFocusedCell] = useState<ProgVideoExpertFocusedCell | null>(
    null
  )
  const focusedCellRef = useRef<ProgVideoExpertFocusedCell | null>(null)
  ...
  const [weekRectSelection, setWeekRectSelection] =
    useState<ProgVideoWeekRectSelection | null>(null)
  // Multi-cell selection state for copy/paste operations.
  const [multiCellSelection, setMultiCellSelection] =
    useState<ProgVideoMultiCellSelection | null>(null)
  const [isSelecting, setIsSelecting] = useState(false)
  const [copiedCells, setCopiedCells] = useState<ProgVideoCopiedCells | null>(null)
  ...
  const lastDragRectDuringGestureRef = useRef<ProgVideoWeekRectSelection | null>(null)
  /**
   ...
   */
  const postDragWeekClickRectRef = useRef<ProgVideoWeekRectSelection | null>(null)
  const weekRectSelectionRef = useRef<ProgVideoWeekRectSelection | null>(null)
  ...
  const progVideoDescriptorKeys = useMemo(
    () =>
      [
        ...PROGVIDEO_DESCRIPTOR_CORE,
        ...PROGVIDEO_BILLING_FLAG_KEYS,
        ...PROGVIDEO_DESCRIPTOR_TAIL,
      ] as (keyof ProgVideoExpertScheduleRow)[],
    []
  )
  ...
  const rowMergeMaps = useMemo<readonly ProgVideoRowMergeMap[]>(() => {
    const maps = normalizedRows.map((row) => {
      ...
      const spanMetaByAnchorWeekKey: Record<string, ProgVideoRowMergeSpanMeta> = {}
      ...
      for (const span of row.mergedWeekSpans ?? []) {
        if (spanById[span.id]) {
          if (DEBUG_PROGVIDEO_MERGE) {
            console.debug("[Programmatic Video merge] occupancy duplicate span id ignored", {
              rowId: row.id,
        ...
        if (keys.some((k) => occupiedWeekKeys.has(k))) {
          if (DEBUG_PROGVIDEO_MERGE) {
            console.debug("[Programmatic Video merge] occupancy overlap ignored", {
              rowId: row.id,
              rowIndex: normalizedRows.indexOf(row),
              spanId: span.id,
              keys,
  ...
  useEffect(() => {
    if (!DEBUG_PROGVIDEO_MERGE) return
    ...
    console.debug("[Programmatic Video merge] pending selection updated", pendingMergeSelection)
  ...
  const updateRow = useCallback(
    (rowIndex: number, patch: Partial<ProgVideoExpertScheduleRow>) => {
  ...
  const tryFuzzyMatch = useCallback(
    (rowIndex: number, field: FuzzyMatchField, value: string) => {
      ...
      if (corrected) {
        updateRow(rowIndex, { [field]: corrected } as Partial<ProgVideoExpertScheduleRow>)
        return
      }
      ...
      if (fuzzyMatchAutoApplyRef.current) {
        updateRow(rowIndex, { [field]: match.matched } as Partial<ProgVideoExpertScheduleRow>)
      } else {
  ...
  const navColCount =
    progVideoDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekKeys.length
  const firstWeekNavColIndex = progVideoDescriptorKeys.length + WEEK_GRID_COL_OFFSET
  const unitRateNavColIndex = progVideoDescriptorKeys.indexOf("unitRate")
  ...
  const unmergeWeekSpan = useCallback(
    (rowIndex: number, spanId: string) => {
      ...
      if (DEBUG_PROGVIDEO_MERGE) {
        console.debug("[Programmatic Video merge] unmerge applied", { rowIndex, spanId })
      }
    },
  ...
  const duplicateRow = useCallback(
    (rowIndex: number) => {
      ...
      const id =
        typeof crypto !== "undefined" && crypto.randomUUID
          ? crypto.randomUUID()
          : `progvideo-expert-${Date.now()}-${rowIndex}`
  ...
  const clearWeekSelectionWhereMergedAnchorInvolved = useCallback(
    (rowIndex: number, weekKey: string) => {
      setWeekRectSelection((prev) =>
        prev && progVideoWeekCellInRect(rowIndex, weekKey, prev, weekKeys)
          ? null
          : prev
      )
  ...
  const rangeWeekMultiSelect = useCallback(
    (rowIndex: number, anchorKey: string, endKey: string) => {
      ...
      setWeekRectSelection(
        normalizeProgVideoWeekRect(rowIndex, anchorKey, rowIndex, endKey, weekKeys)
      )
      setWeekStripSelection(null)
    },
    [weekKeys]
  ...
  const lockPendingMergeSelectionFromCurrentSelection = useCallback(() => {
    const n = deriveProgVideoMergeEligibility(
  ...
  const derivedMergeTarget = useMemo(() => {
    const n = deriveProgVideoMergeEligibility(weekRectSelection, weekMultiSelect, weekKeys)
  ...
  useEffect(() => {
    const endDrag = () => {
      ...
      if (DEBUG_PROGVIDEO_MERGE) {
        console.debug("[Programmatic Video merge] drag end", {
          lastRect: r,
  ...
  const handleMergeSelectedWeeks = useCallback(() => {
    ...
    if (selectionOverlapsMergedSpan(rowMergeMaps[rowIndex], sorted, weekKeys)) {
      if (DEBUG_PROGVIDEO_MERGE) {
        console.debug("[Programmatic Video merge] validation overlap failure", {
          rowIndex,
    ...
    if (DEBUG_PROGVIDEO_MERGE) {
      console.debug("[Programmatic Video merge] merge applied", {
        rowIndex,
        spanId: newSpan.id,
  ...
  useEffect(() => {
    ...
    const cellId = expertGridCellId(
      ...
      progVideoDescriptorKeys.length + WEEK_GRID_COL_OFFSET + lastWi
    )
    document
      .getElementById(cellId)
      ?.scrollIntoView({ behavior: "smooth", block: "nearest", inline: "nearest" })
  }, [domGridId, mergeTarget, mergeWeeksReady, progVideoDescriptorKeys.length, weekKeys])

  useEffect(() => {
    if (!DEBUG_PROGVIDEO_MERGE) return
    console.debug("[Programmatic Video merge] merge eligibility derived", {
      mergeTarget,
      derivedMergeTarget,
      mergeWeeksReady,
      weekRectSelection,
  ...
  const pasteMatrixIntoGrid = useCallback(
    (matrix: string[][]) => {
      ...
      if (fcIsWeek) {
        ...
        const nextRows: ProgVideoExpertScheduleRow[] = normalizedRows.map((r) => ({
          ...r,
      ...
      for (let dr = 0; dr < working.length; dr++) {
        ...
        for (let dc = 0; dc < pasteRow.length; dc++) {
          ...
          const field = target.field as keyof ProgVideoExpertScheduleRow
          ...
          if (field === "unitRate") {
            ...
            const v = raw.trim()
            nextRows[targetRow] = { ...cur, [field]: v } as ProgVideoExpertScheduleRow
            applied += 1
          }
        }
      }
  ...
  const containerTotals = useMemo(() => {
    ...
    const fee =
      feeprogvideo > 0 && feeprogvideo < 100 ? (sumGross * feeprogvideo) / (100 - feeprogvideo) : 0
    const totalWithFee = sumGross + fee

    return { sumGross, sumQty, perWeek, fee, totalWithFee }
  }, [feeprogvideo, normalizedRows, weekKeys])
  ...
  const colIndexOf = useCallback(
    (key: keyof ProgVideoExpertScheduleRow) => progVideoDescriptorKeys.indexOf(key),
    [progVideoDescriptorKeys]
  )
      ...
      <Card className="relative overflow-hidden border-0 shadow-md">
        ...
        <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2 border-b border-border/60 bg-muted/5 pb-3">
          <CardTitle className="text-base font-semibold tracking-tight">
            Programmatic Video — Expert Schedule
          </CardTitle>
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2">
              <Label htmlFor="progvideo-expert-row-count" className="text-sm whitespace-nowrap">
                Rows:
              </Label>
              <Input
                id="progvideo-expert-row-count"
                type="number"
                min={1}
                max={500}
          ...
          <div className="overflow-hidden rounded-lg border border-border/80 bg-card/30 shadow-sm">
            {normalizedRows.length === 0 ? (
              ...
              <div
                ...
                data-progvideo-expert-grid-scroll=""
              >
                    ...
                    <tr>
                        ...
                        <th
                          ...
                          className={stickyThCorner(
                            i === descriptorHeadLabels.length - 1
                              ? PROGVIDEO_EXPERT_WEEK_SCROLLER_EDGE
                              : undefined
                          )}
                          style={{
                      ...
                      {weekColumns.map((col) => (
                        <th
                          ...
                          style={{
                            ...progVideoExpertWeekColLayoutStyle,
                            ...progVideoExpertHeaderCellBgStyle,
                          }}
                          title={col.labelFull}
                        >
                  ...
                  <tbody>
                    {normalizedRows.map((row, rowIndex) => {
                      ...
                      const stripeStyle =
                        rowIndex % 2 === 0
                          ...
                          : undefined
                      const grossCol = progVideoDescriptorKeys.length
                      const actionsCol = progVideoDescriptorKeys.length + 1
                      const sigmaCol = progVideoDescriptorKeys.length + 2
                        ...
                        >
                          ...
                          {(() => {
                            ...
                            for (let wi = 0; wi < weekColumns.length; wi += 1) {
                              ...
                              const colIndex =
                                progVideoDescriptorKeys.length +
                              ...
                              const tdClassName = cn(
                                ...
                                isPopulatedNonMergedCell &&
                                  PROGVIDEO_WEEK_CELL_VISUAL_CLASSES.populatedSingleTd,
                                ...
                                !isMergedAnchorCell &&
                                  mergeReadyOnCell &&
                                  "progvideo-expert-week-cell--merge-ready z-[6] bg-amber-500/24 ring-2 ring-inset ring-amber-600/75 shadow-sm dark:bg-amber-500/20 dark:ring-amber-400/70",
                              ...
                              const mergedAnchorWrapperClassName = cn(
                                ...
                                isMergedAnchorCell &&
                                  PROGVIDEO_WEEK_CELL_VISUAL_CLASSES.mergedSurface,
                              ...
                              const inputClassName = cn(
                                ...
                                isMergedAnchorCell
                                  ? PROGVIDEO_WEEK_CELL_VISUAL_CLASSES.mergedInput
                              ...
                              renderedWeekCells.push(
                                <td
                                  ...
                                  style={
                                    isMergedAnchorCell
                                      ? {
                                          width: PROGVIDEO_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                          minWidth:
                                            PROGVIDEO_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                          maxWidth:
                                            PROGVIDEO_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                  ...
                                  onMouseEnter={(e) => {
                                    ...
                                    const next = normalizeProgVideoWeekRect(
                                      drag.rowIndex,
                                      drag.weekKey,
                                      rowIndex,
                                      col.weekKey,
                                    ...
                                    setWeekStripSelection(null)
                                    if (DEBUG_PROGVIDEO_MERGE) {
                                      console.debug("[Programmatic Video merge] drag update", {
                                        rect: next,
                                      })
                                    }
                                  }}
                                ...
                                >
                                  ...
                                  >
                                    <Input
                                      ...
                                      setIsSelecting(true)
                                      if (DEBUG_PROGVIDEO_MERGE) {
                                        console.debug("[Programmatic Video merge] drag start", {
                                          rowIndex,
                                          weekKey: col.weekKey,
                                        })
                                      }
                                      ...
                                      if (shift && lastWeekAnchorRef.current) {
                                        ...
                                        if (
                                          ...
                                          const rect = normalizeProgVideoWeekRect(
                                            lastWeekAnchorRef.current.rowIndex,
                                            lastWeekAnchorRef.current.weekKey,
                                            rowIndex,
                                            col.weekKey,
                    ...
                    >
                      <td
                        ...
                        colSpan={progVideoDescriptorKeys.length}
                      >
                      ...
                      <td
                        className={cn(
                          stickyTd(progVideoDescriptorKeys.length),
                          "h-8 px-1 text-xs tabular-nums"
                        )}
                        style={{
                          ...stickyStyleBody(progVideoDescriptorKeys.length),
                          ...progVideoExpertTotalsRowBgStyle,
                        }}
                      >
                      ...
                      <td
                        className={cn(
                          stickyTd(progVideoDescriptorKeys.length + 1),
                          "h-8"
                        )}
                        style={{
                          ...stickyStyleBody(progVideoDescriptorKeys.length + 1),
                          ...progVideoExpertTotalsRowBgStyle,
                        }}
                      />
                      <td
                        className={cn(
                          stickyTd(progVideoDescriptorKeys.length + 2),
                          "h-8 px-1 text-xs tabular-nums text-muted-foreground",
                          PROGVIDEO_EXPERT_WEEK_SCROLLER_EDGE
                        )}
                        style={{
                          ...stickyStyleBody(progVideoDescriptorKeys.length + 2),
                          ...progVideoExpertTotalsRowBgStyle,
                        }}
                      >
