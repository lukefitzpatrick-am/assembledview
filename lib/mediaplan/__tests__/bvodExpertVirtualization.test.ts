/**
 * F-28 Phase 2 — BVOD Expert row virtualization smoke (Prompt A items 1,3,5,7,8).
 */
import assert from "node:assert/strict"
import test from "node:test"
import { format, startOfDay } from "date-fns"

import { mapBvodExpertRowsToStandardLineItems } from "@/lib/mediaplan/expertChannelMappings"
import {
  reorderExpertRows,
  mergedSpanWidthPx,
...
const BVOD_EXPERT_ROW_OVERSCAN = EXPERT_GRID_ROW_OVERSCAN_DEFAULT
...
function emptyRow(id: string): BvodExpertScheduleRow {
...
function buildRows(n: number): BvodExpertScheduleRow[] {
  return Array.from({ length: n }, (_, i) => emptyRow(`bvod-v-${i}`))
}

/** Mirrors BVODExpertGrid containerTotals over the FULL logical array. */
function computeBvodWeeklyTotals(
...
test("3. DRAG REORDER across virtual boundary lands on correct logical index", () => {
  ...
  const dropIndex = virtualRowIndexFromOffsetY(
    280 * BVOD_EXPERT_ROW_HEIGHT_PX + 1,
    BVOD_EXPERT_ROW_HEIGHT_PX,
    300
  )
...
test("5. WEEK-COLUMN WIDTH respects resize overrides (sticky header/week resize path)", () => {
  ...
  assert.equal(BVOD_EXPERT_ROW_HEIGHT_PX, 41)
})
...
test("7. WEEKLY TOTALS derived from full 300-row state", () => {
  ...
  const totals = computeBvodWeeklyTotals(rows, 0)
  ...
  const after = computeBvodWeeklyTotals(edited, 0)
...
test("8. PERF: mounted DOM window stays bounded while scrolling 300 rows", () => {
  ...
  const nearTop = expectedMountedRowRange(
    ...
    BVOD_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    BVOD_EXPERT_ROW_OVERSCAN
  )
  const mid = expectedMountedRowRange(
    150 * BVOD_EXPERT_ROW_HEIGHT_PX,
    viewportHeight,
    BVOD_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    BVOD_EXPERT_ROW_OVERSCAN
  ...
  const maxWindow =
    Math.ceil(viewportHeight / BVOD_EXPERT_ROW_HEIGHT_PX) +
    2 * BVOD_EXPERT_ROW_OVERSCAN +
    2
