...
import {
  ...
  WEEK_COL_WIDTH_PX as INTEGRATION_EXPERT_WEEK_COL_WIDTH_PX,
  WEEK_SCROLLER_EDGE as INTEGRATION_EXPERT_WEEK_SCROLLER_EDGE,
  WEEK_CELL_VISUAL_CLASSES as INTEGRATION_WEEK_CELL_VISUAL_CLASSES,
  ...
  normalizeWeekRect as normalizeIntegrationWeekRect,
  weekCellInRect as integrationWeekCellInRect,
  ...
  normalizeWeekMergeSelection as normalizeIntegrationWeekMergeSelection,
  weekPlainClickPreservesWeekAreaSelection,
  deriveMergeEligibility as deriveIntegrationMergeEligibility,
  weekCellExportText,
...
type IntegrationWeekMergeSelectionNormalized = WeekMergeSelectionNormalized
...
export function IntegrationExpertGrid({
  ...
  const normalizeRowCacheRef = useRef(
    new Map<string, NormalizeRowCacheEntry<IntegrationExpertScheduleRow>>()
  )
  ...
  const rowMergeMapCacheRef = useRef(
    new Map<
      string,
      MapBuildCacheEntry<
        IntegrationExpertScheduleRow["mergedWeekSpans"],
        IntegrationRowMergeMap
      >
    >()
  )
  ...
  const pushRows = useCallback(
    (next: IntegrationExpertScheduleRow[]) => {
      const withDates = finalizeRowsPreservingIdentity(next, (r) => {
        ...
        const dates = deriveIntegrationExpertRowScheduleYmdFromRow(
          cleared,
          weekColumns,
          campaignStartDate,
          campaignEndDate,
  ...
  const widthForWeekKey = useCallback(
    (k: string): number => {
      ...
      return weekColumnWidths[k] ?? INTEGRATION_EXPERT_WEEK_COL_WIDTH_PX
    },
  ...
  const containerTotals = useMemo(() => {
    ...
    for (const row of normalizedRows) {
      const { net, fee } = expertRowCostSplit(row, weekKeys, feeintegration)
      sumNet += net
      sumFee += fee
      for (const k of weekKeys) {
  ...
  }, [dayKeysByWeekKey, feeintegration, normalizedRows, weekKeys])
        ...
        <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2 border-b border-border/60 bg-muted/5 pb-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <Label htmlFor="integration-expert-row-count" className="sr-only">Add rows count</Label>
              <Input
                  ...
                  <thead className="[&_tr]:border-b-0">
                    <tr>
                      <ExpertGridRowReorderHeaderCell
                        ...
                        style={integrationExpertHeaderCellBgStyle}
                      />
                        ...
                        return (
                          ...
                          >
                            ...
                            <ExpertGridWeekResizeHandle
                              weekKey={col.weekKey}
                              currentWidth={weekColumnWidths[col.weekKey] ?? INTEGRATION_EXPERT_WEEK_COL_WIDTH_PX}
                    ...
                    {normalizedRows.map((row, rowIndex) => {
                      ...
                      return (
                        <MemoExpertGridRow
                          ...
                          rowUiSig={rowUiSig}
                          render={() => {
                      const net = expertRowNetMedia(row, weekKeys, feeintegration)
                          ...
                          {(() => {
                            ...
                            for (let wi = 0; wi < weekColumns.length; wi += 1) {
                              ...
                              const tdClassName = cn(
                                ...
                                !isMergedAnchorCell &&
                                  mergeReadyOnCell &&
                                  "integration-expert-week-cell--merge-ready z-[6] bg-pacing-behind-bg ring-2 ring-inset ring-pacing-behind shadow-e0",
