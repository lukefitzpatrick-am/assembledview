...
- **`DATA_BACKEND=xano|shadow|postgres`** (default `xano`) — Phase 2 cutover switch. Reference-table GETs (`tv_stations`, `radio_stations`, `newspapers`(+adsizes), `magazines`(+adsizes), `audio`/`bvod`/`display`/`video_site`) go through `lib/data/readReferenceMediaDetail.ts`: `xano` = Xano only; `shadow` = serve Xano + async Supabase field-level diff into an in-memory store (`lib/data/shadowDiff.ts`); `postgres` = serve Supabase via `db/`. Wired in `/api/media-details` GETs and server-side `fetchMediaDetail` in `lib/api.ts`. Admin summary: `GET /api/admin/migration-diffs` (`requireAdmin`).
