...
- `app/admin/fireflies-unattributed` — Fireflies notes filtered by `attributed_type` (Unattributed default / Publisher / Internal / New Business). Assign is a grouped searchable dropdown (clients by `mp_client_name — mbaidentifier`, publishers excluding id 61, Internal, New Business) via `GET|POST /api/admin/fireflies-unattributed`. Client assign writes `client_domains` (MR-5); publisher assign writes `publisher_domains` unless free-mail, roster, or already in `client_domains`; internal / new_business upsert `meeting_title_rules` (no domain learning). **Sync now** (`POST /api/admin/fireflies-sync`, `requireAdmin`) runs the same pull as cron. Cron `GET/POST /api/cron/fireflies-sync` (`FIREFLIES_API_KEY` + `CRON_SECRET`) pulls transcripts into `client_notes` and advances `fireflies_sync_state` cursor. Attribution is client-name-in-title first, then client domain, then publisher domain, then title rules, then roster-internal; MBA is a client-only refinement. Human accept lives on `/tasks` Inbox. Publisher Hub Meetings (`PublisherMeetingsSection`) is read-only `client_notes` for this `publisher_id` + `attributed_type='publisher'` via `GET /api/publishers/[publisherId]/meetings` (`requireAdmin`); empty state points here.
