...
function line(bursts: any[]): StandardCinemaFormLineItem {
  return {
    network: "N", station: "", buyType: "spots", placement: "", format: "", duration: "",
...
test("sub-week interior burst imports as day-detail and round-trips", () => {
  // Wed 7 – Fri 9 Jan (inside week 1, a full 7-day interior week), 6 spots
  const std = [line([{ budget: "600", buyAmount: "100", startDate: new Date(2026,0,7), endDate: new Date(2026,0,9), calculatedValue: 6 }])]
  const rows = mapStandardCinemaLineItemsToExpertRows(std, cols, CS, CE)
  ...
  const back = mapCinemaExpertRowsToStandardLineItems(rows, cols, CS, CE, { feePctCinema: 0 })
  const b = back[0]!.bursts
  const total = b.reduce((s, x) => s + Number(x.calculatedValue ?? 0), 0)
  assert.equal(total, 6)                       // deliverables preserved
  // window preserved to the day (Wed–Fri), not widened to the whole week
...
test("full-week burst stays weekly (no dailyValues)", () => {
  // Full interior week Sun 11 – Sat 17, 10 spots
