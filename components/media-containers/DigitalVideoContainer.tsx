...
export default function DigiVideoContainer({
  ...
  useEffect(() => {
    ...
    const stableItems = assignStableLineItemNumbers<any>(formLineItems, mbaNumber, MEDIA_TYPE_ID_CODES.digitalVideo);
    ...
    const lineItemTotals = watchedLineItems.map((lineItem, index) => {
      ...
      lineItem.bursts.forEach((burst) => {
        ...
        if (lineItem.budgetIncludesFees) {
          ...
          burstMedia = budget;
          burstFee = feedigivideo ? (budget / (100 - feedigivideo)) * feedigivideo : 0;
        }
  ...
  const handleValueChange = useCallback((lineItemIndex: number, burstIndex: number, budgetIncludesFeesOverride?: boolean) => {
    ...
    const buyTypeRaw = form.getValues(`digivideolineItems.${lineItemIndex}.buyType`);
    ...
    const bt = coerceBuyTypeWithDevWarn(String(buyTypeRaw || ""), "DigiVideoContainer.handleValueChange");
                                ...
                                <div className={MP_BURST_ROW_SHELL}>
                                  ...
                                  <div className={showAdServingOverrideInput ? AD_SERVING_OVERRIDE_BURST_GRID : "grid grid-cols-7 gap-3 items-center flex-grow"}>
                                    ...
                                    {(buyType === "cpc" || buyType === "cpv") && (
                                      <FormField
                                        control={form.control}
                                        name={`digivideolineItems.${lineItemIndex}.bursts.${burstIndex}.adServingRatePct`}
                                        render={({ field }) => (
                                          <FormItem>
                                    ...
                                    {buyType === "fixed_cost" && (
                                      <FormField
                                        control={form.control}
                                        name={`digivideolineItems.${lineItemIndex}.bursts.${burstIndex}.adServingImpressions`}
                                        render={({ field }) => (
                                          <FormItem>
