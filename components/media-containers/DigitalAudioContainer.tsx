...
export default function DigiAudioContainer({
    ...
    const transformedLineItems = formLineItems.map((lineItem, index) => {
      ...
      lineItem.bursts.forEach((burst) => {
        ...
        if (lineItem.budgetIncludesFees) {
          ...
          const base = budget / (1 + (feedigiaudio || 0) / 100);
          totalMedia += base;
        } else {
          // Budget is net media
          totalMedia += budget;
      ...
      return {
        ...
        line_item_id: `${mbaNumber || 'DA'}${index + 1}`,
  ...
  }, [watchedLineItems, mbaNumber, feedigiaudio, onMediaLineItemsChange]);
  ...
  const handleLineItemValueChange = useCallback((lineItemIndex: number) => {
    ...
    digiaudiolineItems.forEach((lineItem) => {
      ...
      lineFee = feedigiaudio ? (lineMedia / (100 - feedigiaudio)) * feedigiaudio : 0;
      overallMedia += lineMedia;
      overallFee += lineFee;
  ...
  const handleValueChange = useCallback((lineItemIndex: number, burstIndex: number) => {
    ...
    const currentValue = form.getValues(`digiaudiolineItems.${lineItemIndex}.bursts.${burstIndex}.calculatedValue`);
...
useEffect(() => {
  ...
  const items: LineItem[] = form.getValues('digiaudiolineItems').flatMap((lineItem, lineItemIndex) =>
    lineItem.bursts.map(burst => {
      ...
      const lineItemId = `${mbaNumber || 'DA'}${lineItemIndex + 1}`;
...
}, [watchedLineItems, feedigiaudio, mbaNumber]);
  ...
  }, [watchedLineItems, feedigiaudio]); // Removed callback dependencies to prevent infinite loops
  ...
  return (
    <div className="space-y-6">
      <div className="mb-6">
        <Card>
          <CardHeader>
            <CardTitle className="pt-4 border-t font-bold text-lg flex justify-between">Digital Audio</CardTitle>
          </CardHeader>
                              ...
                              <CardContent className="py-2 px-4">
                                <div className="flex items-center gap-3">
                                  ...
                                  <div className="grid grid-cols-7 gap-3 items-center flex-grow">
                                    ...
                                    <div className="space-y-1">
                                      <FormLabel className="text-xs leading-tight">Fee ({feedigiaudio}%)</FormLabel>
                                      <Input
                                        type="text"
                                        className="w-full h-10 text-sm"
                                        value={new Intl.NumberFormat("en-US", {
