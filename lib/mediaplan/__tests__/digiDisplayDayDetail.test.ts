...
function line(bursts: any[]): StandardDigiDisplayFormLineItem {
  return {
    platform: "P", site: "S", buyType: "bonus", publisher: "Pub", creativeTargeting: "", creative: "", buyingDemo: "", market: "",
...
test("sub-week interior burst imports as day-detail and round-trips", () => {
  ...
  const rows = mapStandardDigiDisplayLineItemsToExpertRows(std, cols, CS, CE)
  const wk0 = cols[0]!
  assert.equal(weekHasDailyValues(rows[0]!.dailyValues ?? {}, weekDayKeys(wk0, CS, CE)), true)
  const back = mapDigitalDisplayExpertRowsToStandardLineItems(rows, cols, CS, CE, { feePctDigiDisplay: 0 })
