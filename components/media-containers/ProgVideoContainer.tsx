...
export default function ProgVideoContainer({
    ...
    const transformedLineItems = formLineItems.map((lineItem, index) => {
      ...
      lineItem.bursts.forEach((burst) => {
        ...
        if (lineItem.budgetIncludesFees) {
          ...
          const base = budget / (1 + (feeprogvideo || 0) / 100);
          totalMedia += base;
        } else {
          // Budget is net media
          totalMedia += budget;
  ...
  }, [watchedLineItems, mbaNumber, feeprogvideo, onMediaLineItemsChange]);
  ...
  const handleLineItemValueChange = useCallback((lineItemIndex: number) => {
    ...
    lineItems.forEach((lineItem) => {
      ...
      lineFee = feeprogvideo ? (lineMedia / (100 - feeprogvideo)) * feeprogvideo : 0;
      overallMedia += lineMedia;
      overallFee += lineFee;
  ...
  }, [form, feeprogvideo]); // Removed onTotalMediaChange dependency to prevent infinite loops
...
useEffect(() => {
  ...
  }, [watchedLineItems, feeprogvideo]); // Removed callback dependencies to prevent infinite loops
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="pt-4 border-t font-medium flex justify-between">
              <span>Prog Video Media Totals:</span>
              <div className="flex space-x-4">
                <span>Media: ${overallTotals.overallMedia.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                <span>Fees ({feeprogvideo}%): ${overallTotals.overallFee.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                      ...
                      <CardHeader className="pb-2">
                        <div className="flex justify-between items-center">
                          <div className="flex items-center space-x-2">
                            <CardTitle className="text-lg font-medium">Prog Video Line Item {lineItemIndex + 1}</CardTitle>
                                  ...
                                  <div className="grid grid-cols-5 gap-4 items-center flex-grow">
                                    ...
                                    <div className="flex flex-col space-y-2">
                                      <div className="grid grid-cols-2 gap-2">
                                        ...
                                        <div className="flex flex-col">
                                          <FormLabel className="text-xs">Fee ({feeprogvideo}%)</FormLabel>
                                          <Input
                                            type="text"
                                            className="w-full"
                                            value={new Intl.NumberFormat("en-US", {
