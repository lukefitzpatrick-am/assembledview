...
import type {
  ExpertWeeklyValues,
  IntegrationExpertMergedWeekSpan,
...
const INTEGRATION_EXPERT_WEEK_COL_WIDTH_PX = 112

/** Week column width on `<th>` and week `<td>`; inputs use `w-full` inside the cell. */
const integrationExpertWeekColLayoutStyle = {
  width: INTEGRATION_EXPERT_WEEK_COL_WIDTH_PX,
  minWidth: INTEGRATION_EXPERT_WEEK_COL_WIDTH_PX,
  maxWidth: INTEGRATION_EXPERT_WEEK_COL_WIDTH_PX,
  boxSizing: "border-box" as const,
}

const MEDIA_ACCENT_HEX = getMediaTypeThemeHex("integration")

const integrationExpertHeaderCellBgStyle = {
  backgroundColor: rgbaFromHex(MEDIA_ACCENT_HEX, 0.08),
}

const integrationExpertTotalsRowBgStyle = {
  backgroundColor: rgbaFromHex(MEDIA_ACCENT_HEX, 0.06),
}

/** Right edge of sticky block before the horizontally scrolling week area. */
const INTEGRATION_EXPERT_WEEK_SCROLLER_EDGE =
...
function sumMergedQuantities(row: IntegrationExpertScheduleRow): number {
...
): IntegrationExpertMergedWeekSpan | null {
...
type IntegrationMultiCellSelection = {
  startRow: number
  endRow: number
  startCol: number
  endCol: number
...
type IntegrationCopiedCells = {
  data: (string | number)[][]
  sourceRows: number
  sourceCols: number
  selection: IntegrationMultiCellSelection
}

function weeklyCellDisplayValue(
  cellValue: number | string | undefined,
  mergedSpan: IntegrationExpertMergedWeekSpan | null
): string {
  if (mergedSpan) {
...
const INTEGRATION_WEEK_CELL_VISUAL_CLASSES = {
...
function selectionOverlapsMergedSpan(
  rowMergeMap: IntegrationRowMergeMap | undefined,
...
type IntegrationWeekRectSelection = {
  rowStart: number
  rowEnd: number
  weekKeyStart: string
  weekKeyEnd: string
...
function normalizeIntegrationWeekRect(
  row0: number,
  wk0: string,
  row1: number,
  wk1: string,
  weekKeys: readonly string[]
): IntegrationWeekRectSelection {
...
function integrationWeekCellInRect(
  rowIndex: number,
  weekKey: string,
  rect: IntegrationWeekRectSelection | null,
  weekKeys: readonly string[]
): boolean {
...
function rectToMultiCellSelection(
  rect: IntegrationWeekRectSelection | null,
  weekKeys: readonly string[]
): IntegrationMultiCellSelection | null {
  if (!rect) return null
  const i0 = weekKeys.indexOf(rect.weekKeyStart)
  const i1 = weekKeys.indexOf(rect.weekKeyEnd)
...
function coerceWeeklySelectionBounds(
  ...
  weekRectSelection: IntegrationWeekRectSelection | null,
  ...
  if (
    weekRectSelection &&
    integrationWeekCellInRect(anchorRow, anchorWeekKey, weekRectSelection, weekKeys)
  ) {
...
function applyWeeklyPasteMatrixToSelection(args: {
  ...
  weekKeys: readonly string[]
  rowCount: number
  nextRows: IntegrationExpertScheduleRow[]
}): {
  applied: number
  errorReasons: string[]
  layout: WeeklyPasteLayoutMode
...
function oohMergeKeysFromRect(
  rect: IntegrationWeekRectSelection,
  weekKeys: readonly string[]
): string[] | null {
...
type IntegrationWeekMergeSelectionNormalized = {
  rowIndex: number
 ...
 */
function normalizeIntegrationWeekMergeSelection(
  rect: IntegrationWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): IntegrationWeekMergeSelectionNormalized | null {
...
function weekPlainClickPreservesWeekAreaSelection(
  ...
  const mergeNorm = normalizeIntegrationWeekMergeSelection(rect, multi, weekKeys)
  if (
    mergeNorm &&
  ...
  if (rect && integrationWeekCellInRect(rowIndex, weekKey, rect, weekKeys)) {
    const isSingleCell =
...
function deriveIntegrationMergeEligibility(
  rect: IntegrationWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): IntegrationWeekMergeSelectionNormalized | null {
  return normalizeIntegrationWeekMergeSelection(rect, multi, weekKeys)
}
...
function mergedWeekSpansAfterCutRect(
  spans: IntegrationExpertMergedWeekSpan[] | undefined,
  wi0: number,
  wi1: number,
  weekKeys: readonly string[]
): IntegrationExpertMergedWeekSpan[] {
...
type WeeklyExportSelection =
  | { kind: "rect"; rect: IntegrationWeekRectSelection }
...
function resolveWeeklyExportSelection(
  ...
  weekKeys: readonly string[],
  rows: readonly IntegrationExpertScheduleRow[] | null
...
function buildWeeklyExportTsv(
  selection: WeeklyExportSelection,
  rows: readonly IntegrationExpertScheduleRow[],
  weekKeys: readonly string[]
): string | null {
  switch (selection.kind) {
    case "rect": {
...
function applyWeeklyCutToRows(
  ...
  rows: readonly IntegrationExpertScheduleRow[],
  weekKeys: readonly string[]
): IntegrationExpertScheduleRow[] | null {
  switch (selection.kind) {
    case "rect": {
      const rect = selection.rect
      const wi0 = Math.min(
      ...
      if (wi0 < 0 || wi1 < 0) return null
      const next: IntegrationExpertScheduleRow[] = rows.map((r) => ({
        ...r,
    ...
    case "mergeContiguous": {
      ...
      const next: IntegrationExpertScheduleRow[] = rows.map((row) => ({
        ...row,
...
/** Match labels/values on {@link IntegrationContainer} buy-type combobox. */
const INTEGRATION_BUY_TYPE_OPTIONS: ComboboxOption[] = [
...
const INTEGRATION_BID_STRATEGY_OPTIONS: ComboboxOption[] = [
...
function normalizeIntegrationBuyTypePaste(raw: string): string {
  const v = raw.trim()
  if (!v) return ""
  const byValue = INTEGRATION_BUY_TYPE_OPTIONS.find(
    (o) => o.value.toLowerCase() === v.toLowerCase()
  )
  if (byValue) return byValue.value
  const byLabel = INTEGRATION_BUY_TYPE_OPTIONS.find(
...
function normalizeIntegrationBidStrategyPaste(raw: string): string {
  const v = raw.trim()
  if (!v) return ""
  const byValue = INTEGRATION_BID_STRATEGY_OPTIONS.find((o) => o.value.toLowerCase() === v.toLowerCase())
  if (byValue) return byValue.value
  const byLabel = INTEGRATION_BID_STRATEGY_OPTIONS.find((o) => o.label.toLowerCase() === v.toLowerCase())
  if (byLabel) return byLabel.value
  return v
}

function normalizeIntegrationPlatformPaste(raw: string, platformNames: string[]): string {
  const v = raw.trim()
  if (!v) return ""
...
function rowGrossCost(row: IntegrationExpertScheduleRow, weekKeys: string[]): number {
...
const INTEGRATION_DESCRIPTOR_CORE: readonly (keyof IntegrationExpertScheduleRow)[] = [
  "startDate",
  "endDate",
  "platform",
  "bidStrategy",
...
const INTEGRATION_BILLING_FLAG_KEYS: readonly (keyof IntegrationExpertScheduleRow)[] = [
  "fixedCostMedia",
  "clientPaysForMedia",
  "budgetIncludesFees",
]

const INTEGRATION_DESCRIPTOR_TAIL: readonly (keyof IntegrationExpertScheduleRow)[] = [
  "market",
  "buyingDemo",
  "unitRate",
]
...
export interface IntegrationExpertGridProps {
  ...
  /** Platform names (integration publishers API) for platform combobox + fuzzy matching */
  publishers?: { publisher_name: string }[]
}

const moneyOpts = { locale: "en-US" as const, currency: "USD" as const }

type IntegrationExpertFocusedCell = { rowIndex: number; columnKey: string }
type WeekDragSource =
  | {
      type: "single"
      rowIndex: number
...
type IntegrationRowMergeSpanMeta = Readonly<{
  id: string
  startWeekKey: string
  endWeekKey: string
  totalQty: number
...
type IntegrationRowMergeMap = Readonly<{
  anchorByWeekKey: Readonly<Record<string, string>>
  interiorByWeekKey: Readonly<Record<string, string>>
  spanById: Readonly<Record<string, IntegrationExpertMergedWeekSpan>>
  spanMetaByAnchorWeekKey: Readonly<Record<string, IntegrationRowMergeSpanMeta>>
}>
...
export function IntegrationExpertGrid({
  ...
  const [focusedCell, setFocusedCell] = useState<IntegrationExpertFocusedCell | null>(
    null
  )
  const focusedCellRef = useRef<IntegrationExpertFocusedCell | null>(null)
  ...
  const [weekRectSelection, setWeekRectSelection] =
    useState<IntegrationWeekRectSelection | null>(null)
  // Multi-cell selection state for copy/paste operations.
  const [multiCellSelection, setMultiCellSelection] =
    useState<IntegrationMultiCellSelection | null>(null)
  const [isSelecting, setIsSelecting] = useState(false)
  const [copiedCells, setCopiedCells] = useState<IntegrationCopiedCells | null>(null)
  ...
  const lastDragRectDuringGestureRef = useRef<IntegrationWeekRectSelection | null>(null)
  /**
   ...
   */
  const postDragWeekClickRectRef = useRef<IntegrationWeekRectSelection | null>(null)
  const weekRectSelectionRef = useRef<IntegrationWeekRectSelection | null>(null)
  ...
  const integrationDescriptorKeys = useMemo(
    () =>
      [
        ...(showBillingCols ? INTEGRATION_BILLING_FLAG_KEYS : []),
        ...INTEGRATION_DESCRIPTOR_CORE,
        ...INTEGRATION_DESCRIPTOR_TAIL,
      ] as (keyof IntegrationExpertScheduleRow)[],
    [showBillingCols]
  )

  const descriptorColWidths = useMemo(() => {
    const billing = showBillingCols ? [56, 56, 56] : []
  ...
  const rowMergeMaps = useMemo<readonly IntegrationRowMergeMap[]>(() => {
    const maps = normalizedRows.map((row) => {
      const anchorByWeekKey: Record<string, string> = {}
      const interiorByWeekKey: Record<string, string> = {}
      const spanById: Record<string, IntegrationExpertMergedWeekSpan> = {}
      const spanMetaByAnchorWeekKey: Record<string, IntegrationRowMergeSpanMeta> = {}
      ...
      for (const span of row.mergedWeekSpans ?? []) {
        if (spanById[span.id]) {
          if (DEBUG_INTEGRATION_MERGE) {
            console.debug("[Integration merge] occupancy duplicate span id ignored", {
              rowId: row.id,
        ...
        if (keys.some((k) => occupiedWeekKeys.has(k))) {
          if (DEBUG_INTEGRATION_MERGE) {
            console.debug("[Integration merge] occupancy overlap ignored", {
              rowId: row.id,
              rowIndex: normalizedRows.indexOf(row),
              spanId: span.id,
              keys,
  ...
  useEffect(() => {
    if (!DEBUG_INTEGRATION_MERGE) return
    ...
    console.debug("[Integration merge] pending selection updated", pendingMergeSelection)
  ...
  const updateRow = useCallback(
    (rowIndex: number, patch: Partial<IntegrationExpertScheduleRow>) => {
  ...
  const tryFuzzyMatch = useCallback(
    (rowIndex: number, field: FuzzyMatchField, value: string) => {
      ...
      if (corrected) {
        updateRow(rowIndex, { [field]: corrected } as Partial<IntegrationExpertScheduleRow>)
        return
      }
      ...
      if (fuzzyMatchAutoApplyRef.current) {
        updateRow(rowIndex, { [field]: match.matched } as Partial<IntegrationExpertScheduleRow>)
      } else {
  ...
  const navColCount =
    integrationDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekKeys.length
  const firstWeekNavColIndex = integrationDescriptorKeys.length + WEEK_GRID_COL_OFFSET
  const unitRateNavColIndex = integrationDescriptorKeys.indexOf("unitRate")
  ...
  const unmergeWeekSpan = useCallback(
    (rowIndex: number, spanId: string) => {
      ...
      if (DEBUG_INTEGRATION_MERGE) {
        console.debug("[Integration merge] unmerge applied", { rowIndex, spanId })
      }
    },
  ...
  const duplicateRow = useCallback(
    (rowIndex: number) => {
      ...
      const id =
        typeof crypto !== "undefined" && crypto.randomUUID
          ? crypto.randomUUID()
          : `integration-expert-${Date.now()}-${rowIndex}`
  ...
  const clearWeekSelectionWhereMergedAnchorInvolved = useCallback(
    (rowIndex: number, weekKey: string) => {
      setWeekRectSelection((prev) =>
        prev && integrationWeekCellInRect(rowIndex, weekKey, prev, weekKeys)
          ? null
          : prev
      )
  ...
  const rangeWeekMultiSelect = useCallback(
    (rowIndex: number, anchorKey: string, endKey: string) => {
      ...
      setWeekRectSelection(
        normalizeIntegrationWeekRect(rowIndex, anchorKey, rowIndex, endKey, weekKeys)
      )
      setWeekStripSelection(null)
    },
    [weekKeys]
  ...
  const lockPendingMergeSelectionFromCurrentSelection = useCallback(() => {
    const n = deriveIntegrationMergeEligibility(
  ...
  const derivedMergeTarget = useMemo(() => {
    const n = deriveIntegrationMergeEligibility(weekRectSelection, weekMultiSelect, weekKeys)
  ...
  useEffect(() => {
    const endDrag = () => {
      ...
      if (DEBUG_INTEGRATION_MERGE) {
        console.debug("[Integration merge] drag end", {
          lastRect: r,
  ...
  const handleMergeSelectedWeeks = useCallback(() => {
    ...
    if (selectionOverlapsMergedSpan(rowMergeMaps[rowIndex], sorted, weekKeys)) {
      if (DEBUG_INTEGRATION_MERGE) {
        console.debug("[Integration merge] validation overlap failure", {
          rowIndex,
    ...
    const newId =
      typeof crypto !== "undefined" && crypto.randomUUID
        ? crypto.randomUUID()
        : `merge-${Date.now()}`
    const newSpan: IntegrationExpertMergedWeekSpan = {
      id: newId,
    ...
    if (DEBUG_INTEGRATION_MERGE) {
      console.debug("[Integration merge] merge applied", {
        rowIndex,
        spanId: newSpan.id,
  ...
  useEffect(() => {
    ...
    const cellId = expertGridCellId(
      ...
      integrationDescriptorKeys.length + WEEK_GRID_COL_OFFSET + lastWi
    )
    document
      .getElementById(cellId)
      ?.scrollIntoView({ behavior: "smooth", block: "nearest", inline: "nearest" })
  }, [domGridId, mergeTarget, mergeWeeksReady, integrationDescriptorKeys.length, weekKeys])

  useEffect(() => {
    if (!DEBUG_INTEGRATION_MERGE) return
    console.debug("[Integration merge] merge eligibility derived", {
      mergeTarget,
      derivedMergeTarget,
      mergeWeeksReady,
      weekRectSelection,
  ...
  const pasteMatrixIntoGrid = useCallback(
    (matrix: string[][]) => {
      ...
      if (fcIsWeek) {
        ...
        const nextRows: IntegrationExpertScheduleRow[] = normalizedRows.map((r) => ({
          ...r,
      ...
      for (let dr = 0; dr < working.length; dr++) {
        ...
        for (let dc = 0; dc < pasteRow.length; dc++) {
          ...
          const field = target.field as keyof IntegrationExpertScheduleRow
          ...
          if (field === "unitRate") {
            ...
            nextRows[targetRow] = {
              ...cur,
              [field]: truthy,
            } as IntegrationExpertScheduleRow
            applied += 1
          } else if (field === "buyType") {
            nextRows[targetRow] = {
              ...cur,
              buyType: normalizeIntegrationBuyTypePaste(raw),
            }
            applied += 1
          } else if (field === "bidStrategy") {
            nextRows[targetRow] = {
              ...cur,
              bidStrategy: normalizeIntegrationBidStrategyPaste(raw),
            }
            applied += 1
          } else if (field === "platform") {
            const plt = normalizeIntegrationPlatformPaste(raw, platformNames)
            nextRows[targetRow] = {
              ...cur,
              platform: plt,
            }
            ...
            const v = raw.trim()
            nextRows[targetRow] = { ...cur, [field]: v } as IntegrationExpertScheduleRow
            applied += 1
          }
        }
      }
  ...
  }, [feeintegration, normalizedRows, weekKeys])
  ...
  const colIndexOf = useCallback(
    (key: keyof IntegrationExpertScheduleRow) => integrationDescriptorKeys.indexOf(key),
    [integrationDescriptorKeys]
  )
      ...
      <Card className="relative overflow-hidden border-0 shadow-md">
        ...
        <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2 border-b border-border/60 bg-muted/5 pb-3">
          <CardTitle className="text-base font-semibold tracking-tight">
            Integration Media — Expert Schedule
          </CardTitle>
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2">
              <Label htmlFor="integration-expert-row-count" className="text-sm whitespace-nowrap">
                Rows:
              </Label>
              <Input
                id="integration-expert-row-count"
                type="number"
                min={1}
                max={500}
                ...
                <table className="w-max min-w-full border-collapse text-sm">
                    ...
                    <tr>
                        ...
                        <th
                          ...
                          className={stickyThCorner(
                            i === descriptorHeadLabels.length - 1
                              ? INTEGRATION_EXPERT_WEEK_SCROLLER_EDGE
                              : undefined
                          )}
                          style={{
                            ...stickyStyleHeaderCorner(i),
                            ...integrationExpertHeaderCellBgStyle,
                          }}
                        >
                          {label === "Unit Rate" ? (
                            <Tooltip>
                      ...
                      {weekColumns.map((col) => (
                        <th
                          ...
                          style={{
                            ...integrationExpertWeekColLayoutStyle,
                            ...integrationExpertHeaderCellBgStyle,
                          }}
                          title={col.labelFull}
                        >
                  ...
                  <tbody>
                    {normalizedRows.map((row, rowIndex) => {
                      const net = rowNetMedia(row, weekKeys, feeintegration)
                      const qtySum =
                      ...
                      const stripeStyle =
                        rowIndex % 2 === 0
                          ...
                          : undefined
                      const grossCol = integrationDescriptorKeys.length
                      const actionsCol = integrationDescriptorKeys.length + 1
                      const sigmaCol = integrationDescriptorKeys.length + 2
                            ...
                            for (let wi = 0; wi < weekColumns.length; wi += 1) {
                              ...
                              const tdClassName = cn(
                                ...
                                isPopulatedNonMergedCell &&
                                  INTEGRATION_WEEK_CELL_VISUAL_CLASSES.populatedSingleTd,
                                ...
                                !isMergedAnchorCell &&
                                  mergeReadyOnCell &&
                                  "integration-expert-week-cell--merge-ready z-[6] bg-amber-500/24 ring-2 ring-inset ring-amber-600/75 shadow-sm dark:bg-amber-500/20 dark:ring-amber-400/70",
                              ...
                              const mergedAnchorWrapperClassName = cn(
                                ...
                                isMergedAnchorCell &&
                                  INTEGRATION_WEEK_CELL_VISUAL_CLASSES.mergedSurface,
                              ...
                              const inputClassName = cn(
                                ...
                                isMergedAnchorCell
                                  ? INTEGRATION_WEEK_CELL_VISUAL_CLASSES.mergedInput
                              ...
                              renderedWeekCells.push(
                                <td
                                  ...
                                  style={
                                    isMergedAnchorCell
                                      ? {
                                          width: INTEGRATION_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                          minWidth:
                                            INTEGRATION_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                          maxWidth:
                                            INTEGRATION_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                  ...
                                  onMouseEnter={(e) => {
                                    ...
                                    const next = normalizeIntegrationWeekRect(
                                      drag.rowIndex,
                                      drag.weekKey,
                                      rowIndex,
                                      col.weekKey,
                                    ...
                                    setWeekStripSelection(null)
                                    if (DEBUG_INTEGRATION_MERGE) {
                                      console.debug("[Integration merge] drag update", {
                                        rect: next,
                                      })
                                    }
                                  }}
                                ...
                                >
                                  ...
                                  >
                                    <Input
                                      ...
                                      setIsSelecting(true)
                                      if (DEBUG_INTEGRATION_MERGE) {
                                        console.debug("[Integration merge] drag start", {
                                          rowIndex,
                                          weekKey: col.weekKey,
                                        })
                                      }
                                      ...
                                      if (shift && lastWeekAnchorRef.current) {
                                        ...
                                        if (
                                          ...
                                          const rect = normalizeIntegrationWeekRect(
                                            lastWeekAnchorRef.current.rowIndex,
                                            lastWeekAnchorRef.current.weekKey,
                                            rowIndex,
                                            col.weekKey,
                    ...
                    >
                      <td
                        ...
                        style={{
                          ...
                          ...integrationExpertTotalsRowBgStyle,
                        }}
                        colSpan={integrationDescriptorKeys.length}
                      >
                      ...
                      <td
                        className={cn(
                          stickyTd(integrationDescriptorKeys.length),
                          "h-8 px-1 text-xs tabular-nums"
                        )}
                        style={{
                          ...stickyStyleBody(integrationDescriptorKeys.length),
                          ...integrationExpertTotalsRowBgStyle,
                        }}
                      >
                      ...
                      <td
                        className={cn(
                          stickyTd(integrationDescriptorKeys.length + 1),
                          "h-8"
                        )}
                        style={{
                          ...stickyStyleBody(integrationDescriptorKeys.length + 1),
                          ...integrationExpertTotalsRowBgStyle,
                        }}
                      />
                      <td
                        className={cn(
                          stickyTd(integrationDescriptorKeys.length + 2),
                          "h-8 px-1 text-xs tabular-nums text-muted-foreground",
                          INTEGRATION_EXPERT_WEEK_SCROLLER_EDGE
                        )}
                        style={{
                          ...stickyStyleBody(integrationDescriptorKeys.length + 2),
                          ...integrationExpertTotalsRowBgStyle,
                        }}
                      >
        ...
        {DEBUG_INTEGRATION_MERGE ? (
          <div className="pointer-events-none absolute bottom-2 right-2 z-50 max-w-[360px] rounded-md border bg-background/95 p-2 text-[11px] leading-snug shadow-lg">
            <div className="mb-1 font-medium text-foreground">Integration merge debug</div>
