 ...
 */
const DIGITALDISPLAY_EXPERT_ROW_VIRTUALIZATION = true
const DIGITALDISPLAY_EXPERT_ROW_HEIGHT_PX = OOH_EXPERT_ROW_HEIGHT_PX
const DIGITALDISPLAY_EXPERT_ROW_OVERSCAN = OOH_EXPERT_ROW_OVERSCAN
...
export function DigitalDisplayExpertGrid({
  ...
  const normalizeRowCacheRef = useRef(
    new Map<string, NormalizeRowCacheEntry<DigitalDisplayExpertScheduleRow>>()
  )
  ...
  const rowMergeMapCacheRef = useRef(
    new Map<
      string,
      MapBuildCacheEntry<
        DigitalDisplayExpertScheduleRow["mergedWeekSpans"],
        DigitalDisplayRowMergeMap
      >
    >()
  )
  ...
  const pushRows = useCallback(
    (next: DigitalDisplayExpertScheduleRow[]) => {
      const withDates = finalizeRowsPreservingIdentity(next, (r) => {
        ...
        const dates = deriveDigitalDisplayExpertRowScheduleYmdFromRow(
          cleared,
          weekColumns,
          campaignStartDate,
          campaignEndDate,
  ...
  const rowReorderVirtual = useMemo(
    () =>
      DIGITALDISPLAY_EXPERT_ROW_VIRTUALIZATION
        ? {
            rowCount: normalizedRows.length,
            estimateSizePx: DIGITALDISPLAY_EXPERT_ROW_HEIGHT_PX,
  ...
  const ensureRowVisible = useCallback((rowIndex: number) => {
    if (!DIGITALDISPLAY_EXPERT_ROW_VIRTUALIZATION) return
            ...
            ? Math.min(
                ...
                Math.max(
                  ...
                  deltaDaysFromPx(prev.edgeYmd, dx, {
                    ...
                    dayKeysByWeekKey,
                    weekColumnWidths,
                    defaultWeekColWidthPx: DIGITALDISPLAY_EXPERT_WEEK_COL_WIDTH_PX,
  ...
  const {
    ...
    getScrollElement: () => gridScrollRef.current,
    estimateSize: DIGITALDISPLAY_EXPERT_ROW_HEIGHT_PX,
    overscan: DIGITALDISPLAY_EXPERT_ROW_OVERSCAN,
  ...
  const virtualSpacerColSpan = useMemo(() => {
    ...
    return (
      1 + digitalDisplayDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekCells
    )
  }, [
    weekColumns,
    expandedWeekKeys,
        ...
        <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2 border-b border-border/60 bg-muted/5 pb-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <Label htmlFor="digitaldisplay-expert-row-count" className="sr-only">Add rows count</Label>
              <Input
                  ...
                  <tbody>
                    {(() => {
                      ...
                      return (
                        <tr
                          ...
                          data-digitaldisplay-expert-row-index={rowIndex}
                          style={{
                            ...stripeStyle,
                            height: DIGITALDISPLAY_EXPERT_ROW_HEIGHT_PX,
                            maxHeight: DIGITALDISPLAY_EXPERT_ROW_HEIGHT_PX,
                          }}
                      ...
                      if (!DIGITALDISPLAY_EXPERT_ROW_VIRTUALIZATION) {
                    ...
                    >
                      ...
                      <td
                        ...
                        style={stickyStyleBody(digitalDisplayDescriptorKeys.length)}
                      >
                      ...
                      <td
                        ...
                        style={stickyStyleBody(digitalDisplayDescriptorKeys.length + 1)}
                      />
                      <td
                        className={cn(
                        ...
                        style={stickyStyleBody(digitalDisplayDescriptorKeys.length + 2)}
                      >
