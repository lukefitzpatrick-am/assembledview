...
import {
  ...
  WEEK_COL_WIDTH_PX as SOCIALMEDIA_EXPERT_WEEK_COL_WIDTH_PX,
  expertWeekColLayoutStyle as socialMediaExpertWeekColLayoutStyle,
  ...
  normalizeWeekRect as normalizeSocialMediaWeekRect,
  weekCellInRect as socialMediaWeekCellInRect,
  ...
  normalizeWeekMergeSelection as normalizeSocialMediaWeekMergeSelection,
  weekPlainClickPreservesWeekAreaSelection,
  deriveMergeEligibility as deriveSocialMediaMergeEligibility,
  weekCellExportText,
...
type SocialMediaWeekRectSelection = ExpertWeekRectSelection
type SocialMediaMultiCellSelection = ExpertMultiCellSelection

const MEDIA_ACCENT_HEX = getMediaTypeThemeHex("socialmedia")

const socialMediaExpertHeaderCellBgStyle = {
  backgroundColor: rgbaFromHex(MEDIA_ACCENT_HEX, 0.08),
}

const socialMediaExpertTotalsRowBgStyle = {
  backgroundColor: rgbaFromHex(MEDIA_ACCENT_HEX, 0.06),
}

const DEBUG_SOCIALMEDIA_MERGE = false

function normalizeSocialMediaKey(input: unknown): string {
  return String(input ?? "")
    .trim()
    .toLowerCase()
    .replace(/\s+/g, " ")
...
function sumMergedQuantities(row: SocialMediaExpertScheduleRow): number {
...
type SocialMediaCopiedCells = {
  data: (string | number)[][]
  sourceRows: number
  sourceCols: number
  selection: ExpertMultiCellSelection
}

type SocialMediaWeekMergeSelectionNormalized = WeekMergeSelectionNormalized

/** Match labels/values on {@link SocialMediaContainer} buy-type combobox. */
const SOCIALMEDIA_BUY_TYPE_OPTIONS: ComboboxOption[] = [
...
const SOCIALMEDIA_BID_STRATEGY_OPTIONS: ComboboxOption[] = [
...
function normalizeSocialMediaBuyTypePaste(raw: string): string {
  const v = raw.trim()
  if (!v) return ""
  const byValue = SOCIALMEDIA_BUY_TYPE_OPTIONS.find(
    (o) => o.value.toLowerCase() === v.toLowerCase()
  )
  if (byValue) return byValue.value
  const byLabel = SOCIALMEDIA_BUY_TYPE_OPTIONS.find(
...
function normalizeSocialMediaBidStrategyPaste(raw: string): string {
  const v = raw.trim()
  if (!v) return ""
  const byValue = SOCIALMEDIA_BID_STRATEGY_OPTIONS.find((o) => o.value.toLowerCase() === v.toLowerCase())
  if (byValue) return byValue.value
  const byLabel = SOCIALMEDIA_BID_STRATEGY_OPTIONS.find((o) => o.label.toLowerCase() === v.toLowerCase())
  if (byLabel) return byLabel.value
  return v
}

function normalizeSocialMediaPlatformPaste(raw: string, platformNames: string[]): string {
  const v = raw.trim()
  if (!v) return ""
...
function rowGrossCost(row: SocialMediaExpertScheduleRow, weekKeys: string[]): number {
...
const SOCIALMEDIA_DESCRIPTOR_CORE: readonly (keyof SocialMediaExpertScheduleRow)[] = [
  "startDate",
  "endDate",
  "platform",
  "bidStrategy",
...
const SOCIALMEDIA_BILLING_FLAG_KEYS: readonly (keyof SocialMediaExpertScheduleRow)[] = [
  "fixedCostMedia",
  "clientPaysForMedia",
  "budgetIncludesFees",
]

const SOCIALMEDIA_DESCRIPTOR_TAIL: readonly (keyof SocialMediaExpertScheduleRow)[] = ["unitRate"]
...
type SocialMediaRowMergeSpanMeta = Readonly<{
  id: string
  startWeekKey: string
  endWeekKey: string
  totalQty: number
...
type SocialMediaRowMergeMap = Readonly<{
  anchorByWeekKey: Readonly<Record<string, string>>
  interiorByWeekKey: Readonly<Record<string, string>>
  spanById: Readonly<Record<string, SocialMediaExpertMergedWeekSpan>>
  spanMetaByAnchorWeekKey: Readonly<Record<string, SocialMediaRowMergeSpanMeta>>
}>
...
export function SocialMediaExpertGrid({
  ...
  const [focusedCell, setFocusedCell] = useState<SocialMediaExpertFocusedCell | null>(
    null
  )
  const focusedCellRef = useRef<SocialMediaExpertFocusedCell | null>(null)
  ...
  const [weekRectSelection, setWeekRectSelection] =
    useState<SocialMediaWeekRectSelection | null>(null)
  // Multi-cell selection state for copy/paste operations.
  const [multiCellSelection, setMultiCellSelection] =
    useState<SocialMediaMultiCellSelection | null>(null)
  const [isSelecting, setIsSelecting] = useState(false)
  const [copiedCells, setCopiedCells] = useState<SocialMediaCopiedCells | null>(null)
  ...
  const lastDragRectDuringGestureRef = useRef<SocialMediaWeekRectSelection | null>(null)
  /**
   ...
   */
  const postDragWeekClickRectRef = useRef<SocialMediaWeekRectSelection | null>(null)
  const weekRectSelectionRef = useRef<SocialMediaWeekRectSelection | null>(null)
  ...
  const socialMediaDescriptorKeys = useMemo(
    () =>
      [
        ...SOCIALMEDIA_DESCRIPTOR_CORE,
        ...SOCIALMEDIA_BILLING_FLAG_KEYS,
        ...SOCIALMEDIA_DESCRIPTOR_TAIL,
      ] as (keyof SocialMediaExpertScheduleRow)[],
    []
  )

  const descriptorColWidths = useMemo(
    () => [48, 48, 120, 110, 96, 120, 110, 110, 96, 40, 40, 40, 88],
    []
  )
  ...
  useEffect(() => {
    if (!DEBUG_SOCIALMEDIA_MERGE) return
    ...
    console.debug("[Social Media merge] pending selection updated", pendingMergeSelection)
  ...
  const tryFuzzyMatch = useCallback(
    (rowIndex: number, field: FuzzyMatchField, value: string) => {
      ...
      if (corrected) {
        updateRow(rowIndex, { [field]: corrected } as Partial<SocialMediaExpertScheduleRow>)
        return
      }
      ...
      if (fuzzyMatchAutoApplyRef.current) {
        updateRow(rowIndex, { [field]: match.matched } as Partial<SocialMediaExpertScheduleRow>)
      } else {
  ...
  const navColCount =
    socialMediaDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekKeys.length
  const firstWeekNavColIndex = socialMediaDescriptorKeys.length + WEEK_GRID_COL_OFFSET
  const unitRateNavColIndex = socialMediaDescriptorKeys.indexOf("unitRate")
  ...
  const duplicateRow = useCallback(
    (rowIndex: number) => {
      ...
      const id =
        typeof crypto !== "undefined" && crypto.randomUUID
          ? crypto.randomUUID()
          : `socialmedia-expert-${Date.now()}-${rowIndex}`
  ...
  const clearWeekSelectionWhereMergedAnchorInvolved = useCallback(
    (rowIndex: number, weekKey: string) => {
      setWeekRectSelection((prev) =>
        prev && socialMediaWeekCellInRect(rowIndex, weekKey, prev, weekKeys)
          ? null
          : prev
      )
  ...
  const rangeWeekMultiSelect = useCallback(
    (rowIndex: number, anchorKey: string, endKey: string) => {
      ...
      setWeekRectSelection(
        normalizeSocialMediaWeekRect(rowIndex, anchorKey, rowIndex, endKey, weekKeys)
      )
      setWeekStripSelection(null)
    },
    [weekKeys]
  ...
  const lockPendingMergeSelectionFromCurrentSelection = useCallback(() => {
    const n = deriveSocialMediaMergeEligibility(
  ...
  const derivedMergeTarget = useMemo(() => {
    const n = deriveSocialMediaMergeEligibility(weekRectSelection, weekMultiSelect, weekKeys)
  ...
  useEffect(() => {
    const endDrag = () => {
      ...
      if (DEBUG_SOCIALMEDIA_MERGE) {
        console.debug("[Social Media merge] drag end", {
          lastRect: r,
  ...
  const handleMergeSelectedWeeks = useCallback(() => {
    ...
    if (selectionOverlapsMergedSpan(rowMergeMaps[rowIndex], sorted, weekKeys)) {
      if (DEBUG_SOCIALMEDIA_MERGE) {
        console.debug("[Social Media merge] validation overlap failure", {
          rowIndex,
    ...
    const newId =
      typeof crypto !== "undefined" && crypto.randomUUID
        ? crypto.randomUUID()
        : `merge-${Date.now()}`
    const newSpan: SocialMediaExpertMergedWeekSpan = {
      id: newId,
    ...
    if (DEBUG_SOCIALMEDIA_MERGE) {
      console.debug("[Social Media merge] merge applied", {
        rowIndex,
        spanId: newSpan.id,
  ...
  useEffect(() => {
    ...
    const cellId = expertGridCellId(
      ...
      socialMediaDescriptorKeys.length + WEEK_GRID_COL_OFFSET + lastWi
    )
    document
      .getElementById(cellId)
      ?.scrollIntoView({ behavior: "smooth", block: "nearest", inline: "nearest" })
  }, [domGridId, mergeTarget, mergeWeeksReady, socialMediaDescriptorKeys.length, weekKeys])

  useEffect(() => {
    if (!DEBUG_SOCIALMEDIA_MERGE) return
    console.debug("[Social Media merge] merge eligibility derived", {
      mergeTarget,
      derivedMergeTarget,
      mergeWeeksReady,
      weekRectSelection,
  ...
  const pasteMatrixIntoGrid = useCallback(
    (matrix: string[][]) => {
      ...
      if (fcIsWeek) {
        ...
        const nextRows: SocialMediaExpertScheduleRow[] = normalizedRows.map((r) => ({
          ...r,
      ...
      for (let dr = 0; dr < working.length; dr++) {
        ...
        for (let dc = 0; dc < pasteRow.length; dc++) {
          ...
          const field = target.field as keyof SocialMediaExpertScheduleRow
          ...
          if (field === "unitRate") {
            ...
            nextRows[targetRow] = {
              ...cur,
              [field]: truthy,
            } as SocialMediaExpertScheduleRow
            applied += 1
          } else if (field === "buyType") {
            nextRows[targetRow] = {
              ...cur,
              buyType: normalizeSocialMediaBuyTypePaste(raw),
            }
            applied += 1
          } else if (field === "bidStrategy") {
            nextRows[targetRow] = {
              ...cur,
              bidStrategy: normalizeSocialMediaBidStrategyPaste(raw),
            }
            applied += 1
          } else if (field === "platform") {
            const plt = normalizeSocialMediaPlatformPaste(raw, platformNames)
            nextRows[targetRow] = {
              ...cur,
              platform: plt,
            }
            ...
            const v = raw.trim()
            nextRows[targetRow] = { ...cur, [field]: v } as SocialMediaExpertScheduleRow
            applied += 1
          }
        }
      }
  ...
  const containerTotals = useMemo(() => {
    ...
    const fee =
      feesocial > 0 && feesocial < 100 ? (sumGross * feesocial) / (100 - feesocial) : 0
    const totalWithFee = sumGross + fee

    return { sumGross, sumQty, perWeek, fee, totalWithFee }
  }, [feesocial, normalizedRows, weekKeys])
  ...
  const colIndexOf = useCallback(
    (key: keyof SocialMediaExpertScheduleRow) => socialMediaDescriptorKeys.indexOf(key),
    [socialMediaDescriptorKeys]
  )
      ...
      <Card className="relative overflow-hidden border-0 shadow-md">
        ...
        <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2 border-b border-border/60 bg-muted/5 pb-3">
          <CardTitle className="text-base font-semibold tracking-tight">
            Social Media — Expert Schedule
          </CardTitle>
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2">
              <Label htmlFor="socialmedia-expert-row-count" className="text-sm whitespace-nowrap">
                Rows:
              </Label>
              <Input
                id="socialmedia-expert-row-count"
                type="number"
                min={1}
                max={500}
                    ...
                    <tr>
                        ...
                        <th
                          ...
                          className={stickyThCorner(
                            i === descriptorHeadLabels.length - 1
                              ? SOCIALMEDIA_EXPERT_WEEK_SCROLLER_EDGE
                              : undefined
                          )}
                          style={{
                            ...stickyStyleHeaderCorner(i),
                            ...socialMediaExpertHeaderCellBgStyle,
                          }}
                        >
                          {label === "Unit Rate" ? (
                            <Tooltip>
                      ...
                      {weekColumns.map((col) => (
                        <th
                          ...
                          style={{
                            ...socialMediaExpertWeekColLayoutStyle,
                            ...socialMediaExpertHeaderCellBgStyle,
                          }}
                          title={col.labelFull}
                        >
                  ...
                  <tbody>
                    {normalizedRows.map((row, rowIndex) => {
                      ...
                      const stripeStyle =
                        rowIndex % 2 === 0
                          ...
                          : undefined
                      const grossCol = socialMediaDescriptorKeys.length
                      const actionsCol = socialMediaDescriptorKeys.length + 1
                      const sigmaCol = socialMediaDescriptorKeys.length + 2
                            ...
                            for (let wi = 0; wi < weekColumns.length; wi += 1) {
                              ...
                              const tdClassName = cn(
                                ...
                                isPopulatedNonMergedCell &&
                                  SOCIALMEDIA_WEEK_CELL_VISUAL_CLASSES.populatedSingleTd,
                                ...
                                !isMergedAnchorCell &&
                                  mergeReadyOnCell &&
                                  "socialmedia-expert-week-cell--merge-ready z-[6] bg-amber-500/24 ring-2 ring-inset ring-amber-600/75 shadow-sm dark:bg-amber-500/20 dark:ring-amber-400/70",
                              ...
                              const mergedAnchorWrapperClassName = cn(
                                ...
                                isMergedAnchorCell &&
                                  SOCIALMEDIA_WEEK_CELL_VISUAL_CLASSES.mergedSurface,
                              ...
                              const inputClassName = cn(
                                ...
                                isMergedAnchorCell
                                  ? SOCIALMEDIA_WEEK_CELL_VISUAL_CLASSES.mergedInput
                              ...
                              renderedWeekCells.push(
                                <td
                                  ...
                                  style={
                                    isMergedAnchorCell
                                      ? {
                                          width: SOCIALMEDIA_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                          minWidth:
                                            SOCIALMEDIA_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                          maxWidth:
                                            SOCIALMEDIA_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                  ...
                                  onMouseEnter={(e) => {
                                    ...
                                    const next = normalizeSocialMediaWeekRect(
                                      drag.rowIndex,
                                      drag.weekKey,
                                      rowIndex,
                                      col.weekKey,
                                    ...
                                    setWeekStripSelection(null)
                                    if (DEBUG_SOCIALMEDIA_MERGE) {
                                      console.debug("[Social Media merge] drag update", {
                                        rect: next,
                                      })
                                    }
                                  }}
                                ...
                                >
                                  ...
                                  >
                                    <Input
                                      ...
                                      setIsSelecting(true)
                                      if (DEBUG_SOCIALMEDIA_MERGE) {
                                        console.debug("[Social Media merge] drag start", {
                                          rowIndex,
                                          weekKey: col.weekKey,
                                        })
                                      }
                                      ...
                                      if (shift && lastWeekAnchorRef.current) {
                                        ...
                                        if (
                                          ...
                                          const rect = normalizeSocialMediaWeekRect(
                                            lastWeekAnchorRef.current.rowIndex,
                                            lastWeekAnchorRef.current.weekKey,
                                            rowIndex,
                                            col.weekKey,
                    ...
                    >
                      <td
                        ...
                        colSpan={socialMediaDescriptorKeys.length}
                      >
                      ...
                      <td
                        className={cn(
                          stickyTd(socialMediaDescriptorKeys.length),
                          "h-8 px-1 text-xs tabular-nums"
                        )}
                        style={{
                          ...stickyStyleBody(socialMediaDescriptorKeys.length),
                          ...socialMediaExpertTotalsRowBgStyle,
                        }}
                      >
                      ...
                      <td
                        className={cn(
                          stickyTd(socialMediaDescriptorKeys.length + 1),
                          "h-8"
                        )}
                        style={{
                          ...stickyStyleBody(socialMediaDescriptorKeys.length + 1),
                          ...socialMediaExpertTotalsRowBgStyle,
                        }}
                      />
                      <td
                        className={cn(
                          stickyTd(socialMediaDescriptorKeys.length + 2),
                          "h-8 px-1 text-xs tabular-nums text-muted-foreground",
                          SOCIALMEDIA_EXPERT_WEEK_SCROLLER_EDGE
                        )}
                        style={{
                          ...stickyStyleBody(socialMediaDescriptorKeys.length + 2),
                          ...socialMediaExpertTotalsRowBgStyle,
                        }}
                      >
