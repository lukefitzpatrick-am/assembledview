/**
 * F-28 Phase 2 — Programmatic Video Expert row virtualization smoke (Prompt A items 1,3,5,7,8).
 */
import assert from "node:assert/strict"
import test from "node:test"
import { format, startOfDay } from "date-fns"

import { mapProgVideoExpertRowsToStandardLineItems } from "@/lib/mediaplan/expertChannelMappings"
import {
  reorderExpertRows,
  mergedSpanWidthPx,
...
const PROGVIDEO_EXPERT_ROW_OVERSCAN = EXPERT_GRID_ROW_OVERSCAN_DEFAULT
...
function emptyRow(id: string): ProgVideoExpertScheduleRow {
...
function buildRows(n: number): ProgVideoExpertScheduleRow[] {
  return Array.from({ length: n }, (_, i) => emptyRow(`progvideo-v-${i}`))
}

/** Mirrors ProgVideoExpertGrid containerTotals over the FULL logical array. */
function computeProgVideoWeeklyTotals(
...
test("3. DRAG REORDER across virtual boundary lands on correct logical index", () => {
  ...
  const dropIndex = virtualRowIndexFromOffsetY(
    280 * PROGVIDEO_EXPERT_ROW_HEIGHT_PX + 1,
    PROGVIDEO_EXPERT_ROW_HEIGHT_PX,
    300
  )
...
test("5. WEEK-COLUMN WIDTH respects resize overrides (sticky header/week resize path)", () => {
  ...
  assert.equal(PROGVIDEO_EXPERT_ROW_HEIGHT_PX, 41)
})
...
test("7. WEEKLY TOTALS derived from full 300-row state", () => {
  ...
  const totals = computeProgVideoWeeklyTotals(rows, 0)
  ...
  const after = computeProgVideoWeeklyTotals(edited, 0)
...
test("8. PERF: mounted DOM window stays bounded while scrolling 300 rows", () => {
  ...
  const nearTop = expectedMountedRowRange(
    ...
    PROGVIDEO_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    PROGVIDEO_EXPERT_ROW_OVERSCAN
  )
  const mid = expectedMountedRowRange(
    150 * PROGVIDEO_EXPERT_ROW_HEIGHT_PX,
    viewportHeight,
    PROGVIDEO_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    PROGVIDEO_EXPERT_ROW_OVERSCAN
  ...
  const nearBottom = expectedMountedRowRange(
    280 * PROGVIDEO_EXPERT_ROW_HEIGHT_PX,
    viewportHeight,
    PROGVIDEO_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    PROGVIDEO_EXPERT_ROW_OVERSCAN
  ...
  const maxWindow =
    Math.ceil(viewportHeight / PROGVIDEO_EXPERT_ROW_HEIGHT_PX) +
    2 * PROGVIDEO_EXPERT_ROW_OVERSCAN +
    2
