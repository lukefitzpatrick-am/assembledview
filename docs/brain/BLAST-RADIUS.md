...
| `lib/mediaplans/ingest/*` / `app/admin/schedule-ingest` / `POST /api/admin/ingest/{review,remap,accept}` / `0023_line_item_panels` / `0027_line_item_panel_flights` / `0024_publisher_profiles` | Human gate for publisher schedules. Accept → `savePlanVersion` then panel+flight insert in one txn (never bypass). Remap mutates `publisher_profiles.column_map`. Proposal never mints `line_item_id`. Panels collapsed by default in review UI (flight summary per panel). AVA mapping proposals only when publisher confidence < 90% + unmapped columns (`avaColumnMapping`) — never auto-accept; never parses values/bursts/grid_semantics. **OOH panel-line count > `OOH_PANEL_LINE_EXPERT_THRESHOLD` (30)** → Accept sets `preferOohExpertView`; `OOHContainer` suppresses card list (summary + open expert). Flights: no money; dark letters = no row. |
