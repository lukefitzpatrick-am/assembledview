...
- `0080_campaign_kpi_target_source.sql` — `campaign_kpi.target_source` text NULL default `target` + `benchmark_ref` text NULL. Existing rows stamped `target`. Industry benchmark backfill is a later data step. Marker `0080_campaign_kpi_target_source`. AUTHOR ONLY. Do not apply until Luke runs it. Do not drizzle-kit. Mirror: `db/schema/ported.ts`. Do not SELECT these columns against live Postgres before applying (C-76).
