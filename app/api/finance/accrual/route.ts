  ...
  try {
    ...
    try {
      for (const v of chosen) {
        const partial = collectClientPaysForMediaFlagsFromSchedule(v.deliverySchedule ?? v.delivery_schedule)
        for (const [k, val] of Object.entries(partial)) {
          if (val) clientPaysForMediaByLineItemId[k] = true
          else if (!(k in clientPaysForMediaByLineItemId)) clientPaysForMediaByLineItemId[k] = false
        }
      }
    } catch {
      // ignore (non-fatal)
