...
import {
  ...
  WEEK_COL_WIDTH_PX as INFLUENCERS_EXPERT_WEEK_COL_WIDTH_PX,
  WEEK_SCROLLER_EDGE as INFLUENCERS_EXPERT_WEEK_SCROLLER_EDGE,
  WEEK_CELL_VISUAL_CLASSES as INFLUENCERS_WEEK_CELL_VISUAL_CLASSES,
  ...
  normalizeWeekRect as normalizeInfluencersWeekRect,
  weekCellInRect as influencersWeekCellInRect,
  ...
  normalizeWeekMergeSelection as normalizeInfluencersWeekMergeSelection,
  weekPlainClickPreservesWeekAreaSelection,
  deriveMergeEligibility as deriveInfluencersMergeEligibility,
  weekCellExportText,
...
type InfluencersWeekRectSelection = ExpertWeekRectSelection
type InfluencersMultiCellSelection = ExpertMultiCellSelection
type InfluencersWeekMergeSelectionNormalized = WeekMergeSelectionNormalized
 ...
 */
const INFLUENCERS_EXPERT_ROW_VIRTUALIZATION = true
...
export function InfluencersExpertGrid({
  ...
  const normalizeRowCacheRef = useRef(
    new Map<string, NormalizeRowCacheEntry<InfluencersExpertScheduleRow>>()
  )
  ...
  const rowMergeMapCacheRef = useRef(
    new Map<
      string,
      MapBuildCacheEntry<
        InfluencersExpertScheduleRow["mergedWeekSpans"],
        InfluencersRowMergeMap
      >
    >()
  )
  ...
  const pushRows = useCallback(
    (next: InfluencersExpertScheduleRow[]) => {
      const withDates = finalizeRowsPreservingIdentity(next, (r) => {
        ...
        const dates = deriveInfluencersExpertRowScheduleYmdFromRow(
          cleared,
          weekColumns,
          campaignStartDate,
          campaignEndDate,
  ...
  const rowReorderVirtual = useMemo(
    () =>
      INFLUENCERS_EXPERT_ROW_VIRTUALIZATION
        ? {
            rowCount: normalizedRows.length,
            estimateSizePx: INFLUENCERS_EXPERT_ROW_HEIGHT_PX,
  ...
  const ensureRowVisible = useCallback((rowIndex: number) => {
    if (!INFLUENCERS_EXPERT_ROW_VIRTUALIZATION) return
  ...
  const widthForWeekKey = useCallback(
    (k: string): number => {
      ...
      return weekColumnWidths[k] ?? INFLUENCERS_EXPERT_WEEK_COL_WIDTH_PX
    },
            ...
            ? Math.min(
                ...
                Math.max(
                  ...
                  deltaDaysFromPx(prev.edgeYmd, dx, {
                    ...
                    dayKeysByWeekKey,
                    weekColumnWidths,
                    defaultWeekColWidthPx: INFLUENCERS_EXPERT_WEEK_COL_WIDTH_PX,
  ...
  const {
    ...
    getScrollElement: () => gridScrollRef.current,
    estimateSize: INFLUENCERS_EXPERT_ROW_HEIGHT_PX,
    overscan: INFLUENCERS_EXPERT_ROW_OVERSCAN,
  ...
  const virtualSpacerColSpan = useMemo(() => {
    ...
    return (
      1 + influencersDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekCells
    )
  }, [
    weekColumns,
    expandedWeekKeys,
  ...
  const containerTotals = useMemo(() => {
    ...
    for (const row of normalizedRows) {
      const { net, fee } = expertRowCostSplit(row, weekKeys, feeinfluencers)
      sumNet += net
      sumFee += fee
      for (const k of weekKeys) {
  ...
  }, [dayKeysByWeekKey, feeinfluencers, normalizedRows, weekKeys])
        ...
        <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2 border-b border-border/60 bg-muted/5 pb-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <Label htmlFor="influencers-expert-row-count" className="sr-only">Add rows count</Label>
              <Input
                  ...
                  <thead ref={theadRef} className="[&_tr]:border-b-0">
                    <tr>
                      <ExpertGridRowReorderHeaderCell
                        ...
                        style={influencersExpertHeaderCellBgStyle}
                      />
                        ...
                        return (
                          ...
                          >
                            ...
                            <ExpertGridWeekResizeHandle
                              weekKey={col.weekKey}
                              currentWidth={weekColumnWidths[col.weekKey] ?? INFLUENCERS_EXPERT_WEEK_COL_WIDTH_PX}
                    ...
                    {(() => {
                      ...
                      return (
                        <MemoExpertGridRow
                          ...
                          rowUiSig={rowUiSig}
                          render={() => {
                      const net = expertRowNetMedia(row, weekKeys, feeinfluencers)
                      ...
                      return (
                        <tr
                          ...
                          data-influencers-expert-row-index={rowIndex}
                          style={{
                            ...stripeStyle,
                            height: INFLUENCERS_EXPERT_ROW_HEIGHT_PX,
                            maxHeight: INFLUENCERS_EXPERT_ROW_HEIGHT_PX,
                          }}
                          ...
                          {(() => {
                            ...
                            for (let wi = 0; wi < weekColumns.length; wi += 1) {
                              ...
                              const tdClassName = cn(
                                ...
                                !isMergedAnchorCell &&
                                  mergeReadyOnCell &&
                                  "influencers-expert-week-cell--merge-ready z-[6] bg-pacing-behind-bg ring-2 ring-inset ring-pacing-behind shadow-e0",
                      ...
                      if (!INFLUENCERS_EXPERT_ROW_VIRTUALIZATION) {
