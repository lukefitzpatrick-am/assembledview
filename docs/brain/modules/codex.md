...
| Table | Status | Notes |
|---|---|---|
| `tasks` | **Live** | Create / list / patch / soft-delete; `auto_created` + `ava_auto_key` (0040); `estimated_minutes` (0048 AUTHOR ONLY — not in Drizzle until applied; repo probes `information_schema`) |
...
- 2026-08-14 — Task detail is a right slide-over over the list (`/tasks/[id]` still pasteable; list + panel). Left-edge resize persists in localStorage. MBA is a client-scoped searchable combobox labelled `<mba> — <campaign>` (desc). Planner `estimated_minutes` (0048 AUTHOR ONLY — do not apply from the app; do not smoke estimates until the column exists). Inbox GET paginates by meeting; dismiss-all is one UPDATE of the note’s full proposed set. Filter toolbar state is URL params; board is full-width.
