...
- Two catch-all proxies (`/api/media_plans/[...path]`, `/api/media-details/[...path]`) gated by `lib/security/proxyAllowlist.ts` (path depth ≤2, numeric second segment, per-path method sets) **and** `requireRole(["admin","manager"])` on every method (SEC-1 Constrained — dies with Xano reads at T6).
