...
**Billing-record writes** go through `lib/data/writeFinance.ts` (Postgres, `invoice_key`, never `xero:`). Xero ingest is the only writer of `xero:` keys. `finance_edits` POST still Xano (T1). Internal finance money is **ex-GST**; Xero Total stays on `xero_ar_*` / `xero_ap_*` only.
