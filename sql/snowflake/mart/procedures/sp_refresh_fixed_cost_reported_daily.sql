...
-- June GET_DDL retained. Live procedure is ACCOUNTADMIN-owned; AV_APP_WRITE_ROLE cannot
-- GET_DDL it. Paste after `show procedures like 'SP_REFRESH_FIXED_COST_REPORTED_DAILY'
-- in schema ASSEMBLEDVIEW.MART` then
-- `select get_ddl('procedure','ASSEMBLEDVIEW.MART.SP_REFRESH_FIXED_COST_REPORTED_DAILY(<signature>)');`
-- CPV still reads VIDEO_3S_VIEWS; VW_PACING_PARTNER_FILE dual-writes COMPLETED_VIEWS there.
