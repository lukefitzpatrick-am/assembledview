...
function line(bursts: any[]): StandardProgBvodFormLineItem {
  return {
...
test("sub-week interior burst imports as day-detail and round-trips", () => {
  ...
  const rows = mapStandardProgBvodLineItemsToExpertRows(std, cols, CS, CE)
  const wk0 = cols[0]!
  assert.equal(weekHasDailyValues(rows[0]!.dailyValues ?? {}, weekDayKeys(wk0, CS, CE)), true)
  const back = mapProgBvodExpertRowsToStandardLineItems(rows, cols, CS, CE, { feePctProgBvod: 0 })
