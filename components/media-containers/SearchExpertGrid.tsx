...
import {
  ...
  WEEK_COL_WIDTH_PX as SEARCH_EXPERT_WEEK_COL_WIDTH_PX,
  WEEK_SCROLLER_EDGE as SEARCH_EXPERT_WEEK_SCROLLER_EDGE,
  WEEK_CELL_VISUAL_CLASSES as SEARCH_WEEK_CELL_VISUAL_CLASSES,
  ...
  normalizeWeekRect as normalizeSearchWeekRect,
  weekCellInRect as searchWeekCellInRect,
...
type SearchWeekRectSelection = ExpertWeekRectSelection
type SearchMultiCellSelection = ExpertMultiCellSelection
type SearchWeekMergeSelectionNormalized = WeekMergeSelectionNormalized
...
export function SearchExpertGrid({
  ...
  const pushRows = useCallback(
    (next: SearchExpertScheduleRow[]) => {
      const withDates = finalizeRowsPreservingIdentity(next, (r) => {
        ...
        const dates = deriveSearchExpertRowScheduleYmdFromRow(
          cleared,
          weekColumns,
          campaignStartDate,
          campaignEndDate,
  ...
  const ensureRowVisible = useCallback((rowIndex: number) => {
    if (!SEARCH_EXPERT_ROW_VIRTUALIZATION) return
  ...
  const widthForWeekKey = useCallback(
    (k: string): number => {
      ...
      return weekColumnWidths[k] ?? SEARCH_EXPERT_WEEK_COL_WIDTH_PX
    },
            ...
            ? Math.min(
                ...
                Math.max(
                  ...
                  deltaDaysFromPx(prev.edgeYmd, dx, {
                    ...
                    dayKeysByWeekKey,
                    weekColumnWidths,
                    defaultWeekColWidthPx: SEARCH_EXPERT_WEEK_COL_WIDTH_PX,
  ...
  const virtualSpacerColSpan = useMemo(() => {
    ...
    return (
      1 + searchDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekCells
    )
  }, [
    weekColumns,
    expandedWeekKeys,
  ...
  const containerTotals = useMemo(() => {
    ...
    for (const row of normalizedRows) {
      const { net, fee } = expertRowCostSplit(row, weekKeys, feesearch)
      sumNet += net
      sumFee += fee
      for (const k of weekKeys) {
  ...
  }, [dayKeysByWeekKey, feesearch, normalizedRows, weekKeys])
        ...
        <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2 border-b border-border/60 bg-muted/5 pb-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <Label htmlFor="search-expert-row-count" className="sr-only">Add rows count</Label>
              <Input
                  ...
                  <thead ref={theadRef} className="[&_tr]:border-b-0">
                    <tr>
                      <ExpertGridRowReorderHeaderCell
                        ...
                        style={searchExpertHeaderCellBgStyle}
                      />
                        ...
                        return (
                          ...
                          >
                            ...
                            <ExpertGridWeekResizeHandle
                              weekKey={col.weekKey}
                              currentWidth={weekColumnWidths[col.weekKey] ?? SEARCH_EXPERT_WEEK_COL_WIDTH_PX}
                    ...
                    {(() => {
                      ...
                      return (
                        <MemoExpertGridRow
                          ...
                          rowUiSig={rowUiSig}
                          render={() => {
                      const net = expertRowNetMedia(row, weekKeys, feesearch)
                      ...
                      if (!SEARCH_EXPERT_ROW_VIRTUALIZATION) {
