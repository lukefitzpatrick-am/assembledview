/**
 * F-28 Phase 2 — Programmatic Audio Expert row virtualization smoke (Prompt A items 1,3,5,7,8).
 */
import assert from "node:assert/strict"
import test from "node:test"
import { format, startOfDay } from "date-fns"

import { mapProgAudioExpertRowsToStandardLineItems } from "@/lib/mediaplan/expertChannelMappings"
import {
  reorderExpertRows,
  mergedSpanWidthPx,
...
const PROGAUDIO_EXPERT_ROW_OVERSCAN = EXPERT_GRID_ROW_OVERSCAN_DEFAULT
...
function emptyRow(id: string): ProgAudioExpertScheduleRow {
...
function buildRows(n: number): ProgAudioExpertScheduleRow[] {
  return Array.from({ length: n }, (_, i) => emptyRow(`progaudio-v-${i}`))
}

/** Mirrors ProgAudioExpertGrid containerTotals over the FULL logical array. */
function computeProgAudioWeeklyTotals(
...
test("3. DRAG REORDER across virtual boundary lands on correct logical index", () => {
  ...
  const dropIndex = virtualRowIndexFromOffsetY(
    280 * PROGAUDIO_EXPERT_ROW_HEIGHT_PX + 1,
    PROGAUDIO_EXPERT_ROW_HEIGHT_PX,
    300
  )
...
test("5. WEEK-COLUMN WIDTH respects resize overrides (sticky header/week resize path)", () => {
  ...
  assert.equal(PROGAUDIO_EXPERT_ROW_HEIGHT_PX, 41)
})
...
test("7. WEEKLY TOTALS derived from full 300-row state", () => {
  ...
  const totals = computeProgAudioWeeklyTotals(rows, 0)
  ...
  const after = computeProgAudioWeeklyTotals(edited, 0)
...
test("8. PERF: mounted DOM window stays bounded while scrolling 300 rows", () => {
  ...
  const nearTop = expectedMountedRowRange(
    ...
    PROGAUDIO_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    PROGAUDIO_EXPERT_ROW_OVERSCAN
  )
  const mid = expectedMountedRowRange(
    150 * PROGAUDIO_EXPERT_ROW_HEIGHT_PX,
    viewportHeight,
    PROGAUDIO_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    PROGAUDIO_EXPERT_ROW_OVERSCAN
  ...
  const nearBottom = expectedMountedRowRange(
    280 * PROGAUDIO_EXPERT_ROW_HEIGHT_PX,
    viewportHeight,
    PROGAUDIO_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    PROGAUDIO_EXPERT_ROW_OVERSCAN
  ...
  const maxWindow =
    Math.ceil(viewportHeight / PROGAUDIO_EXPERT_ROW_HEIGHT_PX) +
    2 * PROGAUDIO_EXPERT_ROW_OVERSCAN +
    2
