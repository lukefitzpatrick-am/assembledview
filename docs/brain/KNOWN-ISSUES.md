...
| C-31 | **MBA edit "Maximum update depth exceeded"** — `@radix-ui/react-switch` Root composes inline `(node) => setButton(node)` into `useComposedRefs`; React 19 ref detach/attach during channel-enable Switch remount churn calls setState in commitMutationEffects (HAR: 0 failed requests, edit document fetched 3×). Latent upstream (radix-ui/primitives#3963); not introduced by tonight's UI commits. Reproduced path-independent (`DATA_BACKEND_PLAN_DETAIL` unset and postgres) | FIXED — `components/ui/switch.tsx` no longer mounts Radix Switch Root; stable ref callback, no node-in-useState (form bubble path unused — RHF `onCheckedChange`); commit `fix(ui): stop Switch remount loop… (C-31)` |
...
| P-3 | N+1 to Xano per MBA with up to 5 param-shape attempts per channel; cold MBA GET = 20 channels × up to 5 attempts (`maxDuration=60` is load-bearing). Mitigated when `DATA_BACKEND_PLAN_DETAIL=postgres` (one `line_items` query set; C-22 / `6f1f0323`) | Open — Xano default; postgres path ready |
