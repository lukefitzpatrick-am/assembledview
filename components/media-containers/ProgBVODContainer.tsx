...
import { getPublishersForProgBvod, getClientInfo } from "@/lib/api"
...
} from "@/components/media-containers/ProgBVODExpertGrid"
...
export function calculateInvestmentPerMonth(form, feeprogbvod) {
...
export default function ProgBVODContainer({
  ...
  const progbvodStandardBaselineRef = useRef("")
  const mediaLineItemsPublishFpRef = useRef("")
  const [expertProgBvodRows, setExpertProgBvodRows] = useState<
    ProgBvodExpertScheduleRow[]
  >([])
  const [progbvodExpertModalOpen, setProgBvodExpertModalOpen] =
    useState(false)
  ...
  const [progbvodExpertExitConfirmOpen, setProgBvodExpertExitConfirmOpen] =
    useState(false)
  ...
  const progbvodExpertWeekColumns = useMemo(
  ...
  const handleExpertProgBvodRowsChange = useCallback(
    (next: ProgBvodExpertScheduleRow[]) => {
      setExpertProgBvodRows(next)
    },
    []
  )

  const openProgBvodExpertModal = useCallback(() => {
    const mapped = mapStandardProgBvodLineItemsToExpertRows(
      (form.getValues("lineItems") || []) as StandardProgBvodFormLineItem[],
      progbvodExpertWeekColumns,
      campaignStartDate,
      campaignEndDate
    )
    const weekKeys = progbvodExpertWeekColumns.map((c) => c.weekKey)
    const rows: ProgBvodExpertScheduleRow[] =
      mapped.length > 0
        ? mapped
        : [
            createEmptyProgBvodExpertRow(
    ...
    progbvodExpertRowsBaselineRef.current =
      serializeProgBvodExpertRowsBaseline(rows)
    setExpertProgBvodRows(rows)
  ...
  const confirmProgBvodExpertExitWithoutSaving = useCallback(() => {
    setProgBvodExpertExitConfirmOpen(false)
    collapseAllLineItems()
    setProgBvodExpertModalOpen(false)
  }, [collapseAllLineItems])

  const handleProgBvodExpertModalOpenChange = useCallback(
    (open: boolean) => {
      if (open) {
        setProgBvodExpertModalOpen(true)
        return
      ...
      const dirty =
        serializeProgBvodExpertRowsBaseline(expertProgBvodRows) !==
        progbvodExpertRowsBaselineRef.current
      if (!dirty) {
        collapseAllLineItems()
        setProgBvodExpertModalOpen(false)
      ...
      setProgBvodExpertExitConfirmOpen(true)
    },
    [collapseAllLineItems, expertProgBvodRows]
  )

  const handleProgBvodExpertApply = useCallback(() => {
    const prevLineItems = form.getValues("lineItems") || []
    const standard = mapProgBvodExpertRowsToStandardLineItems(
      expertProgBvodRows,
      progbvodExpertWeekColumns,
      campaignStartDate,
      campaignEndDate,
      {
        feePctProgBvod: feeprogbvod,
        budgetIncludesFees: Boolean(prevLineItems[0]?.budgetIncludesFees),
      }
    )
    const merged = mergeProgBvodStandardFromExpertWithPrevious(
      standard,
  ...
  useEffect(() => {
    ...
    const stableItems = assignStableLineItemNumbers<any>(formLineItems, mbaNumber, MEDIA_TYPE_ID_CODES.progBVOD);
  ...
  }, [watchedLineItems, mbaNumber, feeprogbvod, form, onMediaLineItemsChange]);
    ...
    const lineItemTotals = watchedLineItems.map((lineItem, index) => {
      ...
      lineItem.bursts.forEach((burst) => {
        ...
        if (lineItem.budgetIncludesFees) {
          ...
          burstMedia = budget;
          burstFee = feeprogbvod ? (budget / (100 - feeprogbvod)) * feeprogbvod : 0;
        }
  ...
  }, [form, feeprogbvod, onTotalMediaChange]);
  ...
  const handleValueChange = useCallback((lineItemIndex: number, burstIndex: number, budgetIncludesFeesOverride?: boolean) => {
    ...
    const bt = coerceBuyTypeWithDevWarn(String(buyTypeRaw || ""), "ProgBVODContainer.handleValueChange");
  ...
  useEffect(() => {
    const fetchPublishers = async () => {
      try {
        ...
        const fetchedPublishers = await getPublishersForProgBvod();
...
useEffect(() => {
  ...
  const items: LineItem[] = form.getValues('lineItems').flatMap((lineItem, lineItemIndex) =>
    lineItem.bursts.map(burst => {
      ...
      const lineItemId = buildLineItemId(mbaNumber, MEDIA_TYPE_ID_CODES.progBVOD, lineItemIndex + 1);
...
}, [watchedLineItems, feeprogbvod, form, mbaNumber, onLineItemsChange]);
  ...
  return (
    <div className="space-y-6">
        ...
        <Card className="overflow-hidden border-0 shadow-md">
            ...
            <div className="flex flex-col gap-3">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex min-w-0 flex-wrap items-center gap-2">
                  ...
                  {progbvodExpertModalOpen ? (
                    <Badge
                      variant="outline"
                      className="border-2 text-[10px] font-semibold uppercase tracking-wider shadow-sm"
                      style={{
                        borderColor: rgbaFromHex(PROG_BVOD_MEDIA_HEX, 0.55),
                        backgroundColor: rgbaFromHex(PROG_BVOD_MEDIA_HEX, 0.14),
                ...
                <div
                  role="group"
                  aria-label="Prog BVOD Media entry mode"
                ...
                >
                  <button
                    ...
                    style={
                      !progbvodExpertModalOpen
                        ? { backgroundColor: PROG_BVOD_MEDIA_HEX }
                        : undefined
                    }
                    onClick={() => {
                  ...
                  <button
                    ...
                    style={{
                      ...(progbvodExpertModalOpen
                        ? { backgroundColor: PROG_BVOD_MEDIA_HEX }
                        : {}),
                      ...(expertSegmentAttention && !progbvodExpertModalOpen
                        ? {
                            boxShadow: `0 0 0 2px ${rgbaFromHex(PROG_BVOD_MEDIA_HEX, 0.45)}`,
                          }
                        : {}),
                    }}
                    onClick={() => {
          ...
          <CardContent className="space-y-0">
            <MediaContainerSummarySection
              ...
              feeLabel={`Fee (${feeprogbvod}%)`}
              accentHex={PROG_BVOD_MEDIA_HEX}
      ...
      <div>
        {isLoading ? (
          <MediaContainerLoadState loading label="Programmatic BVOD" />
        ) : (
          <div className="space-y-6">
            {progbvodExpertModalOpen ? null : (
            <Form {...form}>
              <div className="space-y-6">
                            ...
                            <Button
                              ...
                              aria-label={
                                collapsedLineItems.has(lineItemIndex)
                                  ? `Expand details for prog BVOD line item ${lineItemIndex + 1}`
                                  : `Collapse details for prog BVOD line item ${lineItemIndex + 1}`
                              }
...
<FormItem>
  <FormControl>
      ...
      <Dialog
        open={progbvodExpertModalOpen}
        onOpenChange={handleProgBvodExpertModalOpenChange}
      >
        <DialogContent className="max-w-[95vw] w-[95vw] max-h-[95vh] h-[95vh] flex flex-col p-4 gap-0 overflow-hidden">
          <DialogHeader className="flex-shrink-0 pb-2">
            <DialogTitle>Prog BVOD Media Expert Mode</DialogTitle>
          </DialogHeader>
          ...
          <DialogFooter className="flex-shrink-0 border-t pt-3 mt-2">
            <div className="mr-auto flex flex-col gap-1.5">
              <ExpertIncompleteRowsSummary rows={expertProgBvodRows} />
              {expertApplyPendingPageSave ? (
            ...
            <Button type="button" onClick={handleProgBvodExpertApply}>
      ...
      <Dialog
        open={progbvodExpertExitConfirmOpen}
        onOpenChange={(open) => {
          if (!open) dismissProgBvodExpertExitConfirm()
        }}
      >
        <DialogContent
          ...
          onClick={(e) => {
            if (
              (e.target as HTMLElement).closest(
                "[data-progbvodexpert-exit-yes]"
              )
            ) {
              return
            }
            dismissProgBvodExpertExitConfirm()
          }}
        >
          <DialogHeader>
            <DialogTitle>Leave Prog BVOD Media Expert Mode?</DialogTitle>
            <DialogDescription>
              You have unsaved changes in Expert Mode. Apply saves them to the
              Prog BVOD Media section; leaving now discards those edits.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
              type="button"
              variant="outline"
              onClick={dismissProgBvodExpertExitConfirm}
            >
              No, keep editing
            </Button>
            <Button
              ...
              variant="destructive"
              data-progbvodexpert-exit-yes
              onClick={confirmProgBvodExpertExitWithoutSaving}
            >
