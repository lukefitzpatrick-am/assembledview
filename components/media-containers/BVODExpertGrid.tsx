...
function sumMergedQuantities(row: BvodExpertScheduleRow): number {
...
type BvodCopiedCells = {
  data: (string | number)[][]
  sourceRows: number
  sourceCols: number
  selection: BvodMultiCellSelection
}

function weeklyCellDisplayValue(
  cellValue: number | string | undefined,
  mergedSpan: BvodExpertMergedWeekSpan | null
): string {
  if (mergedSpan) {
...
const BVOD_WEEK_CELL_VISUAL_CLASSES = {
...
function selectionOverlapsMergedSpan(
  rowMergeMap: BvodRowMergeMap | undefined,
...
function normalizeBvodWeekRect(
  row0: number,
  wk0: string,
  row1: number,
  wk1: string,
  weekKeys: readonly string[]
): BvodWeekRectSelection {
...
function bvodWeekCellInRect(
  rowIndex: number,
  weekKey: string,
  rect: BvodWeekRectSelection | null,
  weekKeys: readonly string[]
): boolean {
...
function coerceWeeklySelectionBounds(
  ...
  weekRectSelection: BvodWeekRectSelection | null,
  ...
  if (
    weekRectSelection &&
    bvodWeekCellInRect(anchorRow, anchorWeekKey, weekRectSelection, weekKeys)
  ) {
...
function applyWeeklyPasteMatrixToSelection(args: {
  ...
  weekKeys: readonly string[]
  rowCount: number
  nextRows: BvodExpertScheduleRow[]
}): {
  applied: number
  errorReasons: string[]
  layout: WeeklyPasteLayoutMode
...
type BvodWeekMergeSelectionNormalized = {
  rowIndex: number
 ...
 */
function normalizeBvodWeekMergeSelection(
  rect: BvodWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): BvodWeekMergeSelectionNormalized | null {
...
function weekPlainClickPreservesWeekAreaSelection(
  ...
  const mergeNorm = normalizeBvodWeekMergeSelection(rect, multi, weekKeys)
  if (
    mergeNorm &&
  ...
  if (rect && bvodWeekCellInRect(rowIndex, weekKey, rect, weekKeys)) {
    const isSingleCell =
...
function deriveBvodMergeEligibility(
  rect: BvodWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): BvodWeekMergeSelectionNormalized | null {
  return normalizeBvodWeekMergeSelection(rect, multi, weekKeys)
}
...
function mergedWeekSpansAfterCutRect(
  spans: BvodExpertMergedWeekSpan[] | undefined,
  wi0: number,
  wi1: number,
  weekKeys: readonly string[]
): BvodExpertMergedWeekSpan[] {
...
type WeeklyExportSelection =
  | { kind: "rect"; rect: BvodWeekRectSelection }
...
function resolveWeeklyExportSelection(
  ...
  weekKeys: readonly string[],
  rows: readonly BvodExpertScheduleRow[] | null
...
function buildWeeklyExportTsv(
  selection: WeeklyExportSelection,
  rows: readonly BvodExpertScheduleRow[],
  weekKeys: readonly string[]
): string | null {
  switch (selection.kind) {
    case "rect": {
...
function applyWeeklyCutToRows(
  ...
  switch (selection.kind) {
    case "rect": {
      ...
      if (wi0 < 0 || wi1 < 0) return null
      const next: BvodExpertScheduleRow[] = rows.map((r) => ({
        ...r,
    ...
    case "mergeContiguous": {
      ...
      const next: BvodExpertScheduleRow[] = rows.map((row) => ({
        ...row,
...
/** Match labels/values on {@link BVODContainer} buy-type combobox. */
const BVOD_BUY_TYPE_OPTIONS: ComboboxOption[] = [
...
function normalizeBvodBuyTypePaste(raw: string): string {
  const v = raw.trim()
  if (!v) return ""
  const byValue = BVOD_BUY_TYPE_OPTIONS.find(
    (o) => o.value.toLowerCase() === v.toLowerCase()
  )
  if (byValue) return byValue.value
  const byLabel = BVOD_BUY_TYPE_OPTIONS.find(
...
function normalizeBvodPublisherPaste(raw: string, publisherNames: string[]): string {
  const v = raw.trim()
  if (!v) return ""
...
function normalizeBvodSitePaste(raw: string, siteNames: string[]): string {
  const v = raw.trim()
  if (!v) return ""
...
export interface BVODExpertGridProps {
  ...
  /** BVOD sites (platform + site label) for site combobox + fuzzy matching */
  bvodSites?: {
...
type BvodExpertFocusedCell = { rowIndex: number; columnKey: string }
type WeekDragSource =
  | {
      type: "single"
      rowIndex: number
...
type BvodRowMergeSpanMeta = Readonly<{
  id: string
  startWeekKey: string
  endWeekKey: string
  totalQty: number
...
type BvodRowMergeMap = Readonly<{
  anchorByWeekKey: Readonly<Record<string, string>>
  interiorByWeekKey: Readonly<Record<string, string>>
  spanById: Readonly<Record<string, BvodExpertMergedWeekSpan>>
  spanMetaByAnchorWeekKey: Readonly<Record<string, BvodRowMergeSpanMeta>>
}>
...
export function BVODExpertGrid({
  ...
  const [focusedCell, setFocusedCell] = useState<BvodExpertFocusedCell | null>(
    null
  )
  const focusedCellRef = useRef<BvodExpertFocusedCell | null>(null)
  ...
  const siteComboboxOptionsForRow = useCallback(
    (row: BvodExpertScheduleRow): ComboboxOption[] => {
      const key = normalizeBvodKey(row.publisher)
      const src = !key
        ? bvodSites
        : bvodSites.filter((s) => normalizeBvodKey(s.platform) === key)
      const names = [
        ...new Set(
          src
            .map((s) => String(s.site ?? "").trim())
  ...
  const [weekRectSelection, setWeekRectSelection] =
    useState<BvodWeekRectSelection | null>(null)
  // Multi-cell selection state for copy/paste operations.
  const [multiCellSelection, setMultiCellSelection] =
    useState<BvodMultiCellSelection | null>(null)
  const [isSelecting, setIsSelecting] = useState(false)
  const [copiedCells, setCopiedCells] = useState<BvodCopiedCells | null>(null)
  ...
  const lastDragRectDuringGestureRef = useRef<BvodWeekRectSelection | null>(null)
  /**
   ...
   */
  const postDragWeekClickRectRef = useRef<BvodWeekRectSelection | null>(null)
  const weekRectSelectionRef = useRef<BvodWeekRectSelection | null>(null)
  ...
  const rowMergeMaps = useMemo<readonly BvodRowMergeMap[]>(() => {
    const maps = normalizedRows.map((row) => {
      const anchorByWeekKey: Record<string, string> = {}
      const interiorByWeekKey: Record<string, string> = {}
      const spanById: Record<string, BvodExpertMergedWeekSpan> = {}
      const spanMetaByAnchorWeekKey: Record<string, BvodRowMergeSpanMeta> = {}
      ...
      for (const span of row.mergedWeekSpans ?? []) {
        if (spanById[span.id]) {
          if (DEBUG_BVOD_MERGE) {
            console.debug("[BVOD merge] occupancy duplicate span id ignored", {
              rowId: row.id,
        ...
        if (keys.some((k) => occupiedWeekKeys.has(k))) {
          if (DEBUG_BVOD_MERGE) {
            console.debug("[BVOD merge] occupancy overlap ignored", {
              rowId: row.id,
              rowIndex: normalizedRows.indexOf(row),
              spanId: span.id,
              keys,
  ...
  useEffect(() => {
    ...
    console.debug("[BVOD merge] pending selection updated", pendingMergeSelection)
  ...
  const updateRow = useCallback(
    (rowIndex: number, patch: Partial<BvodExpertScheduleRow>) => {
  ...
  const tryFuzzyMatch = useCallback(
    (rowIndex: number, field: FuzzyMatchField, value: string) => {
      ...
      if (corrected) {
        updateRow(rowIndex, { [field]: corrected } as Partial<BvodExpertScheduleRow>)
        return
      }
      ...
      if (fuzzyMatchAutoApplyRef.current) {
        updateRow(rowIndex, { [field]: match.matched } as Partial<BvodExpertScheduleRow>)
      } else {
  ...
  const navColCount =
    bvodDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekKeys.length
  const firstWeekNavColIndex = bvodDescriptorKeys.length + WEEK_GRID_COL_OFFSET
  const unitRateNavColIndex = bvodDescriptorKeys.indexOf("unitRate")
  ...
  const unmergeWeekSpan = useCallback(
    (rowIndex: number, spanId: string) => {
      ...
      if (DEBUG_BVOD_MERGE) {
        console.debug("[BVOD merge] unmerge applied", { rowIndex, spanId })
      }
    },
  ...
  const clearWeekSelectionWhereMergedAnchorInvolved = useCallback(
    (rowIndex: number, weekKey: string) => {
      setWeekRectSelection((prev) =>
        prev && bvodWeekCellInRect(rowIndex, weekKey, prev, weekKeys)
          ? null
          : prev
      )
  ...
  const rangeWeekMultiSelect = useCallback(
    (rowIndex: number, anchorKey: string, endKey: string) => {
      ...
      setWeekRectSelection(
        normalizeBvodWeekRect(rowIndex, anchorKey, rowIndex, endKey, weekKeys)
      )
      setWeekStripSelection(null)
    },
    [weekKeys]
  ...
  const lockPendingMergeSelectionFromCurrentSelection = useCallback(() => {
    const n = deriveBvodMergeEligibility(
  ...
  const derivedMergeTarget = useMemo(() => {
    const n = deriveBvodMergeEligibility(weekRectSelection, weekMultiSelect, weekKeys)
  ...
  useEffect(() => {
    const endDrag = () => {
      ...
      if (DEBUG_BVOD_MERGE) {
        console.debug("[BVOD merge] drag end", {
          lastRect: r,
  ...
  const handleMergeSelectedWeeks = useCallback(() => {
    ...
    if (selectionOverlapsMergedSpan(rowMergeMaps[rowIndex], sorted, weekKeys)) {
      if (DEBUG_BVOD_MERGE) {
        console.debug("[BVOD merge] validation overlap failure", {
          rowIndex,
    ...
    const newId =
      typeof crypto !== "undefined" && crypto.randomUUID
        ? crypto.randomUUID()
        : `merge-${Date.now()}`
    const newSpan: BvodExpertMergedWeekSpan = {
      id: newId,
    ...
    if (DEBUG_BVOD_MERGE) {
      console.debug("[BVOD merge] merge applied", {
        rowIndex,
        spanId: newSpan.id,
  ...
  useEffect(() => {
    ...
    const cellId = expertGridCellId(
      ...
      bvodDescriptorKeys.length + WEEK_GRID_COL_OFFSET + lastWi
    )
    document
      .getElementById(cellId)
      ?.scrollIntoView({ behavior: "smooth", block: "nearest", inline: "nearest" })
  }, [domGridId, mergeTarget, mergeWeeksReady, bvodDescriptorKeys.length, weekKeys])

  useEffect(() => {
    if (!DEBUG_BVOD_MERGE) return
    console.debug("[BVOD merge] merge eligibility derived", {
      mergeTarget,
      derivedMergeTarget,
      mergeWeeksReady,
      weekRectSelection,
  ...
  const pasteMatrixIntoGrid = useCallback(
    (matrix: string[][]) => {
      ...
      if (fcIsWeek) {
        ...
        const nextRows: BvodExpertScheduleRow[] = normalizedRows.map((r) => ({
          ...r,
      ...
      for (let dr = 0; dr < working.length; dr++) {
        ...
        for (let dc = 0; dc < pasteRow.length; dc++) {
          ...
          const field = target.field as keyof BvodExpertScheduleRow
          ...
          if (field === "unitRate") {
            ...
            nextRows[targetRow] = {
              ...cur,
              [field]: truthy,
            } as BvodExpertScheduleRow
            applied += 1
          } else if (field === "buyType") {
            nextRows[targetRow] = {
              ...cur,
              buyType: normalizeBvodBuyTypePaste(raw),
            }
            applied += 1
          } else if (field === "publisher") {
            const pub = normalizeBvodPublisherPaste(raw, publisherNames)
            nextRows[targetRow] = {
              ...cur,
              publisher: pub,
            ...
            nextRows[targetRow] = {
              ...cur,
              site: normalizeBvodSitePaste(raw, siteNames),
            }
            applied += 1
          } else {
            const v = raw.trim()
            nextRows[targetRow] = { ...cur, [field]: v } as BvodExpertScheduleRow
            applied += 1
          }
        }
      }
                        ...
                        <th
                          ...
                          className={stickyThCorner(
                            i === descriptorHeadLabels.length - 1
                              ? BVOD_EXPERT_WEEK_SCROLLER_EDGE
                              : undefined
                          )}
                          style={{
                  ...
                  <tbody>
                    {normalizedRows.map((row, rowIndex) => {
                      ...
                      const stripeStyle =
                        rowIndex % 2 === 0
                          ...
                          : undefined
                      const grossCol = bvodDescriptorKeys.length
                      const actionsCol = bvodDescriptorKeys.length + 1
                          ...
                          {(() => {
                            ...
                            for (let wi = 0; wi < weekColumns.length; wi += 1) {
                              ...
                              const tdClassName = cn(
                                ...
                                !isMergedAnchorCell &&
                                  mergeReadyOnCell &&
                                  "bvod-expert-week-cell--merge-ready z-[6] bg-amber-500/24 ring-2 ring-inset ring-amber-600/75 shadow-sm dark:bg-amber-500/20 dark:ring-amber-400/70",
                              ...
                              const mergedAnchorWrapperClassName = cn(
                                ...
                                isMergedAnchorCell &&
                                  BVOD_WEEK_CELL_VISUAL_CLASSES.mergedSurface,
                              ...
                              const inputClassName = cn(
                                ...
                                isMergedAnchorCell
                                  ? BVOD_WEEK_CELL_VISUAL_CLASSES.mergedInput
                              ...
                              renderedWeekCells.push(
                                <td
                                  ...
                                  onMouseEnter={(e) => {
                                    ...
                                    const next = normalizeBvodWeekRect(
                                      drag.rowIndex,
                                      drag.weekKey,
                                      rowIndex,
                                      col.weekKey,
                                    ...
                                    setWeekStripSelection(null)
                                    if (DEBUG_BVOD_MERGE) {
                                      console.debug("[BVOD merge] drag update", {
                                        rect: next,
                                      })
                                    }
                                  }}
                                ...
                                >
                                  ...
                                  >
                                    <Input
                                      ...
                                      setIsSelecting(true)
                                      if (DEBUG_BVOD_MERGE) {
                                        console.debug("[BVOD merge] drag start", {
                                          rowIndex,
                                          weekKey: col.weekKey,
                                        })
                                      }
                                      ...
                                      if (shift && lastWeekAnchorRef.current) {
                                        ...
                                        if (
                                          ...
                                          const rect = normalizeBvodWeekRect(
                                            lastWeekAnchorRef.current.rowIndex,
                                            lastWeekAnchorRef.current.weekKey,
                                            rowIndex,
                                            col.weekKey,
                    ...
                    >
                      <td
                        ...
                        colSpan={bvodDescriptorKeys.length}
                      >
                      ...
                      <td
                        className={cn(
                          stickyTd(bvodDescriptorKeys.length),
                          "h-8 px-1 text-xs tabular-nums"
                        )}
                        style={{
                          ...stickyStyleBody(bvodDescriptorKeys.length),
                          ...bvodExpertTotalsRowBgStyle,
                        }}
                      >
                      ...
                      <td
                        className={cn(
                          stickyTd(bvodDescriptorKeys.length + 1),
                          "h-8"
                        )}
                        style={{
                          ...stickyStyleBody(bvodDescriptorKeys.length + 1),
                          ...bvodExpertTotalsRowBgStyle,
                        }}
                      />
                      <td
                        className={cn(
                          stickyTd(bvodDescriptorKeys.length + 2),
                          "h-8 px-1 text-xs tabular-nums text-muted-foreground",
                          BVOD_EXPERT_WEEK_SCROLLER_EDGE
                        )}
                        style={{
                          ...stickyStyleBody(bvodDescriptorKeys.length + 2),
                          ...bvodExpertTotalsRowBgStyle,
                        }}
                      >
