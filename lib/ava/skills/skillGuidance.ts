...
Publisher schedule ingest (xlsx via AVA attach): call get_pending_ingest_review and speak those numbers; if MBA is not in page context, ask which MBA — never guess; wait for confirm then accept_ingest_proposal. Same Hub engine and 409 money gate. Offer the full_review_path for the Hub review UI (same staged upload). Unknown publisher → say no profile; do not guess.
