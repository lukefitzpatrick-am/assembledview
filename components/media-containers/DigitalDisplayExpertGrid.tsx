...
const DIGITALDISPLAY_EXPERT_WEEK_COL_WIDTH_PX = 112

/** Week column width on `<th>` and week `<td>`; inputs use `w-full` inside the cell. */
const digitalDisplayExpertWeekColLayoutStyle = {
  width: DIGITALDISPLAY_EXPERT_WEEK_COL_WIDTH_PX,
  minWidth: DIGITALDISPLAY_EXPERT_WEEK_COL_WIDTH_PX,
  maxWidth: DIGITALDISPLAY_EXPERT_WEEK_COL_WIDTH_PX,
...
const DIGITALDISPLAY_EXPERT_WEEK_SCROLLER_EDGE =
...
): DigitalDisplayExpertMergedWeekSpan | null {
...
type DigitalDisplayMultiCellSelection = {
  startRow: number
  endRow: number
  startCol: number
  endCol: number
...
function weeklyCellDisplayValue(
  ...
  mergedSpan: DigitalDisplayExpertMergedWeekSpan | null
): string {
  if (mergedSpan) {
...
const DIGITALDISPLAY_WEEK_CELL_VISUAL_CLASSES = {
...
function selectionOverlapsMergedSpan(
  rowMergeMap: DigitalDisplayRowMergeMap | undefined,
...
type DigitalDisplayWeekRectSelection = {
  rowStart: number
  rowEnd: number
  weekKeyStart: string
  weekKeyEnd: string
...
function normalizeDigitalDisplayWeekRect(
  row0: number,
  wk0: string,
  row1: number,
  wk1: string,
  weekKeys: readonly string[]
): DigitalDisplayWeekRectSelection {
...
function digitalDisplayWeekCellInRect(
  rowIndex: number,
  weekKey: string,
  rect: DigitalDisplayWeekRectSelection | null,
  weekKeys: readonly string[]
): boolean {
...
function rectToMultiCellSelection(
  rect: DigitalDisplayWeekRectSelection | null,
  weekKeys: readonly string[]
): DigitalDisplayMultiCellSelection | null {
  if (!rect) return null
  const i0 = weekKeys.indexOf(rect.weekKeyStart)
  const i1 = weekKeys.indexOf(rect.weekKeyEnd)
...
function coerceWeeklySelectionBounds(
  ...
  weekRectSelection: DigitalDisplayWeekRectSelection | null,
  ...
  if (
    weekRectSelection &&
    digitalDisplayWeekCellInRect(anchorRow, anchorWeekKey, weekRectSelection, weekKeys)
  ) {
...
function applyWeeklyPasteMatrixToSelection(args: {
  ...
  weekKeys: readonly string[]
  rowCount: number
  nextRows: DigitalDisplayExpertScheduleRow[]
}): {
  applied: number
  errorReasons: string[]
  layout: WeeklyPasteLayoutMode
...
function oohMergeKeysFromRect(
  rect: DigitalDisplayWeekRectSelection,
  weekKeys: readonly string[]
): string[] | null {
...
type DigitalDisplayWeekMergeSelectionNormalized = {
  rowIndex: number
 ...
 */
function normalizeDigitalDisplayWeekMergeSelection(
  rect: DigitalDisplayWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): DigitalDisplayWeekMergeSelectionNormalized | null {
...
function weekPlainClickPreservesWeekAreaSelection(
  ...
  const mergeNorm = normalizeDigitalDisplayWeekMergeSelection(rect, multi, weekKeys)
  if (
    mergeNorm &&
  ...
  if (rect && digitalDisplayWeekCellInRect(rowIndex, weekKey, rect, weekKeys)) {
    const isSingleCell =
...
function deriveDigitalDisplayMergeEligibility(
  rect: DigitalDisplayWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): DigitalDisplayWeekMergeSelectionNormalized | null {
  return normalizeDigitalDisplayWeekMergeSelection(rect, multi, weekKeys)
}
...
function mergedWeekSpansAfterCutRect(
  spans: DigitalDisplayExpertMergedWeekSpan[] | undefined,
  wi0: number,
  wi1: number,
  weekKeys: readonly string[]
): DigitalDisplayExpertMergedWeekSpan[] {
...
type WeeklyExportSelection =
  | { kind: "rect"; rect: DigitalDisplayWeekRectSelection }
...
function resolveWeeklyExportSelection(
  ...
  weekKeys: readonly string[],
  rows: readonly DigitalDisplayExpertScheduleRow[] | null
...
function buildWeeklyExportTsv(
  selection: WeeklyExportSelection,
  rows: readonly DigitalDisplayExpertScheduleRow[],
  weekKeys: readonly string[]
): string | null {
  switch (selection.kind) {
    case "rect": {
...
function applyWeeklyCutToRows(
  ...
  rows: readonly DigitalDisplayExpertScheduleRow[],
  weekKeys: readonly string[]
): DigitalDisplayExpertScheduleRow[] | null {
  switch (selection.kind) {
    case "rect": {
      const rect = selection.rect
      const wi0 = Math.min(
      ...
      if (wi0 < 0 || wi1 < 0) return null
      const next: DigitalDisplayExpertScheduleRow[] = rows.map((r) => ({
        ...r,
    ...
    case "mergeContiguous": {
      ...
      const next: DigitalDisplayExpertScheduleRow[] = rows.map((row) => ({
        ...row,
...
const DIGITALDISPLAY_DESCRIPTOR_TAIL: readonly (keyof DigitalDisplayExpertScheduleRow)[] = ["unitRate"]
...
export function DigitalDisplayExpertGrid({
  ...
  const digitalDisplayDescriptorKeys = useMemo(
    () =>
      [
        ...
        ...DIGITALDISPLAY_BILLING_FLAG_KEYS,
  ...
  const descriptorColWidths = useMemo(
    () => [
      48, 48, 120, 120, 120, 96, 120, 110, 110, 96, 40, 40, 40, 88,
    ],
    []
  )
  ...
  const containerTotals = useMemo(() => {
    ...
    const fee =
      feedigidisplay > 0 && feedigidisplay < 100 ? (sumGross * feedigidisplay) / (100 - feedigidisplay) : 0
    const totalWithFee = sumGross + fee

    return { sumGross, sumQty, perWeek, fee, totalWithFee }
  }, [feedigidisplay, normalizedRows, weekKeys])
                    ...
                    <tr>
                      ...
                      {weekColumns.map((col) => (
                        <th
                          ...
                          style={{
                            ...digitalDisplayExpertWeekColLayoutStyle,
                            ...digitalDisplayExpertHeaderCellBgStyle,
                          }}
                          title={col.labelFull}
                        >
                          ...
                          {(() => {
                            ...
                            for (let wi = 0; wi < weekColumns.length; wi += 1) {
                              ...
                              const tdClassName = cn(
                                ...
                                !isMergedAnchorCell &&
                                  mergeReadyOnCell &&
                                  "digitaldisplay-expert-week-cell--merge-ready z-[6] bg-amber-500/24 ring-2 ring-inset ring-amber-600/75 shadow-sm dark:bg-amber-500/20 dark:ring-amber-400/70",
                              ...
                              renderedWeekCells.push(
                                <td
                                  ...
                                  style={
                                    isMergedAnchorCell
                                      ? {
                                          width: DIGITALDISPLAY_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                          minWidth:
                                            DIGITALDISPLAY_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
                                          maxWidth:
                                            DIGITALDISPLAY_EXPERT_WEEK_COL_WIDTH_PX * spanLen,
