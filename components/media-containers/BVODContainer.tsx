    ...
    (items) => {
      ...
      const transformedLineItems = items.map((item: any, index: number) => {
        ...
        const bursts = parsedBursts.length > 0 ? parsedBursts.map((burst: any) => ({
          ...
          startDate: coerceBurstDateLocal(burst.startDate ?? burst.start_date) ?? defaultMediaBurstStartDate(campaignStartDate, campaignEndDate),
          endDate: coerceBurstDateLocal(burst.endDate ?? burst.end_date) ?? defaultMediaBurstEndDate(campaignStartDate, campaignEndDate),
    ...
    const lineItemTotals = watchedLineItems.map((lineItem, index) => {
      ...
      lineItem.bursts.forEach((burst) => {
        ...
        if (lineItem.budgetIncludesFees) {
          ...
          // Media = Budget * ((100 - Fee) / 100)
          // Fees = Budget * (Fee / 100)
          burstMedia = (budget * (100 - (feebvod || 0))) / 100;
          burstFee = (budget * (feebvod || 0)) / 100;
        } else {
          // Budget is net media, fee calculated on top
          // Media = Budget (unchanged)
          // Fees = Budget * (Fee / (100 - Fee))
          burstMedia = budget;
          burstFee = feebvod ? (budget * feebvod) / (100 - feebvod) : 0;
        }
...
useEffect(() => {
  ...
  return (
    <div className="space-y-6">
        ...
        <Card className="overflow-hidden border-0 shadow-md">
          ...
          <CardContent className="space-y-0">
            <MediaContainerSummarySection
              ...
              feeLabel={`Fee (${feebvod}%)`}
              accentHex={MEDIA_ACCENT_HEX}
              dimensions={["Publisher", "Platform", "Bid Strategy", "Buy Type"]}
      ...
      <div>
        {isLoading ? (
          <MediaContainerLoadState loading label="BVOD" />
        ) : (
          <div className="space-y-6">
            <Form {...form}>
              <div className="space-y-6">
        ...
        <DialogContent className="max-w-[95vw] w-[95vw] max-h-[95vh] h-[95vh] flex flex-col p-4 gap-0 overflow-hidden">
          ...
          <DialogFooter className="flex-shrink-0 border-t pt-3 mt-2">
            <div className="mr-auto flex flex-col gap-1.5">
              <ExpertIncompleteRowsSummary rows={expertBvodRows} />
              {expertApplyPendingPageSave ? (
