...
- **Xero sync (T5, parity mode):** `app/api/cron/xero-sync` writes Postgres only via `lib/xero/*` (ingest → billing import → PDF Blob → contacts). Xano `daily_xero_sync` stays active until T6. Matcher = `lib/xero/matchMba.ts` (finance/match_mba port). Client aliases live in `xero_client_aliases` (migration 0006). Never touch `media:`/`sow:`/`retainer:` invoice_key rows. Parity: `npm run db:xero-parity` after a cron run, before `db:etl`.
- T4a plan save writes `schedule_months` from T4a.0-enriched `computeCampaignFinancials` schedules (explode via `_scheduleTransform`); `billing_overrides` rows for the version retag matching billing months as `source='override'`. Editor still saves via Xano until `WRITE_BACKEND=postgres` + T4c.
