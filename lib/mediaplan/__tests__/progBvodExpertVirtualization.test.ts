/**
 * F-28 Phase 2 — Programmatic BVOD Expert row virtualization smoke (Prompt A items 1,3,5,7,8).
 */
import assert from "node:assert/strict"
import test from "node:test"
import { format, startOfDay } from "date-fns"

import { mapProgBvodExpertRowsToStandardLineItems } from "@/lib/mediaplan/expertChannelMappings"
import {
  reorderExpertRows,
  mergedSpanWidthPx,
...
const PROGBVOD_EXPERT_ROW_OVERSCAN = EXPERT_GRID_ROW_OVERSCAN_DEFAULT
...
function emptyRow(id: string): ProgBvodExpertScheduleRow {
...
function buildRows(n: number): ProgBvodExpertScheduleRow[] {
  return Array.from({ length: n }, (_, i) => emptyRow(`progbvod-v-${i}`))
}

/** Mirrors ProgBVODExpertGrid containerTotals over the FULL logical array. */
function computeProgBvodWeeklyTotals(
...
test("3. DRAG REORDER across virtual boundary lands on correct logical index", () => {
  ...
  const dropIndex = virtualRowIndexFromOffsetY(
    280 * PROGBVOD_EXPERT_ROW_HEIGHT_PX + 1,
    PROGBVOD_EXPERT_ROW_HEIGHT_PX,
    300
  )
...
test("5. WEEK-COLUMN WIDTH respects resize overrides (sticky header/week resize path)", () => {
  ...
  assert.equal(PROGBVOD_EXPERT_ROW_HEIGHT_PX, 41)
})
...
test("7. WEEKLY TOTALS derived from full 300-row state", () => {
  ...
  const totals = computeProgBvodWeeklyTotals(rows, 0)
  ...
  const after = computeProgBvodWeeklyTotals(edited, 0)
...
test("8. PERF: mounted DOM window stays bounded while scrolling 300 rows", () => {
  ...
  const nearTop = expectedMountedRowRange(
    ...
    PROGBVOD_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    PROGBVOD_EXPERT_ROW_OVERSCAN
  )
  const mid = expectedMountedRowRange(
    150 * PROGBVOD_EXPERT_ROW_HEIGHT_PX,
    viewportHeight,
    PROGBVOD_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    PROGBVOD_EXPERT_ROW_OVERSCAN
  ...
  const nearBottom = expectedMountedRowRange(
    280 * PROGBVOD_EXPERT_ROW_HEIGHT_PX,
    viewportHeight,
    PROGBVOD_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    PROGBVOD_EXPERT_ROW_OVERSCAN
  ...
  const maxWindow =
    Math.ceil(viewportHeight / PROGBVOD_EXPERT_ROW_HEIGHT_PX) +
    2 * PROGBVOD_EXPERT_ROW_OVERSCAN +
    2
