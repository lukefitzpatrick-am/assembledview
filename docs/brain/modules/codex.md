...
| **Task detail** | Right `SlideOver` over the list (`TaskDetailSlideOver` + `TaskDetailClient`). Left-edge drag handle resizes the panel (min ~420px, max ~80vw, localStorage; double-click resets). Panel header: inline-editable title + **Mark complete**. Fields: status / priority / category / due / **estimate** (`1h 30m` / `45m` / `2h` → `estimated_minutes`) / assignee / client / MBA searchable combobox of that client's campaigns labelled `<mba_number> — <campaign name>` (`GET /api/codex/client-mbas`, MBA number descending) — empty + disabled until a client is chosen. MBA links **one-way** to `/mediaplans/mba/[mba]/edit` (containment: nothing outside `/tasks` links back). Checklist tickboxes optimistic-PATCH (`applyChecklistToggle` / `persistChecklistToggle`); add/reorder/delete + "N of M"; comments (newest last); activity from `codex_activity` (`entity_type=task`) with legible before→after diffs via `formatActivityDiff`. Failed inline saves restore previous value + destructive toast. |
...
| Table | Status | Notes |
|---|---|---|
| `tasks` | **Live** | Create / list / patch / soft-delete; `auto_created` + `ava_auto_key` (0040, partial unique `uq_tasks_ava_auto_key` when set); `estimated_minutes` integer NULL (0048; mirrored in `db/schema/codex.ts`) |
...
- `lib/codex/{repo,types,flag,shadowRoles,queryHelpers,activityDiff,quickAddParse,recurringRule,runRecurring,seedTasks,auth0LoginUpsert,auth0RosterSync,rosterEligibility,auth0RosterStore,rosterEmailAlias,rosterAliasGuard,rosterLoginCheck,clientMbas,checklistToggle}.ts`
...
- `components/tasks/{TaskBoard,TaskBulkBar,TaskDetailClient,TaskDetailSlideOver,TaskMbaSelect,TaskChecklist,TaskFormDialog,TaskQuickAdd,TeamMemberFormDialog,TemplateFormDialog,Auth0RosterSyncButton}.tsx`
