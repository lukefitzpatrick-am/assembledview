...
- `lib/nav/routeManifest.ts` is the single source of truth for user-facing route **labels**, document **titles**, breadcrumb copy, and command-palette destinations. `AppSidebar`, `DynamicBreadcrumbs`, and `CommandPalette` derive from it; `DocumentTitleFromManifest` + `pageMetadata()` keep `<title>` aligned. Do not invent parallel label maps. Admin sidebar IA is `ADMIN_SIDEBAR_GROUPS` (Plan / Deliver / Money / Reference; ungrouped **Today** + **Tasks** at top; **Admin** in footer beside UserMenu). Create Campaign is palette-only (verb). `/management` is removed — user invites live at `/admin/users/new`.
