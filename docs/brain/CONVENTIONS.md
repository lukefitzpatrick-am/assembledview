...
- `db/index.ts` is `server-only`. Never let it reach an Edge or client bundle. `postgres` is in `serverExternalPackages`; `instrumentation.ts` must not import anything that pulls in `db/`. `await import("@/db")` defers loading, not bundling — Webpack still pulls the module into the client compilation. Gate: `npm run check:client-server-only` (static and dynamic imports from every `"use client"` seed; `import type` skipped; `webpackIgnore` only for the isomorphic `lib/api.ts` allowlist). Ingest DB writers live in a `*.server.ts` sibling beside client-safe helpers (`avaColumnMapping.server.ts` / `lineAudit.server.ts` pattern): `persistColumnRemap.server.ts`, `valueSynonymRepo.server.ts`, `loadPublisherProfiles.server.ts`, `ingestStageStore.server.ts`. `@/db/schema` is not server-only (pure table definitions); modules that only need types or table shapes may import it statically.
