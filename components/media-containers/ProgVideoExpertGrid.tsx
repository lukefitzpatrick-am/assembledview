...
import {
  ...
  WEEK_COL_WIDTH_PX as PROGVIDEO_EXPERT_WEEK_COL_WIDTH_PX,
  WEEK_SCROLLER_EDGE as PROGVIDEO_EXPERT_WEEK_SCROLLER_EDGE,
  WEEK_CELL_VISUAL_CLASSES as PROGVIDEO_WEEK_CELL_VISUAL_CLASSES,
  ...
  normalizeWeekRect as normalizeProgVideoWeekRect,
  weekCellInRect as progVideoWeekCellInRect,
  ...
  normalizeWeekMergeSelection as normalizeProgVideoWeekMergeSelection,
  weekPlainClickPreservesWeekAreaSelection,
  deriveMergeEligibility as deriveProgVideoMergeEligibility,
  weekCellExportText,
...
type ProgVideoWeekRectSelection = ExpertWeekRectSelection
type ProgVideoMultiCellSelection = ExpertMultiCellSelection
type ProgVideoWeekMergeSelectionNormalized = WeekMergeSelectionNormalized
 ...
 */
const PROGVIDEO_EXPERT_ROW_VIRTUALIZATION = true
const PROGVIDEO_EXPERT_ROW_HEIGHT_PX = OOH_EXPERT_ROW_HEIGHT_PX
const PROGVIDEO_EXPERT_ROW_OVERSCAN = OOH_EXPERT_ROW_OVERSCAN
...
export function ProgVideoExpertGrid({
  ...
  const normalizeRowCacheRef = useRef(
    new Map<string, NormalizeRowCacheEntry<ProgVideoExpertScheduleRow>>()
  )
  ...
  const rowMergeMapCacheRef = useRef(
    new Map<
      string,
      MapBuildCacheEntry<
        ProgVideoExpertScheduleRow["mergedWeekSpans"],
        ProgVideoRowMergeMap
      >
    >()
  )
  ...
  const rowReorderVirtual = useMemo(
    () =>
      PROGVIDEO_EXPERT_ROW_VIRTUALIZATION
        ? {
            rowCount: normalizedRows.length,
            estimateSizePx: PROGVIDEO_EXPERT_ROW_HEIGHT_PX,
  ...
  const ensureRowVisible = useCallback((rowIndex: number) => {
    if (!PROGVIDEO_EXPERT_ROW_VIRTUALIZATION) return
  ...
  const widthForWeekKey = useCallback(
    (k: string): number => {
      ...
      return weekColumnWidths[k] ?? PROGVIDEO_EXPERT_WEEK_COL_WIDTH_PX
    },
            ...
            ? Math.min(
                ...
                Math.max(
                  ...
                  deltaDaysFromPx(prev.edgeYmd, dx, {
                    ...
                    dayKeysByWeekKey,
                    weekColumnWidths,
                    defaultWeekColWidthPx: PROGVIDEO_EXPERT_WEEK_COL_WIDTH_PX,
  ...
  const {
    ...
    getScrollElement: () => gridScrollRef.current,
    estimateSize: PROGVIDEO_EXPERT_ROW_HEIGHT_PX,
    overscan: PROGVIDEO_EXPERT_ROW_OVERSCAN,
  ...
  const virtualSpacerColSpan = useMemo(() => {
    ...
    return (
      1 + progVideoDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekCells
    )
  }, [
    weekColumns,
    expandedWeekKeys,
  ...
  const containerTotals = useMemo(() => {
    ...
    for (const row of normalizedRows) {
      const { net, fee } = expertRowCostSplit(row, weekKeys, feeprogvideo)
      sumNet += net
      sumFee += fee
      for (const k of weekKeys) {
  ...
  }, [dayKeysByWeekKey, feeprogvideo, normalizedRows, weekKeys])
        ...
        <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2 border-b border-border/60 bg-muted/5 pb-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <Label htmlFor="progvideo-expert-row-count" className="sr-only">Add rows count</Label>
              <Input
                        ...
                        return (
                          ...
                          >
                            ...
                            <ExpertGridWeekResizeHandle
                              weekKey={col.weekKey}
                              currentWidth={weekColumnWidths[col.weekKey] ?? PROGVIDEO_EXPERT_WEEK_COL_WIDTH_PX}
                  ...
                  <tbody>
                    {(() => {
                      ...
                      return (
                        <MemoExpertGridRow
                          ...
                          rowUiSig={rowUiSig}
                          render={() => {
                      const net = expertRowNetMedia(row, weekKeys, feeprogvideo)
                      ...
                      return (
                        <tr
                          ...
                          data-progvideo-expert-row-index={rowIndex}
                          style={{
                            ...stripeStyle,
                            height: PROGVIDEO_EXPERT_ROW_HEIGHT_PX,
                            maxHeight: PROGVIDEO_EXPERT_ROW_HEIGHT_PX,
                          }}
                          ...
                          {(() => {
                            ...
                            for (let wi = 0; wi < weekColumns.length; wi += 1) {
                              ...
                              const tdClassName = cn(
                                ...
                                !isMergedAnchorCell &&
                                  mergeReadyOnCell &&
                                  "progvideo-expert-week-cell--merge-ready z-[6] bg-pacing-behind-bg ring-2 ring-inset ring-pacing-behind shadow-e0",
                      ...
                      if (!PROGVIDEO_EXPERT_ROW_VIRTUALIZATION) {
                    ...
                    >
                      ...
                      <td
                        ...
                        style={stickyStyleBody(progVideoDescriptorKeys.length)}
                      >
                      ...
                      <td
                        ...
                        style={stickyStyleBody(progVideoDescriptorKeys.length + 1)}
                      />
                      <td
                        className={cn(
                        ...
                        style={stickyStyleBody(progVideoDescriptorKeys.length + 2)}
                      >
