...
const PROGDISPLAY_EXPERT_WEEK_COL_WIDTH_PX = 112

/** Week column width on `<th>` and week `<td>`; inputs use `w-full` inside the cell. */
const progDisplayExpertWeekColLayoutStyle = {
  width: PROGDISPLAY_EXPERT_WEEK_COL_WIDTH_PX,
  minWidth: PROGDISPLAY_EXPERT_WEEK_COL_WIDTH_PX,
  maxWidth: PROGDISPLAY_EXPERT_WEEK_COL_WIDTH_PX,
...
const PROGDISPLAY_EXPERT_WEEK_SCROLLER_EDGE =
...
type ProgDisplayMultiCellSelection = {
  startRow: number
  endRow: number
  startCol: number
  endCol: number
...
const PROGDISPLAY_WEEK_CELL_VISUAL_CLASSES = {
...
function selectionOverlapsMergedSpan(
  rowMergeMap: ProgDisplayRowMergeMap | undefined,
...
type ProgDisplayWeekRectSelection = {
  rowStart: number
  rowEnd: number
  weekKeyStart: string
  weekKeyEnd: string
...
function normalizeProgDisplayWeekRect(
  row0: number,
  wk0: string,
  row1: number,
  wk1: string,
  weekKeys: readonly string[]
): ProgDisplayWeekRectSelection {
...
function progDisplayWeekCellInRect(
  rowIndex: number,
  weekKey: string,
  rect: ProgDisplayWeekRectSelection | null,
  weekKeys: readonly string[]
): boolean {
...
function rectToMultiCellSelection(
  rect: ProgDisplayWeekRectSelection | null,
  weekKeys: readonly string[]
): ProgDisplayMultiCellSelection | null {
  if (!rect) return null
  const i0 = weekKeys.indexOf(rect.weekKeyStart)
  const i1 = weekKeys.indexOf(rect.weekKeyEnd)
...
function coerceWeeklySelectionBounds(
  ...
  weekRectSelection: ProgDisplayWeekRectSelection | null,
  ...
  if (
    weekRectSelection &&
    progDisplayWeekCellInRect(anchorRow, anchorWeekKey, weekRectSelection, weekKeys)
  ) {
...
function applyWeeklyPasteMatrixToSelection(args: {
  ...
  weekKeys: readonly string[]
  rowCount: number
  nextRows: ProgDisplayExpertScheduleRow[]
}): {
  applied: number
  errorReasons: string[]
  layout: WeeklyPasteLayoutMode
...
function oohMergeKeysFromRect(
  rect: ProgDisplayWeekRectSelection,
  weekKeys: readonly string[]
): string[] | null {
...
type ProgDisplayWeekMergeSelectionNormalized = {
  rowIndex: number
 ...
 */
function normalizeProgDisplayWeekMergeSelection(
  rect: ProgDisplayWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): ProgDisplayWeekMergeSelectionNormalized | null {
...
function weekPlainClickPreservesWeekAreaSelection(
  ...
  const mergeNorm = normalizeProgDisplayWeekMergeSelection(rect, multi, weekKeys)
  if (
    mergeNorm &&
  ...
  if (rect && progDisplayWeekCellInRect(rowIndex, weekKey, rect, weekKeys)) {
    const isSingleCell =
...
function deriveProgDisplayMergeEligibility(
  rect: ProgDisplayWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): ProgDisplayWeekMergeSelectionNormalized | null {
  return normalizeProgDisplayWeekMergeSelection(rect, multi, weekKeys)
}
...
type WeeklyExportSelection =
  | { kind: "rect"; rect: ProgDisplayWeekRectSelection }
...
function resolveWeeklyExportSelection(
  ...
  weekKeys: readonly string[],
  rows: readonly ProgDisplayExpertScheduleRow[] | null
...
function buildWeeklyExportTsv(
  selection: WeeklyExportSelection,
  rows: readonly ProgDisplayExpertScheduleRow[],
  weekKeys: readonly string[]
): string | null {
  switch (selection.kind) {
    case "rect": {
...
function applyWeeklyCutToRows(
  ...
  rows: readonly ProgDisplayExpertScheduleRow[],
  weekKeys: readonly string[]
): ProgDisplayExpertScheduleRow[] | null {
  switch (selection.kind) {
    case "rect": {
      const rect = selection.rect
      const wi0 = Math.min(
      ...
      if (wi0 < 0 || wi1 < 0) return null
      const next: ProgDisplayExpertScheduleRow[] = rows.map((r) => ({
        ...r,
    ...
    case "mergeContiguous": {
      ...
      const next: ProgDisplayExpertScheduleRow[] = rows.map((row) => ({
        ...row,
...
const PROGDISPLAY_DESCRIPTOR_TAIL: readonly (keyof ProgDisplayExpertScheduleRow)[] = ["unitRate"]
...
export function ProgDisplayExpertGrid({
  ...
  const containerTotals = useMemo(() => {
    ...
    const fee =
      feeprogdisplay > 0 && feeprogdisplay < 100 ? (sumGross * feeprogdisplay) / (100 - feeprogdisplay) : 0
