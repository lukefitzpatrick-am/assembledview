...
function sumMergedQuantities(row: MagazinesExpertScheduleRow): number {
...
): MagazinesExpertMergedWeekSpan | null {
...
type MagazinesMultiCellSelection = {
  startRow: number
  endRow: number
  startCol: number
  endCol: number
...
type MagazinesCopiedCells = {
  data: (string | number)[][]
  sourceRows: number
  sourceCols: number
  selection: MagazinesMultiCellSelection
}

function weeklyCellDisplayValue(
  cellValue: number | string | undefined,
  mergedSpan: MagazinesExpertMergedWeekSpan | null
): string {
  if (mergedSpan) {
...
const MAGAZINES_WEEK_CELL_VISUAL_CLASSES = {
...
function selectionOverlapsMergedSpan(
  rowMergeMap: MagazinesRowMergeMap | undefined,
...
type MagazinesWeekRectSelection = {
  rowStart: number
  rowEnd: number
  weekKeyStart: string
  weekKeyEnd: string
...
function normalizeMagazinesWeekRect(
  row0: number,
  wk0: string,
  row1: number,
  wk1: string,
  weekKeys: readonly string[]
): MagazinesWeekRectSelection {
...
function magazinesWeekCellInRect(
  rowIndex: number,
  weekKey: string,
  rect: MagazinesWeekRectSelection | null,
  weekKeys: readonly string[]
): boolean {
...
function rectToMultiCellSelection(
  rect: MagazinesWeekRectSelection | null,
  weekKeys: readonly string[]
): MagazinesMultiCellSelection | null {
  if (!rect) return null
  const i0 = weekKeys.indexOf(rect.weekKeyStart)
  const i1 = weekKeys.indexOf(rect.weekKeyEnd)
...
function coerceWeeklySelectionBounds(
  ...
  weekRectSelection: MagazinesWeekRectSelection | null,
  ...
  if (
    weekRectSelection &&
    magazinesWeekCellInRect(anchorRow, anchorWeekKey, weekRectSelection, weekKeys)
  ) {
...
function applyWeeklyPasteMatrixToSelection(args: {
  ...
  weekKeys: readonly string[]
  rowCount: number
  nextRows: MagazinesExpertScheduleRow[]
}): {
  applied: number
  errorReasons: string[]
  layout: WeeklyPasteLayoutMode
...
function radioWeekRangeOutlineFlags(
  rowIndex: number,
  weekIndex: number,
  rect: MagazinesWeekRectSelection | null,
...
function radioWeekOutlineEdgeClasses(flags: {
  inRange: boolean
  top: boolean
  bottom: boolean
  left: boolean
...
function radioWeekCellInMergePulseHighlight(
  rowIndex: number,
  weekKey: string,
...
function oohMergeKeysFromRect(
  rect: MagazinesWeekRectSelection,
  weekKeys: readonly string[]
): string[] | null {
...
type MagazinesWeekMergeSelectionNormalized = {
  rowIndex: number
 ...
 */
function normalizeMagazinesWeekMergeSelection(
  rect: MagazinesWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): MagazinesWeekMergeSelectionNormalized | null {
...
function weekPlainClickPreservesWeekAreaSelection(
  ...
  const mergeNorm = normalizeMagazinesWeekMergeSelection(rect, multi, weekKeys)
  if (
    mergeNorm &&
  ...
  if (rect && magazinesWeekCellInRect(rowIndex, weekKey, rect, weekKeys)) {
    const isSingleCell =
...
function deriveMagazinesMergeEligibility(
  rect: MagazinesWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): MagazinesWeekMergeSelectionNormalized | null {
  return normalizeMagazinesWeekMergeSelection(rect, multi, weekKeys)
}
...
function mergedWeekSpansAfterCutRect(
  spans: MagazinesExpertMergedWeekSpan[] | undefined,
  wi0: number,
  wi1: number,
  weekKeys: readonly string[]
): MagazinesExpertMergedWeekSpan[] {
...
type WeeklyExportSelection =
  | { kind: "rect"; rect: MagazinesWeekRectSelection }
...
function resolveWeeklyExportSelection(
  ...
  weekKeys: readonly string[],
  rows: readonly MagazinesExpertScheduleRow[] | null
...
function buildWeeklyExportTsv(
  selection: WeeklyExportSelection,
  rows: readonly MagazinesExpertScheduleRow[],
  weekKeys: readonly string[]
): string | null {
  switch (selection.kind) {
    case "rect": {
...
function applyWeeklyCutToRows(
  ...
  rows: readonly MagazinesExpertScheduleRow[],
  weekKeys: readonly string[]
): MagazinesExpertScheduleRow[] | null {
  switch (selection.kind) {
    case "rect": {
      const rect = selection.rect
      const wi0 = Math.min(
      ...
      if (wi0 < 0 || wi1 < 0) return null
      const next: MagazinesExpertScheduleRow[] = rows.map((r) => ({
        ...r,
    ...
    case "mergeContiguous": {
      ...
      const next: MagazinesExpertScheduleRow[] = rows.map((row) => ({
        ...row,
...
/** Match labels/values on {@link MagazinesContainer} buy-type combobox. */
const MAGAZINES_BUY_TYPE_OPTIONS: ComboboxOption[] = [
...
function normalizeMagazinesBuyTypePaste(raw: string): string {
  const v = raw.trim()
  if (!v) return ""
  const byValue = MAGAZINES_BUY_TYPE_OPTIONS.find(
    (o) => o.value.toLowerCase() === v.toLowerCase()
  )
  if (byValue) return byValue.value
  const byLabel = MAGAZINES_BUY_TYPE_OPTIONS.find(
...
function normalizeMagazinesNetworkPaste(raw: string, networkNames: string[]): string {
  const v = raw.trim()
  if (!v) return ""
...
function rowGrossCost(row: MagazinesExpertScheduleRow, weekKeys: string[]): number {
...
const MAGAZINES_DESCRIPTOR_CORE: readonly (keyof MagazinesExpertScheduleRow)[] = [
  "startDate",
  "endDate",
  "network",
  "title",
...
const MAGAZINES_BILLING_FLAG_KEYS: readonly (keyof MagazinesExpertScheduleRow)[] = [
  "fixedCostMedia",
  "clientPaysForMedia",
  "budgetIncludesFees",
]

const MAGAZINES_DESCRIPTOR_TAIL: readonly (keyof MagazinesExpertScheduleRow)[] = ["unitRate"]
...
type MagazinesExpertFocusedCell = { rowIndex: number; columnKey: string }
type WeekDragSource =
  | {
      type: "single"
      rowIndex: number
...
type MagazinesRowMergeSpanMeta = Readonly<{
  id: string
  startWeekKey: string
  endWeekKey: string
  totalQty: number
...
type MagazinesRowMergeMap = Readonly<{
  anchorByWeekKey: Readonly<Record<string, string>>
  interiorByWeekKey: Readonly<Record<string, string>>
  spanById: Readonly<Record<string, MagazinesExpertMergedWeekSpan>>
  spanMetaByAnchorWeekKey: Readonly<Record<string, MagazinesRowMergeSpanMeta>>
}>
...
export function MagazinesExpertGrid({
  ...
  const [focusedCell, setFocusedCell] = useState<MagazinesExpertFocusedCell | null>(
    null
  )
  const focusedCellRef = useRef<MagazinesExpertFocusedCell | null>(null)
  ...
  const [weekRectSelection, setWeekRectSelection] =
    useState<MagazinesWeekRectSelection | null>(null)
  // Multi-cell selection state for copy/paste operations.
  const [multiCellSelection, setMultiCellSelection] =
    useState<MagazinesMultiCellSelection | null>(null)
  const [isSelecting, setIsSelecting] = useState(false)
  const [copiedCells, setCopiedCells] = useState<MagazinesCopiedCells | null>(null)
  ...
  const lastDragRectDuringGestureRef = useRef<MagazinesWeekRectSelection | null>(null)
  /**
   ...
   */
  const postDragWeekClickRectRef = useRef<MagazinesWeekRectSelection | null>(null)
  const weekRectSelectionRef = useRef<MagazinesWeekRectSelection | null>(null)
  ...
  const magazinesDescriptorKeys = useMemo(
    () =>
      [
        ...MAGAZINES_DESCRIPTOR_CORE,
        ...MAGAZINES_BILLING_FLAG_KEYS,
        ...MAGAZINES_DESCRIPTOR_TAIL,
      ] as (keyof MagazinesExpertScheduleRow)[],
    []
  )

  const descriptorColWidths = useMemo(
    () => [
      48, 48, 120, 120, 96, 80, 120, 110, 110, 96, 40, 40, 40, 88,
    ],
    []
  )
  ...
  const rowMergeMaps = useMemo<readonly MagazinesRowMergeMap[]>(() => {
    const maps = normalizedRows.map((row) => {
      const anchorByWeekKey: Record<string, string> = {}
      const interiorByWeekKey: Record<string, string> = {}
      const spanById: Record<string, MagazinesExpertMergedWeekSpan> = {}
      const spanMetaByAnchorWeekKey: Record<string, MagazinesRowMergeSpanMeta> = {}
      ...
      for (const span of row.mergedWeekSpans ?? []) {
        if (spanById[span.id]) {
          if (DEBUG_MAGAZINES_MERGE) {
            console.debug("[Magazines merge] occupancy duplicate span id ignored", {
              rowId: row.id,
        ...
        if (keys.some((k) => occupiedWeekKeys.has(k))) {
          if (DEBUG_MAGAZINES_MERGE) {
            console.debug("[Magazines merge] occupancy overlap ignored", {
              rowId: row.id,
              rowIndex: normalizedRows.indexOf(row),
              spanId: span.id,
              keys,
  ...
  useEffect(() => {
    if (!DEBUG_MAGAZINES_MERGE) return
    ...
    console.debug("[Magazines merge] pending selection updated", pendingMergeSelection)
  ...
  const updateRow = useCallback(
    (rowIndex: number, patch: Partial<MagazinesExpertScheduleRow>) => {
  ...
  const tryFuzzyMatch = useCallback(
    (rowIndex: number, field: FuzzyMatchField, value: string) => {
      ...
      if (corrected) {
        updateRow(rowIndex, { [field]: corrected } as Partial<MagazinesExpertScheduleRow>)
        return
      }
      ...
      if (fuzzyMatchAutoApplyRef.current) {
        updateRow(rowIndex, { [field]: match.matched } as Partial<MagazinesExpertScheduleRow>)
      } else {
  ...
  const navColCount =
    magazinesDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekKeys.length
  const firstWeekNavColIndex = magazinesDescriptorKeys.length + WEEK_GRID_COL_OFFSET
  const unitRateNavColIndex = magazinesDescriptorKeys.indexOf("unitRate")
  ...
  const unmergeWeekSpan = useCallback(
    (rowIndex: number, spanId: string) => {
      ...
      if (DEBUG_MAGAZINES_MERGE) {
        console.debug("[Magazines merge] unmerge applied", { rowIndex, spanId })
      }
    },
  ...
  const clearWeekSelectionWhereMergedAnchorInvolved = useCallback(
    (rowIndex: number, weekKey: string) => {
      setWeekRectSelection((prev) =>
        prev && magazinesWeekCellInRect(rowIndex, weekKey, prev, weekKeys)
          ? null
          : prev
      )
  ...
  const rangeWeekMultiSelect = useCallback(
    (rowIndex: number, anchorKey: string, endKey: string) => {
      ...
      setWeekRectSelection(
        normalizeMagazinesWeekRect(rowIndex, anchorKey, rowIndex, endKey, weekKeys)
      )
      setWeekStripSelection(null)
    },
    [weekKeys]
  ...
  const lockPendingMergeSelectionFromCurrentSelection = useCallback(() => {
    const n = deriveMagazinesMergeEligibility(
  ...
  const derivedMergeTarget = useMemo(() => {
    const n = deriveMagazinesMergeEligibility(weekRectSelection, weekMultiSelect, weekKeys)
  ...
  useEffect(() => {
    const endDrag = () => {
      ...
      if (DEBUG_MAGAZINES_MERGE) {
        console.debug("[Magazines merge] drag end", {
          lastRect: r,
  ...
  const handleMergeSelectedWeeks = useCallback(() => {
    ...
    if (selectionOverlapsMergedSpan(rowMergeMaps[rowIndex], sorted, weekKeys)) {
      if (DEBUG_MAGAZINES_MERGE) {
        console.debug("[Magazines merge] validation overlap failure", {
          rowIndex,
    ...
    const newId =
      typeof crypto !== "undefined" && crypto.randomUUID
        ? crypto.randomUUID()
        : `merge-${Date.now()}`
    const newSpan: MagazinesExpertMergedWeekSpan = {
      id: newId,
    ...
    if (DEBUG_MAGAZINES_MERGE) {
      console.debug("[Magazines merge] merge applied", {
        rowIndex,
        spanId: newSpan.id,
  ...
  useEffect(() => {
    ...
    const cellId = expertGridCellId(
      ...
      magazinesDescriptorKeys.length + WEEK_GRID_COL_OFFSET + lastWi
    )
    document
      .getElementById(cellId)
      ?.scrollIntoView({ behavior: "smooth", block: "nearest", inline: "nearest" })
  }, [domGridId, mergeTarget, mergeWeeksReady, magazinesDescriptorKeys.length, weekKeys])

  useEffect(() => {
    if (!DEBUG_MAGAZINES_MERGE) return
    console.debug("[Magazines merge] merge eligibility derived", {
      mergeTarget,
      derivedMergeTarget,
      mergeWeeksReady,
      weekRectSelection,
  ...
  const pasteMatrixIntoGrid = useCallback(
    (matrix: string[][]) => {
      ...
      if (fcIsWeek) {
        ...
        const nextRows: MagazinesExpertScheduleRow[] = normalizedRows.map((r) => ({
          ...r,
      ...
      for (let dr = 0; dr < working.length; dr++) {
        ...
        for (let dc = 0; dc < pasteRow.length; dc++) {
          ...
          const field = target.field as keyof MagazinesExpertScheduleRow
          ...
          if (field === "unitRate") {
            ...
            nextRows[targetRow] = {
              ...cur,
              [field]: truthy,
            } as MagazinesExpertScheduleRow
            applied += 1
          } else if (field === "buyType") {
            nextRows[targetRow] = {
              ...cur,
              buyType: normalizeMagazinesBuyTypePaste(raw),
            }
            applied += 1
          } else if (field === "network") {
            nextRows[targetRow] = {
              ...cur,
              network: normalizeMagazinesNetworkPaste(raw, networkNames),
            }
            applied += 1
          } else {
            const v = raw.trim()
            nextRows[targetRow] = { ...cur, [field]: v } as MagazinesExpertScheduleRow
            applied += 1
          }
        }
      }
  ...
  const containerTotals = useMemo(() => {
    ...
    const fee =
      feemagazines > 0 && feemagazines < 100 ? (sumGross * feemagazines) / (100 - feemagazines) : 0
  ...
  const colIndexOf = useCallback(
    (key: keyof MagazinesExpertScheduleRow) => magazinesDescriptorKeys.indexOf(key),
    [magazinesDescriptorKeys]
  )
      ...
      <Card className="relative overflow-hidden border-0 shadow-md">
          ...
          <div className="overflow-hidden rounded-lg border border-border/80 bg-card/30 shadow-sm">
            {normalizedRows.length === 0 ? (
              ...
              <div
                ...
                data-magazines-expert-grid-scroll=""
              >
                        ...
                        <th
                          ...
                          className={stickyThCorner(
                            i === descriptorHeadLabels.length - 1
                              ? MAGAZINES_EXPERT_WEEK_SCROLLER_EDGE
                              : undefined
                          )}
                          style={{
                  ...
                  <tbody>
                    {normalizedRows.map((row, rowIndex) => {
                      ...
                      const stripeStyle =
                        rowIndex % 2 === 0
                          ...
                          : undefined
                      const grossCol = magazinesDescriptorKeys.length
                      const actionsCol = magazinesDescriptorKeys.length + 1
                      const sigmaCol = magazinesDescriptorKeys.length + 2
                        ...
                        >
                          ...
                          >
                            <Input
                              ...
                              className="h-8 border-0 bg-transparent px-1 text-xs shadow-none focus-visible:ring-1"
                              value={row.size}
                              onFocus={() => handleCellFocus(rowIndex, "size")}
                              onKeyDown={(e) =>
                          ...
                          >
                            <Input
                              ...
                              onKeyDown={(e) =>
                                handleGridInputKeyDown(rowIndex, cPub, e)
                              }
                              onChange={(e) =>
                                updateRow(rowIndex, {
                            ...
                            for (let wi = 0; wi < weekColumns.length; wi += 1) {
                              ...
                              const tdClassName = cn(
                                ...
                                isPopulatedNonMergedCell &&
                                  MAGAZINES_WEEK_CELL_VISUAL_CLASSES.populatedSingleTd,
                                ...
                                !isMergedAnchorCell &&
                                  mergeReadyOnCell &&
                                  "magazines-expert-week-cell--merge-ready z-[6] bg-amber-500/24 ring-2 ring-inset ring-amber-600/75 shadow-sm dark:bg-amber-500/20 dark:ring-amber-400/70",
                              ...
                              const mergedAnchorWrapperClassName = cn(
                                ...
                                isMergedAnchorCell &&
                                  MAGAZINES_WEEK_CELL_VISUAL_CLASSES.mergedSurface,
                              ...
                              const inputClassName = cn(
                                ...
                                isMergedAnchorCell
                                  ? MAGAZINES_WEEK_CELL_VISUAL_CLASSES.mergedInput
                              ...
                              renderedWeekCells.push(
                                <td
                                  ...
                                  onMouseEnter={(e) => {
                                    ...
                                    const next = normalizeMagazinesWeekRect(
                                      drag.rowIndex,
                                      drag.weekKey,
                                      rowIndex,
                                      col.weekKey,
                                    ...
                                    setWeekStripSelection(null)
                                    if (DEBUG_MAGAZINES_MERGE) {
                                      console.debug("[Magazines merge] drag update", {
                                        rect: next,
                                      })
                                    }
                                  }}
                                ...
                                >
                                  ...
                                  >
                                    <Input
                                      ...
                                      setIsSelecting(true)
                                      if (DEBUG_MAGAZINES_MERGE) {
                                        console.debug("[Magazines merge] drag start", {
                                          rowIndex,
                                          weekKey: col.weekKey,
                                        })
                                      }
                                      ...
                                      if (shift && lastWeekAnchorRef.current) {
                                        ...
                                        if (
                                          ...
                                          const rect = normalizeMagazinesWeekRect(
                                            lastWeekAnchorRef.current.rowIndex,
                                            lastWeekAnchorRef.current.weekKey,
                                            rowIndex,
                                            col.weekKey,
                    ...
                    >
                      <td
                        ...
                        colSpan={magazinesDescriptorKeys.length}
                      >
                      ...
                      <td
                        className={cn(
                          stickyTd(magazinesDescriptorKeys.length),
                          "h-8 px-1 text-xs tabular-nums"
                        )}
                        style={{
                          ...stickyStyleBody(magazinesDescriptorKeys.length),
                          ...magazinesExpertTotalsRowBgStyle,
                        }}
                      >
                      ...
                      <td
                        className={cn(
                          stickyTd(magazinesDescriptorKeys.length + 1),
                          "h-8"
                        )}
                        style={{
                          ...stickyStyleBody(magazinesDescriptorKeys.length + 1),
                          ...magazinesExpertTotalsRowBgStyle,
                        }}
                      />
                      <td
                        className={cn(
                          stickyTd(magazinesDescriptorKeys.length + 2),
                          "h-8 px-1 text-xs tabular-nums text-muted-foreground",
                          MAGAZINES_EXPERT_WEEK_SCROLLER_EDGE
                        )}
                        style={{
                          ...stickyStyleBody(magazinesDescriptorKeys.length + 2),
                          ...magazinesExpertTotalsRowBgStyle,
                        }}
                      >
        ...
        {DEBUG_MAGAZINES_MERGE ? (
          <div className="pointer-events-none absolute bottom-2 right-2 z-50 max-w-[360px] rounded-md border bg-background/95 p-2 text-[11px] leading-snug shadow-lg">
            <div className="mb-1 font-medium text-foreground">Magazines merge debug</div>
