...
| C-29 | **Costs payables under reconciliation** — after FN7, Costs is the only payables surface; legacy hub composition (version pool / production / month-header fee+adserving / status filters) still diverges from sections published-tip + dual-shape join until Luke signs FN-FIX-1 D1–D4 + FS-2 status/composition one-liners | Open — `PayablesReconBanner` on `/finance/costs` + `/finance/costs/invoices` (“do not use for month-end”); figures + decisions in `docs/superpowers/fs2-payables-status-scoping-2026-08-01.md`; composition notes in `fn-fix-1-legacy-payables-composition-2026-08-01.md` |
