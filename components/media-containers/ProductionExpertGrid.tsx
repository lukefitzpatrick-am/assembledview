...
import type {
  ExpertWeeklyValues,
  ProductionExpertMergedWeekSpan,
...
import {
  ...
  WEEK_COL_WIDTH_PX as PRODUCTION_EXPERT_WEEK_COL_WIDTH_PX,
  WEEK_SCROLLER_EDGE as PRODUCTION_EXPERT_WEEK_SCROLLER_EDGE,
  WEEK_CELL_VISUAL_CLASSES as PRODUCTION_WEEK_CELL_VISUAL_CLASSES,
  ...
  normalizeWeekRect as normalizeProductionWeekRect,
  weekCellInRect as productionWeekCellInRect,
  ...
  weekRangeOutlineFlags as productionWeekRangeOutlineFlags,
  weekOutlineEdgeClasses as productionWeekOutlineEdgeClasses,
  mergeReadyOutlineFlags as oohMergeReadyOutlineFlags,
  mergeReadyOutlineEdgeClasses as oohMergeReadyOutlineEdgeClasses,
  weekCellInMergePulseHighlight as productionWeekCellInMergePulseHighlight,
  mergeKeysFromRect as oohMergeKeysFromRect,
  normalizeWeekMergeSelection as normalizeProductionWeekMergeSelection,
  weekPlainClickPreservesWeekAreaSelection,
  deriveMergeEligibility as deriveProductionMergeEligibility,
  weekCellExportText,
...
type ProductionWeekRectSelection = ExpertWeekRectSelection
type ProductionMultiCellSelection = ExpertMultiCellSelection
type ProductionWeekMergeSelectionNormalized = WeekMergeSelectionNormalized

const MEDIA_ACCENT_HEX = getMediaTypeThemeHex("production")

const productionExpertHeaderCellBgStyle = {
  backgroundColor: rgbaFromHex(MEDIA_ACCENT_HEX, 0.08),
}

const productionExpertTotalsRowBgStyle = {
...
type ProductionCopiedCells = {
  data: (string | number)[][]
  sourceRows: number
  sourceCols: number
  selection: ProductionMultiCellSelection
}
...
export const productionDescriptorKeys: readonly (keyof ProductionExpertScheduleRow)[] = [
  "startDate",
  "endDate",
  "mediaType",
  "publisher",
...
type ProductionExpertFocusedCell = { rowIndex: number; columnKey: string }
type WeekDragSource =
  | {
      type: "single"
      rowIndex: number
...
type ProductionRowMergeSpanMeta = Readonly<{
  id: string
  startWeekKey: string
  endWeekKey: string
  totalQty: number
...
type ProductionRowMergeMap = Readonly<{
  anchorByWeekKey: Readonly<Record<string, string>>
  interiorByWeekKey: Readonly<Record<string, string>>
  spanById: Readonly<Record<string, ProductionExpertMergedWeekSpan>>
  spanMetaByAnchorWeekKey: Readonly<Record<string, ProductionRowMergeSpanMeta>>
}>
...
export function ProductionExpertGrid({
  ...
  const [focusedCell, setFocusedCell] = useState<ProductionExpertFocusedCell | null>(
    null
  )
  const focusedCellRef = useRef<ProductionExpertFocusedCell | null>(null)
  focusedCellRef.current = focusedCell

  const [rowCountInput, setRowCountInput] = useState<string>("1")

  const productionTypeComboboxOptions: ComboboxOption[] = useMemo(
    () => productionTypeOptions,
    [productionTypeOptions]
  )
  ...
  const [weekRectSelection, setWeekRectSelection] =
    useState<ProductionWeekRectSelection | null>(null)
  // Multi-cell selection state for copy/paste operations.
  const [multiCellSelection, setMultiCellSelection] =
    useState<ProductionMultiCellSelection | null>(null)
  const [isSelecting, setIsSelecting] = useState(false)
  const [copiedCells, setCopiedCells] = useState<ProductionCopiedCells | null>(null)
  ...
  const lastDragRectDuringGestureRef = useRef<ProductionWeekRectSelection | null>(null)
  /**
   ...
   */
  const postDragWeekClickRectRef = useRef<ProductionWeekRectSelection | null>(null)
  const weekRectSelectionRef = useRef<ProductionWeekRectSelection | null>(null)
  ...
  const descriptorColWidths = useMemo(() => {
    const core = [48, 48, 120, 120, 110, 96]
    const tail = [88]
    return [...core, ...tail]
  }, [])
  ...
  const normalizeRowCacheRef = useRef(
    new Map<string, NormalizeRowCacheEntry<ProductionExpertScheduleRow>>()
  )
  ...
  const rowMergeMapCacheRef = useRef(
    new Map<
      string,
      MapBuildCacheEntry<
        ProductionExpertScheduleRow["mergedWeekSpans"],
        ProductionRowMergeMap
      >
    >()
  )
  const rowMergeMaps = useMemo<readonly ProductionRowMergeMap[]>(() => {
    const { maps, cache } = buildMapsPreservingIdentity(
      ...
      (row) => {
        ...
        const spanById: Record<string, ProductionExpertMergedWeekSpan> = {}
        const spanMetaByAnchorWeekKey: Record<string, ProductionRowMergeSpanMeta> = {}
  ...
  useEffect(() => {
    if (!DEBUG_PRODUCTION_MERGE) return
  ...
  const pushRows = useCallback(
    (next: ProductionExpertScheduleRow[]) => {
      const withDates = finalizeRowsPreservingIdentity(next, (r) => {
        ...
        const dates = deriveProductionExpertRowScheduleYmdFromRow(
          cleared,
          weekColumns,
          campaignStartDate,
          campaignEndDate,
  ...
  const updateRow = useCallback(
    (rowIndex: number, patch: Partial<ProductionExpertScheduleRow>) => {
  ...
  const navColCount =
    productionDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekKeys.length
  const firstWeekNavColIndex = productionDescriptorKeys.length + WEEK_GRID_COL_OFFSET
  const unitRateNavColIndex = productionDescriptorKeys.indexOf("unitRate")
  ...
  const widthForWeekKey = useCallback(
    (k: string): number => {
      ...
      return weekColumnWidths[k] ?? PRODUCTION_EXPERT_WEEK_COL_WIDTH_PX
    },
            ...
            ? Math.min(
                ...
                Math.max(
                  ...
                  deltaDaysFromPx(prev.edgeYmd, dx, {
                    ...
                    dayKeysByWeekKey,
                    weekColumnWidths,
                    defaultWeekColWidthPx: PRODUCTION_EXPERT_WEEK_COL_WIDTH_PX,
  ...
  const clearWeekSelectionWhereMergedAnchorInvolved = useCallback(
    (rowIndex: number, weekKey: string) => {
      setWeekRectSelection((prev) =>
        prev && productionWeekCellInRect(rowIndex, weekKey, prev, weekKeys)
          ? null
          : prev
      )
  ...
  const rangeWeekMultiSelect = useCallback(
    (rowIndex: number, anchorKey: string, endKey: string) => {
      ...
      setWeekRectSelection(
        normalizeProductionWeekRect(rowIndex, anchorKey, rowIndex, endKey, weekKeys)
      )
      setWeekStripSelection(null)
    },
    [weekKeys]
  ...
  const lockPendingMergeSelectionFromCurrentSelection = useCallback(() => {
    const n = deriveProductionMergeEligibility(
  ...
  const derivedMergeTarget = useMemo(() => {
    const n = deriveProductionMergeEligibility(weekRectSelection, weekMultiSelect, weekKeys)
  ...
  const handleMergeSelectedWeeks = useCallback(() => {
    ...
    const newId =
      typeof crypto !== "undefined" && crypto.randomUUID
        ? crypto.randomUUID()
        : `merge-${Date.now()}`
    const newSpan: ProductionExpertMergedWeekSpan = {
      id: newId,
  ...
  useEffect(() => {
    ...
    const cellId = expertGridCellId(
      ...
      productionDescriptorKeys.length + WEEK_GRID_COL_OFFSET + lastWi
    )
    document
      .getElementById(cellId)
      ?.scrollIntoView({ behavior: "smooth", block: "nearest", inline: "nearest" })
  }, [domGridId, mergeTarget, mergeWeeksReady, productionDescriptorKeys.length, weekKeys])
  ...
  const pasteMatrixIntoGrid = useCallback(
    (matrix: string[][]) => {
      ...
      if (fcIsWeek) {
        ...
        const nextRows: ProductionExpertScheduleRow[] = normalizedRows.map((r) => ({
          ...r,
      ...
      for (let dr = 0; dr < working.length; dr++) {
        ...
        for (let dc = 0; dc < pasteRow.length; dc++) {
          ...
          const field = target.field as keyof ProductionExpertScheduleRow
          ...
          if (field === "unitRate") {
            ...
            nextRows[targetRow] = { ...cur, market: raw.trim() }
            applied += 1
          }
        }
      }
  ...
  const containerTotals = useMemo(() => {
    ...
    for (const row of normalizedRows) {
      sumGross += expertRowGrossCost(row, weekKeys)
      for (const k of weekKeys) {
    ...
    return { sumGross, sumQty, perWeek }
  }, [dayKeysByWeekKey, normalizedRows, weekKeys])
  ...
  const colIndexOf = useCallback(
    (key: keyof ProductionExpertScheduleRow) => productionDescriptorKeys.indexOf(key),
    [productionDescriptorKeys]
  )
      ...
      <Card className="relative overflow-hidden border-0 shadow-md">
        ...
        <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2 border-b border-border/60 bg-muted/5 pb-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <Label htmlFor="production-expert-row-count" className="sr-only">Add rows count</Label>
              <Input
                id="production-expert-row-count"
                type="number"
                min={1}
                max={500}
              ...
              <div className="flex flex-col items-center justify-center gap-4 px-6 py-14 text-center">
                ...
                <div className="space-y-2 max-w-md">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Add a row to enter production line items and weekly quantities, or
                  ...
                  <thead className="[&_tr]:border-b-0">
                    <tr>
                      <ExpertGridRowReorderHeaderCell
                        ...
                        style={productionExpertHeaderCellBgStyle}
                      />
                        ...
                        <th
                          ...
                          className={stickyThCorner(
                            i === descriptorHeadLabels.length - 1
                              ? PRODUCTION_EXPERT_WEEK_SCROLLER_EDGE
                              : undefined
                          )}
                          style={{
                            ...stickyStyleHeaderCorner(i),
                            ...productionExpertHeaderCellBgStyle,
                          }}
                        >
                          {label === "Unit Cost" ? (
                            <Tooltip>
                              ...
                              <TooltipContent side="bottom" className="max-w-xs text-xs">
                                Unit cost per deliverable (fee-free qty × rate)
                              </TooltipContent>
                            </Tooltip>
                          ) : (
                            label
                        ...
                        return (
                          ...
                          >
                            ...
                            <ExpertGridWeekResizeHandle
                              weekKey={col.weekKey}
                              currentWidth={weekColumnWidths[col.weekKey] ?? PRODUCTION_EXPERT_WEEK_COL_WIDTH_PX}
                    ...
                    {normalizedRows.map((row, rowIndex) => {
                      ...
                      return (
                        <MemoExpertGridRow
                          ...
                          rowUiSig={rowUiSig}
                          render={() => {
                      const totalCost = expertRowGrossCost(row, weekKeys)
                      ...
                      const totalCostTooltip =
                        qtySum > 0
                          ? `${qtySum.toLocaleString()} × ${formatAUD(rowUnitRate)}`
                          : "Enter unit cost and quantities"
                      const stripe =
                      ...
                      const stripeStyle =
                        rowIndex % 2 === 0
                          ...
                          : undefined
                      const totalCostCol = productionDescriptorKeys.length
                      const actionsCol = productionDescriptorKeys.length + 1
                      const sigmaCol = productionDescriptorKeys.length + 2
                      const cStart = colIndexOf("startDate")
                      const cEnd = colIndexOf("endDate")
                      const cMedia = colIndexOf("mediaType")
                      const cPub = colIndexOf("publisher")
                      const cMkt = colIndexOf("market")
                      const cDesc = colIndexOf("description")
                      ...
                      return (
                        ...
                        >
                          ...
                          <td
                            className={stickyTd(cMedia)}
                            style={stickyStyleBody(cMedia)}
                          >
                            <Combobox
                              id={expertGridCellId(
                                domGridId,
                              ...
                              options={productionTypeComboboxOptions}
                              value={row.mediaType}
                              onValueChange={(v) =>
                                updateRow(rowIndex, { mediaType: v })
                              }
                              placeholder="Select"
                              searchPlaceholder="Search production types…"
                              emptyText={
                                productionTypeComboboxOptions.length === 0
                                  ? "No production types."
                                  : "No match."
                              }
                              buttonClassName="h-8 border-0 bg-transparent px-1 text-xs shadow-none focus-visible:ring-1"
                              onTriggerFocus={() =>
                                handleCellFocus(rowIndex, "mediaType")
                              }
                              onOpenChange={(open) => {
                                if (open) {
                                  handleCellFocus(rowIndex, "mediaType")
                          ...
                          <td
                            className={stickyTd(cDesc)}
                            style={stickyStyleBody(cDesc)}
                          >
                            <Input
                              id={expertGridCellId(
                                domGridId,
                              ...
                              onFocus={() =>
                                handleCellFocus(rowIndex, "description")
                              }
                              onKeyDown={(e) =>
                                handleGridInputKeyDown(rowIndex, cDesc, e)
                              }
                              onChange={(e) =>
                                updateRow(rowIndex, {
                          ...
                          <td
                            className={stickyTd(totalCostCol)}
                            style={stickyStyleBody(totalCostCol)}
                          >
                            <div
                          ...
                          {(() => {
                            ...
                            for (let wi = 0; wi < weekColumns.length; wi += 1) {
                              ...
                              const budgetEntryBlocked =
                                entryMode === "budget" && !rowBudgetEntryOk
                              const colIndex =
                                productionDescriptorKeys.length +
                                WEEK_GRID_COL_OFFSET +
                                wi
                              ...
                              const stripOutlineRow =
                                ...
                                weekStripSelection.rowIndex === rowIndex
                                  ...
                                  : null
                              const rangeOutline = productionWeekRangeOutlineFlags(
                                rowIndex,
                                wi,
                                weekRectSelection,
                                stripOutlineRow,
                              ...
                              const inMergePulseHighlight = spanKeys.some(
                                (key) =>
                                  productionWeekCellInMergePulseHighlight(
                                    rowIndex,
                                    key,
                              ...
                              const tdClassName = cn(
                                ...
                                isPopulatedNonMergedCell &&
                                  PRODUCTION_WEEK_CELL_VISUAL_CLASSES.populatedSingleTd,
                                ...
                                isDragDropTargetCell &&
                                  !isDragDropTargetValid &&
                                  "cursor-not-allowed",
                                productionWeekOutlineEdgeClasses(rangeOutline),
                                !isMergedAnchorCell &&
                              ...
                              const mergedAnchorWrapperClassName = cn(
                                ...
                                isMergedAnchorCell &&
                                  PRODUCTION_WEEK_CELL_VISUAL_CLASSES.mergedSurface,
                              ...
                              const inputClassName = cn(
                                ...
                                isMergedAnchorCell
                                  ? PRODUCTION_WEEK_CELL_VISUAL_CLASSES.mergedInput
                              ...
                              renderedWeekCells.push(
                                <td
                                  ...
                                  onMouseEnter={(e) => {
                                    ...
                                    const next = normalizeProductionWeekRect(
                                      drag.rowIndex,
                                      drag.weekKey,
                                      rowIndex,
                                      col.weekKey,
                                    ...
                                    <Input
                                      ...
                                      if (shift && lastWeekAnchorRef.current) {
                                        ...
                                        if (
                                          ...
                                          const rect = normalizeProductionWeekRect(
                                            lastWeekAnchorRef.current.rowIndex,
                                            lastWeekAnchorRef.current.weekKey,
                                            rowIndex,
                                            col.weekKey,
                    ...
                    >
                      <td
                        ...
                        style={{
                          ...
                          ...productionExpertTotalsRowBgStyle,
                        }}
                      />
                      <td
                        className={stickyTd(0)}
                        ...
                        colSpan={productionDescriptorKeys.length}
                      >
                      ...
                      <td
                        className={cn(
                          stickyTd(productionDescriptorKeys.length),
                          "h-8 px-1 text-xs tabular-nums"
                        )}
                        style={{
                          ...stickyStyleBody(productionDescriptorKeys.length),
                          ...productionExpertTotalsRowBgStyle,
                        }}
                      >
                        <div className="flex h-full items-center">
                          {formatAUD(containerTotals.sumGross)}
                        </div>
                      </td>
                      <td
                        className={cn(
                          stickyTd(productionDescriptorKeys.length + 1),
                          "h-8"
                        )}
                        style={{
                          ...stickyStyleBody(productionDescriptorKeys.length + 1),
                          ...productionExpertTotalsRowBgStyle,
                        }}
                      />
                      <td
                        className={cn(
                          stickyTd(productionDescriptorKeys.length + 2),
                          "h-8 px-1 text-xs tabular-nums text-muted-foreground",
                          PRODUCTION_EXPERT_WEEK_SCROLLER_EDGE
                        )}
                        style={{
                          ...stickyStyleBody(productionDescriptorKeys.length + 2),
                          ...productionExpertTotalsRowBgStyle,
                        }}
                      >
        ...
        {DEBUG_PRODUCTION_MERGE ? (
          <div className="pointer-events-none absolute bottom-2 right-2 z-50 max-w-[360px] rounded-md border bg-background/95 p-2 text-[11px] leading-snug shadow-lg">
            <div className="mb-1 font-medium text-foreground">Production Merge Debug</div>
