...
/**
 * Display-facing media + fee split. Always shows full media regardless of
 * `clientPaysForMedia` (matches UI comment "always show media for display")
 * by forcing `clientPaysForMedia: false` into {@link computeBurstAmounts} and
 * returning `deliveryMediaAmount` so client-pays rows still display media.
 */
export function computeBurstMediaFee(input: {
  rawBudget: number
  budgetIncludesFees: boolean
  feePct: number
  buyType?: string
}): { media: number; fee: number } {
  const a = computeBurstAmounts({
  ...
  return { media: a.deliveryMediaAmount, fee: a.feeAmount }
}
