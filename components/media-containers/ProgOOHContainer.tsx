...
} from "@/components/media-containers/ProgOOHExpertGrid"
...
export function getProgOohBursts(
  form: UseFormReturn<ProgOOHFormValues>,
  feeprogooh: number,
  mbaNumber?: string,
): BillingBurst[] {
...
export function calculateInvestmentPerMonth(form, feeprogooh) {
  const lineItems = form.getValues("lineItems") || [];
  ...
  lineItems.forEach((lineItem) => {
    lineItem.bursts.forEach((burst) => {
      ...
      const feePercentage = feeprogooh || 0;
...
export function calculateBurstInvestmentPerMonth(form, feeprogooh) {
  const lineItems = form.getValues("lineItems") || [];
  let monthlyInvestment: Record<string, number> = {};
...
export default function ProgOOHContainer({
  ...
  // Form initialization
  const form = useForm<ProgOOHFormValues>({
    defaultValues: {
      lineItems: [
        {
          platform: "",
          ...
          bursts: [
            {
              ...
              calculatedValue: 0,
              fee: 0,
            } as ProgOOHFormValues["lineItems"][number]["bursts"][number] & { _reactKey: string },
          ],
          totalMedia: 0,
          totalDeliverables: 0,
          totalFee: 0,
  ...
  const progoohExpertWeekColumns = useMemo(
    () => buildWeeklyGanttColumnsFromCampaign(campaignStartDate, campaignEndDate),
    [campaignStartDate, campaignEndDate]
  )

  useLayoutEffect(() => {
    progoohStandardBaselineRef.current =
      serializeProgOohStandardLineItemsBaseline(
        form.getValues("lineItems") as StandardProgOohFormLineItem[]
      )
  }, [form])
  ...
  const handleExpertProgOohRowsChange = useCallback(
    (next: ProgOohExpertScheduleRow[]) => {
      setExpertProgOohRows(next)
    },
    []
  )

  const openProgOohExpertModal = useCallback(() => {
    const mapped = mapStandardProgOohLineItemsToExpertRows(
      (form.getValues("lineItems") || []) as StandardProgOohFormLineItem[],
      progoohExpertWeekColumns,
      campaignStartDate,
      campaignEndDate
    )
    const weekKeys = progoohExpertWeekColumns.map((c) => c.weekKey)
    const rows: ProgOohExpertScheduleRow[] =
      mapped.length > 0
        ? mapped
        : [
            createEmptyProgOohExpertRow(
    ...
    progoohExpertRowsBaselineRef.current =
      serializeProgOohExpertRowsBaseline(rows)
    setExpertProgOohRows(rows)
  ...
  const confirmProgOohExpertExitWithoutSaving = useCallback(() => {
    setProgOohExpertExitConfirmOpen(false)
    collapseAllLineItems()
    setProgOohExpertModalOpen(false)
  }, [collapseAllLineItems])

  const handleProgOohExpertModalOpenChange = useCallback(
    (open: boolean) => {
      if (open) {
        setProgOohExpertModalOpen(true)
        return
      ...
      const dirty =
        serializeProgOohExpertRowsBaseline(expertProgOohRows) !==
        progoohExpertRowsBaselineRef.current
      if (!dirty) {
        collapseAllLineItems()
        setProgOohExpertModalOpen(false)
      ...
      setProgOohExpertExitConfirmOpen(true)
    },
    [collapseAllLineItems, expertProgOohRows]
  )

  const handleProgOohExpertApply = useCallback(() => {
    ...
    const standard = mapProgOohExpertRowsToStandardLineItems(
      ...
      campaignEndDate,
      {
        feePctProgOoh: feeprogooh,
        budgetIncludesFees: Boolean(prevLineItems[0]?.budgetIncludesFees),
      }
    )
    const merged = mergeProgOohStandardFromExpertWithPrevious(
      standard,
    ...
    form.setValue("lineItems", keyedMerged as ProgOOHFormValues["lineItems"], {
      shouldDirty: true,
  ...
  useEffect(() => {
    ...
    const stableItems = assignStableLineItemNumbers<any>(formLineItems, mbaNumber, MEDIA_TYPE_ID_CODES.progOOH);
    ...
    const transformedLineItems = stableItems.map((lineItem, index) => {
      ...
      return {
        ...
        environment: lineItem.environment || "",
    ...
    const lineItemTotals = watchedLineItems.map((lineItem, index) => {
      ...
      lineItem.bursts.forEach((burst) => {
        ...
        if (lineItem.budgetIncludesFees) {
          ...
          lineMedia += budget;
          const fee = feeprogooh ? (budget / (100 - feeprogooh)) * feeprogooh : 0;
          lineFee += fee;
        }
  ...
  }, [form, feeprogooh, onTotalMediaChange]);
  ...
  }, [feeprogooh, form, handleLineItemValueChange]);
  ...
  useEffect(() => {
    const fetchPublishers = async () => {
      try {
        ...
        const fetchedPublishers = await getPublishersForProgOoh();
...
useEffect(() => {
  ...
  const calculatedBursts = getProgOohBursts(form, feeprogooh || 0, mbaNumber);
  let burstIndex = 0;
...
}, [watchedLineItems, feeprogooh, form, mbaNumber, onLineItemsChange]);

  useEffect(() => {
    const timeoutId = setTimeout(() => {
      const investmentByMonth = calculateInvestmentPerMonth(form, feeprogooh || 0);
      const bursts = getProgOohBursts(form, feeprogooh || 0, mbaNumber);
  ...
  }, [watchedLineItems, feeprogooh, form, mbaNumber, onBurstsChange, onInvestmentChange]);
    ...
    return formLineItems.flatMap(item =>
      item.bursts.map(burst => {
        ...
        if (item.budgetIncludesFees && item.clientPaysForMedia) {
          ...
          // Media = 0
          // Fees = Budget * (Fee / 100)
          feeAmount = budget * ((feeprogooh || 0) / 100);
          mediaAmount = 0;
          ...
          feeAmount = (budget / (100 - (feeprogooh || 0))) * (feeprogooh || 0);
          mediaAmount = 0;
        } else {
            ...
            <div className="flex flex-col gap-3">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex min-w-0 flex-wrap items-center gap-2">
                  ...
                  {progoohExpertModalOpen ? (
                    <Badge
                      variant="outline"
                      className="border-2 text-[10px] font-semibold uppercase tracking-wider shadow-sm"
                      style={{
                        borderColor: rgbaFromHex(PROG_OOH_MEDIA_HEX, 0.55),
                        backgroundColor: rgbaFromHex(PROG_OOH_MEDIA_HEX, 0.14),
                ...
                <div
                  role="group"
                  aria-label="Prog OOH Media entry mode"
                ...
                >
                  <button
                    ...
                    style={
                      !progoohExpertModalOpen
                        ? { backgroundColor: PROG_OOH_MEDIA_HEX }
                        : undefined
                    }
                    onClick={() => {
                  ...
                  <button
                    ...
                    style={{
                      ...(progoohExpertModalOpen
                        ? { backgroundColor: PROG_OOH_MEDIA_HEX }
                        : {}),
                      ...(expertSegmentAttention && !progoohExpertModalOpen
                        ? {
                            boxShadow: `0 0 0 2px ${rgbaFromHex(PROG_OOH_MEDIA_HEX, 0.45)}`,
                          }
                        : {}),
                    }}
                    onClick={() => {
          ...
          <CardContent className="space-y-0">
            ...
            <div className="flex items-center justify-between pt-3 mt-1 border-t-2 border-primary/20">
              ...
              <div className="flex items-center gap-6 text-sm font-semibold tabular-nums">
                ...
                <div className="text-right">
                  <span className="text-[11px] text-muted-foreground font-normal block">Fee ({feeprogooh}%)</span>
                    ...
                    <Card key={field.id} className="overflow-hidden border border-border/50 shadow-sm hover:shadow-md transition-shadow duration-200 space-y-6">
                              ...
                              <span className="text-sm font-bold tabular-nums">
                                {formatMoney(
                                  form.getValues(`lineItems.${lineItemIndex}.budgetIncludesFees`)
                                    ? totalMedia
                                    : totalMedia + (totalMedia / (100 - (feeprogooh || 0))) * (feeprogooh || 0),
                            ...
                            <Button
                              ...
                              aria-label={
                                collapsedLineItems.has(lineItemIndex)
                                  ? `Expand details for prog OOH line item ${lineItemIndex + 1}`
                                  : `Collapse details for prog OOH line item ${lineItemIndex + 1}`
                              }
                      ...
                      <div className={MP_BURST_SECTION_OUTER}>
                        <div className={MP_BURST_HEADER_SHELL}>
                          <div className={MP_BURST_HEADER_INNER}>
                            ...
                            <div className={MP_BURST_HEADER_ROW}>
                              <div
                                className={`${shouldShowAdServingOverrideInput(form.watch(`lineItems.${lineItemIndex}.buyType`)) ? AD_SERVING_OVERRIDE_BURST_GRID : MP_BURST_GRID_7} text-[11px] font-semibold uppercase tracking-wider text-muted-foreground`}
                              >
                                <span>Budget</span>
                        ...
                        {form.watch(`lineItems.${lineItemIndex}.bursts`, []).map((burstField, burstIndex) => {
                          ...
                          return (
                            <Card key={burstField._reactKey ?? `${lineItemIndex}-${burstIndex}`} className={MP_BURST_CARD}>
...
<FormItem>
  <FormControl>
                                    ...
                                    <Input
                                        ...
                                        value={formatMoney(
                                          form.getValues(`lineItems.${lineItemIndex}.budgetIncludesFees`)
                                            ? (parseFloat(form.getValues(`lineItems.${lineItemIndex}.bursts.${burstIndex}.budget`)?.replace(/[^0-9.]/g, "") || "0") / 100) * (100 - (feeprogooh || 0))
                                    ...
                                    <Input
                                        ...
                                        value={formatMoney(
                                          form.getValues(`lineItems.${lineItemIndex}.budgetIncludesFees`)
                                            ? (parseFloat(form.getValues(`lineItems.${lineItemIndex}.bursts.${burstIndex}.budget`)?.replace(/[^0-9.]/g, "") || "0") / 100) * (feeprogooh || 0)
                                            : (parseFloat(form.getValues(`lineItems.${lineItemIndex}.bursts.${burstIndex}.budget`)?.replace(/[^0-9.]/g, "") || "0") / (100 - (feeprogooh || 0))) * (feeprogooh || 0)
                                        , { locale: "en-AU", currency: "AUD" })}
                                        readOnly
                                      />
                                  </div>
      ...
      <Dialog
        open={progoohExpertModalOpen}
        onOpenChange={handleProgOohExpertModalOpenChange}
      >
        <DialogContent className="max-w-[95vw] w-[95vw] max-h-[95vh] h-[95vh] flex flex-col p-4 gap-0 overflow-hidden">
          <DialogHeader className="flex-shrink-0 pb-2">
            <DialogTitle>Prog OOH Media Expert Mode</DialogTitle>
          </DialogHeader>
      ...
      <Dialog
        open={progoohExpertExitConfirmOpen}
        onOpenChange={(open) => {
          if (!open) dismissProgOohExpertExitConfirm()
        }}
      >
        <DialogContent
          ...
          onClick={(e) => {
            if (
              (e.target as HTMLElement).closest(
                "[data-progoohexpert-exit-yes]"
              )
            ) {
              return
            }
            dismissProgOohExpertExitConfirm()
          }}
        >
          <DialogHeader>
            <DialogTitle>Leave Prog OOH Media Expert Mode?</DialogTitle>
            <DialogDescription>
              You have unsaved changes in Expert Mode. Apply saves them to the
              Prog OOH Media section; leaving now discards those edits.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
              type="button"
              variant="outline"
              onClick={dismissProgOohExpertExitConfirm}
            >
              No, keep editing
            </Button>
            <Button
              ...
              variant="destructive"
              data-progoohexpert-exit-yes
              onClick={confirmProgOohExpertExitWithoutSaving}
            >
