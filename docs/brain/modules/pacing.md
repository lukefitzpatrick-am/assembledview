...
- `lib/data/readPacing.ts` — pacing-owned Xano reads: `media_plan_master`, `media_plan_versions`, `pacing_orphan_fixes`. Channel `media_plan_*` line GETs remain direct Xano until T2e. Snapshot sync (`fetchAllLineItems` → `XANO_LINE_ITEMS_SNAPSHOT`) frozen on Xano until T6.
...
- **Security:** the 4 per-channel POST routes (prog display/video, social meta/tiktok) have no authz gate (SEC-7).
