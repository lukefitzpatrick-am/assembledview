...
- `components/kpis/{KPIEditModal,KPISection,kpiHost}` — shared modal + the `KpiHost` contract with two persistence semantics: the media-plan host persists `campaign_kpi` immediately when the plan has an identity (create defers until the first save in session); the pacing host syncs immediately. Plan save still runs its own sync for the version it just wrote (including VP-1 increments).
