...
const STORAGE_KEY = "dashboard.pacing.platform-expansion.v1"
    ...
    <section className="rounded-2xl border border-border/60 bg-card p-4 md:p-6">
      <header className="flex flex-wrap items-start justify-between gap-3 border-b border-border/50 pb-4">
        <div className="flex min-w-0 flex-1 flex-wrap items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-semibold tracking-tight text-foreground">Live Pacing</h3>
          </div>
        ...
        <Button
          ...
          onClick={onRefresh}
          disabled={isLoading}
          aria-label="Refresh pacing data"
        >
      ...
      {isLoading ? (
        <div className="mt-6 space-y-4">
          ...
          <p className="text-sm text-muted-foreground">Fetching latest pacing data...</p>
        </div>
      ) : null}
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          {(
            [
              ...
              ["Average pacing", `${summary.avgPacingPct.toFixed(1)}%`],
      ...
      {!isLoading && error ? (
        <div className="mt-6 rounded-2xl border border-rose-300/40 bg-rose-500/5 p-4">
          <p className="text-sm font-medium text-rose-700">Unable to load live pacing data.</p>
          <p className="mt-1 text-sm text-rose-700/80">{error.message || "Unknown pacing error"}</p>
      ...
      {isEmpty ? (
        <div className="mt-6 flex flex-col items-center gap-3 rounded-2xl border border-dashed border-border/70 bg-muted/10 p-8 text-center">
          ...
          <p className="text-sm font-medium text-foreground">No active line items with pacing data</p>
