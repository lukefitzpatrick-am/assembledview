...
function line(bursts: any[]): StandardMagazineFormLineItem {
  return {
    network: "N", title: "T", buyType: "bonus", size: "", publisher: "Pub", placement: "", buyingDemo: "", market: "",
...
test("sub-week interior burst imports as day-detail and round-trips", () => {
  ...
  const rows = mapStandardMagazineLineItemsToExpertRows(std, cols, CS, CE)
  const wk0 = cols[0]!
  assert.equal(weekHasDailyValues(rows[0]!.dailyValues ?? {}, weekDayKeys(wk0, CS, CE)), true)
  const back = mapMagazineExpertRowsToStandardLineItems(rows, cols, CS, CE, { feePctMagazines: 0 })
