/**
 * F-28 Phase 2 — Cinema Expert row virtualization smoke (Prompt A items 1,3,5,7,8).
 */
import assert from "node:assert/strict"
import test from "node:test"
import { format, startOfDay } from "date-fns"

import { mapCinemaExpertRowsToStandardLineItems } from "@/lib/mediaplan/expertChannelMappings"
import {
  reorderExpertRows,
  mergedSpanWidthPx,
...
const CINEMA_EXPERT_ROW_HEIGHT_PX = OOH_EXPERT_ROW_HEIGHT_PX
const CINEMA_EXPERT_ROW_OVERSCAN = EXPERT_GRID_ROW_OVERSCAN_DEFAULT
...
function emptyRow(id: string): CinemaExpertScheduleRow {
...
function buildRows(n: number): CinemaExpertScheduleRow[] {
  return Array.from({ length: n }, (_, i) => emptyRow(`cinema-v-${i}`))
}

/** Mirrors CinemaExpertGrid containerTotals over the FULL logical array. */
function computeCinemaWeeklyTotals(
  rows: readonly CinemaExpertScheduleRow[],
  feePct: number
) {
  let sumNet = 0
  let sumFee = 0
...
test("3. DRAG REORDER across virtual boundary lands on correct logical index", () => {
  ...
  const dropIndex = virtualRowIndexFromOffsetY(
    280 * CINEMA_EXPERT_ROW_HEIGHT_PX + 1,
    CINEMA_EXPERT_ROW_HEIGHT_PX,
    300
  )
...
test("5. WEEK-COLUMN WIDTH respects resize overrides (sticky header/week resize path)", () => {
  ...
  assert.equal(CINEMA_EXPERT_ROW_HEIGHT_PX, 41)
})
...
test("7. WEEKLY TOTALS derived from full 300-row state", () => {
  ...
  const totals = computeCinemaWeeklyTotals(rows, 0)
  ...
  const after = computeCinemaWeeklyTotals(edited, 0)
...
test("8. PERF: mounted DOM window stays bounded while scrolling 300 rows", () => {
  ...
  const nearTop = expectedMountedRowRange(
    ...
    CINEMA_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    CINEMA_EXPERT_ROW_OVERSCAN
  )
  const mid = expectedMountedRowRange(
    150 * CINEMA_EXPERT_ROW_HEIGHT_PX,
    viewportHeight,
    CINEMA_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    CINEMA_EXPERT_ROW_OVERSCAN
  ...
  const nearBottom = expectedMountedRowRange(
    280 * CINEMA_EXPERT_ROW_HEIGHT_PX,
    viewportHeight,
    CINEMA_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    CINEMA_EXPERT_ROW_OVERSCAN
  ...
  const maxWindow =
    Math.ceil(viewportHeight / CINEMA_EXPERT_ROW_HEIGHT_PX) +
    2 * CINEMA_EXPERT_ROW_OVERSCAN +
    2
