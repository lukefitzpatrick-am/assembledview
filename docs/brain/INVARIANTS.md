...
- **Manual billing status (MB-9 / MB-21):** vocabulary is Manual / Media prepaid / Prepaid / Fee adjusted / Client pays (`manualBillingVocabulary.ts`), with provenance axis **· saved** | **· unsaved** (and **· unsaved · differs from saved** when pending contradicts table). Pending carrier → unsaved; table-only → saved. `Matches MBA · unsaved` when pending exists — reconciliation is not persistence. Divergence Acknowledge banner fires only for unintended cases (`isUnintendedBillingDivergence`: line total / adserving total / stranded line) — never for deliberate reconciling month redistribution.
...
- **MBA billing override row precedence (MB-20):** `pendingBillingOverrideRows` (Applied, unsaved) > `billingOverrideRowsForPanels` (saved table) > computed auto. Pending is derived only via `layerDraftMonthsOntoOverrideRows` (`buildPendingBillingOverrideRows`). Do not invent a second draft→rows path. Apply still persists until MB-23; Cancel / X / Escape discard pending; Done keeps it.
