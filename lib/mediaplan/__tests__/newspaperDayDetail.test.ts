...
function line(bursts: any[]): StandardNewspaperFormLineItem {
  return {
    network: "N", publisher: "Pub", title: "T", buyType: "bonus", size: "", format: "", placement: "", buyingDemo: "", market: "",
...
test("sub-week interior burst imports as day-detail and round-trips", () => {
  ...
  const rows = mapStandardNewspaperLineItemsToExpertRows(std, cols, CS, CE)
  const wk0 = cols[0]!
  assert.equal(weekHasDailyValues(rows[0]!.dailyValues ?? {}, weekDayKeys(wk0, CS, CE)), true)
  const back = mapNewspaperExpertRowsToStandardLineItems(rows, cols, CS, CE, { feePctNewspaper: 0 })
