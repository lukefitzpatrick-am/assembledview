...
import { getPublishersForProgDisplay, getClientInfo } from "@/lib/api"
...
import {
  ProgDisplayExpertGrid,
  createEmptyProgDisplayExpertRow,
} from "@/components/media-containers/ProgDisplayExpertGrid"
...
const PROG_DISPLAY_MEDIA_HEX = getMediaTypeThemeHex("progdisplay")
...
/** Net media for deliverable math when budget is gross (fee as markup on net). Matches overallTotals. */
...
const progDisplayFormSchema = z.object({
  lineItems: z.array(lineItemSchema),
  overallDeliverables: z.number().optional(),
})

type ProgDisplayFormValues = z.infer<typeof progDisplayFormSchema>
...
export function calculateInvestmentPerMonth(form, feeprogdisplay) {
  const lineItems = form.getValues("lineItems") || [];
  ...
  lineItems.forEach((lineItem) => {
    lineItem.bursts.forEach((burst) => {
      ...
      const feePercentage = feeprogdisplay || 0;
...
export function calculateBurstInvestmentPerMonth(form, feeprogdisplay) {
  const lineItems = form.getValues("lineItems") || [];
  let monthlyInvestment: Record<string, number> = {};
...
export default function ProgDisplayContainer({
  ...
  const progdisplayStandardBaselineRef = useRef("")
  const [expertProgDisplayRows, setExpertProgDisplayRows] = useState<
    ProgDisplayExpertScheduleRow[]
  >([])
  const [progdisplayExpertModalOpen, setProgDisplayExpertModalOpen] =
    useState(false)
  const [progdisplayExpertExitConfirmOpen, setProgDisplayExpertExitConfirmOpen] =
    useState(false)
  ...
  const progdisplayExpertWeekColumns = useMemo(
    () => buildWeeklyGanttColumnsFromCampaign(campaignStartDate, campaignEndDate),
    [campaignStartDate, campaignEndDate]
  )

  useLayoutEffect(() => {
    progdisplayStandardBaselineRef.current =
      serializeProgDisplayStandardLineItemsBaseline(
        form.getValues("lineItems") as StandardProgDisplayFormLineItem[]
      )
  }, [form])
  ...
  const handleExpertProgDisplayRowsChange = useCallback(
    (next: ProgDisplayExpertScheduleRow[]) => {
      setExpertProgDisplayRows(next)
    },
    []
  )

  const openProgDisplayExpertModal = useCallback(() => {
    const mapped = mapStandardProgDisplayLineItemsToExpertRows(
      (form.getValues("lineItems") || []) as StandardProgDisplayFormLineItem[],
      progdisplayExpertWeekColumns,
      campaignStartDate,
      campaignEndDate
    )
    const weekKeys = progdisplayExpertWeekColumns.map((c) => c.weekKey)
    const rows: ProgDisplayExpertScheduleRow[] =
      mapped.length > 0
        ? mapped
        : [
            createEmptyProgDisplayExpertRow(
    ...
    progdisplayExpertRowsBaselineRef.current =
      serializeProgDisplayExpertRowsBaseline(rows)
    setExpertProgDisplayRows(rows)
  ...
  const confirmProgDisplayExpertExitWithoutSaving = useCallback(() => {
    setProgDisplayExpertExitConfirmOpen(false)
    setProgDisplayExpertModalOpen(false)
  }, [])

  const handleProgDisplayExpertModalOpenChange = useCallback(
    (open: boolean) => {
      if (open) {
        setProgDisplayExpertModalOpen(true)
        return
      ...
      const dirty =
        serializeProgDisplayExpertRowsBaseline(expertProgDisplayRows) !==
        progdisplayExpertRowsBaselineRef.current
      if (!dirty) {
        setProgDisplayExpertModalOpen(false)
        return
      }
      setProgDisplayExpertExitConfirmOpen(true)
    },
    [expertProgDisplayRows]
  )

  const handleProgDisplayExpertApply = useCallback(() => {
    ...
    const standard = mapProgDisplayExpertRowsToStandardLineItems(
      ...
      campaignEndDate,
      {
        feePctProgDisplay: feeprogdisplay,
        budgetIncludesFees: Boolean(prevLineItems[0]?.budgetIncludesFees),
      }
    )
    const merged = mergeProgDisplayStandardFromExpertWithPrevious(
      standard,
      prevLineItems as StandardProgDisplayFormLineItem[]
    )
    form.setValue("lineItems", merged as ProgDisplayFormValues["lineItems"], {
      shouldDirty: true,
  ...
  useEffect(() => {
    if (progdisplayExpertModalOpenRef.current) return
    ...
    const transformedLineItems = formLineItems.map((lineItem, index) => {
      ...
      lineItem.bursts.forEach((burst) => {
        ...
        if (lineItem.budgetIncludesFees) {
          ...
          const base = budget / (1 + (feeprogdisplay || 0) / 100);
          totalMedia += base;
        } else {
          // Budget is net media
          totalMedia += budget;
  ...
  }, [watchedLineItems, mbaNumber, feeprogdisplay, form, onMediaLineItemsChange]);
    ...
    const lineItemTotals = watchedLineItems.map((lineItem, index) => {
      ...
      lineItem.bursts.forEach((burst) => {
        ...
        if (lineItem.budgetIncludesFees) {
          ...
          lineMedia += budget;
          const fee = feeprogdisplay ? (budget / (100 - feeprogdisplay)) * feeprogdisplay : 0;
          lineFee += fee;
        }
  ...
  }, [watchedLineItems, feeprogdisplay]);
  ...
  }, [form, feeprogdisplay, onTotalMediaChange]);
  ...
  const handleValueChange = useCallback((lineItemIndex: number, burstIndex: number, budgetIncludesFeesOverride?: boolean) => {
    ...
    const budget = netMediaFeeMarkup(rawBudget, budgetIncludesFees, feeprogdisplay || 0);
  ...
  }, [feeprogdisplay, form, handleLineItemValueChange]);
  ...
  useEffect(() => {
    const fetchPublishers = async () => {
      try {
        ...
        const fetchedPublishers = await getPublishersForProgDisplay();
...
useEffect(() => {
  ...
  const items: LineItem[] = form.getValues('lineItems').flatMap((lineItem, lineItemIndex) =>
    lineItem.bursts.map(burst => {
      ...
      const lineItemId = buildLineItemId(mbaNumber, MEDIA_TYPE_ID_CODES.progDisplay, lineItemIndex + 1);
...
}, [watchedLineItems, feeprogdisplay, form, mbaNumber, onLineItemsChange]);
  ...
  }, [watchedLineItems, feeprogdisplay, form, onBurstsChange, onInvestmentChange]);
    ...
    return formLineItems.flatMap(item =>
      item.bursts.map(burst => {
        ...
        if (item.budgetIncludesFees && item.clientPaysForMedia) {
          ...
          // Media = 0
          // Fees = Budget * (Fee / 100)
          feeAmount = budget * ((feeprogdisplay || 0) / 100);
          mediaAmount = 0;
          ...
          feeAmount = (budget / (100 - (feeprogdisplay || 0))) * (feeprogdisplay || 0);
          mediaAmount = 0;
        } else {
            ...
            <div className="flex flex-col gap-3">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex min-w-0 flex-wrap items-center gap-2">
                  ...
                  {progdisplayExpertModalOpen ? (
                    <Badge
                      variant="outline"
                      className="border-2 text-[10px] font-semibold uppercase tracking-wider shadow-sm"
                      style={{
                        borderColor: rgbaFromHex(PROG_DISPLAY_MEDIA_HEX, 0.55),
                        backgroundColor: rgbaFromHex(PROG_DISPLAY_MEDIA_HEX, 0.14),
                ...
                <div
                  role="group"
                  aria-label="Prog Display Media entry mode"
                ...
                >
                  <button
                    ...
                    style={
                      !progdisplayExpertModalOpen
                        ? { backgroundColor: PROG_DISPLAY_MEDIA_HEX }
                        : undefined
                    }
                    onClick={() => {
                  ...
                  <button
                    ...
                    style={{
                      ...(progdisplayExpertModalOpen
                        ? { backgroundColor: PROG_DISPLAY_MEDIA_HEX }
                        : {}),
                      ...(expertSegmentAttention && !progdisplayExpertModalOpen
                        ? {
                            boxShadow: `0 0 0 2px ${rgbaFromHex(PROG_DISPLAY_MEDIA_HEX, 0.45)}`,
                          }
                        : {}),
                    }}
                    onClick={() => {
          ...
          <CardContent className="space-y-0">
            ...
            <div className="flex items-center justify-between pt-3 mt-1 border-t-2 border-primary/20">
              ...
              <div className="flex items-center gap-6 text-sm font-semibold tabular-nums">
                ...
                <div className="text-right">
                  <span className="text-[11px] text-muted-foreground font-normal block">Fee ({feeprogdisplay}%)</span>
            ...
            <Form {...form}>
              <div className="space-y-6">
                {lineItemFields.map((field, lineItemIndex) => {
                  const sectionId = `progdisplay-line-item-${lineItemIndex}`;
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <span className="font-mono text-[11px] text-muted-foreground">{`${mbaNumber}PD${lineItemIndex + 1}`}</span>
                            </div>
                          </div>
                          <div className="flex items-center gap-3">
                            <div className="text-right">
...
<FormItem>
  <FormControl>
      ...
      <Dialog
        open={progdisplayExpertModalOpen}
        onOpenChange={handleProgDisplayExpertModalOpenChange}
      >
        <DialogContent className="max-w-[95vw] w-[95vw] max-h-[95vh] h-[95vh] flex flex-col p-4 gap-0 overflow-hidden">
          <DialogHeader className="flex-shrink-0 pb-2">
            <DialogTitle>Prog Display Media Expert Mode</DialogTitle>
          </DialogHeader>
          ...
          <DialogFooter className="flex-shrink-0 border-t pt-3 mt-2">
            <Button type="button" onClick={handleProgDisplayExpertApply}>
              Apply
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog
        open={progdisplayExpertExitConfirmOpen}
        onOpenChange={(open) => {
          if (!open) dismissProgDisplayExpertExitConfirm()
        }}
      >
        <DialogContent
          ...
          onClick={(e) => {
            if (
              (e.target as HTMLElement).closest(
                "[data-progdisplayexpert-exit-yes]"
              )
            ) {
              return
            }
            dismissProgDisplayExpertExitConfirm()
          }}
        >
          <DialogHeader>
            <DialogTitle>Leave Prog Display Media Expert Mode?</DialogTitle>
            <DialogDescription>
              You have unsaved changes in Expert Mode. Apply saves them to the
              Prog Display Media section; leaving now discards those edits.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
              type="button"
              variant="outline"
              onClick={dismissProgDisplayExpertExitConfirm}
            >
              No, keep editing
            </Button>
            <Button
              ...
              variant="destructive"
              data-progdisplayexpert-exit-yes
              onClick={confirmProgDisplayExpertExitWithoutSaving}
            >
