...
type OohCopiedCells = {
  ...
  sourceRows: number
  sourceCols: number
  selection: OohMultiCellSelection
}

function weeklyCellDisplayValue(
  cellValue: number | string | undefined,
  mergedSpan: OohExpertMergedWeekSpan | null
): string {
  if (mergedSpan) {
...
const OOH_WEEK_CELL_VISUAL_CLASSES = {
...
function selectionOverlapsMergedSpan(
  rowMergeMap: OohRowMergeMap | undefined,
...
function normalizeOohWeekRect(
  row0: number,
  wk0: string,
  row1: number,
  wk1: string,
  weekKeys: readonly string[]
): OohWeekRectSelection {
...
function oohWeekCellInRect(
  rowIndex: number,
  weekKey: string,
  rect: OohWeekRectSelection | null,
  weekKeys: readonly string[]
): boolean {
...
function coerceWeeklySelectionBounds(
  ...
  weekRectSelection: OohWeekRectSelection | null,
  ...
  if (
    weekRectSelection &&
    oohWeekCellInRect(anchorRow, anchorWeekKey, weekRectSelection, weekKeys)
  ) {
...
function oohWeekRangeOutlineFlags(
  rowIndex: number,
  weekIndex: number,
  rect: OohWeekRectSelection | null,
...
function oohWeekOutlineEdgeClasses(flags: {
  inRange: boolean
  top: boolean
  bottom: boolean
  left: boolean
...
function oohWeekCellInMergePulseHighlight(
  rowIndex: number,
  weekKey: string,
...
type OohWeekMergeSelectionNormalized = {
  rowIndex: number
 ...
 */
function normalizeOohWeekMergeSelection(
  rect: OohWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): OohWeekMergeSelectionNormalized | null {
...
function weekPlainClickPreservesWeekAreaSelection(
  ...
  const mergeNorm = normalizeOohWeekMergeSelection(rect, multi, weekKeys)
  if (
    mergeNorm &&
  ...
  if (rect && oohWeekCellInRect(rowIndex, weekKey, rect, weekKeys)) {
    const isSingleCell =
...
function deriveOohMergeEligibility(
  rect: OohWeekRectSelection | null,
  multi: { rowIndex: number; keys: string[] } | null,
  weekKeys: readonly string[]
): OohWeekMergeSelectionNormalized | null {
  return normalizeOohWeekMergeSelection(rect, multi, weekKeys)
}
...
function mergedWeekSpansAfterCutRect(
  spans: OohExpertMergedWeekSpan[] | undefined,
  wi0: number,
  wi1: number,
  weekKeys: readonly string[]
): OohExpertMergedWeekSpan[] {
...
type WeeklyExportSelection =
  | { kind: "rect"; rect: OohWeekRectSelection }
...
function resolveWeeklyExportSelection(
  ...
  weekKeys: readonly string[],
  rows: readonly OohExpertScheduleRow[] | null
...
function applyWeeklyCutToRows(
  ...
  switch (selection.kind) {
    case "rect": {
      ...
      if (wi0 < 0 || wi1 < 0) return null
      const next: OohExpertScheduleRow[] = rows.map((r) => ({
        ...r,
    ...
    case "mergeContiguous": {
      ...
      const next: OohExpertScheduleRow[] = rows.map((row) => ({
        ...row,
...
function normalizeOohNetworkPaste(raw: string, networkNames: string[]): string {
  const v = raw.trim()
  if (!v) return ""
...
const OOH_FORMAT_OPTIONS: ComboboxOption[] = [
...
const OOH_BUY_TYPE_OPTIONS: ComboboxOption[] = [
...
const OOH_DESCRIPTOR_CORE: readonly (keyof OohExpertScheduleRow)[] = [
  "startDate",
  "endDate",
  "network",
  "format",
...
const OOH_BILLING_FLAG_KEYS: readonly (keyof OohExpertScheduleRow)[] = [
  "fixedCostMedia",
  "clientPaysForMedia",
  "budgetIncludesFees",
]

const OOH_DESCRIPTOR_TAIL: readonly (keyof OohExpertScheduleRow)[] = ["unitRate"]
...
type FuzzyMatchField = "network" | "format" | "buyType"
...
type OohRowMergeSpanMeta = Readonly<{
  id: string
  startWeekKey: string
  endWeekKey: string
  totalQty: number
...
type OohRowMergeMap = Readonly<{
  ...
  spanMetaByAnchorWeekKey: Readonly<Record<string, OohRowMergeSpanMeta>>
}>

export function OohExpertGrid({
  campaignStartDate,
  campaignEndDate,
  feeooh,
  rows,
  ...
  const focusedCellRef = useRef<OohExpertFocusedCell | null>(null)
  ...
  const [showBillingCols, setShowBillingCols] = useState(true)
  ...
  const [weekRectSelection, setWeekRectSelection] =
    useState<OohWeekRectSelection | null>(null)
  // Multi-cell selection state for copy/paste operations.
  const [multiCellSelection, setMultiCellSelection] =
    useState<OohMultiCellSelection | null>(null)
  const [isSelecting, setIsSelecting] = useState(false)
  const [copiedCells, setCopiedCells] = useState<OohCopiedCells | null>(null)
  ...
  const lastDragRectDuringGestureRef = useRef<OohWeekRectSelection | null>(null)
  /**
   ...
   */
  const postDragWeekClickRectRef = useRef<OohWeekRectSelection | null>(null)
  const weekRectSelectionRef = useRef<OohWeekRectSelection | null>(null)
  ...
  const oohDescriptorKeys = useMemo(
    () =>
      [
        ...OOH_DESCRIPTOR_CORE,
        ...(showBillingCols ? [...OOH_BILLING_FLAG_KEYS] : []),
        ...OOH_DESCRIPTOR_TAIL,
      ] as (keyof OohExpertScheduleRow)[],
    [showBillingCols]
  )

  const descriptorColWidths = useMemo(() => {
    const core = [48, 48, 130, 130, 110, 120, 120, 96, 96, 96]
    const billing = [40, 40, 40]
    const tail = [88, 100, 76, 68]
    return showBillingCols ? [...core, ...billing, ...tail] : [...core, ...tail]
  }, [showBillingCols])
  ...
  useEffect(() => {
    ...
    console.debug("[OOH merge] pending selection updated", pendingMergeSelection)
  ...
  const tryFuzzyMatch = useCallback(
    (rowIndex: number, field: FuzzyMatchField, value: string) => {
      ...
      if (corrected) {
        updateRow(rowIndex, { [field]: corrected } as Partial<OohExpertScheduleRow>)
        return
      }
      if (field === "format") {
        const ex = exactCanonicalFormat(value)
        if (ex) {
          updateRow(rowIndex, { format: ex })
          return
        }
      }
      if (field === "buyType") {
        const ex = exactCanonicalBuyType(value)
        if (ex) {
          updateRow(rowIndex, { buyType: ex })
          return
        }
      }
      ...
      if (field === "network") {
        ...
        match = fuzzyMatchFormat(value)
      } else if (field === "buyType") {
        match = fuzzyMatchBuyType(value)
      }
      if (!match) return
      if (fuzzyMatchAutoApplyRef.current) {
        updateRow(rowIndex, { [field]: match.matched } as Partial<OohExpertScheduleRow>)
      } else {
  ...
  const navColCount =
    oohDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekKeys.length
  ...
  const clearWeekSelectionWhereMergedAnchorInvolved = useCallback(
    (rowIndex: number, weekKey: string) => {
      setWeekRectSelection((prev) =>
        prev && oohWeekCellInRect(rowIndex, weekKey, prev, weekKeys)
          ? null
          : prev
      )
  ...
  const rangeWeekMultiSelect = useCallback(
    (rowIndex: number, anchorKey: string, endKey: string) => {
      ...
      setWeekRectSelection(
        normalizeOohWeekRect(rowIndex, anchorKey, rowIndex, endKey, weekKeys)
      )
      setWeekStripSelection(null)
    },
    [weekKeys]
  ...
  const lockPendingMergeSelectionFromCurrentSelection = useCallback(() => {
    const n = deriveOohMergeEligibility(
  ...
  const derivedMergeTarget = useMemo(() => {
    const n = deriveOohMergeEligibility(weekRectSelection, weekMultiSelect, weekKeys)
  ...
  useEffect(() => {
    const endDrag = () => {
      ...
      if (DEBUG_OOH_MERGE) {
        console.debug("[OOH merge] drag end", {
          lastRect: r,
  ...
  const handleMergeSelectedWeeks = useCallback(() => {
    ...
    if (selectionOverlapsMergedSpan(rowMergeMaps[rowIndex], sorted, weekKeys)) {
      if (DEBUG_OOH_MERGE) {
        console.debug("[OOH merge] validation overlap failure", {
          rowIndex,
    ...
    if (DEBUG_OOH_MERGE) {
      console.debug("[OOH merge] merge applied", {
        rowIndex,
        spanId: newSpan.id,
  ...
  useEffect(() => {
    ...
    const cellId = expertGridCellId(
      ...
      oohDescriptorKeys.length + WEEK_GRID_COL_OFFSET + lastWi
    )
    document
      .getElementById(cellId)
      ?.scrollIntoView({ behavior: "smooth", block: "nearest", inline: "nearest" })
  }, [domGridId, mergeTarget, mergeWeeksReady, oohDescriptorKeys.length, weekKeys])

  useEffect(() => {
    if (!DEBUG_OOH_MERGE) return
    console.debug("[OOH merge] merge eligibility derived", {
      mergeTarget,
      derivedMergeTarget,
      mergeWeeksReady,
      weekRectSelection,
  ...
  const pasteMatrixIntoGrid = useCallback(
    (matrix: string[][]) => {
      ...
      if (fcIsWeek) {
        ...
        const nextRows: OohExpertScheduleRow[] = normalizedRows.map((r) => ({
          ...r,
      ...
      for (let dr = 0; dr < working.length; dr++) {
        ...
        for (let dc = 0; dc < pasteRow.length; dc++) {
          ...
          const field = target.field as keyof OohExpertScheduleRow
          ...
          if (field === "unitRate") {
            ...
            nextRows[targetRow] = {
              ...cur,
              [field]: truthy,
            } as OohExpertScheduleRow
            applied += 1
          } else if (field === "buyType") {
            nextRows[targetRow] = {
              ...cur,
              buyType: normalizeOohBuyTypePaste(raw),
            }
            applied += 1
          } else if (field === "format") {
            nextRows[targetRow] = {
              ...cur,
              format: normalizeOohFormatPaste(raw),
            }
            applied += 1
          } else if (field === "network") {
            nextRows[targetRow] = {
              ...cur,
              network: normalizeOohNetworkPaste(raw, networkNames),
            }
            applied += 1
          } else {
            const v = raw.trim()
            nextRows[targetRow] = { ...cur, [field]: v } as OohExpertScheduleRow
            applied += 1
          }
        }
      }
  ...
  const containerTotals = useMemo(() => {
    ...
    const fee =
      feeooh > 0 && feeooh < 100 ? (sumGross * feeooh) / (100 - feeooh) : 0
    const totalWithFee = sumGross + fee

    return { sumGross, sumQty, perWeek, fee, totalWithFee }
  }, [feeooh, normalizedRows, weekKeys])
  ...
  const colIndexOf = useCallback(
    (key: keyof OohExpertScheduleRow) => oohDescriptorKeys.indexOf(key),
    [oohDescriptorKeys]
  )
      ...
      <Card className="relative overflow-hidden border-0 shadow-md">
                ...
                <table className="w-max min-w-full border-collapse text-sm">
                  ...
                  <tbody>
                    {normalizedRows.map((row, rowIndex) => {
                      ...
                      const stripeStyle =
                        rowIndex % 2 === 0
                          ...
                          : undefined
                      const grossCol = oohDescriptorKeys.length
                      const actionsCol = oohDescriptorKeys.length + 1
                      const sigmaCol = oohDescriptorKeys.length + 2
                      ...
                      return (
                        ...
                        >
                          ...
                          <td
                            className={stickyTd(cType)}
                            style={stickyStyleBody(cType)}
                          >
                            <Input
                              id={expertGridCellId(
                                domGridId,
                              ...
                              value={row.type}
                              onFocus={() =>
                                handleCellFocus(rowIndex, "type")
                              }
                              onKeyDown={(e) =>
                                handleGridInputKeyDown(rowIndex, cType, e)
                              }
                              onChange={(e) =>
                                updateRow(rowIndex, { type: e.target.value })
                              }
                            />
                          </td>
                          <td
                            ...
                            for (let wi = 0; wi < weekColumns.length; wi += 1) {
                              ...
                              const stripOutlineRow =
                                ...
                                weekStripSelection.rowIndex === rowIndex
                                  ...
                                  : null
                              const rangeOutline = oohWeekRangeOutlineFlags(
                                rowIndex,
                                wi,
                                weekRectSelection,
                                stripOutlineRow,
                              ...
                              const inMergePulseHighlight = spanKeys.some(
                                (key) =>
                                  oohWeekCellInMergePulseHighlight(
                                    rowIndex,
                                    key,
                              ...
                              const tdClassName = cn(
                                ...
                                isPopulatedNonMergedCell &&
                                  OOH_WEEK_CELL_VISUAL_CLASSES.populatedSingleTd,
                                ...
                                !isMergedAnchorCell &&
                                  mergeReadyOnCell &&
                                  "ooh-expert-week-cell--merge-ready z-[6] bg-amber-500/24 ring-2 ring-inset ring-amber-600/75 shadow-sm dark:bg-amber-500/20 dark:ring-amber-400/70",
                                ...
                                isDragDropTargetCell &&
                                  !isDragDropTargetValid &&
                                  "cursor-not-allowed",
                                oohWeekOutlineEdgeClasses(rangeOutline),
                                !isMergedAnchorCell &&
                              ...
                              const mergedAnchorWrapperClassName = cn(
                                ...
                                isMergedAnchorCell &&
                                  OOH_WEEK_CELL_VISUAL_CLASSES.mergedSurface,
                              ...
                              const inputClassName = cn(
                                ...
                                isMergedAnchorCell
                                  ? OOH_WEEK_CELL_VISUAL_CLASSES.mergedInput
                              ...
                              renderedWeekCells.push(
                                <td
                                  ...
                                  onMouseEnter={(e) => {
                                    ...
                                    const next = normalizeOohWeekRect(
                                      drag.rowIndex,
                                      drag.weekKey,
                                      rowIndex,
                                      col.weekKey,
                                    ...
                                    setWeekStripSelection(null)
                                    if (DEBUG_OOH_MERGE) {
                                      console.debug("[OOH merge] drag update", {
                                        rect: next,
                                      })
                                    }
                                  }}
                                ...
                                >
                                  ...
                                  >
                                    <Input
                                      ...
                                      setIsSelecting(true)
                                      if (DEBUG_OOH_MERGE) {
                                        console.debug("[OOH merge] drag start", {
                                          rowIndex,
                                          weekKey: col.weekKey,
                                        })
                                      }
                                      ...
                                      if (shift && lastWeekAnchorRef.current) {
                                        ...
                                        if (
                                          ...
                                          const rect = normalizeOohWeekRect(
                                            lastWeekAnchorRef.current.rowIndex,
                                            lastWeekAnchorRef.current.weekKey,
                                            rowIndex,
                                            col.weekKey,
                    ...
                    >
                      ...
                      <td
                        className={cn(
                          stickyTd(oohDescriptorKeys.length + 2),
        ...
        {DEBUG_OOH_MERGE ? (
          <div className="pointer-events-none absolute bottom-2 right-2 z-50 max-w-[360px] rounded-md border bg-background/95 p-2 text-[11px] leading-snug shadow-lg">
            <div className="mb-1 font-medium text-foreground">OOH Merge Debug</div>
