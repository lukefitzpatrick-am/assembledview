...
export default function BVODContainer({
  ...
  // Form initialization
  const form = useForm<BVODFormValues>({
    ...
    const lineItemTotals = watchedLineItems.map((lineItem, index) => {
      ...
      lineItem.bursts.forEach((burst) => {
        ...
        if (lineItem.budgetIncludesFees) {
          lineFee += (budget / 100) * (feebvod || 0);
          lineMedia += (budget / 100) * (100 - (feebvod || 0));
        } else {
          lineMedia += budget;
  ...
  const handleValueChange = useCallback((lineItemIndex: number, burstIndex: number) => {
    ...
    if (form.getValues(`bvodlineItems.${lineItemIndex}.bursts.${burstIndex}.calculatedValue`) !== calculatedValue) {
...
useEffect(() => {
  ...
  const items: LineItem[] = form.getValues('bvodlineItems').flatMap(lineItem =>
    lineItem.bursts.map(burst => ({
  ...
  }, [watchedLineItems, feebvod, onInvestmentChange, onBurstsChange, onTotalMediaChange, form]);
    ...
    return formLineItems.flatMap(item =>
      item.bursts.map(burst => {
        ...
        if (item.budgetIncludesFees) {
          // budget was gross (media+fee)
          const base = budget / (1 + (feebvod || 0)/100);
                    ...
                    <Card key={field.id} className="space-y-6">
                      ...
                      <div className="px-6 py-2 border-b">
                        <div className="grid grid-cols-4 gap-4 text-sm">
                          ...
                          <div>
                            <span className="font-medium">Buy Type:</span> {form.watch(`bvodlineItems.${lineItemIndex}.buyType`) || 'Not selected'}
                          </div>
                          <div>
                                    ...
                                    <FormField
                                      ...
                                      render={({ field }) => {
                                        ...
                                        const calculatedValue = useMemo(() => {
                                          const budget = parseFloat(form.getValues(`bvodlineItems.${lineItemIndex}.bursts.${burstIndex}.budget`)?.replace(/[^0-9.]/g, "") || "0");
                                          const buyAmount = parseFloat(form.getValues(`bvodlineItems.${lineItemIndex}.bursts.${burstIndex}.buyAmount`)?.replace(/[^0-9.]/g, "") || "1");
                                          ...
                                          form.getValues(`bvodlineItems.${lineItemIndex}.bursts.${burstIndex}.budget`),
                                          form.getValues(`bvodlineItems.${lineItemIndex}.bursts.${burstIndex}.buyAmount`),
                                          buyType
                                        ]);
