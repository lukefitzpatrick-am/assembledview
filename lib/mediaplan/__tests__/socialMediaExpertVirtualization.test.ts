/**
 * F-28 Phase 2 — Social Media Expert row virtualization smoke (Prompt A items 1,3,5,7,8).
 */
import assert from "node:assert/strict"
import test from "node:test"
import { format, startOfDay } from "date-fns"

import { mapSocialMediaExpertRowsToStandardLineItems } from "@/lib/mediaplan/expertChannelMappings"
import {
  reorderExpertRows,
  mergedSpanWidthPx,
...
function emptyRow(id: string): SocialMediaExpertScheduleRow {
...
function buildRows(n: number): SocialMediaExpertScheduleRow[] {
  return Array.from({ length: n }, (_, i) => emptyRow(`socialmedia-v-${i}`))
}

/** Mirrors SocialMediaExpertGrid containerTotals over the FULL logical array. */
function computeSocialMediaWeeklyTotals(
  rows: readonly SocialMediaExpertScheduleRow[],
  feePct: number
) {
  let sumNet = 0
  let sumFee = 0
...
test("3. DRAG REORDER across virtual boundary lands on correct logical index", () => {
  ...
  const dropIndex = virtualRowIndexFromOffsetY(
    280 * SOCIALMEDIA_EXPERT_ROW_HEIGHT_PX + 1,
    SOCIALMEDIA_EXPERT_ROW_HEIGHT_PX,
    300
  )
...
test("5. WEEK-COLUMN WIDTH respects resize overrides (sticky header/week resize path)", () => {
  ...
  assert.equal(SOCIALMEDIA_EXPERT_ROW_HEIGHT_PX, 41)
})
...
test("7. WEEKLY TOTALS derived from full 300-row state", () => {
  ...
  const totals = computeSocialMediaWeeklyTotals(rows, 0)
  ...
  const after = computeSocialMediaWeeklyTotals(edited, 0)
...
test("8. PERF: mounted DOM window stays bounded while scrolling 300 rows", () => {
  ...
  const nearTop = expectedMountedRowRange(
    ...
    SOCIALMEDIA_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    SOCIALMEDIA_EXPERT_ROW_OVERSCAN
  )
  const mid = expectedMountedRowRange(
    150 * SOCIALMEDIA_EXPERT_ROW_HEIGHT_PX,
    viewportHeight,
    SOCIALMEDIA_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    SOCIALMEDIA_EXPERT_ROW_OVERSCAN
  ...
  const nearBottom = expectedMountedRowRange(
    280 * SOCIALMEDIA_EXPERT_ROW_HEIGHT_PX,
    viewportHeight,
    SOCIALMEDIA_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    SOCIALMEDIA_EXPERT_ROW_OVERSCAN
  ...
  const maxWindow =
    Math.ceil(viewportHeight / SOCIALMEDIA_EXPERT_ROW_HEIGHT_PX) +
    2 * SOCIALMEDIA_EXPERT_ROW_OVERSCAN +
    2
