...
import {
  ...
  WEEK_COL_WIDTH_PX as TV_EXPERT_WEEK_COL_WIDTH_PX,
  WEEK_SCROLLER_EDGE as TV_EXPERT_WEEK_SCROLLER_EDGE,
  WEEK_CELL_VISUAL_CLASSES as TV_WEEK_CELL_VISUAL_CLASSES,
  ...
  normalizeWeekRect as normalizeTelevisionWeekRect,
  weekCellInRect as tvWeekCellInRect,
  ...
  normalizeWeekMergeSelection as normalizeTelevisionWeekMergeSelection,
  weekPlainClickPreservesWeekAreaSelection,
  deriveMergeEligibility as deriveTelevisionMergeEligibility,
  weekCellExportText,
...
type TelevisionWeekRectSelection = ExpertWeekRectSelection
type TelevisionMultiCellSelection = ExpertMultiCellSelection
type TelevisionWeekMergeSelectionNormalized = WeekMergeSelectionNormalized
 ...
 */
const TV_EXPERT_ROW_VIRTUALIZATION = true
const TV_EXPERT_ROW_HEIGHT_PX = OOH_EXPERT_ROW_HEIGHT_PX
const TV_EXPERT_ROW_OVERSCAN = OOH_EXPERT_ROW_OVERSCAN
...
/** Row gross from Σ qty × unit rate via shared deliverable ↔ budget lib. */
...
const TV_DESCRIPTOR_TAIL: readonly (keyof TelevisionExpertScheduleRow)[] = [
  "market",
  "buyingDemo",
  "unitRate",
]
...
export function TelevisionExpertGrid({
  ...
  const tvDescriptorKeys = useMemo(
    () =>
      [
        ...(showBillingCols ? TV_BILLING_FLAG_KEYS : []),
        ...TV_DESCRIPTOR_CORE,
        ...TV_DESCRIPTOR_TAIL,
  ...
  const descriptorColWidths = useMemo(() => {
    ...
    const core = [48, 48, 120, 120, 96, 120, 110, 80, 80]
  ...
  const normalizeRowCacheRef = useRef(
    new Map<string, NormalizeRowCacheEntry<TelevisionExpertScheduleRow>>()
  )
  ...
  const rowMergeMapCacheRef = useRef(
    new Map<
      string,
      MapBuildCacheEntry<
        TelevisionExpertScheduleRow["mergedWeekSpans"],
        TelevisionRowMergeMap
      >
    >()
  )
  ...
  const pushRows = useCallback(
    (next: TelevisionExpertScheduleRow[]) => {
      const withDates = finalizeRowsPreservingIdentity(next, (r) => {
        ...
        const dates = deriveTelevisionExpertRowScheduleYmdFromRow(
          cleared,
          weekColumns,
          campaignStartDate,
          campaignEndDate,
  ...
  const rowReorderVirtual = useMemo(
    () =>
      TV_EXPERT_ROW_VIRTUALIZATION
        ? {
            rowCount: normalizedRows.length,
            estimateSizePx: TV_EXPERT_ROW_HEIGHT_PX,
  ...
  const ensureRowVisible = useCallback((rowIndex: number) => {
    if (!TV_EXPERT_ROW_VIRTUALIZATION) return
  ...
  const widthForWeekKey = useCallback(
    (k: string): number => {
      ...
      return weekColumnWidths[k] ?? TV_EXPERT_WEEK_COL_WIDTH_PX
    },
            ...
            ? Math.min(
                ...
                Math.max(
                  ...
                  deltaDaysFromPx(prev.edgeYmd, dx, {
                    ...
                    dayKeysByWeekKey,
                    weekColumnWidths,
                    defaultWeekColWidthPx: TV_EXPERT_WEEK_COL_WIDTH_PX,
  ...
  const {
    ...
    getScrollElement: () => gridScrollRef.current,
    estimateSize: TV_EXPERT_ROW_HEIGHT_PX,
    overscan: TV_EXPERT_ROW_OVERSCAN,
  ...
  const virtualSpacerColSpan = useMemo(() => {
    ...
    return (
      1 + tvDescriptorKeys.length + WEEK_GRID_COL_OFFSET + weekCells
    )
  }, [
    weekColumns,
    expandedWeekKeys,
  ...
  const containerTotals = useMemo(() => {
    ...
    for (const row of normalizedRows) {
      const { net, fee } = expertRowCostSplit(row, weekKeys, feetelevision)
      sumNet += net
      sumFee += fee
      for (const k of weekKeys) {
  ...
  }, [dayKeysByWeekKey, feetelevision, normalizedRows, weekKeys])
        ...
        <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2 border-b border-border/60 bg-muted/5 pb-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <Label htmlFor="tv-expert-row-count" className="sr-only">Add rows count</Label>
              <Input
                        ...
                        return (
                          ...
                          >
                            ...
                            <ExpertGridWeekResizeHandle
                              weekKey={col.weekKey}
                              currentWidth={weekColumnWidths[col.weekKey] ?? TV_EXPERT_WEEK_COL_WIDTH_PX}
                  ...
                  <tbody>
                    {(() => {
                      ...
                      return (
                        <MemoExpertGridRow
                          ...
                          rowUiSig={rowUiSig}
                          render={() => {
                      const net = expertRowNetMedia(row, weekKeys, feetelevision)
                      ...
                      return (
                        <tr
                          ...
                          data-tv-expert-row-index={rowIndex}
                          style={{
                            ...stripeStyle,
                            height: TV_EXPERT_ROW_HEIGHT_PX,
                            maxHeight: TV_EXPERT_ROW_HEIGHT_PX,
                          }}
                          ...
                          {(() => {
                            ...
                            for (let wi = 0; wi < weekColumns.length; wi += 1) {
                              ...
                              const tdClassName = cn(
                                ...
                                !isMergedAnchorCell &&
                                  mergeReadyOnCell &&
                                  "tv-expert-week-cell--merge-ready z-[6] bg-pacing-behind-bg ring-2 ring-inset ring-pacing-behind shadow-e0",
                      ...
                      if (!TV_EXPERT_ROW_VIRTUALIZATION) {
                    ...
                    >
                      ...
                      <td
                        ...
                        style={stickyStyleBody(tvDescriptorKeys.length)}
                      >
                      ...
                      <td
                        ...
                        style={stickyStyleBody(tvDescriptorKeys.length + 1)}
                      />
                      <td
                        className={cn(
                        ...
                        style={stickyStyleBody(tvDescriptorKeys.length + 2)}
                      >
