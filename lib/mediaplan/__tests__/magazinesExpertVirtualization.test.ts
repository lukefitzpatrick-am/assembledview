/**
 * F-28 Phase 2 — Magazines Expert row virtualization smoke (Prompt A items 1,3,5,7,8).
 */
import assert from "node:assert/strict"
import test from "node:test"
import { format, startOfDay } from "date-fns"

import { mapMagazineExpertRowsToStandardLineItems } from "@/lib/mediaplan/expertChannelMappings"
import {
  reorderExpertRows,
  mergedSpanWidthPx,
...
const MAGAZINES_EXPERT_ROW_OVERSCAN = EXPERT_GRID_ROW_OVERSCAN_DEFAULT
...
function emptyRow(id: string): MagazinesExpertScheduleRow {
...
function buildRows(n: number): MagazinesExpertScheduleRow[] {
  return Array.from({ length: n }, (_, i) => emptyRow(`magazines-v-${i}`))
}

/** Mirrors MagazinesExpertGrid containerTotals over the FULL logical array. */
function computeMagazinesWeeklyTotals(
...
test("3. DRAG REORDER across virtual boundary lands on correct logical index", () => {
  ...
  const dropIndex = virtualRowIndexFromOffsetY(
    280 * MAGAZINES_EXPERT_ROW_HEIGHT_PX + 1,
    MAGAZINES_EXPERT_ROW_HEIGHT_PX,
    300
  )
...
test("5. WEEK-COLUMN WIDTH respects resize overrides (sticky header/week resize path)", () => {
  ...
  assert.equal(MAGAZINES_EXPERT_ROW_HEIGHT_PX, 41)
})
...
test("7. WEEKLY TOTALS derived from full 300-row state", () => {
  ...
  const totals = computeMagazinesWeeklyTotals(rows, 0)
  ...
  const after = computeMagazinesWeeklyTotals(edited, 0)
...
test("8. PERF: mounted DOM window stays bounded while scrolling 300 rows", () => {
  ...
  const nearTop = expectedMountedRowRange(
    ...
    MAGAZINES_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    MAGAZINES_EXPERT_ROW_OVERSCAN
  )
  const mid = expectedMountedRowRange(
    150 * MAGAZINES_EXPERT_ROW_HEIGHT_PX,
    viewportHeight,
    MAGAZINES_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    MAGAZINES_EXPERT_ROW_OVERSCAN
  ...
  const nearBottom = expectedMountedRowRange(
    280 * MAGAZINES_EXPERT_ROW_HEIGHT_PX,
    viewportHeight,
    MAGAZINES_EXPERT_ROW_HEIGHT_PX,
    rowCount,
    MAGAZINES_EXPERT_ROW_OVERSCAN
  ...
  const maxWindow =
    Math.ceil(viewportHeight / MAGAZINES_EXPERT_ROW_HEIGHT_PX) +
    2 * MAGAZINES_EXPERT_ROW_OVERSCAN +
    2
