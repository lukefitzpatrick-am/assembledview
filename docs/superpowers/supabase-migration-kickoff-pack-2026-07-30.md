...
- **Prompt 2 is therefore adjusted:** do NOT author the schema blind and do NOT generate/apply an initial migration — the database is ahead of the code. Author `db/schema/*.ts` in Drizzle to match the two SQL files exactly (or start from `drizzle-kit pull` introspection and clean up), then verify parity: `drizzle-kit generate` against the live DB must produce an **empty diff**. Everything else in Prompt 2 (client setup, env, scripts, attrs zod validators, golden test) stands.
- **Prompt 2 DONE** (`270ba7b8`): pull via DIRECT_URL succeeded, 45 tables / 588 columns / 4 enums match, generate → no changes.
- **Prompt 3 DONE**: `scripts/migration/etl-xano-to-supabase.ts` + `recon.ts` (helpers `_shared`, `_lineItemTransform`, `_scheduleTransform`). Live load + recon against snapshot `2026-07-30`: 1:1 counts match, money deltas $0, disposition CSVs under `exports/xano/<date>/recon/` (`duplicates.csv` ~270, `version-duplicates.csv` 14, `parse-failures.csv` 3, `schedule-divergence.csv` 47). npm: `db:etl`, `db:recon`.

### Prompt 3 addendum (read together with the Prompt 3 text below)

1. **Fresh snapshot first**: re-run `npm run xano:export` immediately before the ETL; never load from a stale dated folder.
2. **Expect unique-constraint collisions on `line_items` — this is a feature.** Production Xano is known to contain ~42 duplicated version-groups (PENFOLD016-class corruption; repair was only partially applied). When the load hits `UNIQUE(version_id, line_item_id)`, do NOT crash and do NOT silently dedupe: detect duplicate groups per (version, line_item_id) *before* insert, collapse to the newest row (highest Xano id), and write every collapse to `duplicates.csv` (version, line_item_id, kept id, dropped ids, dropped $ sum). That file joins the disposition set alongside `parse-failures.csv` and `schedule-divergence.csv`.
3. **Renames**: `finance_saved_views.user` → `user_id`; `clientdashboard.Client_dashboard` → `client_dashboard`.
4. **Dual invoice_key schemes are real data, not a bug**: `finance_billing_records.invoice_key` contains both app-written (`media:{mba}:{month}`, `sow:...`, `retainer:...`) and Xano-sync-written (`xero:{invoice_id}`) keys. Port rows verbatim; do not normalise.
5. **Money to cents**: schedule blobs may contain formatted strings (e.g. "$1,234.56") and month labels like "July 2026" — reuse the app's existing parsers from `lib/**` (do not reimplement); parse to integer cents with banker's-safe rounding only at the final assignment; any row that doesn't parse cleanly goes to `parse-failures.csv`, never a guessed zero.
6. **`media_plan_production` mapping**: no `media_plan_version` field exists on that table — map via `mba_number` + `mp_plannumber` to that MBA's versions, collapse per line identity to latest, and log every collapse (expected heavy: e.g. PENFOLD015 has ~220 rows for 1 real line).
7. **Recon must also cross-check against live Xano dashboard totals** captured 2026-07-30: 28,585 records / 66 tables (manifest counts from the export are the authority; the dashboard figure is a sanity band, it moves daily).
8. If `lib/**` schedule parsers turn out to be entangled with Next-only imports and can't run under `tsx`, extract the pure functions to `lib/mediaplan/scheduleParse.ts` first (no behaviour change, add a golden test) and import from both places — do not fork the logic.
