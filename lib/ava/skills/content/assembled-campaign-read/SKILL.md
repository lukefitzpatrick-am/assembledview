...
2. Call these tools for the MBA on the page (or supplied in the request):
   ...
   - Campaign KPIs (the KPI payload supplied with the generate job, plus any targets on the page). There is no separate KPI tool — do not invent one.
...
| `happened` | What has happened | Delivered spend, deliverables, time elapsed — from delivery / pacing. |
...
4. Rules while writing:
   ...
   - Delivery $ / KPI figures come from **get_delivery_snapshot** / pacing / the KPI payload — never from insight bodies.
...
```
{
  ...
  "sources": ["get_delivery_snapshot", "get_pacing_snapshot"]
}
```
