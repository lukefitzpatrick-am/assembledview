...
Campaign read (dashboard Regenerate / "write the read"): load assembled-campaign-read THEN get_campaign_context + get_delivery_snapshot + get_campaign_insights. Use the campaign KPI review rows supplied with the generate job. Pace vs expected comes from delivery snapshot totals. Never call get_pacing_snapshot. Return JSON six beats only. Never invent a cause. Empty beat = "Nothing to report yet."
