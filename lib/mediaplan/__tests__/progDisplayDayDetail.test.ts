...
function line(bursts: any[]): StandardProgDisplayFormLineItem {
  return {
...
test("sub-week interior burst imports as day-detail and round-trips", () => {
  ...
  const rows = mapStandardProgDisplayLineItemsToExpertRows(std, cols, CS, CE)
  const wk0 = cols[0]!
  assert.equal(weekHasDailyValues(rows[0]!.dailyValues ?? {}, weekDayKeys(wk0, CS, CE)), true)
  const back = mapProgDisplayExpertRowsToStandardLineItems(rows, cols, CS, CE, { feePctProgDisplay: 0 })
